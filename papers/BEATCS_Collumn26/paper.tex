...
Note that the semantics is remarkably similar to that of expressions and assertions (or formulas) in the quantifier-free fragment of standard first-order logic. Graphs $H$ in density expressions correspond to real-valued variables, density expressions to arithmetic expressions over these variables, and assertions to logical formulas over these arithmetic expressions. What distinguishes flag algebra from first-order logic is that values at different graphs (or variables) are not set independently, but have strong correlation; they all come from the same limiting graph. As a result, two syntactically different density expressions or assertions can be semantically equivalent, if their
difference lies only in the use of different isomorphic graphs in density expressions. Also, the sum of all non-isomophic graphs on a fixed vertex set is semantically equal to the constant density expression $1$, since this normalisation property is enjoyed by all limiting graphs. We formalise these observations in the next two lemmas. Finally, every graph is non-negative.
\begin{lem}
