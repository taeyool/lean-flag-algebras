...
Using flag algebra, we can express asymptotic extremal problems mentioned in the previous section. For a graph $H$ and a finite set of graphs $\mathcal{F}$, the 
...
\]
holds, i.e., it is valid. The condition $\bigwedge_{F \in \mathcal{F}} F = 0$ here expresses that a limiting graph $\phi$ avoids all forbidden graphs in $\mathcal{F}$, and the conclusion $H \leq c$ states that the density of $H$ in $\phi$ is at most $c$.
...
I_s + K_t \geq c'
\]
holds. We remind the reader that $I_s$ here is the edgeless graph with $s$ vertices.
...
Before moving on to reasoning principles of flag algebras, we present two examples of expressing and proving extremal graph theory problems using the syntax and semantics introduced so far. The first example is concerned with a Tur\'an-type problem that asks us to find a tightest upper bound on the asymptotic edge density in triangle-free graphs. In terms of flag algebra, this problem asks us to find a tightest $c$ that makes the below assertion valid:
\[
K_3 = 0 \implies K_2 \leq c
\]
The answer to this problem is well-known to be $1/2$ by  Mantel's theorem \hy{Citation}. We prove this optimal bound in flag algebra as follows. We start by noting the following key inequality:
\begin{equation}
  \label{eqn:mantel-key-inequality}
0 \leq \tfrac{1}{2} \cdot \Ithree - \tfrac{1}{6} \cdot \Ethree - \tfrac{1}{6} \cdot \Pthree
+ \tfrac{1}{2} \cdot \Kthree
\end{equation}
How to prove the validity of this inequality will be explained later in the next section where we go deeper into reasoning principles of flag algebras. For now, we just accept this inequality as valid, and use it to derive the desired upper bound on $K_2$ under the assumption $K_3 = 0$. The derivation is given below: 
\begin{align*}
  K_2 
