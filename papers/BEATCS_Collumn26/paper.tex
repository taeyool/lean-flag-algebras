...
Throughout the paper, we are concerned with finite simple graphs $G$ whose vertex set is a subset of $\mathbb{N}$, and call them simply as graphs. For a graph $G$, we let $V(G)$ and $E(G)$ be the vertex set and the edge set of $G$, respectively, and write $v(G)$ and $e(G)$ 
for their cardinalities. Also, for a set $U \subseteq V(G)$, we define $G[U]$ to be the subgraph of $G$ induced by $U$. Finally, we use $\mathcal{G}$ to mean the set of all graphs.

\section{Induced Subgraph Density and Limiting Density Maps}

For graphs $H$ and $G$, the \emph{induced subgraph density} of $H$ in $G$ or simply \emph{$H$-density} in $G$ is defined as
\[
p(H, G) \defeq \Pr(G[U] \simeq H)
\]
where $U$ is a uniformly random subset of $V(G)$ with size $v(H)$, and $\simeq$ denotes the graph isomorphism. 

The density function $p$ provides two new ways of viewing graphs. First, a graph $G$ can be understood as a feature vector $p(\_,G)$ residing in an infinite-dimensional space indexed by all graphs in $\mathcal{G}$. \hy{Perhaps this feature reprentation has nice properties, like injectivity. Check this.} Second, the partially applied function $p(H, \_)$ for a given graph $H$ can be seen as a statistic measuring how frequently $H$ appears as an induced subgraph in other graphs. 

Let $(G_n)_{n \in \mathbb{N}}$ be a sequence of graphs. The sequence is \emph{increasing} if $v(G_n) < v(G_{n+1})$ for all $n \in \mathbb{N}$. It is \emph{convergent} if the sequence is increasing and the limit $\lim_{n \to \infty} p(H, G_n)$ exists for every graph $H$. 
