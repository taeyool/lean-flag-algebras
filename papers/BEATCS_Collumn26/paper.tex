...
The previous section illustrated how to reason about limiting graphs using flag algebra with two examples. In this section, we explain the reasoning principles of flag algebra that enable such reasoning. 

A \emph{type} is a graph $\tau$ with the vertex set $[k]$ for some $k \in \mathbb{N}$. We call $k$ the \emph{size} of $\tau$. A \emph{$\tau$-labeled graph} is a pair $(G, \theta)$ of a graph $G$ and an injective map $\theta : [k] \to V(G)$ such that the map $\theta$ is a graph isomorphism from $\tau$ to the induced subgraph $G[\theta([k])]$. We call $\tau$ the \emph{type} of $(G,\theta)$. For a type $\tau$, we write $\mathcal{G}^\tau$ for the set of all $\tau$-labeled graphs. 

Consider types $\tau,\sigma$, and a function $\gamma$ from $\mathcal{G}^\tau$ to $\Pr(\mathcal{G}^{\sigma})$ where we regard $\mathcal{G}^{\sigma}$ as a measurable space with the discrete sigma algebra. Then, 
\[
p(\alpha(H), G) = p(H, \gamma(G)).
\]
where $p$ is ...
