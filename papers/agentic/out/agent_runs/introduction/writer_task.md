...
\paragraph{Extremal densities.}
Extremal graph theory often asks how dense one pattern can be inside graphs
that avoid another pattern.  Let $F$ and $H$ be finite simple graphs.  We write
\[
  \mathrm{ex}(n;\,F,H)
  =
  \max\bigl\{
...
triangle-free graphs.  From this point onward the problem is no longer about a
single finite graph but about limiting densities over all sufficiently large
graphs, which is the setting of asymptotic extremal combinatorics.

Razborov's flag algebra method~\cite{razborov2007flag} is a framework for
proving upper bounds on such limits.  A flag is a finite graph together with a
distinguished labeled subgraph; flag algebras organize the densities of these
finite patterns into an algebraic calculus.  In many applications, including
the Erd\H{o}s pentagon problem~\cite{hatami2012,grzesik2012}, an upper bound is
obtained by finding a semidefinite-programming (SDP) certificate: a finite matrix
certificate whose positive semidefiniteness implies the required density
inequality.  The solver that finds the certificate is external; the
mathematical proof still has to justify, exactly, that the certificate proves
the intended inequality.

\paragraph{What has to be formalized.}
A proof assistant cannot treat a flag algebra certificate as just a matrix
file.  There are three separate objects to formalize.  First, there is the
mathematical theory: flags modulo label-preserving isomorphism, the quotient
algebra generated by density-expansion identities, the order given by
non-negativity under all positive homomorphisms (algebra maps to
$\mathbb{R}$ that are non-negative on finite flags), and the theorem
connecting that order to Tur\'an-density bounds.  Second, there is finite
computation: the proof needs many exact rational density values and exact
checks of the SDP certificate.  Third, there is algebraic bookkeeping:
practical proofs contain large linear combinations of flags that must be
expanded, multiplied, and normalized before a human-readable inequality becomes
a Lean goal.

These three objects should not be collapsed into one representation.  Our
formal specification of flag algebras is quotient-based, matching the
mathematics rather than the executable data structures used for computation.
Reflection enters only at a proved boundary: executable graph and flag
operations are connected to the abstract definitions by adequacy theorems,
where an adequacy theorem states that the executable value is equal to the
value specified by the quotient-level definition.  Tactics form a third layer:
they do not define flag algebras and they do not justify SDP certificates; they
perform the syntactic normalization needed to apply already-proved expansion
and multiplication lemmas.
...
Revise the Reference Draft for section 'Introduction' to address the feedback above.
