...
\begin{enumerate}
  ...
  \item \textbf{Data-heavy computation.}  For each flag algebra argument,
    thousands of flag algebra computations must be certified and
    imported into the proof: density values between flag pairs and the entries
    of the SDP certificate matrix must all be verified against the formal
    definitions.
...
\begin{itemize}
  ...
  \item \textbf{Reflection architecture (\S\ref{sec:reflection}).}
    ...
    decomposition over $\mathbb{Q}$.  We deliberate a \emph{trust hierarchy}:
    density tables are checked by Lean's native evaluator (trusting the native
    compiler), while SDP certificate equalities are checked by the kernel alone
    (trusting only the kernel).
  ...
  \item \textbf{Verified results (\S\ref{sec:results}).}
    ...
    ($\pi(C_5; K_3) = 24/625$), the latter including both the upper bound
    via a formally verified SDP certificate.
    ...
    our formalization is, to our knowledge, the first to derive it via the
    flag algebra method.
\end{itemize}
