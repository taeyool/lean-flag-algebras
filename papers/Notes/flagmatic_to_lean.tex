...
Four certificates have been put through the pipeline and produce
files that the Lean kernel accepts with no \texttt{sorry}.
...
The four cases between them exercise every feature of the translation
script.
\begin{itemize}
  ...
  \item \textbf{ErdosPentagon} ($K_3$, $N{=}5$, branch A, three blocks):
    the largest case in the suite, with three quadratic forms of
    dimensions $8$, $6$, $5$ -- a single \texttt{reduce\_downward\_-
    flagmul} call dispatches all $8^2 + 6^2 + 5^2 = 125$ product
    terms.
\end{itemize}

A fifth case, K5turan, exercises a $K_5$ forbid; it is the worked example
of Section~\ref{sec:extension} below.
...
The K5turan certificate has been carried through steps~1--3 (the
$K_5$ branches in \texttt{CommonGraphs.lean} and
\texttt{DensityLoader.lean} are in place, and the K5-specific JSON
files have been produced); step~4 is the remaining work item before
that case can be built and added to Section~\ref{sec:scenarios}.
...
four verified scenarios of Section~\ref{sec:scenarios}, but we have
...
The four certificates of Section~\ref{sec:scenarios} -- Mantel,
K3forbidC4, K4turan, Erd\H{o}s pentagon -- between them exercise every
feature of the pipeline, and a fifth, K5turan, is the in-progress
example that completes the $K_5$-forbid story. None of these required
hand transcription of the certificate into Lean.
