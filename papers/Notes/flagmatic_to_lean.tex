...
The script \texttt{flagmatic\_to\_lean.py} exposes four subcommands:
...
Section~5. Because it resolves every string, it doubles as a validity
check on a fresh certificate (it raises on the first string that fails to
resolve).
...
\end{lstlisting}
where \texttt{inspect} catches identifier mismatches before any Lean
...
unconditioned case) for every density file.
