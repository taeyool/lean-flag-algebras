...
them.  An \emph{induced embedding} of $\sigma$ into a graph $H$ is an injective
...
A \emph{$\sigma$-flag} is a graph in $\T$ with a distinguished induced labelled
...
vertices together receive total mass $\lambda$, and the non-labelled vertices
...
non-labelled clone class.  Second, on the complementary good event, projection
...
non-labelled clone classes.  For each $v\in U$, write $s_v:=|C_v|$ and
...
$X_b$ land in the same non-labelled clone class, still under the conditioning
that the whole sample avoids $R$.  If $X_a\in C_v$, then $X_b$ is uniform among
the remaining $S-1$ vertices in the non-labelled clone classes.  Therefore
\[
...
\[
  \begin{aligned}
  &\Prob[X_a,X_b\text{ lie in the same non-labelled clone class}
...
lie in the same non-labelled clone class.  Equivalently, for the ordered
...
non-labelled clone class.  Combining the two bounds above,
\[
  \Prob[\text{bad}]
  \le
...
all induced embeddings $\widehat\theta:\sigma\to B_N$, where the labels of
...
it is uniform over the finite set of induced labelled copies of $\sigma$ in
$B_N$.  Every ordered choice of
...
\[
  ...
  \frac{\lambda}{2k}.
\]
Hence the number of planted ordered embeddings is at least
\[
  \prod_{i=1}^k |C_{\theta(i)}|
  \ge
  \left(\frac{\lambda}{2k}\right)^k N^k .
\]
The denominator in the random experiment is the total number of induced ordered
embeddings of $\sigma$ into $B_N$.  This denominator is at most the number of all
ordered maps from the $k$ labelled vertices to $V(B_N)$, namely $N^k$.  Therefore
...
induced embedding $\widehat\theta\in\Emb_\sigma(B_j)$, form the point
...
call an induced embedding $\widehat\theta:\sigma\to B_j$ \emph{planted} if
...
when an induced embedding of $\sigma$ into $B_j$ is chosen uniformly, the
...
non-labelled clone class.  The probability of this bad event depends only on the
...
class, and no two sampled vertices lie in the same non-labelled clone class.
...
classes, because $\theta$ is injective; since $\theta$ is an induced embedding of
...
Now consider the planted-mass estimate.  Choose a uniformly random induced
embedding of $\sigma$ into $B_N^+$.  Every ordered choice of one vertex from each
labelled clone class $C_{\theta(i)}$ is an induced copy of $\sigma$: the labelled
clone classes are distinct, and all adjacencies between them are inherited from
the base flag $(G,\theta)$.  Hence the same lower bound on the number of planted
ordered embeddings holds,
\[
  \prod_{i=1}^k |C_{\theta(i)}|
  \ge
  \left(\frac{\lambda}{2k}\right)^k N^k
...
number of induced ordered embeddings is still at most the number $N^k$ of all
ordered maps from the $k$ labelled vertices to $V(B_N^+)$.  Dividing gives the
...
induced embedding of $\sigma$ into $B_j^+$ is planted with probability at least
$c_{|\sigma|,\lambda}$.  In particular, the density of induced copies of
...
choose uniformly an induced embedding
...
We first show that $\K$ is true-clone-closed.  Let $G\in\K$, and write its
connected components as cliques $D_1,\ldots,D_r$.  Form a complete blow-up
$G^{\mathbf m,+}$.  If $v,w$ lie in the same component $D_a$, then either
$v=w$, in which case the clone class $C_v$ is a clique by the definition of
complete blow-up, or $v\ne w$, in which case $vw$ is an edge of $G$ and all
edges between $C_v$ and $C_w$ are present.  Thus the union
\[
  \bigcup_{v\in D_a} C_v
\]
is a clique.  If $v$ and $w$ lie in different components of $G$, then $vw$ is not
an edge of $G$, so no edges are added between $C_v$ and $C_w$.  Hence the
complete blow-up is again a disjoint union of cliques, one clique for each
component of $G$.  Therefore $\K$ is true-clone-closed, and
\cref{thm:true-clone-root-plantable} shows that $\K$ is root-plantable at every
non-degenerate type.

It remains to check that $\K$ is not clone-closed.  The single edge $K_2$
belongs to $\K$.  Take its independent blow-up in which both endpoints are
replaced by independent clone classes of size $2$.  The resulting graph is
$K_{2,2}$: all four cross edges are present and there are no edges inside either
clone class.  Choose two vertices from one side and one vertex from the other.
The induced subgraph on these three vertices has exactly two edges, namely a
path on three vertices.  A path on three vertices is not a disjoint union of
cliques, because its three vertices form one connected component that is not a
clique.  Since $\K$ is hereditary, if $K_{2,2}$ were in $\K$ then this induced
three-vertex path would also be in $\K$, a contradiction.  Thus this independent
blow-up of a graph in $\K$ does not belong to $\K$, and $\K$ is not
clone-closed.
\end{proof}
...
vertex whose internal structure is asymptotically invisible to samples of bounded
labelled flags.
...
Fix $0<\lambda\le1/2$ and assign the same weights $w_v$ as in the planted
blow-up construction.  For all sufficiently large integers $N$, choose positive
integers $s_{v,N}$ summing to $N$ and graphs $H_{v,N}\in\K$ with
$|H_{v,N}|=s_{v,N}$ such that
\[
  \operatorname{err}^{\mathrm{sub}}_N
  :=
  \sum_v \left|\frac{s_{v,N}}{N}-w_v\right|\to0.
\]
Let
\[
...
independent blow-up, with $\operatorname{err}^{\mathrm{sub}}_N$ in place of
$\operatorname{err}_N$ and with the same small-error hypothesis in
...
order inside a larger graph of $\K$.  Choose positive integers $s_{v,N}$ summing
to $N$ and satisfying $s_{v,N}/N\to w_v$.  Since every $w_v$ is positive, all
$s_{v,N}$ are eventually large enough that fibres $H_{v,N}\in\K$ of those exact
orders exist.
...
bounded $\sigma$-flag densities nearly unchanged.
...
$\widetilde C$, because its $F_i$-coordinates differ from those of $\psi$ by at
most the two $\eps/3$ errors above.  The total number of ordered embeddings of
$\sigma$ into $H_t$ is at most $|V(H_t)|^k$, so
\[
...
It remains to check the $\sigma$-flag densities.  Fix a planted embedding
...
ordinary triangle-free quotient: stars labelled at their centres give quotient points with $e=1$, and empty
