...
(replacing each vertex by an independent set of clones): blowing one vertex up into such a
\emph{clone class} of positive density lets one realise at a random---hence typical---vertex
what the quotient could show only at an exceptional one.
...
constrained limit has essentially no neighbours: the \emph{labelled edge density}---the
fraction of the graph a root is joined to---is almost surely $0$.  Yet the class contains arbitrarily large stars, whose centre is a vertex of
...
(\cref{prop:ideal-zero})---and every ensemble-relaxed labelled
