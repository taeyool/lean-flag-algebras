...
\Cref{thm:blowup-root-plantable} is the broad closure theorem above.  It proves
root-plantability by a global operation: whenever the quotient exhibits a finite
labelled witness $(G,\theta)$, one blows up the vertices of $G$ and stays inside the
class.  The planted copies of the labelled vertices then form a positive-density set of
root placements, and bounded flag densities still look almost like they did in
$(G,\theta)$.  In other words, a view that was visible only by hand-picking the roots has
been made visible to random roots.
...
For every good placement, all bounded $\sigma$-flag statistics are nearly the same as in
the original witness.  Thus the quotient view of $(G,\theta)$ can be made typical without
assuming any global blow-up closure of the class.
