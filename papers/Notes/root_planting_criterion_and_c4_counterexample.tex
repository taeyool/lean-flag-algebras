...
\emph{labelled limits}.  Such a limit is a \emph{view} from a copy of $\sigma$:
choose a copy of $\sigma$ in a large graph of the class, treat its vertices as
the roots, record the density of every \emph{flag} (a finite graph with a specified
copy of $\sigma$ inside it), and pass to the limit---the local picture the roots see.
...
limits while the quotient still attains another (\cref{thm:pinning}).  We conjecture that pinning is the only obstruction
...
A \emph{$\sigma$-flag} is a graph in $\T$ with a distinguished copy of
...
type occurs with density zero in every unlabelled limit, so the random rooting below acts
...
the expected density of $g$ is the unlabelled density of $g$, normalised by that
...
the resulting homomorphism evaluates \(g\) as \(c\) almost surely,
...
the original flag.  We first check that this rule is compatible with the
equalities used to define the flag algebra.  In the flag algebra, a flag is
identified with the weighted average of its one-vertex extensions; therefore,
if complementing a flag is to define a map on algebra elements, it must carry
each such equality to the corresponding equality for complemented flags.  If
$F$ is a $\sigma$-flag and $H$ is a larger $\sigma$-flag, then the same set of
unlabelled vertices induces $F$ in $H$ if and only if it
induces $\overline F$ in $\overline H$. Therefore
\[
...
\]
This is just precomposition with the inverse algebra isomorphism.  Its inverse
is $\widehat C_{\overline\sigma}$, so it is a homeomorphism.  Because forbidden
flags correspond under complementation, the intrinsic description of
...
$\Ext_{\overline\sigma}(\overline\phi_0)$.  These identities hold for every
$g$, and the evaluation functions coming from the flag algebra form a
separating algebra of continuous functions on the compact homomorphism space;
hence they determine probability measures.  Thus pushing
$\Ext_\sigma(\phi_0)$ forward by $\widehat C_\sigma$ gives
$\Ext_{\overline\sigma}(\overline\phi_0)$.  As $\phi_0$ ranges over
$Q_0(\K)$, the limits $\overline\phi_0$ range over $Q_0(\overline\K)$.
Since a homeomorphism sends the support of a measure to the support of its
pushforward, $\widehat C_\sigma$ takes each support appearing in the definition
of $S_\sigma(\K)$ to the corresponding support appearing in the definition of
...
Thus the same homeomorphism identifies both pairs of spaces,
$Q_\sigma(\K)$ with $Q_{\overline\sigma}(\overline\K)$ and $S_\sigma(\K)$ with
...
admissible one-root random extension; while the co-star rooted at its isolated vertex
...
$c\in[0,1]$.  Suppose the density of $g$ is almost surely pinned to $c$ across
...
a root, or that two further vertices are adjacent, is at most $\lambda$.  Hence as
$\lambda\to0$ the density of a $\sigma$-flag $g$ tends to $1$ when $g$ extends
...
$\lambda W$ supply, as $\lambda\to0$, a deterministic rooted view of $\sigma$ from which
...
the type consisting of two roots joined by an edge, and let $g\in\A^\tau[\T_{\mathrm{Graph}}]$
be the three-vertex $\tau$-flag in which the one unlabelled vertex is adjacent
to both roots; equivalently, $g$ is the triangle $\tau$-flag over
the edge type.
...
every such labelled quotient limit can be planted.  A typical two-root edge is much
...
though a specially chosen quotient limit at this type can give \(g\) a different
...
for the exhibited classes: a degenerate root choice carries no information, so its only
