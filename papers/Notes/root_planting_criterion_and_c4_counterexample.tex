...
underlying many computer-assisted density bounds such as Mantel's and Tur\'an's theorems
and the Erd\H{o}s pentagon problem.  Often one wants such a bound only inside a hereditary
...
clique-free and triangle-free classes---and more generally under substitution.  It can
also fail: the $C_4$-free graphs are a counterexample, and because the condition is
preserved by complementation, so are some very dense classes, so neither sparsity nor
density is the dividing line.  The common cause is \emph{pinning}, a local quantity frozen
...
holds for a single root and for a non-adjacent pair of roots but fails for an edge, where
a ``book'' graph obstructs it, refuting the natural all-types conjecture.  Finally,
...
extremal configurations.  For instance, read through this lens the verified $K_4$-free
$P_4$ certificate forces every extremiser to be the balanced complete tripartite graph,
...
underlie many computer-assisted density bounds, from Mantel's and Tur\'an's theorems to
the Erd\H{o}s pentagon problem.  This paper concerns a foundational question that the
...
induced subgraphs, equivalently the graphs avoiding every member of some forbidden family.
...
\emph{roots}.  The two semantics then live in one and the same space of labelled limits:
those the quotient can exhibit form a set $Q_\sigma$, while those arising as random local
views of constrained \emph{unlabelled} limits form a set $S_\sigma$.  Soundness is the
...
\[
  S_\sigma=Q_\sigma .
\]
We call this condition \emph{root-plantability}: every constrained labelled limit must be
approximable by the \emph{typical} local views---those occurring with positive
probability---of constrained unlabelled limits.  Informally, any configuration the
...
(replacing each vertex by an independent set of clones): a clone class of positive density
lets one realise at a random vertex what the quotient saw only at an exceptional one.
This already covers the clique-free classes, the triangle-free graphs among them
...
common generalisation for classes closed under \emph{substitution}
(\cref{thm:substitution-root-plantable}).  We also isolate a finite, checkable form of the
argument---a \emph{local planting} criterion---that applies when the planted graph must be
repaired locally rather than produced by a global closure operation
...
sparse---every $n$-vertex $C_4$-free graph has $o(n^2)$ edges---so a typical vertex of a
constrained limit has essentially no neighbours, and the labelled edge density is almost
surely $0$.  Yet the class contains arbitrarily large stars, whose centre is a vertex of
edge density $1$.  The quotient sees this exceptional centre; the ensemble, sampling a
typical vertex, never does.  So the inequality ``the labelled edge density is $0$'' holds
in the ensemble but fails in the quotient, and the class is not root-plantable
...
unlabel to zero (\cref{prop:ideal-zero}), and every ensemble-relaxed labelled
contribution is, after unlabelling, a limit of ordinary quotient sum-of-squares
contributions (\cref{thm:no-closed-certificate-gap}).  What could in principle remain is
...
the one-vertex type, and at the two-vertex non-edge type, a local repair of the blow-up
construction works, so root-plantability holds
(\cref{thm:c5-one-root,thm:c5-nonedge-root}).  At the two-vertex \emph{edge} type it
...
equality slice this recovers the local statistics of the extremal complete bipartite
...
space.  We write $Q_0:=Q_\emptyset$ for the empty-type instance, the constrained
unlabelled limits (the limits of graph sequences drawn from $\T_1$).  The set $Q_\sigma$
has a simple intrinsic description, phrased entirely in terms of $\chi$: a
