...
pinning mechanism: \emph{degeneracy}.  Here degeneracy means that graphs in the class
have vanishing edge density asymptotically: every $n$-vertex member has $o(n^2)$ edges,
equivalently every dense limit of the class has edge density zero.  In such a limit a
typical vertex sees no labelled edge, yet the class may still contain finite graphs with an
\emph{exceptional} high-degree vertex, such as the centre of a star.  The labelled edge
flag separates the two.

For the degeneracy obstruction below, $\sigma=1$ is the one-vertex type,
$e\in\A^1[\T_{\mathrm{Graph}}]$ is the labelled edge flag (the two-vertex flag in which
the labelled vertex is adjacent to the one unlabelled vertex), and
...
root-plantable.  Concretely, for $f:=-e\in\A^1[\T_{\mathrm{Graph}}]$,
\[
  \Prob_{\phi^1\sim\Ext_1(\phi_0)}[\phi^1(f)\ge0]=1
  \qquad\text{for every }\phi_0\in Q_0,
\]
but some $\psi\in Q_1$ has $\psi(f)<0$; in fact
\[
...
\begin{proof}
Let $\phi_0\in Q_0$.  Since $\brak1=1$ we
...
surely, hence $\phi^1(-e)=0\ge0$ almost surely.  Moreover,
...
$\psi(-e)=-1<0$.  Thus, $\psi\in Q_1$ witnesses
$\{\chi\in Q_1:\chi(e)=0\}\subsetneq Q_1$; in particular $\psi\in Q_1\setminus S_1$,
so $S_1\ne Q_1$.
\end{proof}

\begin{remark}
The hypothesis ``contains arbitrarily large stars'' is only used to exhibit, in $Q_1$, a labelled limit of positive labelled edge density; it may be replaced by the
...
$\sigma=1$: the element $-e$ is non-negative almost surely under every admissible
one-root random extension, but is negative at some point of $Q_1$.
\end{corollary}
...
$\sigma=1$, witnessed by $f:=e-\one_1$ (with $\one_1$ the unit of
$\A^1[\T_{\mathrm{Graph}}]$).  In
...
$\phi^1(e)=1$ almost surely, so $e-\one_1$ is non-negative almost surely under every
admissible one-root random extension; while the co-star labelled at its isolated vertex
gives a $\psi\in Q_1$ with $\psi(e)=0$, whence $\psi(e-\one_1)=-1<0$ and quotient
semantics fails.
\end{proof}
