...
nonnegative building blocks of a quotient certificate, each obtained by squaring a
...
a non-$\T_1$ graph as an induced subgraph is itself outside $\T_1$.  So the forbidden flags
already span an ideal, not merely a subspace, and the ideal they generate is exactly their
span.  Hence the elements that $\qmap_\sigma$ collapses to $0$ are exactly the linear
...
from its quotient.  By the universal property it therefore descends: the assignment
$\psi(\qmap_\sigma a):=\chi(a)$ is well defined, and it is the unique homomorphism $\psi$ on
