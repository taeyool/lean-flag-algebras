...
independent blow-ups (which holds for the clique-free classes, including triangle-free) implies root-plantability and ensures that these two notions of non-negativity coincide.  The true-twin analogue, closure under complete blow-ups, gives further all-type positive examples that are not clone-closed, such as cluster graphs; more generally, infinite substitution-closed hereditary classes are root-plantable.  We also isolate a finite local planting criterion that captures local repair arguments beyond global closure assumptions, and verify it through a sparse root-blow-up repair condition. But heredity alone does not lead to root-plantability.  Every class so
...
This incompleteness, however, never improves an asymptotic density bound at the
closed-cone level.  Such a bound is an inequality about unlabelled graphs, whereas
the gap between the two semantics lives in the auxiliary inequalities---about
...
examples witnessing the gap vanish once averaged back into unlabelled-graph statements,
and, more generally, that every ensemble-relaxed labelled contribution is a limit of
ordinary quotient sum-of-squares contributions after unlabelling.  What can remain is
only an exact or finite-truncation certificate question, not a new asymptotic density
bound.  Finally, we point out a different advantage of ensemble semantics: it can impose
non-hereditary limit constraints directly.  On the Mantel equality slice, for example,
the relative ensemble support records the labelled local structure of the balanced
complete bipartite extremal limit, including the identity that a typical vertex has
degree $1/2$.  More interestingly, applying the same idea to the verified $K_4$-free
$P_4$ certificate extracts labelled equations that every $P_4$-extremal $K_4$-free
limit must satisfy; these equations force edge density $2/3$, and hence recover the
balanced complete tripartite extremal graphon.  We also record the corresponding
stability mechanism: near equality in the certificate forces the three rooted square
terms to be small, these approximate local equations force the edge density close to
$2/3$, and graphon Turán stability then gives closeness to the tripartite graphon.  The
same extraction works for the parametric $K_{r+1}$-free $P_4$ certificate: equality
forces the $r$-partite local laws, and the only remaining external input needed to
identify the whole graphon is the classical equality case of Zykov's clique-density
theorem.
\end{abstract}
...
correspondence for the clique-free classes, including triangle-free.  The same proof also
has a true-twin version: closure under complete blow-ups implies root-plantability, giving
non-clone-closed all-type examples such as cluster graphs.  More generally, infinite
substitution-closed hereditary classes are root-plantable, since the planted-module
sampling estimate never sees the internal structure of a fibre unless two sampled
vertices collide inside it.  Finally, a finite local planting criterion captures the
same support argument when the planted graph must be repaired locally rather than
obtained from a global closure operation, and a sparse root-blow-up repair theorem
gives a checkable way to verify it.  Heredity alone, however,
...
improves the closed cone of density bounds.  A density bound is a statement about
unlabelled graphs---the \emph{empty type}---where the two semantics provably coincide
(\cref{prop:empty-type}); the labelled examples that witness the gap vanish when averaged
back into unlabelled-graph statements (\cref{prop:ideal-zero}); and, in fact, every
ensemble-relaxed labelled contribution is a $Q_0$-limit of ordinary quotient
sum-of-squares contributions (\cref{thm:no-closed-certificate-gap}).  The incompleteness
is thus confined to the intermediate, labelled certificates a proof passes through.  The
remaining practical question is whether exact finite-degree or rational certificates can
separate before taking this closure.  A separate, positive flexibility of ensemble
semantics is that it can be enhanced by non-hereditary limit constraints.  We formulate
this relative semantics and use the Mantel equality slice to show how it recovers labelled
identities of extremizers, such as $e=\frac12\one_1$ at a typical vertex of a maximum
triangle-free limit, which ordinary hereditary quotient semantics cannot impose.  We then
apply the same mechanism to the verified $K_4$-free $P_4$ certificate: equality in the
bound forces the labelled certificate terms to vanish on the relative one- and two-root
supports, yielding concrete local equations for extremal limits.  Integrating those
local equations gives a short certificate-based derivation that every extremal
$K_4$-free $P_4$ limit has edge density $2/3$, and therefore is the balanced complete
tripartite limit by the equality case of Turán's theorem.  The same proof gives
qualitative stability by compactness.  More quantitatively, if the $P_4$ defect is
$\Delta$, then the certificate controls the three rooted square terms by $O(\Delta)$,
and the integrated local equations give
$|p-2/3|\le C\Delta^{1/4}$ for the edge density $p$; composing this with any graphon
Turán-stability modulus gives an explicit certificate-to-tripartite stability modulus.
Finally, the parametric \texttt{CompleteGraphFreeP4} certificate gives the corresponding
$r$-partite local equations for every $r\ge3$; for $r\ge4$, the equality case of Zykov's
$K_4$-density theorem turns these equations into a full recovery of the balanced complete
$r$-partite limit.
...
\[
  Y\subseteq Q_0
\]
of admissible unlabelled limits inside an existing hereditary theory.  The set $Y$ may
...
genuine theory enhancement: it can impose limit-level conditions that are meaningful for
...
Let $\T_1$ be the triangle-free graph theory, let $\rho\in\A^0[\T_1]$ be the
...
\begin{enumerate}[label=\textup{(\roman*)}, leftmargin=2.5em]
  ...
  \item At the ordered edge type $\tau$, let $a_\tau$ and $b_\tau$ be the two
        ...
        theory.  On $S_\tau(Y_{\mathrm{Mantel}})$,
        \[
...
local information about extremizers that the hereditary quotient deliberately forgets.
\end{remark}
...
Let $\T_1$ be the $K_4$-free graph theory, and let $\pi_{P_4}\in\A^0[\T_1]$ denote
...
\begin{theorem}[A certificate route to the tripartite extremizer]\label{thm:k4free-p4-tripartite}
Every $K_4$-free graphon in the equality slice $Y_{P_4}$ has edge density $2/3$.
Consequently, by the equality case of Turán's theorem, every element of $Y_{P_4}$ is the
...
equality case of Turán's theorem applies: $W$ is equivalent, up to a measure-preserving
relabeling, to the balanced complete tripartite graphon.
\end{proof}

\begin{remark}
The point of \cref{thm:k4free-p4-tripartite} is not that Turán's equality case is new;
...
\]
Then $W_n$ converges in cut distance, up to measure-preserving relabeling, to the
...
\begin{proof}
The graphon space modulo measure-preserving relabeling is compact.  If a subsequence of
...
\begin{theorem}[Certificate-to-Turán stability]\label{thm:k4free-p4-quant-stability}
...
Moreover, let $\omega_{\mathrm{Tur}}$ be any graphon Turán-stability modulus for
...
The final implication is just Turán stability applied after this edge-density estimate:
...
Fix an integer $r\ge3$.  Let $\T_r$ be the $K_{r+1}$-free graph theory, and let
...
\begin{enumerate}[label=\textup{(\arabic*)}, leftmargin=2.8em]
  ...
  leans on classical Turán/Zykov equality cases.  The next genuinely new test is to mine
