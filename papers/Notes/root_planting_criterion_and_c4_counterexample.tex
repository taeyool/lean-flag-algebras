...
Fix the constants first.  Let $C_m$ be a constant, depending only on $m$ (with $k=|\sigma|$
fixed), large enough for the two sampling estimates used for (iii) below.  Choose
...
Finally, we compare the two sampling experiments.  
Outside both bad events that we handled in the previous two steps, the sampled vertices lie in $U$ and span no altered pair, so $H$
and $G$ agree on them; together with conditions (i) and (ii) of \cref{def:sparse-root-repair}---which
make $\operatorname{im}\widehat\theta$ induce $\sigma$ and match its adjacencies to $U$ with
those of the original roots $\operatorname{im}\theta$---the $\sigma$-flag induced by
$(\operatorname{im}\widehat\theta,\text{sample})$ in $H$ equals the one induced by
$(\operatorname{im}\theta,\text{sample})$ in $G$.  So the densities differ by at most the total probability of these two bad events, which we bounded by $2mk\lambda+4m^2\rho$:
\[
  \bigl|p(F,(H,\widehat\theta))-p(F,(G,\theta))\bigr|
  \le 2mk\lambda+4m^2\rho
  \le 4m^2(\lambda+\rho).
\]
The two sampling estimates thus hold with $C_m=4m^2$ (which bounds both $2mk$ and $4m^2$,
as $k\le m$), the constant fixed at the outset; by the choice of $\lambda$ and $\rho$,
$C_m(\lambda+\rho)<\eps$, which is (iii).
