...
probability at least $c_{k,\lambda}$ under a uniformly random induced embedding of
...
For the planted-mass bound, every ordered choice of one vertex from each labelled clone
class $H_{\theta(i)}$ is an induced copy of $\sigma$: the labelled clone classes are
distinct and their mutual adjacencies are inherited from $(G,\theta)$.  So the planted
count $\prod_i s_{\theta(i)}$ and its lower bound are unchanged.  By the same token an
induced copy of $\sigma$ already arises from any transversal of $k$ labelled clone classes
spanning an induced $\sigma$ in $G$, so the positive lower bound on the $\sigma$-type
...
many induced copies of the type in $B_j$: for $k>0$, choosing one vertex from each
labelled clone class gives $j^k=(1/n)^k|V(B_j)|^k$ ordered induced copies of $\sigma$,
and for $k=0$ the empty type has density $1$.  Thus the $\sigma$-density of $B_j$ is
...
independent of $j$, for the probability that a uniformly random induced embedding of
$\sigma$ into $B_j$ is one of these planted embeddings.

Let $Y_\sigma=[0,1]^{\F^\sigma[\T_{\mathrm{Graph}}]}$, and let $P_j$ be the pushforward
to $Y_\sigma$ of the uniform measure on induced embeddings
...
We now turn from global closure hypotheses to a local question: can a specific labelled
witness be planted without requiring the whole class to be closed under blow-ups?  In the
proof of \cref{thm:blowup-root-plantable}, the starting point was a constrained labelled
limit $\psi\in Q_\sigma$, approximated by large finite $\sigma$-flags $(G,\theta)$ in the
class.  The proof then made the roots in such a finite witness typical by a global
operation: blow up the vertices of $G$ while staying inside the class.  The planted copies
of the labelled vertices form a positive-density set of root placements, and the densities
of flags of any fixed size still look almost like they did in $(G,\theta)$.  In other
words, a view that was visible only by hand-picking the roots has been made visible to
random roots.
...
is more local: after planting the roots, one can repair the few dangerous edges and still
change so little of the graph that no fixed flag-density test notices.  The purpose of
this section is to isolate exactly the finite statement that such a repair must prove.
The payoff is \cref{thm:finite-local-planting}: this finite planting property already
implies root-plantability.

The definition below says the following.  Fix a resolution $m$ and a tolerance
$\eps$: we are only asking to preserve the densities of flags with at most $m$ vertices,
up to error $\eps$.  Then every sufficiently large labelled witness $(G,\theta)$ in the
class should have a larger in-class graph $H$ containing many good placements of the
roots.  ``Many'' means a positive proportion on the $|V(H)|^k$ scale, encoded by the
constant $\delta$; this is what will become positive probability under random labelling.
For every good placement, all $\sigma$-flag statistics involving at most $m$ vertices are
nearly the same as in the original witness.  Thus the quotient view of $(G,\theta)$ can be
made typical without assuming any global blow-up closure of the class.
...
\begin{remark}
The closure theorems above prove finite local planting by
