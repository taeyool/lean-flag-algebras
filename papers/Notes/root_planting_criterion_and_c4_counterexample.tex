...
\[
  ...
  \{\chi\in X_\sigma:
    |\chi(F_i)-\psi(F_i)|<\eps\text{ for all }i\}.
\]
Write $k=|\sigma|$ for the number of labelled vertices, and pick an integer $m\ge k$
bounding the sizes of the $F_i$.
Choose $n_0=n_0(m,\eps/3)$ and $\delta=\delta(m,\eps/3)$ supplied by the finite
local planting property.  Also, choose a convergent sequence of $\sigma$-flags
...
Now apply the finite local planting property to each such $(G_t,\theta_t)$, with the integer
...
(\cref{thm:clone-root-plantable,thm:true-clone-root-plantable}) verify the finite local
planting property by taking $\Theta$ to be the embeddings obtained by choosing one vertex
from each labelled clone class.  The one-root and two-root non-edge $C_5$-free constructions
