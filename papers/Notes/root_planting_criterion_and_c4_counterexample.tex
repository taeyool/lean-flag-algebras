...
$\Homp(\A^0[\T],\R)$, while a \emph{$\sigma$-labelled} or \emph{$\sigma$-rooted limit} is
a point of $\Homp(\A^\sigma[\T],\R)$.  We freely pass between the two names: $\phi$ may be
called a limit or a positive homomorphism, and $\phi(F)$ is the density of the flag $F$ in
that limit.  On a single flag this value lies in $[0,1]$: for each size the flags of that
...
\]
If $k=0$, set $w_v=1/n$ for all $v\in V(G)$.  For an integer $N\ge n$, choose
positive integer clone-class sizes $|C_v|$ summing to $N$, and let
$B_N(G,\theta,\lambda)$ be the corresponding independent blow-up.  The estimates
below measure how close these choices are to the target proportions $w_v$ by
\[
  \operatorname{err}_N
  :=
  \sum_{v\in V(G)}\left|\frac{|C_v|}{N}-w_v\right|.
\]
For fixed $G,\theta,\lambda$, the sizes can be chosen, for instance by rounding
the numbers $w_vN$, so that $\operatorname{err}_N\to0$ as $N\to\infty$.
...
that the following holds for every $0<\lambda\le 1/2$.  Let $F_0$ be any
$\sigma$-flag with at most $m$ vertices, let $(G,\theta)$ be a $\sigma$-flag on
$n$ vertices with $n\ge\max\{m,k+1\}$, and let
$B_N=B_N(G,\theta,\lambda)$ be a planted blow-up with positive clone classes of
total size $N\ge n$.  If
...
The size assumptions have two roles.  First, $|F_0|\le m\le n$, so both
$p(F_0,(G,\theta))$ and $p(F_0,(B_N,\widehat\theta))$ are defined.  Second, the
clone classes are positive and have total size $N$, so $N\ge n$; this is used in
...
probabilities are equal.  Hence assume $q>0$.  To compute
...
from the blow-up to $G$ preserves the induced rooted flag.  The only remaining
...
It remains to compare what happens on the good event with the rooted density in
the base flag $(G,\theta)$.  Let
\[
  \pi:B_N\to G
\]
...
for some constant $K_m^{(4)}$ depending only on $m$,
...
is exactly the adjacency of their two base vertices.  Therefore the rooted flag
seen in $(B_N,\widehat\theta)$ is the same as the rooted flag seen in
...
With the notation above, consider the following finite random labelling
experiment on the unlabelled graph $B_N(G,\theta,\lambda)$: choose uniformly from
all induced embeddings $\widehat\theta:\sigma\to B_N(G,\theta,\lambda)$, where
the labels of $\sigma$ are kept in their prescribed order.  If either $k=0$, or
$k>0$ and $\operatorname{err}_N\le\lambda/(2k)$, then this experiment gives
...
\]
Finally take a sequence $N_j\to\infty$ with rounding errors tending to zero and,
for all large $j$, satisfying
\[
  C_m\operatorname{err}_{N_j}<\eps/10,
  \qquad
  \text{and, if }k>0,\quad \operatorname{err}_{N_j}\le\lambda/(2k);
\]
put
\[
...
random-labelling distribution on $Y_\sigma$ obtained by choosing a random induced
embedding of $\sigma$ into $B_j$ and recording the resulting labelled flag
densities.  By the finite-graph form of the random-extension theorem, these
measures converge weakly to $\Ext_\sigma(\phi_0)$, viewed as a probability
measure on $Y_\sigma$ concentrated on $X_\sigma$.
...
large $j$, \cref{lem:planted-estimate} and the choice of $t,\lambda,N_j$ imply
that every planted root has all $F_i$-coordinates within $\eps/2$ of $\psi$,
hence lies in $\widetilde C$.  By \cref{lem:planted-mass},
\[
...
if the independent blow-up $B_N(G,\theta,\lambda)$ is replaced by the complete
blow-up $B_N^+(G,\theta,\lambda)$ with the same clone class sizes.
\end{lemma}
...
some random $\sigma$-extension of a constrained unlabelled limit, so
