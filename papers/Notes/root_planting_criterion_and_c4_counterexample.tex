...
\begin{proof}
We show that $\K$ has the finite local planting property at $\sigma$;
...
this $\lambda$ and $\rho$, so that $V(H)=U\sqcup R_1\sqcup\cdots\sqcup R_k$ with
...
Finally, compare the two sampling experiments.  Write $B_1$ for the first bad event (the
sample meets $R$) and $B_2$ for the second (the sample contains an altered pair); the two
estimates above give $\Prob[B_1]\le2mk\lambda$ and $\Prob[B_2]\le4m^2\rho$.  Let $A_H$ be the
event that the sample, together with the roots $\operatorname{im}\widehat\theta$, induces $F$
in $H$, so that $p(F,(H,\widehat\theta))=\Prob[A_H]$.  Conditioned on $B_1^c$ the sample
avoids $R$, hence lies in $U$, and is then a uniform $q$-subset of $U$---exactly the sample
defining $p(F,(G,\theta))$.  Accordingly couple the two experiments: draw the $H$-sample $S$
uniformly from the $q$-subsets of $W$, and take the $G$-sample to be $S$ on $B_1^c$ and an
independent uniform $q$-subset of $U$ on $B_1$.  Writing $A_G$ for the event that the
$G$-sample, with the roots $\operatorname{im}\theta$, induces $F$ in $G$, we then have
$p(F,(G,\theta))=\Prob[A_G]$, and the two samples agree on $B_1^c$.  On $B_1^c\cap B_2^c$
the samples agree, lie in $U$, and span no altered pair, so $H$ and $G$ agree on them;
together with conditions (i) and (ii) of \cref{def:sparse-root-repair}---which make
$\operatorname{im}\widehat\theta$ induce $\sigma$ and match its adjacencies to $U$ with those
of the original roots $\operatorname{im}\theta$---the induced $\sigma$-flags coincide, so
$A_H$ and $A_G$ occur together.  Thus $A_H$ and $A_G$ differ only within $B_1\cup B_2$, and
\[
\begin{aligned}
...
This is the finite local planting property at $\sigma$.
\end{proof}
