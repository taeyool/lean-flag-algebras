...
Fix the constants first.  Let $C_m$ be a constant, depending only on $m$ (and so on
$k$), large enough for the two sampling estimates used for (iii) below.  Choose
...
$H\setminus\operatorname{im}\widehat\theta$.  Two events are bad.  First, the sample
may hit one of the sets $R_i$; since $|R_1\cup\cdots\cup R_k|=kL=O_k(\lambda n)$ and
$|V(H)|\ge n-k$, this has probability $O_m(\lambda)$.  Second, once the sample lies
in $U$, it may contain both endpoints of one of the at most $\rho n^2$ old pairs on
which $H[U]$ differs from $G[U]$, which has probability $O_m(\rho)$.  Outside these
events the sampled vertices all lie in $U$, no repaired old edge is seen, the
adjacencies from the images of $\widehat\theta$ to $U$ agree with those from the
original roots in $G$, and the adjacencies among those images agree with $\sigma$;
moreover, conditional on the sample lying in $U$ it is a uniform $q$-subset of $U$,
exactly as in $p(F,(G,\theta))$.  Taking $C_m$ to bound both $O_m$ constants
therefore gives
\[
  \left|p(F,(H,\widehat\theta))-p(F,(G,\theta))\right|\le C_m(\lambda+\rho)<\eps ,
\]
which is (iii).
