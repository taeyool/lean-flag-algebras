...
copy of $\sigma$ singled out inside it---informally, a finite graph in which $k$ marked
vertices form the pattern $\sigma$.  The \emph{empty type}
...
$\Homp(\A^\sigma[\T],\R)$.  We say ``roots'' for the
distinguished labelled vertices and ``labelled'' for the corresponding flags and limits, and use
...
random order, induce a copy of $\sigma$ respecting the labels.
...
meaning, namely containing no $C_5$ as a (not necessarily induced) subgraph.  Equivalently, in the
...
\(\sigma\), element \(g\in\A^\sigma[\T_{\mathrm{Graph}}]\), and constant \(c\),
when any constrained unlabelled limit is rooted at random as a \(\sigma\)-flag,
the resulting labelled homomorphism evaluates \(g\) as \(c\) almost surely,
even though a specially chosen labelled limit can evaluate \(g\) differently.
...
labelled edge, so the density of the labelled edge flag is pinned to \(0\)
under random rooting.  The class may still contain finite graphs with an
\emph{exceptional} high-degree vertex, such as the centre of a star.  The labelled edge flag
...
$e\in\A^{\vtype}[\T_{\mathrm{Graph}}]$ be the labelled edge flag (the
two-vertex flag in which the labelled vertex is adjacent to the one unlabelled vertex), and
...
$Q_{\vtype}$, a labelled limit of positive labelled edge density; it may be replaced by the
...
$\overline{K_{1,N-1}}=K_{N-1}\uplus K_1$.  Then $\K$ is not root-plantable at
$\sigma=\vtype$, witnessed by $f:=e-\one_{\vtype}$ (with \(\one_{\vtype}\)
the unit of $\A^{\vtype}[\T_{\mathrm{Graph}}]$).  In
...
root-plantable at \(\vtype\), and this is \cref{thm:degenerate-obstruction}:
$\overline\K$ is edge-degenerate and contains all stars $K_{1,N-1}$.  The witness
$-e$ for $\overline\K$ pulls back to $-(\one_{\vtype}-e)=e-\one_{\vtype}$ for $\K$.  Directly:
$\avg{e}{\vtype}=\rho$ gives $\E[\phi^{\vtype}(e)]=\phi_0(\rho)=1$, and
$\phi^{\vtype}(e)\le1$ forces
$\phi^{\vtype}(e)=1$ almost surely, so $e-\one_{\vtype}$ is non-negative almost surely under every
admissible one-root random extension; while the co-star labelled at its isolated vertex
gives a $\psi\in Q_{\vtype}$ with $\psi(e)=0$, whence $\psi(e-\one_{\vtype})=-1<0$ and quotient
semantics fails.
\end{proof}
...
$C_5$-free conjecture is false (\cref{sec:c5-edge}), even though the positive
...
counterpart (\cref{cor:codegenerate}) are exactly these, with $\sigma=\vtype$ and
...
\subsection{Boundary pinning for subgraph-forbidden classes}\label{subsec:boundary}
...
induced $\sigma$-density is positive on the set of tuples where every $\sigma$-edge has
$W>0$ and every $\sigma$-non-edge has $W<1$, a set of positive measure since $\phi_0$ has
positive $\sigma$-density; so $\phi_0^{\lambda}$ can be labelled by $\sigma$.
...
labelled triangle flag has density $0$ on \emph{all} of $Q_{\vtype}$: it is pinned at $0$, but
...
$\phi_0(\avg{g}{\tau})=0$.  By \eqref{eq:extension-expectation},
\[
  \E[\phi^\tau(g)]
  =
...
$\tau$.  Concretely,
\[
  \Prob_{\phi^\tau\sim\Ext_\tau(\phi_0)}[\phi^\tau(-g)\ge0]=1
  \qquad\text{for every }\phi_0\in Q_0\text{ with }\phi_0(\brak{\tau})>0,
\]
but $-g$ is negative at some point of $Q_\tau$.
\end{theorem}
...
This is precisely a pinning obstruction, so the characterisation conjecture (\cref{conj:characterisation})
...
The obstructions above all occur at non-empty labelled types.  A \emph{pinning}
obstruction means that some labelled flag \(g\) has a fixed value \(c\) in
almost every random labelled view of every constrained unlabelled limit, even
though a specially chosen labelled quotient limit can give \(g\) a different
value.  Algebraically, the corresponding witness is \(g-c\,\one_\sigma\) or
\(c\,\one_\sigma-g\): it vanishes on the ensemble support but not on all of
...
the empty-type algebra $\A^0[\T_1]$---a statement $f\ge0$ on $Q_0$, such as a
...
reasons: at the empty type the two semantics coincide, and the witnesses that
...
particular the witnesses $-e$ and $e-\one_{\vtype}$ are inequalities at $\sigma=\vtype$, not density
bounds.
\end{corollary}
...
If $\K$ is edge-degenerate, or co-edge-degenerate, then $S_{\vtype}$ is a single point.
Consequently $\C^{\mathrm{ens}}_{\vtype}=\C^{\mathrm{quot}}_{\vtype}=\R_{\ge0}\,\one_0$: at the
...
determined by the chain relations), so $S_{\vtype}=\{o\}$.  On a single point every element
non-negative there agrees with the constant $s(o)\,\one_{\vtype}$, a sum of squares, so
$\avg{s}{\vtype}=s(o)\,\brak{\vtype}=s(o)\,\one_0$ for every such $s$; both cones equal
$\R_{\ge0}\,\one_0$.
\end{proof}
...
\cref{thm:no-interior} pinning is \emph{always} to a boundary value.  The $C_5$-free class
illustrates both possibilities at once: the one-vertex and
...
non-degenerate edge type $\tau$.  The pinned witness $g$ (the triangle $\tau$-flag) is even
more inert: by \cref{prop:ideal-zero}, $g$ and all its flag-multiples unlabel to zero, since
$g=0$ on $S_\tau$ (\cref{cor:c5-edge-pinned}).
\end{proof}
...
balanced complete tripartite graphon.  The slice is moreover non-empty---the balanced
...
$e=(r-1)\one_{\vtype}/r$.  A random ordered edge has its endpoints in two distinct parts; a new
...
\begin{enumerate}[label=\textup{(\arabic*)}, leftmargin=2.8em]
  ...
  already sufficient for root-plantability.  The $C_5$-free class sharpens the test:
