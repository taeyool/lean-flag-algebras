...
analogue, and how does it behave on slices?  Do labelled multipliers that are
non-negative merely on the relative support prove more density consequences over the
slice than sums of squares do?  And is every density inequality valid on a slice
...
multiplier system, and one squared penalty in the constraints is the only genuinely new
ingredient a slice certificate ever needs.
...
\]
Thus, even over a slice, labelled multipliers non-negative merely on the relative
support prove no density consequence beyond sums of squares, except possibly at the
exact, non-closed certificate level.
\end{theorem}
...
\cref{thm:relative-certificate-gap} the labelled multiplier side over the slice
collapses, after closure, to ordinary sums of squares.  Together: every density
...
constraints $\phi_0(h)\ge0$, by contrast, genuinely require localised multipliers in
the Positivstellensatz style; equality constraints cover every slice used in this
paper.
\end{remark}
