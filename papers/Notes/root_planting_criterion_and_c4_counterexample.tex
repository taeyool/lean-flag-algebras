...
this section: they are finite rooted graphs whose $\sigma$-flag densities approximate the
limiting view $\psi$.
...
dangerous edges.  If the repair changes so little of the graph that the $\sigma$-flag densities up to the
...
Every finite $\sigma$-flag $(G,\theta)$ in $\K$ with $|G|\ge n_0$ admits a graph
...
\[
  C_m(\lambda+\rho)<\eps .
\]
Let $n_0$ be the maximum of $m$, $2k$, $\lceil2/\lambda\rceil$, and the threshold from
