...
certificate to confirm the strings map to constants that exist; because it
resolves every string, it doubles as a validity check (it raises on the
first string that fails to resolve).
...
\paragraph{Workflow.}
\begin{lstlisting}[language={}]
inspect  →  gen-skeleton  →  lake build
\end{lstlisting}
\texttt{inspect} catches identifier problems before any Lean compilation.
...
by name. \emph{(This $K_n$ gating was then dropped entirely --- see ``Going
further'' below.)}
...
\paragraph{Going further --- the loaders are not $K_n$-specific.} Nothing in
either loader's generated proof uses completeness of $K_n$: the density proof is
\btt{rw [<tag>_toFinFlag_eq]; ...; native_decide} and the multiplication proof is
\btt{basisVector_quot_mul_forbidEq_sum <graph>.toFinFlag ...}, both valid for any
forbidden graph. So the \btt{"K<r>"} tag parsing above was a restriction, not a
requirement --- and since the JSON tag already \emph{is} the Lean identifier
(\btt{"K3"} names \btt{def K3}), even the parsing was unnecessary. A helper
\btt{resolveForbidGraph} now maps the tag \emph{directly} to its identifier:
tag \btt{C4} resolves to \btt{def C4} and \btt{C4_toFinFlag_eq}, with the
canonical index read off that lemma. The \btt{"K<r>"} parse is kept only to hand
back a clique-specific hint (\btt{generate_complete_graph r ...}) when a $K_r$
definition is missing; every other tag gets the generic ``define \btt{def <tag>}
and \btt{<tag>_toFinFlag_eq}'' hint. On the Python side a \btt{--tag C4} option
gives the raw \btt{--forbid <flagmatic>} path the clean identifier-matching tag
it previously lacked.

\paragraph{Net effect, generalized.} A non-clique forbid $X$ (e.g.\ $C_4$,
$P_4$) now follows the \emph{same} short path as a clique: compute the data with
\btt{--forbid <X> --tag X}, and add \btt{def X} plus its \btt{X_toFinFlag_eq}
(carrying $X$'s canonical index) to \texttt{CommonGraphs.lean}. The loaders need
no edit for \emph{any} forbid --- the lone remaining per-graph datum is the
index, which is genuine information about $X$. $K_3$ and $K_4$ were re-verified
green (Mantel, K4turan) after this change.
