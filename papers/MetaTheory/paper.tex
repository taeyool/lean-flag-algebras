...
\emph{clone part} of positive density---a positive fraction of all the vertices, so that a
...
\[
  P\bigl(\{y\in X:h(y)\ge0\}\bigr)=1
\]
if and only if $h(x)\ge0$ for every $x\in\supp(P)$.
\end{lemma}

\begin{proof}
($\Rightarrow$) Suppose $P\bigl(\{y\in X:h(y)\ge0\}\bigr)=1$, equivalently
$P\bigl(\{y\in X:h(y)<0\}\bigr)=0$, and let $x_0\in\supp(P)$.  If $h(x_0)<0$,
then $\{y\in X:h(y)<0\}$ is an open set (as $h$ is continuous) containing $x_0$, so
by the defining property of the support it has positive $P$-measure, contradicting
$P\bigl(\{y\in X:h(y)<0\}\bigr)=0$.  Hence $h(x_0)\ge0$.

($\Leftarrow$) Suppose $h\ge0$ on $\supp(P)$.  Then
$\{y\in X:h(y)\ge0\}\supseteq\supp(P)$, and the support has full measure, so
$P\bigl(\{y\in X:h(y)\ge0\}\bigr)\ge P(\supp(P))=1$; that is,
$P\bigl(\{y\in X:h(y)\ge0\}\bigr)=1$.
\end{proof}

In applications, $X$ will be a homomorphism space, $f$ will be a fixed element of the
corresponding flag algebra, and $h$ will be the continuous evaluation map
$h_f(\chi):=\chi(f)$.  The lemma lets us replace almost-sure non-negativity under a random
extension by pointwise non-negativity on its support.
...
limits.  To describe $Q_\sigma$ without referring to the quotient, let
\[
  I_\sigma
  :=
  \operatorname{span}_{\R}
  \{F\in\F^\sigma[\T_0]:F\text{ is forbidden}\}
  \subseteq \A^\sigma[\T_0].
\]

\begin{lemma}[Kernel of the constrained quotient]\label{lem:forbidden-kernel}
The subspace $I_\sigma$ is an ideal of $\A^\sigma[\T_0]$, and
\[
  \ker\qmap_\sigma=I_\sigma.
\]
\end{lemma}

\begin{proof}
Let $F$ be a forbidden flag and let $G$ be any flag.  Every flag $H$ occurring with
non-zero coefficient in $F\cdot G$ contains the underlying graph of $F$ as an induced
subgraph.  If the underlying graph of $H$ belonged to $\T_1$, heredity would imply that
the underlying graph of $F$ belonged to $\T_1$, contradicting that $F$ is forbidden.
Thus every such $H$ is forbidden, and hence $F\cdot G\in I_\sigma$.  By bilinearity,
$I_\sigma$ is closed under multiplication by arbitrary elements of
$\A^\sigma[\T_0]$, so it is an ideal.

By construction, $\ker\qmap_\sigma$ is the ideal generated by the forbidden flags: forming
$\A^\sigma[\T_1]$ amounts to declaring precisely those flags equal to $0$.  Since their
linear span $I_\sigma$ is already an ideal, the ideal they generate is $I_\sigma$ itself.
Therefore $\ker\qmap_\sigma=I_\sigma$.
\end{proof}

\begin{proposition}[Intrinsic description of constrained limits]
\label{prop:constrained-intrinsic}
For $\chi\in X_\sigma$,
\[
  \chi\in Q_\sigma
  \quad\Longleftrightarrow\quad
  \chi(F)=0\text{ for every forbidden flag }F.
\]
Consequently, $Q_\sigma$ is closed in $X_\sigma$.
\end{proposition}

\begin{proof}
Suppose first that $\chi\in Q_\sigma$.  Then $\chi=\psi\circ\qmap_\sigma$ for some
$\psi\in\Homp(\A^\sigma[\T_1],\R)$.  If $F$ is forbidden, then
\[
  \chi(F)=\psi(\qmap_\sigma(F))=\psi(0)=0.
\]

Conversely, suppose that $\chi(F)=0$ for every forbidden flag $F$.  By linearity, $\chi$
vanishes on $I_\sigma$, which equals $\ker\qmap_\sigma$ by
\cref{lem:forbidden-kernel}.  By the universal property of the quotient, there is a unique
algebra homomorphism $\psi:\A^\sigma[\T_1]\to\R$ such that
$\chi=\psi\circ\qmap_\sigma$.  Every flag of $\A^\sigma[\T_1]$ has the form
$\qmap_\sigma(G)$ for a non-forbidden flag $G$, and
\[
  \psi(\qmap_\sigma(G))=\chi(G)\ge0.
\]
Hence $\psi\in\Homp(\A^\sigma[\T_1],\R)$ and $\chi\in Q_\sigma$.

For closedness, the characterization just proved gives
\[
  Q_\sigma
  =
  \bigcap_{F\text{ forbidden}}
  \{\chi\in X_\sigma:\chi(F)=0\}.
\]
For each flag $F$, the evaluation $\chi\mapsto\chi(F)$ is continuous in the topology of
pointwise convergence of flag densities (\cref{sec:background}), so each set in the
intersection is closed.  Therefore $Q_\sigma$ is closed in $X_\sigma$.
\end{proof}
...
\]
By \cref{lem:support-as}, ensemble semantics for $f$ is equivalent to
non-negativity of $f$ on every support in the union whose closure defines
$S_\sigma$.  Since $f$ is continuous, this is equivalent to
\[
  f\ge0\qquad\text{on }S_\sigma.
\]
Quotient semantics is exactly
\[
  f\ge0\qquad\text{on }Q_\sigma.
\]

Since $S_\sigma\subseteq Q_\sigma$, quotient semantics always implies ensemble
semantics.  If $S_\sigma=Q_\sigma$, the two conditions are equivalent for every $f$.

Now suppose $S_\sigma\ne Q_\sigma$, and choose
$\psi\in Q_\sigma\setminus S_\sigma$.  The sets $S_\sigma$ and $\{\psi\}$ are disjoint
closed subsets of the compact metrizable space $X_\sigma$.  By Urysohn's lemma, followed
by an affine rescaling, there is a continuous function $h\in C(X_\sigma)$ such that
$h=1$ on $S_\sigma$ and $h(\psi)=-1$.

The evaluation functions coming from $\A^\sigma[\T_0]$ form a unital real subalgebra of
...
$\A^\sigma[\T_0]$.  Choose $f\in\A^\sigma[\T_0]$ with
$\|f-h\|_\infty<1/2$.  Then $f>0$ on $S_\sigma$, so ensemble semantics holds for $f$,
while $f(\psi)<0$, so quotient semantics fails.  Thus equivalence for every $f$ implies
$S_\sigma=Q_\sigma$.
\end{proof}
...
closed under independent blow-ups, a root can be duplicated into a clone part
...
all finite simple graphs.  Let $\K\subseteq\T_{\mathrm{Graph}}$ be hereditary.  When
$\K$ plays the role of the restricted subclass $\T_1$, we denote it by $\T_\K$; this is
only a change of notation, so $\T_\K$ and $\K$ have exactly the same members.  Since
$\K$ is hereditary, it can equivalently be described by forbidding, as induced
subgraphs, all graphs in $\T_{\mathrm{Graph}}\setminus\K$.  Throughout this section we
take $\T_1=\T_\K=\K$.
...
each vertex $v$ by an independent set $C_v$ of $m_v$ clones.  For distinct vertices
$v,w\in V(G)$, every vertex of $C_v$ is joined to every vertex of $C_w$ if and only if
$vw\in E(G)$.  We call $C_v$ the \emph{clone part} over $v$.
\end{definition}
...
an independent blow-up uses at most one vertex from each clone part, and hence
...
clone-part sizes whose normalised sizes approximate these fractions.  Thus, for
...
for the clone part of $v$, so $|C_v|=m_v$.  The graph $B_N$ depends only on
...
Call an embedding $\widehat\theta:\sigma\to B_N$ \emph{planted} if
\[
  \widehat\theta(i)\in C_{\theta(i)}
  \qquad(1\le i\le k).
\]

\begin{lemma}[Planted blow-up approximation]\label{lem:planted-estimate}
For every integer $m\ge k$ there is a constant $L_m$, depending only on $m$,
such that the following holds.  For every choice of $(G,\theta)$, $\lambda$,
$N$, and $\mathbf m$ as above with $n\ge m$, every $\sigma$-flag $F_0$ with at
most $m$ vertices, and every planted embedding
$\widehat\theta:\sigma\to B_N$,
\begin{equation}\label{eq:planted-estimate}
  \left|p(F_0,(B_N,\widehat\theta))-p(F_0,(G,\theta))\right|
  \le
  L_m\left(\lambda+\frac1{n-k}+\operatorname{err}_N\right).
\end{equation}
Consequently, if $f\in\A^\sigma[\T_{\mathrm{Graph}}]$ is represented as
$f=\sum_r a_rF_r$ with $|F_r|\le m$ for every $r$, then
\[
  \left|p(f,(B_N,\widehat\theta))-p(f,(G,\theta))\right|
  \le
  L_f\left(\lambda+\frac1{n-k}+\operatorname{err}_N\right),
  \qquad
  L_f:=L_m\sum_r|a_r|.
\]
\end{lemma}
...
\]
Thus $R$ is the union of the clone parts attached to the roots
...
one of the root clone parts in $R$ or chooses two vertices from the same
unlabelled clone part.  Second, on the complementary good event, projection
...
unlabelled clone parts.  For each $v\in U$, write $s_v:=|C_v|$ and
$S:=\sum_{u\in U}s_u$.  Each exposed position then falls in the clone part
...
$X_b$ land in the same unlabelled clone part, still under the conditioning
that the whole sample avoids $R$.  If $X_a\in C_v$, then $X_b$ is uniform among
the remaining $S-1$ vertices in the unlabelled clone parts.  Therefore
\[
  ...
  \frac{s_v}{S}.
\]
Averaging over the possible clone part $C_v$ of $X_a$ gives the explicit bound
\[
  \begin{aligned}
  &\Prob[X_a,X_b\text{ lie in the same unlabelled clone part}
...
lie in the same unlabelled clone part.  Equivalently, for the ordered
exposure, $E_{\mathrm{coll}}$ is the union over pairs $a<b$ of the events that
$X_a$ and $X_b$ land in the same clone part.  A second union bound over the at
...
unlabelled clone part.  Combining the two bounds above,
\[
  \Prob[\text{bad}]
  \le
...
\[
  \pi:B_N\to G
\]
be the projection sending each clone part $C_v$ to its base vertex $v$.  On the
...
clone part, and $Z_\mu$ is the normalising constant after restricting to
...
two sampled unlabelled vertices lie in distinct clone parts, so their adjacency
...
\end{aligned}
\]
Taking the lemma's constant $L_m$ larger than $K_m^{(3)}+K_m^{(4)}$ proves
\eqref{eq:planted-estimate}.  The final assertion follows by applying this
estimate to each $F_r$ and using linearity and the triangle inequality.
\end{proof}
...
parts $C_v$, target weights defined from $(G,\theta)$ and $\lambda$, and error
...
\[
  ...
  \end{cases}
\]
to the event that the chosen embedding is planted.
\end{lemma}
...
$\operatorname{err}_N\le\lambda/(2k)$, each planted clone part satisfies
\[
  \frac{|C_{\theta(i)}|}{N}
  \ge
  ...
  \frac{\lambda}{2k}.
\]
The clone parts $C_{\theta(1)},\ldots,C_{\theta(k)}$ are distinct, since
...
Choose $0<\lambda\le 1/2$ so small that $L_m\lambda<\eps/10$, where $L_m$ is the
...
\[
  L_m\operatorname{err}_{N_j}<\eps/10
  \qquad
  \text{and}
  \qquad 
...
large $j$, write $C_v^{(j)}$ for the clone part of $v\in V(G_t)$ in $B_j$, and
...
\[
  ...
  \le
  L_m\left(\lambda+\frac1{|G_t|-|\sigma|}+\operatorname{err}_{N_j}\right)
  <\frac{3\eps}{10}
\]
...
by an independent clone part, exactly as in the independent blow-ups of
...
blow-up $B_N^+=G^{\mathbf m,+}$ with the same clone-part sizes.
\end{lemma}
...
blow-up $B_N=G^{\mathbf m}$ using the same clone parts $C_v$ and the same
projection map $\pi:C_v\mapsto v$.  The only difference between the two finite
graphs is internal to the clone parts: in $B_N$ each $C_v$ is independent,
whereas in $B_N^+$ each $C_v$ is a clique.  All edges between distinct clone
parts are governed by the same base graph $G$.
...
one of the root clone parts or if two sampled vertices lie in the same
unlabelled clone part.  The probability of this bad event depends only on the
sizes of the clone parts and on sampling without replacement.  The two
...
parts are never observed.  Indeed, no sampled vertex lies in a root clone
part, and no two sampled vertices lie in the same unlabelled clone part.
Thus every adjacency involving a sampled vertex is an adjacency between two
distinct clone parts, hence is exactly the adjacency of the corresponding base
vertices in $G$.  The planted roots also lie in distinct clone
parts, because $\theta$ is injective; since $\theta$ is an embedding of
...
$|C_v|$ and not the edge relation inside any clone part.  The normalisation
...
\[
  ...
  L_m\left(\lambda+\frac1{n-k}+\operatorname{err}_N\right),
\]
with the same constant $L_m$.  The extension from a single flag $F_0$ to a
...
$\sigma$ into $B_N^+$.  Every choice of one vertex from each root clone part
$C_{\theta(i)}$ gives an embedding of $\sigma$: the root clone parts are
distinct, and all adjacencies between them are inherited from the base flag
$(G,\theta)$.  Since the clone parts are distinct, these choices are independent:
...
$0<\lambda\le1/2$ so small that $L_m\lambda<\eps/10$, where $L_m$ is the
...
\[
  B_j^+:=G_t^{\mathbf m^{(j)},+}.
\]
Write $C_v^{(j)}$ for the clone part of $v\in V(G_t)$ in $B_j^+$.  An embedding
...
\[
  |p(F_i,(B_j^+,\widehat\theta))-p(F_i,(G_t,\theta_t))|
  \le
  L_m\left(\lambda+\frac1{|G_t|-|\sigma|}
...
individual clone part is a clique, and all edges between clone parts over
adjacent vertices of $D_a$ are present.  No edges are added between clone
parts lying over different components.  Thus $G^{\mathbf m,+}$ is again a
...
$\K$, but its independent blow-up with both clone parts of size $2$ is
...
in what is put inside each clone part.  Inspecting the estimates shows that this
interior is never observed.  On the good sampling event, the sampled vertices lie in
distinct clone parts, so every adjacency that matters is between distinct parts and is
governed by the base graph $G$ alone
...
$\sigma$ into $B$.  Here $L_m$ and $c_{k,\lambda}$ are the constants from
...
a root clone part and that no two sampled vertices lie in the same clone part.  On
this event every sampled vertex lies in a distinct clone part, so every adjacency
involving a sampled vertex is an adjacency between two \emph{distinct} clone parts and
...
For the planted-mass bound, every choice of one vertex from each root clone part
$H_{\theta(i)}$ gives an embedding of $\sigma$: the root clone parts are distinct
...
embedding of $\sigma$ already arises from any transversal of $k$ root clone parts
...
\[
  ...
  \frac{L_m}{n-k}<\eps/10.
\]
If $k>0$, also choose $t$ so large that $k/n\le1/2$ and $L_m k/n<\eps/10$, and set
$\lambda:=k/n$.  If $k=0$, choose any $0<\lambda\le1/2$ with
$L_m\lambda<\eps/10$.  This is the small point hidden by the phrase ``uniform blow-up'':
for $k>0$, uniform clone parts have normalised weight $1/n$ at every vertex, exactly
...
\]
Thus the normalised clone-part sizes are all $1/n$.  With the choice of $\lambda$ above,
...
clone part gives $j^k=(1/n)^k|V(B_j)|^k$ embeddings of $\sigma$, and for $k=0$ the
...
\[
  |p(F_i,(B_j,\widehat\theta))-p(F_i,(G_t,\theta_t))|
  \le L_m\left(\lambda+\frac1{n-k}\right)<\frac{2\eps}{10}
\]
...
roots then have large clone parts, and choosing one vertex from each such
part gives a positive-density set of embeddings $\sigma\to H$.  For these embeddings, the
...
from each root clone part.  The one-root and two-root non-edge $C_5$-free constructions
...
Fix the constants first.  Set $D_m:=4m^2$.  Choose
...
\end{aligned}
\]
Since $D_m=4m^2$ bounds both $2mk$ (as $k\le m$) and $4m^2$, the right-hand side is at most
$D_m(\lambda+\rho)$, which by the choice of $\lambda$ and $\rho$ is $<\eps$.  This is (iii).
...
independent blow-up of $K_2$ into two clone parts of size $2$ is $K_{2,2}=C_4$.
