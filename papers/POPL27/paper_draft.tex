...
size is at least \(F_1.1 + F_2.1 - n_0\): the non-type vertices of the two
flags are sampled disjointly, while the \(n_0\) type vertices are shared.  The
theorem then says that any two such admissible output sizes give equivalent
flag vectors, justifying the choice of the minimal size in \lean{flagMul}.
