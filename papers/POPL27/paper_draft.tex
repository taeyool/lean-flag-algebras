...
$G[S]$ is isomorphic to a member of $\mathcal{H}$.  All types, flags, and flag
...
that $\phi$ determines.
Part (a) follows by verifying that the limit satisfies the algebraic properties
of a positive homomorphism.  Part (b) is more demanding: its proof proceeds by sampling a random flag sequence according to $\phi$
and showing that the sequence converges to $\phi$ almost surely.  Measure-theoretic arguments are among the harder proofs to formalize in Lean.
They require explicit measurability proofs that informal arguments routinely omit, as
well as careful conversion between real-valued, non-negative, and extended-real
numeric types.

Lastly, positive homomorphisms let us define the semantic non-negativity cone
$\mathcal{C}^\sigma$ and the order it induces on \lean{FlagAlgebra} $\sigma$.
\lean{semanticCone} $\sigma$ is the set of elements in \lean{FlagAlgebra} $\sigma$ that every positive
homomorphism evaluates non-negatively, and we define $f \leq g$ to mean
$g - f \in \mathcal{C}^\sigma$ via a Lean \lean{LE} instance. \gw{I'm here.}
\begin{lstlisting}
