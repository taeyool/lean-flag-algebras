...
together with a certificate-to-proof compiler that reconstructs, in Lean,
...
used only to suggest finite data: lists of flags, target-density data, and
rational positive-semidefinite matrices.  Lean independently computes the
required density and multiplication facts, proves that these computations
agree with the mathematical definitions, verifies positive-semidefiniteness
exactly over $\mathbb{Q}$, and automates the algebraic normalization steps
that occur in flag-algebra proofs.  As case studies, the compiler yields machine-checked
...
The design of the formalization also led us to new theoretical results about
constrained flag algebras.  We distinguish the built-in-constraint approach,
which builds a hereditary graph constraint, such as \(H\)-freeness,
into the
flag algebra from the start, from the ensemble approach, which
imposes constraints afterwards by testing inequalities on random labeled views
of constrained graph limits.  The two approaches agree for important classes,
but not in general for all densities of finite labeled patterns.  We prove a
root-plantability criterion characterizing when they do agree, establish a
positive result for blow-up-closed classes that also covers
substitution-closed classes, and exhibit a counterexample based on
\(C_4\)-free graphs.  This
...
The Lean development of this meta-theory was generated in a separate
Claude-driven autoformalization workflow, building on the manually developed
library reported in the main part of the paper.
\end{abstract}
...
takes a limit; we call this the \emph{Tur\'an density} of $F$ with
...
for exactly this setting: it reduces the infinitely many graphs one has to
reason about to a finite semidefinite-programming certificate.  A
semidefinite-programming (SDP) solver searches for that certificate, and the
flag algebra framework explains why it implies an asymptotic bound for the
whole graph class.
...
\paragraph{From certificates to formal proofs.}
Stating the method is not the same as checking its applications.  A certificate
found by an SDP search is compact to write down but large to audit: the
...
that \(\phi_0(\langle\sigma\rangle_0)>0\).  By the notation introduced in
\Cref{sec:background-downward}, \(\langle\sigma\rangle_0\) is the empty-type
basis element represented by the underlying graph of \(\sigma\), so the
hypothesis says that this graph has positive density under \(\phi_0\).  Under
this hypothesis Lean constructs a probability measure
\(\mathbb{P}\) on \lean{PositiveHomSpace} with argument \(\sigma\).  This
\(\mathbb{P}\) is the formal
random extension \(\Ext_\sigma(\phi_0)\): it is the measure obtained by
...
a random positive homomorphism drawn from this measure.  The
displayed theorem states the defining identity of that measure:
\begin{lstlisting}
...
downward operator.  It is a positive scalar multiple of
\(\phi_0(\langle\sigma\rangle_0)\), so it is positive under the hypothesis
above.  Thus the theorem gives a concrete way to compute the unlabeled value
...
values, and multiply by the positive normalizing denominator.
...
the expectation of \(\phi(f)\) over \(\phi\sim \mathbb{P}\).  Since~\(f\in\mathcal{C}^\sigma\), every typed positive homomorphism \(\phi\) has
