...
of that equivalence, quotient inequalities always imply ensemble
inequalities, so every typed inequality valid in Razborov's presentation
remains valid in ours.  The case studies themselves depend on neither
direction: their typed \lean{forbidLE} derivations are proved directly in
the ensemble semantics and are ultimately transferred to final unlabeled
Tur\'an bounds.
...
the use of \lean{HEq} arises because pointwise-equal dependent
