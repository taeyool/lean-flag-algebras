...
size is at least \(F_1.1 + F_2.1 - n_0\): the unlabeled vertices outside the flag
...
enumeration of the \(n\)-vertex flags of that flag type.  Both enumerations are in
...
flags of the flag type selected by \(k\) and \(m\); here \(k=1,m=0\) is the unique flag
...
isomorphism and preserved by deleting a vertex, so the proof of the
completeness lemma descends through the size induction; a proved bridge then
equates the
...
form (\Cref{def:ensemble-semantic-order}); the two agree at the empty flag type
...
expression exactly when the constrained class is root-plantable at that flag type.
