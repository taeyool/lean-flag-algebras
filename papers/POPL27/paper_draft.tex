...
the difference between the two sides of the expansion identity is one of the
generators of the zero space, so the identity holds
...
\lean{FlagList}~$\sigma$~\lean{t Vl} assigns to each index \lean{i : Fin t}
a $\sigma$-flag on the vertex type \lean{Vl i}, and \lean{flagListDensity}
...
def FlagList.permute {(*@$\sigma$@*) : FlagType T} {t : (*@$\mathbb{N}$@*)} {Vl : Fin t (*@$\to$@*) Type}
...
\lean{flagListDensity}, so the second statement renders the symmetry from
...
\lean{Vl 0 = U}$_0$ and \lean{Vl 1 = U}$_1$, so that the pair
...
with \lean{Vl' 0 = U}$_1$ and \lean{Vl' 1 = U}$_0$.  The families \lean{Vl'}
and \lean{listTypePermute Vl $\pi$} are pointwise equal, since both send $0$
...
same, and ordinary equality between the two tuples is not available until
the equality of families has been made explicit.  What remains to be proved
is that two Lean terms for the same reordered pair, the permuted tuple and
\lean{[F}$_1$\lean{, F}$_0$\lean{]}$^{\mathtt{f}}$, agree; on paper the two
terms are not even distinguishable.

The bridging lemma \lean{flagListDensity\_HEq\_eq} states that this
agreement suffices: if the two vertex-type families are equal and the two
tuples agree, then the two densities coincide.
\begin{lstlisting}
theorem flagListDensity_HEq_eq
    {Fl : FlagList (*@$\sigma$@*) t Vl} {Fl' : FlagList (*@$\sigma$@*) t Vl'}
    (h_Vl_eq : Vl' = Vl) (h_HEq : HEq Fl Fl') (G : Flag (*@$\sigma$@*) W)
    : flagListDensity Fl G = flagListDensity Fl' G := ...
\end{lstlisting}
The hypothesis \lean{h\_Vl\_eq} is the equality of the two vertex-type
families; in the instantiation
it is \lean{Vl' = listTypePermute Vl $\pi$}, established by checking the two
indices.  The hypothesis \lean{h\_HEq} states the agreement of the tuples
themselves.
Given an equality of types $A=B$, a term of type $A$ can be cast to
type~$B$ without changing the underlying mathematical object.
Heterogeneous equality, written \lean{HEq $x$ $y$}, allows~$x:A$ and~$y:B$
to be compared even when $A$ and $B$ are not definitionally the same; it
asserts that the two terms become equal once one is cast to the type of the
other.  In the instantiation, \lean{h\_HEq} takes the form
\lean{HEq ([F}$_0$\lean{, F}$_1$\lean{]}$^{\mathtt{f}}$\lean{.permute $\pi$) [F}$_1$\lean{, F}$_0$\lean{]}$^{\mathtt{f}}$,
and it holds because casting each entry of
\lean{[F}$_1$\lean{, F}$_0$\lean{]}$^{\mathtt{f}}$ into the corresponding
entry type of the permuted tuple recovers, entry by entry, the permuted
tuple.
...
flag algebras} introduced in their accompanying papers~\cite{davey2026local}.
The local calculus normalizes densities by the maximum degree rather than by
the number of vertices, so that the method remains informative for graphs of
bounded maximum degree.  Under this normalization the exact density-expansion
identity behind the classical quotient construction holds only up to an
$O(1/\Delta)$ error, so no quotient is taken: their algebra is defined
directly on formal combinations of isomorphism classes, evaluation at a
finite graph is multiplicative only asymptotically, and positivity is again
captured by a semantic cone.  Their formalization
machine-certifies the new bounds of those papers, on pentagon counts in
triangle-free graphs and on the strong chromatic index, and arrives
independently at the reflection pattern of \Cref{sec:reflection}: a
...
immediately, extending the \lean{decide +kernel} path that already covers
the other five generated files to the two remaining \lean{native\_decide}
certificates, or replacing them with a formally verified external checker,
would eliminate the remaining dependency on the native compiler.  Its present use is a
