...
whereas in $b$ they are not.  For every positive homomorphism
$\phi\in\poshom{\sigma}$,
\[
  \phi\bigl((a-b)^2\bigr)
  = \phi(a-b)\,\phi(a-b)
...
the three triangle-free graphs on three vertices.  Expanding the unit and the
edge $K_2$ at size three, and omitting the triangle term that has density zero
under the $K_3$-free condition, gives
\[
...
$(a-b)^2=a^2-2a\cdot b+b^2$.  In a rooted three-vertex graph, if the two unlabeled
...
vertex in the $a$ position), so $-2a\cdot b$ gives coefficient $-1$.  The downward
operator averages these coefficients over the three possible choices of the
labeled vertex.  For $G_0$, all three choices give $+1$; for each of $G_1$ and
$G_2$, one choice gives $+1$ and two give $-1$.  Including the factor $1/2$
therefore yields
\[
...
Theorem~\ref{thm:downward-nonneg}, and $G_1$ is non-negative because graph
densities are non-negative.  Hence the right-hand side is non-negative, proving
