...
def sym2FlagDensity(*@$_1$@*) {k m n : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k}
    (F : Sym2LabeledGraph (*@$\sigma$@*) m) (G : Sym2LabeledGraph (*@$\sigma$@*) n)
    [DecidablePred (fun H : Sym2InducedLabeledSubgraph G => candidateMatches F H)]
    : (*@$\mathbb{Q}$@*) :=
...
ordinary finite evaluation.  The definition \lean{sym2FlagDensity}$_1$
...
``tuple should be counted'' condition.  Given the tuple of patterns \lean{Hl}
...
candidates are disjoint away from the shared type vertices.  The finite count
then filters all candidate tuples by this predicate.  The adequacy theorems in
the next subsection rewrite the abstract \lean{flagDensity}$_1$ and
\lean{flagDensity}$_2$ to these executable computations.
