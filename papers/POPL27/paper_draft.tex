...
The (\lean{candidateMatches F H}) below is 
such a predicate. It
turns the induced subgraph \(H\) into its associated labeled graph,  and asks whether the result is isomorphic to $F$
as a labeled graph.
two
labeled graphs are flag-isomorphic.  Once Lean has a decidability instance for
this predicate, the density itself is just the finite count obtained by
filtering the enumeration of candidates.
\begin{lstlisting}
/-- The induced candidate H is isomorphic to the concrete pattern F. -/
def candidateMatches {k m n : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k}
    (F : Sym2LabeledGraph (*@$\sigma$@*) m)
    {G : Sym2LabeledGraph (*@$\sigma$@*) n}
  (H : Sym2InducedLabeledSubgraph G) : Prop :=
  Nonempty ((H.toLabeledSubgraph).coe (*@$\simeq$@*)f F.toLabeledGraph)

/-- A one-pattern baby version of the reflected density computation. -/
def sym2FlagDensity1Baby
    {k m n : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k}
    (F : Sym2LabeledGraph (*@$\sigma$@*) m)
    (G : Sym2LabeledGraph (*@$\sigma$@*) n)
    [DecidablePred (fun H : Sym2InducedLabeledSubgraph G =>
      candidateMatches1 F H)] :
    (*@$\mathbb{Q}$@*) :=
  ((Finset.univ : Finset (Sym2InducedLabeledSubgraph G)).filter
    (fun H => candidateMatches1 F H)).card / Nat.choose (n - k) (m - k)
\end{lstlisting}
The displayed density definition assumes the \lean{DecidablePred} needed by
\lean{Finset.filter}.  In the concrete \lean{Sym2} representation, this
instance is supplied from the data already stored in the objects: \(H\) has a
finite vertex set and an explicit edge set, and \(F\) is also given by an
explicit finite edge set.  Thus the predicate is not merely a mathematical
condition; it is a finite test that the evaluator can run while counting.

The actual repository definition generalizes this baby version rather than
changing its computational idea.  It uses a predicate on lists of candidate
subflags, because product densities must choose several patterns at once and
also enforce disjointness away from the shared type vertices.  The normalizing
factor is correspondingly the multinomial coefficient for those choices.  The
reflected computation is otherwise the same: enumerate finite candidates,
decide flag isomorphism, count, and normalize.  Generated application files
then rewrite abstract densities to \(\mathtt{sym2FlagDensity}_1\) or
\(\mathtt{sym2FlagDensity}_2\) and close the resulting rational equalities by
\lean{native\_decide}.
