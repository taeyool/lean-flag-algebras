...
and its edges are stored as a finite set of unordered pairs
\lean{Finset (Sym2 (Fin n))}, where \lean{Sym2} forms unordered pairs.  The
empty-type case uses the structure \lean{Sym2Graph n}.  For typed flags, the
flag type itself is encoded as \lean{Sym2FlagType k}; since a flag type is just
a graph on the labeled vertices, \lean{Sym2FlagType} has the same fields as
\lean{Sym2Graph}.  The labeled graph is then represented by
\lean{Sym2LabeledGraph}, the edge-set analogue of \lean{LabeledGraph}: \gw{I'm here.}
\begin{lstlisting}
structure Sym2Graph (n : Nat) where
  edges       : Finset (Sym2 (Fin n))
  edges_valid : forall e in edges, not e.IsDiag

structure Sym2FlagType (k : Nat) where
  edges       : Finset (Sym2 (Fin k))
  edges_valid : forall e in edges, not e.IsDiag

structure Sym2LabeledGraph {k : Nat} (sigma : Sym2FlagType k) (n : Nat) where
  edges       : Finset (Sym2 (Fin n))
  edges_valid : forall e in edges, not e.IsDiag
  type_embed  : fromEdgeSet (SetLike.coe sigma.edges) (*@$\hookrightarrow_g$@*) fromEdgeSet (SetLike.coe edges)
\end{lstlisting}
Thus a graph on $\{0,\ldots,n-1\}$ is stored as a finite set of unordered
non-diagonal pairs.  Adjacency is checked by membership in this finite edge set,
and every possible graph on \(\{0,\ldots,n-1\}\) can be enumerated by
enumerating possible edge sets.  The typed versions add the finite edge set for
the type graph and the embedding of those type vertices into the host.  All of
these representations still decode to the abstract graph language by
interpreting adjacency as membership in \lean{edges}.

These edge-set representations also make the two ingredients above concrete.
Candidate induced subflags are represented by finite vertex sets, so their
enumeration is just the enumeration of all finite subsets of
\(\{0,\ldots,n-1\}\):
\begin{lstlisting}
structure Sym2InducedSubgraph (G : Sym2Graph n) where
  verts : Finset (Fin n)

instance (G : Sym2Graph n) : Fintype (Sym2InducedSubgraph G) where
  elems := Finset.map
    { toFun := fun V => ({ verts := V } : Sym2InducedSubgraph G)
      inj'  := ... }
    (Finset.univ : Finset (Finset (Fin n)))
  complete H := ...
\end{lstlisting}
Once a vertex set is chosen, its induced edge set is computed by filtering the
host graph's explicit edge set.

The isomorphism decision procedure is similarly concrete.  For empty-type
graphs, it first rejects graphs with different edge counts, then searches over
vertex permutations and checks edge membership:
\begin{lstlisting}
  ...
  if G1.edges.card != G2.edges.card then false
    ...
    perms.any fun perm =>
      edges.all fun e =>
        ...
        decide (applyPermEdge perm e (*@$\in$@*) G2.edges)

instance fastDecidableSym2GraphEqv
    (G1 G2 : Sym2Graph n) : Decidable (G1 (*@$\sim_{\mathsf{sf}}$@*) G2) :=
  if h : isEmptyIsoFast_bool G1 G2 = true then
    isTrue  (isEmptyIsoFast_bool_true_correct h)
  else
    isFalse (isEmptyIsoFast_bool_false_correct ...)

instance fastDecidableSym2EmptyTypedFlagEqv :
...
isomorphism; only the procedure used to decide it has changed.  For typed flags,
the analogous checker fixes the labeled vertices and permutes only the
remaining vertices.
...
induced-$\mathcal{H}$-free flags, prove their pair-density identities, then their
...
$N$-vertex induced-$\mathcal{H}$-free flags, dropping the forbidden host terms;
...
$N$-vertex induced-\(\mathcal{H}\)-free flags), and the downward-averaged products
...
because it demonstrates that the Automation layer generalizes beyond induced-triangle-free
