...
\(\sigma\) has positive limiting density in the finite graphs approximating
\(\phi_0\).  This is precisely the hypothesis under which the random-extension
...
filtered using the concrete graph $K$.  Bridging the two representations
requires proving that the flags the reflection computation discarded belong to
\lean{forbiddenFlags H}, the set of $\emptyset$-flags whose underlying graphs contain
$H$ as a subgraph.  The $H$-free condition then forces those flags to have
density zero, which justifies removing them from the objective expansion.

The tactic establishes that bridge itself, with no argument beyond $N$ and $K$.
It dispatches on $K$ by the same test the Part~1 commands use, so it always
rewrites onto the filter those commands actually emitted.  When $K$ is complete,
induced and subgraph containment coincide, so Part~1 filtered the enumeration on
the single decoded flag of $K$ having density zero, and the bridge is the
complete-graph membership lemma instantiated at the clique order the tactic
reads off $K$.  When $K$ is not complete, a subgraph copy of $H$ may carry
additional edges and therefore need not be induced; Part~1 filtered instead on
every induced supergraph of $K$ having density zero, and the bridge is the
corresponding membership statement for that finite family.  Either route
performs the same lifting step, and neither introduces additional forbidden
graphs into the certificate.
...
host-size basis, dispatching on the forbidden graph exactly as
...
$LDL^\top$ witnesses; and selects the objective-lifting branch and the form in
which the bound states its forbidden graph.
...
non-complete forbidden graph changes how the bound states that graph and which
unit-expansion lemma Part~4 cites; the generation commands of Part~1 and the
expansion tactics of Parts~3 and~4 need no such selection, since each dispatches
on the forbidden graph itself during elaboration.  Thus, the script varies the
data and the required route, but the argument assembled in Part~4 has the same
structure in every generated file.
