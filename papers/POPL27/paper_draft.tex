...
This section presents a compiler that turns this data into a self-contained
Lean proof.  For each certificate, it generates the required flags, asks Lean
to recompute their density and multiplication identities, verifies the rational
matrix factorizations, and assembles the final inequality.  Thus the external
search proposes the data, while Lean checks every mathematical obligation; no
manual copying of coefficients or reconstruction of the finite bookkeeping is
required.
...
while our compiler reconstructs the decomposition in Lean and proves that
every term in it is non-negative.

Mantel's theorem gives a small example of the whole idea.  Fix the one-vertex
type $\sigma$, and let $A:=\fKtwobull$ and $B:=\fbarKtwobull$ be the two flags
obtained by adding one unlabeled vertex: in $A$ the two vertices are adjacent,
whereas in $B$ they are not.  Write $a:=[A]$ and $b:=[B]$ for their images in
$\mathcal A^\sigma$.  For every positive homomorphism
$\phi\in\poshom{\sigma}$,
\[
  \phi\bigl((a-b)^2\bigr)
  = \phi(a-b)\,\phi(a-b)
  = \bigl(\phi(a)-\phi(b)\bigr)^2
  \geq 0.
\]
...
We next compute what this square contributes after removing the label.  Let
$G_0:=\fbarKthree$, $G_1:=\fbarPthree$, and $G_2:=\fPthree$ denote,
respectively, the three-vertex graphs with zero, one, and two edges.  These are
the three triangle-free graphs on three vertices; write $g_i:=[G_i]$ for their
images in $\mathcal A^\emptyset$.  Expanding the unit and the edge $K_2$ at
size three, and omitting the triangle term that has density zero under the
$K_3$-free condition, gives
\[
  \mathbf 1 = g_0+g_1+g_2,
  \qquad
  K_2 = \tfrac13g_1+\tfrac23g_2.
\]
The coefficients in the second identity are simply the edge densities of the
three host graphs: $0$, $1/3$, and $2/3$.

It remains to expand the square and apply the downward operator.  Write
$(a-b)^2=a^2-2\,(a\cdot b)+b^2$.  In a rooted three-vertex graph, if the two unlabeled
vertices have the same adjacency to the labeled vertex, either $a^2$ or $b^2$
has pair density $1$, giving coefficient $+1$.  If their adjacencies differ,
$a\cdot b$ has pair density $1/2$ (only one of the two assignments puts the adjacent
vertex in the $a$ position), so $-2\,(a\cdot b)$ gives coefficient $-1$.  The downward
operator averages these coefficients over the three possible choices of the
labeled vertex.  For the finite flag $G_0$, all three choices give $+1$; for
each of $G_1$ and $G_2$, one choice gives $+1$ and two give $-1$.  Including the factor $1/2$
therefore yields
\[
  \bigl\llbracket \tfrac12(a-b)^2\bigr\rrbracket_\sigma
  = \tfrac12g_0-\tfrac16g_1-\tfrac16g_2.
\]
...
Theorem~\ref{thm:downward-nonneg}, and the basis element $g_1=[G_1]$ is
non-negative because graph densities are non-negative.  Hence the right-hand side is non-negative, proving
$K_2\leq_{K_3}\tfrac12\cdot\mathbf 1$.  Even this smallest certificate requires
expanding a product, computing a downward average, expanding the objective and
the unit to a common host size, and collecting coefficients.  These are exactly
the calculations that become impractical in larger certificates and that our
compiler automates.
...
matrices PSD, and checks the identity.  The rest of this section explains the
...
$\llbracket \tfrac12(a-b)^2\rrbracket_\sigma$; together with the non-negative
remainder $\tfrac13g_1$, it proves
