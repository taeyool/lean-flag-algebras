...
the tactic layer, the Automation layer, and elaboration-time theorem generation.
...
Section~\ref{sec:flagmatic-shape} isolates it as a four-part skeleton, and
Section~\ref{sec:flagmatic-pipeline} shows how each part is discharged by a
reusable, kernel-checked component and how the translator assembles the whole
proof from a Flagmatic certificate alone.  The translator realizes the second
contribution of Section~\ref{sec:intro}: a certificate-to-proof pipeline whose
output the Lean kernel checks and whose own computation never enters the trusted
base (Section~\ref{sec:flagmatic-trust}), validated on six certificates
...
in turn by the pipeline of the next subsection.
...
self-contained Lean file that compiles under \lean{lake build} as a complete
proof, with no \lean{sorry} gaps:
\begin{lstlisting}[language={}]
python flagmatic_to_lean.py gen-skeleton <cert>.json <target>.lean
\end{lstlisting}
The diagnostic subcommands \lean{inspect} and \lean{check-deps} validate a
certificate before compilation.  The emitted file fills the four parts of
...
% TODO/REVISIT: How much detail Part 1 needs (pruned generation, completeness
% via native_decide, adequacy-based density/product proofs) depends on how much
% the reflection layer (Section~\ref{sec:reflection}) ends up covering the same
% machinery -- revisit once that section's level of detail is settled.
\paragraph{Flags and density data (Part~1).}
The forbidden graph is an explicit \lean{Sym2Graph} term \lean{K}, and the flags
and their density and product identities are generated inside Lean by a handful
of \lean{generate\_pruned\_*} commands parametric in \lean{K}, rather than read
from precomputed tables.  The commands enumerate only the
induced-$\mathcal{H}$-free flags---those containing the forbidden graph are
pruned before they are ever materialized, which keeps the enumeration tractable
at the larger host sizes---and a single \lean{native\_decide} certifies
completeness.  Every density and product is recomputed and re-checked in Lean
each build, so the certificate never asserts a value the kernel does not
reproduce.  Because the commands take the forbidden graph as a term argument, a
new forbid is a new \lean{Sym2Graph} definition, not a new branch in the
generator.

\paragraph{Certificate matrices (Part~2).}
The method proves $P \leq_{\mathcal{H}} c\cdot\mathbf{1}$ not by manipulating $P$
but by adding to it a provably non-negative quantity: a quadratic form
$\langle v_t, M_t\,v_t\rangle$ whose matrix $M_t$ is positive semidefinite---the
flag-algebra analogue of a sum of squares.  The solver supplies the $M_t$; Lean
need only re-establish that each is PSD, which it does from an $LDL^\top$
factorization over $\mathbb{Q}$: exhibiting $M = L D L^\top$ with $d \geq 0$ is a
rational matrix equality the kernel checks exactly (\lean{decide +kernel})---no
eigenvalues, no floating point---and the result lifts to $\mathbb{R}$, where the
flags live.  A wrong factorization therefore surfaces as a compile error, never
as a silently accepted non-PSD matrix; this is the one place we pay for in-kernel
evaluation.

\paragraph{Lifting the objective (Part~3, Branch~B).}
The densities live at the host size $N$, but the objective is not always a flag
of that size.  When it is smaller (\emph{Branch~B}: Mantel, $K_4$- and
...
terms).  Finally, the non-negativity goal is closed by \lean{flag\_nonneg}, which
...
The script's own work is bookkeeping.  Every part above refers to flags by their
canonical index, so it first parses each certificate string and
isomorphism-matches it against a canonical enumeration that reproduces the order
of Lean's flag generators; the index it computes is then already the $i$ in
\lean{Flag\_n\_k\_m\_i}, with no separate table to keep in sync.  With the indices
in hand it fills the four parts---three of them mechanically, as fixed templates
keyed to the sizes, the only real computation being the certificate matrices,
which it reconstructs as $M_t = R_t\,Q_t'\,R_t^\top$ and factors as
$L_t D_t L_t^\top$.  Choosing the branch, the per-block parameters, and the order
of the additions is the entire ``intelligence'' of the generator; why none of it
has to be trusted is the subject of Section~\ref{sec:flagmatic-trust}.
...
The script computes a great deal: it resolves certificate strings to flag
indices, reconstructs and factors the certificate matrices, and decides which
declarations and imports to emit.  \emph{None} of this enters the trusted
base.  Every emitted
...
rejects: the cost of its fragility is a failed compile, never a false theorem.

\subsection{Evaluation}
\label{sec:flagmatic-scenarios}

Six certificates have been carried end-to-end through \lean{gen-skeleton},
producing files the kernel accepts with no \lean{sorry}
...
forbids $K_3$, $K_4$, $K_5$.  None of the six required hand transcription of
...
\begin{table}
  ...
  \begin{tabular}{lrl}
    ...
    Case studies, SDP certificates, Automation, tactics & 3\,483 &
      \lean{MantelTheorem/}, \lean{ErdosPentagon/}, \lean{Logic/}, \lean{Automation/} \\
...
and \lean{prove\_flag\_mul}.  The more uniform Automation-style version
\lean{Mantel\_flagAlgebra\_API} in \lean{API/MantelTheoremAPI.lean} proves
\[
  \lean{FlagAlgebra\_2\_0\_0\_1 <=[K3.toFinFlag] (1/2) * 1}
\]
using the generic Automation tactics: \lean{forbidLE\_add\_QuadraticForm} for the
...
\lean{flag\_nonneg} for the closing step.  The Automation version is roughly thirty
...
the Automation layer: it uses two types rather than one and exercises the
...
is done in \lean{ErdosPentagon/ErdosPentagon.lean}; the Automation-style restatement
\lean{ErdosPentagon\_flagAlgebra\_API} in \lean{API/ErdosPentagonAPI.lean}
shows the same proof in the Automation style of the previous two case studies.
