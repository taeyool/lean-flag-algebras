...
$\llbracket A\rrbracket_\sigma = q_\sigma(A)\cdot A|_\emptyset$.  In Lean,
$\leansmul$ denotes scalar multiplication from the ambient module structure.
\begin{lstlisting}
...
The code below shows this second lift.  We first define
\lean{downwardFlagVectorQuot}, which wraps the output of
\lean{downwardFlagVector} with the quotient map
$\llbracket\cdot\rrbracket$, giving a value of type
...
\end{lstlisting}
The resulting \lean{downward} is the operator from $\mathcal{A}^\sigma$ to
$\mathcal{A}^{\emptyset}$.  In mathematical prose we write it as
$\llbracket-\rrbracket_\sigma$, following
...
empty-typed algebra. \gw{I'm here.}
