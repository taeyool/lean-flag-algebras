...
\(\mathbb{P}[\phi_0]\) denotes the random-extension measure
\(\Ext_\sigma(\phi_0)\).

What remains is to connect this constrained semantic inequality to the finite
extremal quantity that we ultimately want to bound.  In
\lean{Turan/GeneralizedTuran.lean}, the finite extremal number and its limiting
...
Thus \lean{generalizedTuranDensity H F} is the asymptotic maximum density of
copies of \(F\) among \(H\)-free graphs.

The bridge theorem in \lean{Forbid/TuranDensity.lean} is the point at which the
ensemble order becomes an extremal bound:
\begin{lstlisting}
