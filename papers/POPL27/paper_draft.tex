...
positive entries.  Flagmatic's user guide~\cite{vaughan2012flagmaticguide}
explains the format: $Q'_t$ is
chosen to be positive definite, which is easy to check directly, for instance
through Sylvester's criterion, and the congruence then transfers the
non-negativity of $Q'_t$'s quadratic form to $M_t$.
...
The compiler script connects the certificate's external representation to the four
parts above.  Flagmatic represents a graph or flag by a compact string, so the
script parses each string and searches the canonical enumeration used by
...
The script then reads the objective and bound, the forbidden graph, the
...
For each certificate, the script writes one Lean source file containing all
...
generation and normalization steps.  Thus, the script varies the data and
the required route, but the argument assembled in Part~4 has the same structure
in every generated file.

The script itself determines which Lean basis element corresponds to each
...
Within this output discipline, neither the SDP solver nor the compiler script
...
Second, the compiler script is trusted for \emph{translation fidelity}: it parses the
...
We evaluated the compiler on seven certificates spanning both objective
...
\begin{table}
  \centering
\caption{Certificates carried end-to-end through the compiler.  ``Branch''
...
We point out two limitations of the current compiler.
First, this evaluation establishes coverage of the seven cases above, not
completeness of the compiler.  The current input language assumes the
...
certificate compiler operates outside this reflected core and emits source
