...
\lean{FlagAlgebra}~\(\sigma\) to \(\mathbb{R}\).  The second definition is
Lean's subtype construction.  Mathematically, we read it in the usual
set-comprehension form
\[
  \{\phi\in \lean{Hom}~\sigma
    \mid \phi(\llbracket\lean{basisVector F}\rrbracket)\ge 0
    \text{ for every }F\in\mathcal{F}^{\sigma}\}.
\]
In other words, the only extra condition on an algebra homomorphism is the
positivity condition: for every finite \(\sigma\)-flag \(F\),
\lean{basisVector F} is the formal basis vector representing \(F\),
\(\llbracket\cdot\rrbracket\) is the quotient map from
\lean{FlagVector}~\(\sigma\) to \lean{FlagAlgebra}~\(\sigma\), and the
...
convergent flag sequence is equal to the profile \lean{$\phi$.coe} of some
positive homomorphism \(\phi\).  Conversely, the second theorem says that every
positive homomorphism \(\phi\), after passing to this profile
\lean{$\phi$.coe}, arises as the limit of some flag sequence.
...
and then extracting one sequence whose density profile is \lean{$\phi$.coe}.
