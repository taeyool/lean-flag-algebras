...
\end{lstlisting}
The enumeration is deliberately simple.  Since the vertices of \(G\) are
\lean{Fin n}, Lean can enumerate the finite set of all subsets of
\(\{0,\ldots,n-1\}\).  The instance scans this list and keeps a subset \(V\)
precisely when it contains the type vertices \lean{G.type\_verts}; the retained
subset is stored together with the proof of that containment.  Once such a
vertex set is chosen, its induced edge set is computed by filtering the host
graph's explicit edge set. \gw{I'm here.}
