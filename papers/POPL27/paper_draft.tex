...
\lean{generalizedTuranDensity H F}: for each \(n\), it first maximizes the
number of copies of \(F\) over all \(H\)-free graphs on \(n\) vertices, and
then takes the limiting normalized density. The relevant definitions are:
\begin{lstlisting}
noncomputable def generalizedExtremalNumber
    (n : (*@$\mathbb{N}$@*)) (H : SimpleGraph U) (F : SimpleGraph W) : (*@$\mathbb{N}$@*) := by
  classical
...
\lean{subgraphCount F G} counts copies of the target graph \(F\) in \(G\).
