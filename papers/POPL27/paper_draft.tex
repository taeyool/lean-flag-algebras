...
a hereditary graph constraint, such as ordinary \(H\)-freeness, into the
flag algebra
...
counterexample based on ordinary \(C_4\)-free graphs.  The ensemble viewpoint is useful
...
For a non-complete $H$, an ordinary subgraph copy may have additional edges
...
forbidden graph.  For a complete forbidden graph, induced containment and ordinary subgraph
...
proves the ordinary statement that graphs containing no $C_5$ subgraph have
...
than build a new algebra for each forbidden family, it retains the ambient
algebra and restricts the graph limits against which an inequality is tested.
...
\paragraph{The quotient space of constrained limits.}
...
\(G[S]\in\mathcal K\).  This condition does not require closure under deleting
edges.  In particular, the ordinary \(H\)-free classes used earlier are
hereditary, because taking an induced subgraph cannot create a new ordinary
subgraph isomorphic to \(H\).

We write \(\FlagAlg{\sigma}[\mathcal G]\) for the ambient
\(\sigma\)-typed flag algebra and
\(\FlagAlg{\sigma}[\mathcal K]\) for the algebra constructed using only graphs
in \(\mathcal K\).  A \(\sigma\)-flag is \emph{\(\mathcal K\)-forbidden} if its
underlying unlabeled graph does not belong to \(\mathcal K\).  Because
\(\mathcal K\) is hereditary, the span of these forbidden flags is an ideal,
and the usual constrained algebra is canonically isomorphic to the quotient
of the ambient algebra by this ideal.  We therefore have a canonical quotient
map
\[
  q_\sigma:
...
\]
that sends every forbidden flag to zero and every other flag to its
counterpart in the constrained algebra.
...
\]
be the compact space of positive unital algebra homomorphisms on the ambient
algebra, with the topology of pointwise convergence on flags.  Here
\emph{positive} means non-negative on each individual flag.  Pullback along
\(q_\sigma\) embeds the positive homomorphisms of the constrained algebra into
\(X_\sigma\).  Its image is
\begin{equation}\label{eq:Q-sigma}
  ...
  \subseteq X_\sigma.
\end{equation}
Thus \(Q_\sigma\) is the space of all constrained \(\sigma\)-labeled limits
that the built-in quotient semantics admits.  It imposes no random-rooting
requirement, so the labels may be fixed at exceptional vertices that a
uniformly random labeling would miss asymptotically.
...
\begin{equation}\label{eq:Q-sigma-intrinsic}
  ...
  \left\{
    ...
    \;\middle|\;
    \chi([F])=0
    \text{ for every \(\mathcal K\)-forbidden \(\sigma\)-flag \(F\)}
  \right\}.
\end{equation}
The role of heredity is to justify the quotient presentation above.  Every
flag appearing in the product of a forbidden flag with another flag contains
the first flag as an induced subflag.  It therefore also lies outside
\(\mathcal K\), since otherwise heredity would put that induced subflag back
in \(\mathcal K\).  Hence multiplying a forbidden flag by any other flag
produces a linear combination of forbidden flags, so their span is precisely
the ideal killed by \(q_\sigma\).  Once this presentation is established, a
homomorphism vanishing on every forbidden flag vanishes on the whole ideal and
therefore factors through \(q_\sigma\) by the universal property of the quotient.
...
\(Q_0:=Q_{\emptyt}\) for the corresponding space of constrained unlabeled
limits.
...
probability under random rooting in constrained unlabeled limits.
...
\]
Indeed, the downward average of a forbidden \(\sigma\)-flag is a non-negative
multiple of its forbidden underlying graph.  Every \(\phi_0\in Q_0\) evaluates
that graph to zero, so the expected density of the forbidden flag under the
random extension is zero.  Its density is pointwise non-negative and therefore
zero almost surely.  Applying this argument to the countable collection of
forbidden flags shows that the random extension lies in \(Q_\sigma\) almost
surely.  Since \(Q_\sigma\) is closed, \Cref{eq:S-sigma} yields the fundamental
inclusion
...
to hold almost surely after randomly labeling a constrained unlabeled limit.
For the ordinary \(H\)-free class---graphs containing no ordinary subgraph
isomorphic to \(H\)---
...
The proof starts from a labeled limit in \(Q_\sigma\), represented by finite
rooted graphs in \(\mathcal K\), and enlarges the fibers containing the roots
until random rooting hits them with probability bounded away from zero.
...
labeled limit therefore lies in the closure of supports defining
...
clone-closed case and cover all ordinary \(K_r\)-free classes for \(r\geq3\),
...
class of ordinary \(C_4\)-free graphs---those containing no \(C_4\) as a
...
labeled limit belongs to \(Q_{\sigma_1}\) but not to \(S_{\sigma_1}\), so
\[
