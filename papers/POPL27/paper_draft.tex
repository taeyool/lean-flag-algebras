...
Mantel's theorem gives a small example of the whole idea.  Fix one labeled
vertex, and let $a$ and $b$ be the two flags obtained by adding one unlabeled
vertex: in $a$ the two vertices are adjacent, whereas in $b$ they are not.  The
square $(a-b)^2$ is non-negative under every positive homomorphism because it
evaluates to an ordinary real square.  Writing $G_{\mathrm{one}}$ for the
three-vertex graph with exactly one edge, expansion in the triangle-free
algebra gives
\[
  \tfrac12\cdot\mathbf 1-K_2
  =
  \bigl\llbracket \tfrac12(a-b)^2\bigr\rrbracket_1
  + \tfrac13 G_{\mathrm{one}}.
\]
The first term remains non-negative after the label is removed by the downward
operator (Theorem~\ref{thm:downward-nonneg}), and the second is non-negative
because graph densities are non-negative.  The right-hand side is therefore
non-negative, proving $K_2\leq_{K_3}\tfrac12\cdot\mathbf 1$.  Thus, in this
smallest example, the certificate is simply a square together with the
non-negative remainder left by its expansion.

Larger proofs use weighted sums of many such squares.  For flags
...
$\llbracket \tfrac12(a-b)^2\rrbracket_1$; together with the non-negative
remainder $\tfrac13G_{\mathrm{one}}$, it proves
