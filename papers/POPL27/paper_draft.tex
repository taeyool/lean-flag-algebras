...
\lean{$\sigma$ : FlagType (Fin n$_0$)} fix the type graph, while the explicit
...
rewriting it as its induced-forbid-free expansion at size $N$.  Concretely,
...
\lean{flagSet}, and dropping the host terms that contain an induced member of
\(\mathcal{H}\) automatically (no manual \lean{basisVector\_forbidEq\_zero} step).  Its closing
...
which rewrites $\mathbf{1}$ as the sum of $N$-vertex
induced-\(\mathcal{H}\)-free flags by
