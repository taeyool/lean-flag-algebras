...
notions of positivity are \(0\leq^{\mathrm{quot}}_{\mathcal K,\sigma}f\) and
...
continuous function that is positive on \(S_\sigma\) and negative at
\(\chi_*\).  The Stone--Weierstrass theorem then approximates this function by
evaluation of a flag-algebra element.  The resulting element is ensemble
positive but not quotient positive, so the two orders cannot agree for all
elements.  Thus \Cref{thm:support-closure-summary} is a completeness theorem:
...
The criterion reduces semantic completeness to a structural problem about the
...
\(G\in\mathcal K\), \(v\in V(G)\), and \(N\), some graph \(J\) on exactly
...
The proof starts from a \(\sigma\)-typed positive homomorphism in \(Q_\sigma\),
represented by a sequence of finite \(\sigma\)-flags whose underlying graphs
lie in \(\mathcal K\).  It enlarges the fibers containing the labeled vertices
until a uniformly random occurrence of \(\sigma\) selects vertices from those
fibers with probability bounded away from zero.
When a fixed finite flag is sampled from the resulting blow-up, with
probability tending to one its sampled vertices lie in distinct fibers.
Their adjacencies are then determined entirely by the original \(\sigma\)-flag;
the internal structure of the enlarged fibers is invisible.  The target
positive homomorphism therefore lies in the closure of supports defining
\(S_\sigma\).  This proves \(Q_\sigma\subseteq S_\sigma\), while the opposite
inclusion is \eqref{eq:S-subset-Q}.

Independent blow-ups, in which every fiber is an independent set, give the
clone-closed case and cover all \(K_r\)-free classes for \(r\geq3\),
including triangle-free graphs.  Complete blow-ups give the true-clone-closed
case and cover, for example, cluster graphs.  Both are instances of
\Cref{thm:blowup-root-plantability-summary}.  For infinite hereditary classes,
substitution-closure is a stronger sufficient condition.
...
at random has normalized degree tending to zero.  Let \(d\) denote the
\(\sigma_1\)-flag density that the labeled vertex is adjacent to one additional
sampled vertex; then \(-d\) is ensemble positive.  On the other hand,
arbitrarily large stars are \(C_4\)-free, and selecting the center as the
labeled vertex gives \(d=1\).  This
positive homomorphism belongs to \(Q_{\sigma_1}\) but not to
\(S_{\sigma_1}\), so
\[
  S_{\sigma_1}\subsetneq Q_{\sigma_1}.
\]
Accordingly, \(-d\) is not quotient positive.  The example exhibits a genuine
gap between the two typed orders: quotient semantics permits a specially
chosen vertex to carry the label, whereas ensemble semantics obtains the
labeled vertex by uniform random sampling.
...
without incomplete proofs.  Thus the automation concerns the translation of
the mathematics into Lean, not the autonomous discovery of the mathematical
statements.
...
positivity certificates for polynomials in HOL Light, with numerical methods
used to find certificates and the proof assistant used to check them.  We use
