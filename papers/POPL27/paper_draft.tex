...
Sections~\ref{sec:flagmatic-psd}--\ref{sec:flagmatic-close} show how each part
...
Each part of the shape is discharged by a verified, reusable component, and the
four subsections that follow take the recurring steps in turn: verifying the
SDP certificate, generating the forbid-free data, expanding and reducing typed
products, and normalizing and closing.  All of these tactics share one enabler.
Every flag and flag-algebra constant is named \lean{Flag\_n\_k\_m\_i} /
\lean{FlagAlgebra\_n\_k\_m\_i}, where $n$ is the vertex count, $k$ and $m$
identify the type graph, and $i$ is the canonical index within the isomorphism
class; the convention is not merely organizational but a machine-readable
encoding of structure.  By parsing the trailing integers from a constant's
\lean{Name}---a syntactic object the elaboration monad inspects without
triggering type-checking or attribute resolution---a tactic recovers the
canonical order of a flag term, the name of its precomputed density or
multiplication lemma, and the type parameters it needs, all in the fast
elaboration path.  To tolerate the syntactic variation the elaborator
introduces, the tactics normalize the expression tree with a
\lean{collectPrefixConstants} traversal before inspecting names and compare flag
terms with \lean{Expr.eqv} rather than structural equality; this metaprogramming
was among the least portable parts of the development.

\subsection{Verifying and adding the SDP certificates}
...
Once a block is known positive semidefinite, it enters the proof through a
single lemma:
\begin{lstlisting}
...
\end{lstlisting}
which adds the quadratic form $\llbracket\langle v, M\,v\rangle\rrbracket$ to the
target under the forbid relation.  Iterating it once per type builds a
transitive chain of additions, one block at a time.

\subsection{Generating the forbid-free flags and their data}
...
argument} (an \lean{ident} naming a \lean{Sym2Graph}) and build the
forbid-freeness predicate from it, so a new forbidden graph is a new
\lean{Sym2Graph} definition rather than a new per-forbid branch in the
meta-commands.

\subsection{Expanding and reducing typed products}
\label{sec:flagmatic-expand}

Two structural identities recur in every proof: the \emph{expansion} of a flag
at a larger size as a weighted sum of host-size flags, and the \emph{product} of
two typed flags as a linear combination at the product size.  Under a forbid
both are produced in forbid-free form.

Expansion is used in two places, both forbid-free.  In Branch~B the objective is
lifted to host size by \lean{flag\_expand\_hfree N K}, which closes a goal
$F =_{K} \sum_j c_j\,F_j$; the unit is expanded by \lean{expand\_one\_hfree\_at N
K}, which rewrites $\mathbf{1}$ as the sum of $N$-vertex $\mathcal{H}$-free
flags.  Both rewrite with \lean{basisVector\_quot\_forbidEq\_sum} directly onto
the generated \lean{flagSetHfree\_N\_k\_m\_<K>} set---never materializing the
full \lean{flagSet}, and dropping the forbidden host terms automatically (no
manual \lean{basisVector\_forbidEq\_zero} step).  The closing arithmetic of
\lean{flag\_expand\_hfree} needs the relevant \lean{flagDensity\textsubscript{1}}
values present as \lean{@[simp]} lemmas, which the translator emits (each closed
by \lean{native\_decide} after an adequacy rewrite).

Products are reduced by \lean{reduce\_downward\_flagmul}.  The generation step
(Section~\ref{sec:flagmatic-gen}) has already registered a
\lean{flagMul\_<A>\_<B>} expansion for each pattern pair; the tactic
right-associates the sum with \lean{simp only [downward\_add, add\_assoc]} and
then rewrites each head summand $\llbracket c\,(A\cdot B)\rrbracket$ by its
\lean{flagMul} identity, moving each rewritten term across the inequality.  A
single call dispatches all of a block's downward-averaged products.

\subsection{Normalizing and closing}
\label{sec:flagmatic-close}

After expansion and product reduction, the obligation is an equality between two
linear combinations of flag terms that are equal but syntactically reordered.
Generic tactics (\lean{ring}, \lean{ac\_rfl}) handle this in principle but search
over all permutations of summands and become prohibitively slow past ${\sim}20$
terms.  \lean{flagsum\_ac\_sort\_rhs\_pipeline} (in \lean{API/FlagSumSort.lean})
...
${\sim}25$-term expressions, while the index-guided sort completes the same step
in under a second.

What remains is a non-negativity goal, closed by \lean{flag\_nonneg}: it
...
\lean{psd\_real\_ldlt} proof of Section~\ref{sec:flagmatic-psd}.  None of
...
kernel), or a verified tactic of Sections~\ref{sec:flagmatic-psd}--\ref{sec:flagmatic-close}.  A
