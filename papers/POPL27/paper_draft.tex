...
homomorphism to that element.  On paper these shifts are usually harmless and
are left implicit.  In Lean they become the first formalization problem: each
role must be represented by a separate type, and each passage between roles
must be justified.  In particular, constructions on concrete graphs must be
invariant under isomorphism, expansion identities must hold in the quotient,
multiplication must be well defined on the quotient algebra, and positive
homomorphisms must preserve the algebraic operations.
