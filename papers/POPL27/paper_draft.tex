...
\end{lstlisting}
For readers not familiar with Lean syntax, this block declares standing
parameters rather than a new mathematical statement.  Curly braces mark
implicit parameters that Lean usually infers from context.  Square brackets
mark implicit \emph{instance} parameters, called typeclass assumptions.  Here
an ``assumption'' is satisfied by providing data of the required kind: a
\lean{Fintype U} instance witnesses that \lean{U} is finite by giving an
enumeration of its elements, and a \lean{DecidableEq U} instance gives a
decision procedure for equality on \lean{U}.  Lean searches for these instances
automatically when a later definition or theorem needs them.

Thus \lean{T}, \lean{U}, \lean{U$_1$}, \lean{U$_2$}, and \lean{W} are finite
vertex types used below, with enumeration and equality data available where
needed.  The variable $\sigma$ is a flag type on the label type \lean{T};
\lean{FlagType} is introduced in the next subsection.  The type \lean{Fin t}
has elements $0,1,\ldots,t-1$, so \lean{Vl : Fin t -> Type} is a finite list of
vertex types.  The custom typeclass assumptions \lean{FintypeList Vl} and
\lean{DecidableEqList Vl} provide \lean{Fintype (Vl i)} and
\lean{DecidableEq (Vl i)} instances for each index \lean{i : Fin t}.
...
$\theta_A:\sigma\hookrightarrow_g M$.
...
\begin{lstlisting}
structure LabeledGraphIso {V W : Type}
...
def flagEqv (G G' : LabeledGraph (*@$\sigma$@*) V) : Prop :=
  Nonempty (G (*@$\simeq$@*)f G')

instance labeledGraphSetoid ((*@$\sigma$@*) : FlagType T) (V : Type)
...
label-preserving graph isomorphism between them.  Lean's quotient type does not
take an arbitrary relation directly; it expects a \emph{setoid}, namely a type
together with an explicitly supplied equivalence relation.  The instance
