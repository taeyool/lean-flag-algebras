...
The development resolves the proof obligations needed by our case studies, but
it does not settle every design question exposed by the formalization.  We
record four such questions to separate demonstrated properties of the current
system from conjectures about how the approach might generalize.
...
The mitigations of \Cref{sec:obstacles}---explicit \lean{Fintype} bindings and
systematic use of \lean{HEq} and \lean{cast} for size changes---respond to two
features of Lean~4: typeclass coherence by convention rather than by
enforcement, and definitional inequality for dependently typed families that
are propositionally equal.  We do not know which of these issues would survive
a port to a different ITP (Rocq with SSReflect, Agda, or Isabelle/HOL with a
different typeclass story).  In particular, the \lean{Fintype} ambiguity and
the \lean{HEq} discipline depend on specific elaboration and equality
mechanisms; a proof assistant with stricter coherence or a different treatment
of dependent equality might dissolve some of these obstacles.  We report them
as Lean~4 obstacles without claiming that they are foundational.

\paragraph{Is the bridge layer essential or an artifact of our spec choices?}
The reflection and automation layers of
\Cref{sec:reflection,sec:flagmatic-pipeline} are
...
this work into the specification side.  For instance, a specification that
represented each isomorphism class by a chosen canonical graph, rather than as
a quotient, would not need the same adequacy theorems, though it
...
our current case study.  Larger flag-algebra results in the literature, particularly those
...
do not know which part of the compiler---flag enumeration, density-theorem
generation, multiplication-theorem generation, or the growing collection of
named theorems---would be the first bottleneck.
...
keeps the quotient, while a separate executable layer chooses representatives
over the concrete \lean{Sym2Graph} type.  The two
...
that the compilation preserves the specification's meaning.  In this development, when
reflection on the natural specification was computable in principle but
impractical, separating specification from implementation and connecting them
once proved more effective than reshaping the specification around evaluation.

\paragraph{Lesson 3 (Names can serve as elaboration-time metadata).}
...
times out on expressions of around 25 flag-basis terms.  The index-guided
\lean{ac\_sort} completes the same step promptly by exploiting the
...
translator that turns it into Lean source, are both unverified.  Neither has to
be trusted.  The translator emits the chosen flags, matrices, factorizations,
and target inequality, while Lean independently enumerates the required flags
and computes the density and multiplication theorems described in
\Cref{sec:reflection-generation}.  A wrong flag index or coefficient therefore
causes a generated equality or the final algebraic normalization to fail.
...
quadratic form.  The transferable framing is that the external tool proposes
data and proof structure, while the proof assistant reconstructs the finite
calculations and accepts only a complete proof.  External computation remains
a candidate generator, not a proof authority.
...
\lean{decide +kernel} does not complete at this scale on the
proof-relevant encoding used by the specification, even after the optimizations
of \Cref{sec:reflection}.  For the pentagon proof, the three SDP matrix
equalities (sizes
...
\lean{decide +kernel} on the generated density theorems as well---can read this as a
