...
\author[1,3]{Gyeongwon Jeong\thanks{These authors are joint first authors of this work.}}
\author[1,3]{Seonghun Park\protect\footnotemark[1]}
...
root-planting criterion characterizing when they do agree, establish a
...
rainbow-triangle density problem~\cite{balogh2017rainbow}, and it gives the
best known bounds on Tur\'an's tetrahedron
problem~\cite{razborov2010tetrahedron,baber2012turan}.
...
probabilities, and the algebra is a quotient by expansion identities---and its
theorems are the ones a flag-algebra argument actually appeals to, so that such
...
algebraic steps---expanding flags to a common size, multiplying typed flags,
applying the forbidden-graph assumption, and normalizing large linear
...
\Cref{sec:reflection,sec:flagmatic} describe the bridge from
...
\emph{labeled vertices}, or \emph{roots}, of $F$; the remaining $|V(F)|-k$ vertices are
\emph{unlabeled}.
...
intermediate flag $G'$ of size $n$ inside $G$ and then sampling~$F$ inside $G'$.
...
\begin{lemma}[Chain rule]\label{lem:chain-rule}
  ...
  \[
    \den{F}{G}
    \;=\;
    \sum_{G' \in \mathcal{F}^\sigma_n} \den{F}{G'}\,\den{G'}{G}.
  \]
\end{lemma}
...
\begin{equation}
  ...
  \mathbf e_F - \sum_{G' \in \mathcal{F}^\sigma_n} \den{F}{G'} \cdot \mathbf e_{G'},
...
lowercase letters $f,g,h$ denote arbitrary flag-algebra elements.  Thus a
positive homomorphism is applied to $[F]$, not directly to the finite flag
$F$.  For familiar named or pictorial flags, such as $K_2$ or
...
We call $\sum_{G' \in \mathcal{F}^\sigma_n} \den{F}{G'} \cdot \mathbf e_{G'}$ the
...
$\sigma$-flags~\cite{lovasz2006limits}.

\begin{theorem}[Lov\'{a}sz--Szegedy]\label{thm:convergent-hom}
  \begin{enumerate}
...
\begin{lemma}\label{lem:semantic-bound}
  If $[F] \leq c \cdot \mathbf{1}$ for some $c \in \mathbb{R}$, then
...
\begin{proof}
  ...
  $[F] \leq c \cdot \mathbf{1}$ means $\phi([F]) \leq c$ for every
...
A flag algebra proof of $[F] \leq c\cdot\mathbf{1}$ typically proceeds as follows:
...
\eqref{eq:sum-to-one} to conclude $[F] \leq c\cdot\mathbf{1}$.
...
$[F]\leq_{\mathcal H}c\cdot\mathbf 1$.  A \emph{flag-algebra certificate} is a
...
analogous flags, PSD matrices, and remainder coefficients.
...
algebra elements into a vector $v=(e_1,\ldots,e_r)$.  A symmetric matrix $M$ is \emph{positive semidefinite}
(PSD) if $x^\top Mx\geq 0$ for every real vector $x$.  Consequently, when $M$
is PSD, the flag-algebra quadratic form
$\langle v,Mv\rangle=\sum_{i,j}M_{ij}\,(e_i\cdot e_j)$ is non-negative under every
...
\begin{enumerate}
  ...
  \item \textbf{The main theorem.}  With
    ...
    and $r$ the certificate's non-negative remainder, the compiler assembles
...
reflection-layer constant---a \lean{Sym2EmptyTypedFlag} at the empty type or a
\lean{Sym2Flag} at a nonempty type---together with its decoded
...
Mantel example, there is one $Q_t$---the downward image of the halved
...
$h:W=(i\mapsto V_{\pi(i)})$.  Given any equality of types $A=B$ and a term
$x:A$, \lean{cast} transports $x$ across that equality, producing a term of
...
The broader lesson is that explicit bijections provide a common interface
between combinatorial reasoning and Lean's handling of finite cardinalities.
The bijection isolates the mathematical step---exhibiting a one-to-one
...
Let \(\mathcal G\) be the class of all finite simple graphs, let
\(\mathcal K\subseteq\mathcal G\) be a hereditary class, and fix a flag type
...
\(\mathcal K\) is hereditary, the remaining \(\sigma\)-flags---those whose
underlying graphs lie outside \(\mathcal K\)---span an ideal,\footnote{An ideal \(I\) in a
commutative \(\mathbb{R}\)-algebra \(A\) is a linear subspace---hence closed
...
presentation is established, a homomorphism vanishing on every flag outside
\(\mathcal K\) vanishes on the whole ideal and therefore factors through
...
Every \(\phi_0\in Q_0\) evaluates \([F|_\emptyset]\) to zero.  Therefore, if
\(\phi\sim\Ext_\sigma(\phi_0)\), the random-extension
...
\(\sigma\) are often called the \emph{roots}.  A finite \(\sigma\)-flag
\((G,\theta)\) records the flag-density profile around a specified embedding
\(\theta:\sigma\to G\) of these roots.  To \emph{plant} this rooted profile in
a larger graph \(H\in\mathcal K\), with no roots fixed in advance, is to produce
many embeddings \(\widehat\theta:\sigma\to H\) whose flag-density profiles
approximate the profile specified by \((G,\theta)\).  Here ``many'' means that
these embeddings form a fraction bounded away from zero of all embeddings of
\(\sigma\) in \(H\).  Consequently, a uniformly chosen occurrence of \(\sigma\)
reproduces the prescribed profile with non-vanishing probability.  The equality
...
For the \(H\)-free class---graphs containing no subgraph
isomorphic to \(H\)---
...
every \(J_v\) is a clique, cover, for example, cluster graphs---that is, graphs
whose connected components are cliques.  Both are instances of
...
class of \(C_4\)-free graphs---those containing no \(C_4\) as a
subgraph---and let \(\sigma_1\) be the one-vertex type.
Every large \(C_4\)-free graph has \(o(n^2)\) edges, so a vertex chosen uniformly
at random has normalized degree tending to zero.  Let
\(d\in\FlagAlg{\sigma_1}[\mathcal G]\) be the density expression represented by
...
\lean{RootPlantable}, respectively.  The quotient and ensemble non-negativity
notions induced by \Cref{def:two-constrained-orders} are formalized as
\lean{QuotientNonneg} and \lean{EnsembleNonneg}.  At the theorem level,
\lean{support\_criterion} formalizes
\Cref{thm:support-closure-summary};
...
\lean{c4free\_not\_rootPlantable} formalizes the strict inclusion
in \eqref{eq:c4-strict-inclusion}.

These Lean files were generated by Claude agents from an already-developed
...
It will develop finite planting criteria, which describe finite-graph conditions that certify
root-plantability by constructing, within the constrained class, larger graphs
in which a positive fraction of the embeddings of \(\sigma\) have rooted
flag-density profiles approximating a prescribed profile.  It will also study
further graph classes for which quotient and ensemble semantics agree or
disagree.  One source of disagreement is a pinning obstruction: every
 randomly-extended positive homomorphism has a labeled flag density fixed to
one value, while the constrained quotient admits a positive homomorphism 
with a different flag density.
Finally, the forthcoming work will extend ensemble
semantics to restrictions imposed directly on positive homomorphisms, such as fixing
their edge density to a particular value, which need not arise from a hereditary graph
class.  The present section records only the core results arising from the
design of \lean{forbidLE}: quotient inequalities always imply ensemble
inequalities, root-plantability characterizes when the converse holds, and
blow-up closure is a broad sufficient condition for root-plantability.
...
collaborative PFR project~\cite{gowers2025marton,pfr2023} produced a Lean~4
formalization of the Gowers--Green--Manners--Tao proof of the polynomial
...
Lov\'asz~\cite{lovasz2012large}.  Freer has independently formalized a
...
framework---and reflection and automation layers that turn specifications
...
generated density theorems being the most prominent example---we have said so.
...
metaprogramming---the elaboration-time generation commands, the custom
...
Razborov's flag algebras and Lean formalization.
This work was supported by the National Research Foundation of Korea(NRF) grant funded by the Korean Government(MSIT) (No. RS-2023-00279680) and also by the Institute for Basic Science (IBS-R029-C1).
...
\bibitem{pfr2023}
T.~Tao, Y.~Dillies, and contributors.
\newblock The polynomial Freiman--Ruzsa conjecture: A digitisation of the
  proof in Lean~4.
\newblock Formalization project, 2023.
