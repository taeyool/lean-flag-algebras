...
collection and filtering by the decision procedure. \gw{I'm here.}

Section~\ref{sec:abstract} already contains \lean{Fintype} and
\lean{Decidable} assumptions, but they serve the specification rather than the
evaluator.  They let us state finite sums and cardinalities in the abstract
theory.  For example, the abstract graph type stores adjacency as a proposition,
and flags are quotient objects:
\begin{lstlisting}
...
def Flag (sigma : FlagType T) (V : Type) : Type :=
  Quotient (labeledGraphSetoid sigma V)
\end{lstlisting}
This is exactly the right language for the mathematics, but it leaves the
computational data implicit.  When the specification layer needs a finite set,
it can obtain one noncomputably from a proof of finiteness:
\begin{lstlisting}
noncomputable def labeledGraphListCount ... : Nat :=
  have : Fintype (setOfLabeledSubgraphListIsoHl G Hl) :=
    Fintype.ofFinite _
  (setOfLabeledSubgraphListIsoHl G Hl).toFinset.card

noncomputable instance FlagFintype ... : Fintype (Flag sigma V) :=
  Quotient.fintype (labeledGraphSetoid sigma V)
\end{lstlisting}
These definitions are enough to prove theorems about densities, because Lean
can reason classically that the relevant sets are finite and have cardinalities.
They are not, however, the algorithms we want to run.  The first count does not
hand the evaluator a concrete list of vertex subsets to loop over; the quotient
instance does not by itself provide a Boolean isomorphism checker.  The missing
data are precisely the executable enumeration and decision procedures described
above.

The reflection layer therefore introduces data-oriented graph representations.
The empty-type representation is \lean{Sym2Graph n}:
\begin{lstlisting}
...
non-diagonal pairs.  Now adjacency is checked by membership in a finite set of
edges, and every possible graph on \(\{0,\ldots,n-1\}\) can be enumerated by
enumerating possible edge sets.  The representation still decodes to the
abstract graph language by interpreting adjacency as membership in
\lean{edges}.  For typed flags, \lean{Sym2LabeledGraph} adds the embedding of
the type vertices into this explicit edge-set graph.

This representation also makes the two ingredients above concrete.  Candidate
induced subflags are represented by finite vertex sets, so their enumeration is
just the enumeration of all finite subsets of \(\{0,\ldots,n-1\}\):
\begin{lstlisting}
...
\subsection{The recurring proof pattern}
...
$N$-vertex induced-\(\mathcal{H}\)-free flags); the downward-averaged products
