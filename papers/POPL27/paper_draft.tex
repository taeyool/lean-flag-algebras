...
Fix a flag type $\sigma$ with $n_0$ vertices. We regard
...
places these labels on $n_0$ vertices of $G$.  Its normalizing factor is
computed by counting such label placements.  The numerator,
\lean{isomorphismCount G}, counts the injections of the $n_0$ labels into the
...
The definitions appear in the order of this construction.
\lean{downwardNormalizingFactor\_labeledGraph} implements the counting ratio
on concrete $\sigma$-typed labeled graphs, and
\lean{downwardNormalizingFactor} lifts that ratio to isomorphism classes.
...
flags to arbitrary $\sigma$-typed flag vectors.  Its output is an empty-type
