...
The \emph{downward operator} is then
\[
...
there exists a unique probability distribution on $\poshom{\sigma}$ such that for every $f \in \mathcal{A}^\sigma$, the random positive
homomorphism $\boldsymbol{\phi}^\sigma$ drawn from this distribution satisfies
...
\begin{proof}[Proof sketch]
  ...
  Otherwise, let $\boldsymbol{\phi}^\sigma$ be the ensemble rooted at $\phi_0$.
...
We prove $\fKtwo \leq \tfrac{1}{2}\cdot\mathbf{1}$ in the triangle-free flag algebra.
...
This section formalizes the flag algebra framework in Lean~4.
In informal arguments, the same symbol freely denotes a concrete graph,
its isomorphism class, a formal linear combination of flags, or a flag algebra
element.  In our Lean formalization, each role is a distinct type and each transition is an
explicit map for which we prove compatibility with the relevant equivalence relations.

Section~\ref{sec:background} fixed a set $\mathcal{H}$ of forbidden graphs at
the outset and defined all types, flags, and the flag algebra relative to that
fixed~$\mathcal{H}$.  Our Lean formalization takes a different starting point:
the algebra is built without fixing any $\mathcal{H}$, or equivalently with
$\mathcal{H} = \emptyset$, so that no graph is initially excluded.  The
$\mathcal{H}$-free constraint is then introduced as a separate predicate on
the semantic order.  In Razborov's original
presentation~\cite{razborov2007flag}, by contrast, it is baked into the algebra
at setup time.  The two formulations agree on the Tur\'an density bounds we
prove.  Whether they are equivalent for all labeled computations is non-trivial
and depends on the choice of $\mathcal{H}$.  What differs is how much algebraic
infrastructure carries over to a new~$\mathcal{H}$ without reproof.  This is a
tradeoff we report on, not one we resolve.
\gw{This paragraph needs revision.}

Throughout this section, the following variables are in scope:
\begin{lstlisting}
...
\end{lstlisting}
\lean{FlagType} is introduced in the next subsection.
\lean{FintypeList Vl} and \lean{DecidableEqList Vl} are custom typeclass instances
asserting \lean{Fintype (Vl i)} and \lean{DecidableEq (Vl i)} for every \lean{i : Fin t}.
...
Lifting \lean{labeledGraphListDensity} to the quotient type \lean{Flag} requires
several intermediate congruence lemmas.  The result is \lean{flagListDensity},
a density function defined on \lean{Flag}s.
\begin{lstlisting}
...
The downward operator from Section~2.5 is implemented in
\lean{FlagAlgebra/FlagOperators.lean}.  As with the density functions in
Section~3.2, the definition cannot start directly on
\lean{Flag}: the mathematical input is an isomorphism class, but Lean must first
define the corresponding function on concrete \lean{LabeledGraph}s and then
prove that it respects flag isomorphism.

The normalizing factor $q_\sigma(F)$ is formalized in exactly this way.  For a
concrete labeled graph $G$, \lean{isomorphismCount G} counts the label
placements on the same underlying graph that realize a flag isomorphic to
$G$.  Dividing by the number of all injections of the labels gives
\lean{downwardNormalizingFactor\_labeledGraph}; the quotient-level function
\lean{downwardNormalizingFactor} is obtained by a \lean{Quotient.lift}.
\begin{lstlisting}
...
noncomputable def downwardNormalizingFactor
  ...
  Quotient.lift
    (fun G : LabeledGraph (*@$\sigma$@*) (Fin n) =>
      downwardNormalizingFactor_labeledGraph G)
    (fun _ _ h => downwardNormalizingFactor_labeledGraph_respect_eqv h)
\end{lstlisting}
The proof obligation in the last line is the formal counterpart of saying that
$q_\sigma(F)$ depends only on the flag, not on the chosen representative.

The same representative-level pattern appears again when forgetting labels.
We first define \lean{unlabeledGraph} on \lean{LabeledGraph}s by keeping the
underlying graph and replacing the type embedding by the unique empty-type
embedding.  After proving that this operation respects flag isomorphism, it is
lifted to flags as \lean{unlabel}.
\begin{lstlisting}
def unlabeledGraph (G : LabeledGraph (*@$\sigma$@*) V) : LabeledGraph emptyType V :=
  -- same graph, with the empty type embedding

noncomputable def unlabel
    : Flag (*@$\sigma$@*) V (*@$\to$@*) Flag emptyType V :=
  Quotient.lift (fun G : LabeledGraph (*@$\sigma$@*) V => unlabeledGraphQuot G)
    (fun _ _ h => unlabeledGraphQuot_respect_eqv h)
\end{lstlisting}

Only after these two quotient lifts do we define the downward image of a single
flag:
\begin{lstlisting}
noncomputable def downwardFlag (F : Flag (*@$\sigma$@*) (Fin n)) : FlagVector emptyType :=
  downwardNormalizingFactor F (*@$\bullet$@*) basisVector ((*@$n$@*), unlabel F)
\end{lstlisting}
This is the Lean version of $\llbracket F\rrbracket_\sigma =
q_\sigma(F)\cdot F|_\emptyset$.  The remaining task is not merely to extend
this function linearly.  Since \lean{FlagAlgebra} $\sigma$ is a quotient of
\lean{FlagVector} $\sigma$ by the zero-space relations, the linear extension
must be shown to send every zero-space relation in the typed algebra to a
zero-space relation in the empty-typed algebra.

The main combinatorial lemma behind this lift is a double-counting identity.
It compares two ways of first forgetting labels and then expanding to a larger
unlabeled flag: either count copies of \lean{unlabel F} and multiply by the
normalizing factor of $F$, or sum over all label extensions of the larger
unlabeled flag.
\begin{lstlisting}
theorem flagDensity_mul_downwardNormalizingFactor_eq_sum_labelExtensions
    (F : FlagWithSize (*@$\sigma$@*) (*@$\ell$@*))
    (F' : FlagWithSize emptyType (*@$\ell'$@*)) (h : (*@$\ell \leq \ell'$@*)) :
    flagDensity(*@$_1$@*) (unlabel F) F' * downwardNormalizingFactor F =
      (*@$\sum$@*) G (*@$\in$@*) labelExtensions F' (*@$\sigma$@*),
        flagDensity(*@$_1$@*) F G * downwardNormalizingFactor G
\end{lstlisting}
This identity is then used to prove that \lean{downwardFlag} respects the
expansion equations generating the zero space:
\begin{lstlisting}
lemma downwardFlagVector_zeroSpace
    (f : FlagVector (*@$\sigma$@*)) (hf : f (*@$\in$@*) ZeroSpace (*@$\sigma$@*)) :
    downwardFlagVector f (*@$\in$@*) ZeroSpace emptyType

noncomputable def downward
    : FlagAlgebra (*@$\sigma$@*) (*@$\to$@*) FlagAlgebra emptyType :=
...
\end{lstlisting}
Thus the concise paper notation $\llbracket-\rrbracket_\sigma$ hides two
successive quotient lifts and a substantial compatibility proof with the
zero-space presentation of the algebra.
...
object called a random homomorphism; instead, for an empty-typed positive
homomorphism $\phi_0$ with positive $\sigma$-density, Lean constructs a
probability measure on \lean{PositiveHomSpace} $\sigma$.  Its defining property
is the formal version of Equation~\eqref{eq:ensemble}:
\begin{lstlisting}
theorem exists_probMeasure_extend_emptyType_positiveHom
    {(*@$\phi_0$@*) : PositiveHom emptyType}
    ...
    (*@$\exists$@*) (P : ProbabilityMeasure (PositiveHomSpace (*@$\sigma$@*))),
      (*@$\forall$@*) (f : FlagAlgebra (*@$\sigma$@*)),
        ...
        =
        (*@$\phi_0$@*) [[f]]0 / (*@$\phi_0$@*) [[(1 : FlagAlgebra (*@$\sigma$@*))]]0
\end{lstlisting}
...
typed element is semantically non-negative in the empty-typed algebra.
\begin{lstlisting}
theorem downward_preserve_semanticCone
    (f : FlagAlgebra (*@$\sigma$@*)) (hf : f (*@$\in$@*) semanticCone (*@$\sigma$@*)) :
    [[f]]0 (*@$\in$@*) semanticCone emptyType
\end{lstlisting}
...
random-extension measure above rewrites $\phi_0([[f]]_0)$ as a positive
