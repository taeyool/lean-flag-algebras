...
blow-ups of $C_5$ for the Erd\H{o}s pentagon theorem) attains the
...
density.  The further inequalities are two results of Goodman, a lower bound
on the triangle density in terms of the edge density and a bound on Ramsey
multiplicity, proved with the same flag-algebra infrastructure outside the
...
The Mantel development predates the compiler of \Cref{sec:flagmatic} and the
forbidden-graph semantics of \Cref{sec:forbidden}, and uses neither.
\lean{Mantel\_flag\_bound} formalizes the certificate derivation of
\Cref{sec:flagmatic-background} directly, restated in the ambient algebra
with no triangle-free condition anywhere: the
triangle terms that the constrained calculation drops are instead collected
in the extra summand \lean{K3} on the right, rather than removed under a
\lean{forbidLE} hypothesis.  Evaluating at a positive
homomorphism $\phi$ with $\phi([K_3])=0$ recovers the conditional bound
$\phi([K_2])\leq\tfrac12$; this is \lean{Mantel\_flag\_bound'}, and the remaining
argument runs entirely from this pointwise form.

\lean{extremal\_density\_K3\_ge} formalizes the lower-bound construction, the
balanced complete bipartite graph
$K_{\lfloor n/2\rfloor,\lceil n/2\rceil}$: it is triangle-free, since among
any three of its vertices some two lie on the same side and are non-adjacent,
and its $\lfloor n^2/4\rfloor$ edges make up at least half of the
$\binom{n}{2}$ vertex pairs.  In its statement, \lean{extremalNumber} is
Mathlib's classical
...
\lean{Mantel\_Turan} combines the two bounds, mirroring the compactness
proof of \Cref{lem:semantic-bound} directly, again without the machinery of
\Cref{sec:forbidden}: if the normalized extremal numbers, which
...
contradicting \lean{Mantel\_flag\_bound'}.  The normalized extremal numbers
therefore converge to $\tfrac12$, giving \lean{Mantel\_Turan}.
