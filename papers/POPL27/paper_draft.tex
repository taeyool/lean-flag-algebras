...
\end{lstlisting}
This pattern appears throughout the development.  It is used, in
particular, to prove \lean{flagDensity\allowbreak\_permute} from
\Cref{sec:reordered-flag-tuples}.  In that proof, \lean{S0} and
\lean{S1} are the sets of tuples of disjoint induced copies of the entries
of \lean{Fl} and of \lean{Fl.permute $\pi$}, and the bijection reindexes
each tuple by~$\pi$.  Without the
explicit \lean{let} bindings, Lean can synthesize a \lean{Fintype} for each
set through a different path at each use; the equality produced by
\lean{Fintype.card\_congr} then carries one pair of \lean{elems} terms while
the goal carries another, and the two \lean{Fintype.card} expressions fail
to match.  The
...
The second response repairs mismatches that do occur.  The residual goal
then has a characteristic shape: an equality whose two sides print
identically and differ only in the inferred instances inside them.  The
...
enumeration hypotheses for both vertex-type families, and once the families
and the tuples have been identified, the two sides of the goal differ only
in those hypotheses, which \lean{congr!} resolves.

The bookkeeping above is not eliminated, but its location is made explicit.
When a new finite construction is introduced, the formalization chooses the
\lean{Fintype} instance its cardinalities will use and records that choice
once, in a local binding or helper definition.  The subsequent proofs
reuse the choice instead of asking typeclass search to reconstruct it at
every occurrence.  Thus, a new counting construction still
