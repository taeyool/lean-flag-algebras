...
which builds a hereditary graph constraint, such as ordinary \(H\)-freeness,
into the
...
It remains to expand the square in the rooted three-vertex basis and apply the
...
terms in this derivation: the two rooted flags $\fKtwobull$ and
...
Assume that \(\sigma\) is non-degenerate for \(\mathcal K\): equivalently,
...
support \(\operatorname{supp}(\mu)\) consists of those points \(\chi\) for
