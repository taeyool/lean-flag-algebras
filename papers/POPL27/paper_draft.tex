...
The purpose of the specification layer is to make this typed version of the
mathematics usable for later certificate proofs.  It provides the objects and
compatibility theorems corresponding to the informal interface of
...
of algebra elements.  Once these facts are proved once, the later proof scripts
can work with algebraic expressions rather than repeatedly returning to the
underlying graph representatives and finite sampling definitions.

A second, independent design choice concerns where forbidden-subgraph
assumptions enter the development.  In the background section we fixed
...
predicate on the semantic order.  This choice keeps the algebraic infrastructure
reusable across different extremal problems; the price is that we must prove
that the deferred predicate has the same meaning as the usual constrained
flag-algebra semantics when it is used to derive Tur\'an-density bounds.
