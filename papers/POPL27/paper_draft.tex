...
well-definedness on quotient classes, comparisons between flag expansions at
different vertex sizes, and \lean{Fintype}-instance ambiguity, together with
the bijective proof pattern used for density equalities.
...
\Cref{sec:unsure,sec:lessons} discuss open design questions and lessons for
future formalizations, and \Cref{sec:related,sec:conclusion} give related work
and conclusions.
...
For a non-complete $H$, a subgraph copy may have additional edges and therefore
need not be an induced copy of $H$.  The alternative tactic
...
\begin{table}
  ...
  is A when  the size of the target graph $n_{\mathit{obj}}$ is the same as that of the host graph $N$ and B when $n_{\mathit{obj}}<N$; $T$ is the
...
distinct subgraphs.  Thus, for every $n$, there is a triangle-free graph on
$5n$ vertices containing at least $n^5$ induced copies of $C_5$.  Its
normalized $C_5$ count satisfies
\[
  \frac{n^5}{\binom{5n}{5}}
  \;\geq\;
  \frac{n^5}{(5n)^5 / 5!}
...
presentation instead fixes the constraint before constructing the
corresponding algebra.  As \Cref{sec:forbidden} explains, these choices are
not interchangeable for every typed inequality: the ensemble order tests
almost every randomly rooted view, whereas a built-in constrained algebra can
also test specially chosen roots.  For the final unlabeled Tur\'an bounds in
our case studies, the transfer theorem validates the deferred route.

The unresolved question is therefore architectural rather than a claim of
semantic equivalence.  Deferral lets the core definitions and algebraic lemmas
be reused across forbidden graphs; a built-in theory may make
constraint-specific typed arguments more direct.  Comparing the total cost
would require carrying comparable case studies through both designs.
...
two occurrences of the same finite type with different instance terms, and
the use of \lean{cast} and \lean{HEq} arises because pointwise-equal dependent
type families need not be identical by computation.  Other proof assistants
organize finite enumeration, implicit arguments, and dependent equality
differently.  We therefore do not know how much of this local scaffolding
would survive a port to Rocq, Agda, or Isabelle/HOL.  The combinatorial
obligations would remain, but their proof-engineering form may change.

\paragraph{Is the bridge layer essential or an artifact of our specification
choices?}
The reflection layer and the automation built on it
(\Cref{sec:reflection,sec:flagmatic-pipeline}) are necessary in the current
development.  Reflection connects the quotient-based specification to the
executable \lean{Sym2Graph} representation used by the pentagon proof.  A
specification based on canonical graph representatives
might avoid the same adequacy boundary, but would expose canonicalization
choices inside the mathematical layer.  Our development shows that the
separation works and concentrates representation-specific proofs in the
bridge; it does not show that this separation minimizes the total proof or
computation cost.  Answering that question requires an alternative
formalization against which to compare it.

\paragraph{Will theorem generation scale beyond five-vertex hosts?}
The pentagon proof generates 2,832 specialized pair-density theorems, in 15
native-evaluation batches, at elaboration time.  Lean accepts this workload in
our current case study.  Moving to larger host sizes would increase both the
number of flags and the number of flag pairs whose densities are generated.
We have not stress-tested that regime and do not know whether flag
enumeration, density and multiplication theorem generation, or the growing
environment of named theorems would become the first bottleneck.  The present
evaluation therefore establishes feasibility through five-vertex hosts, not
scalability to substantially larger certificates.
...
ambient algebra and imposes the constraint through the ensemble semantic
order when an inequality is stated.  This deferral makes unconstrained
algebraic lemmas reusable, and the transfer theorem recovers the final
unlabeled Tur\'an bounds used in our case studies.

The change is not merely organizational.  As \Cref{sec:forbidden} explains,
the built-in semantics can test a specially chosen root, whereas the ensemble
semantics tests almost every root obtained by random labeling.  The two orders
therefore need not validate the same typed inequalities.  The lesson is not
that constraints should always be deferred, but that moving a constraint
across an abstraction boundary can change the meaning of intermediate
statements.  A formalization should state the resulting semantic order
explicitly and prove the comparison it needs rather than treating deferral as
a semantics-preserving refactoring.
...
The 2,832 generated pair-density theorems are discharged by
\lean{native\_decide}, because kernel evaluation of these numerous closed
reflection-layer computations is not practical at the current scale.  For
the pentagon proof, the three SDP matrix equalities (sizes
...
of flag-algebra certificates, but it does not itself produce proof-assistant
proof terms.  To our knowledge, our work provides the first formally verified
translation from such certificates to combinatorial theorems.
Razborov~\cite{razborov2013flag} gives a
...
graph limit theory exists in any proof assistant; our treatment of flag
sequences and random homomorphisms provides a partial formalization sufficient
for the flag algebra method, but a full formalization of the graphon theory
