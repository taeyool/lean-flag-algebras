...
inequalities in extremal graph theory, often reducing the task to a search for
a finite certificate using semidefinite programming.  The method is
particularly well suited to constrained problems, where one seeks bounds over
...
The formalization covers the mathematical content of the method: partially
...
output of the external semidefinite programming search only as candidate
certificate data: lists of flags, target-density data, and rational
positive-semidefinite matrices.  It then directs Lean to compute the required
density and multiplication facts independently, check those computations
...
flag-algebra proofs.  As case studies, the compiler yields machine-checked
...
$K_4$-free, $K_5$-free, and $C_5$-free graphs, and, most notably, the
Erd\H{o}s pentagon theorem, for which no proof avoiding flag algebras is
known.
...
testing inequalities afterwards on random labeled views of constrained graph
limits.  We briefly state the resulting root-plantability criterion for when
...
bound must guarantee that the proportion of induced copies of $F$ never exceeds
...
over an infinite graph class to checking finitely many algebraic conditions.
A semidefinite-programming (SDP) solver proposes finite data intended to satisfy
these conditions; we call these data a candidate certificate.  The flag algebra
...
density of pentagons in triangle-free graphs, and it determined the minimum
triangle density for a given edge density~\cite{razborov2008triangles}.  It
settled the maximum induced $C_5$ density~\cite{balogh2016induced5} and the
rainbow-triangle density problem~\cite{balogh2017rainbow}.  It also gives the
best known upper bound on Tur\'an's tetrahedron
problem~\cite{razborov2010tetrahedron}, and has produced several exact
...
We formalize the flag algebra framework itself in Lean, following Razborov's
development: partially labeled graphs and their densities in large graphs, the
quotient algebra of density expressions, graph-limit semantics through positive
homomorphisms, and the downward operator that transfers labeled reasoning back
to unlabeled graph statements.  We call this the \emph{specification layer}
(\Cref{sec:abstract}).  Its definitions stay close to the mathematics: flags
are isomorphism classes of labeled graphs, densities are sampling
probabilities, and the algebra is a quotient by expansion identities.  Its
theorems are the ones a flag-algebra argument actually appeals to, so such
an argument can be stated in Lean much as it is stated on paper.

\paragraph{From certificates to formal proofs.}
Stating the method is not the same as checking its applications.  Such a
candidate certificate is compact to write down but large to audit: the
Erd\H{o}s pentagon certificate rests on thousands of exact density values and
on rational matrices whose positive-semidefiniteness must be established
exactly.  Nobody verifies such data by hand, and a formal proof should neither
trust it nor replay it as thousands of hand proofs.  Our certificate-to-proof
compiler closes this gap, supported by two further layers.  The
...
they support agree with the specification; Lean therefore recomputes the
density and multiplication facts a certificate needs instead of trusting them.
The \emph{automation layer} (\Cref{sec:flagmatic}) verifies
positive-semidefiniteness exactly over $\mathbb{Q}$ and packages the recurring
algebraic steps, including expanding flags to a common size, multiplying typed
flags, applying the forbidden-graph assumption, and normalizing large linear
combinations, into tactics and elaboration commands.

\paragraph{Constrained semantics.}
Constrained extremal problems also expose a mathematical choice about when a
constraint is imposed.  In the \emph{built-in-constraint} approach, one builds
a hereditary graph constraint, such as \(H\)-freeness, into the
flag algebra
from the start; this is the traditional way to work in an algebra of
\(H\)-free graphs.  In the \emph{ensemble} approach, one first works
...
finite labeled pattern.  Our investigation proves a \emph{root-plantability} criterion
characterizing when they do agree, gives a positive result for
blow-up-closed classes that also covers substitution-closed classes, and exhibits a
...
to an extremal-density or equality slice.  This is the semantic viewpoint
implemented by \lean{forbidLE} in our formalization.
...
proofs.  Second, it builds a certificate-to-proof compiler that turns
externally generated rational certificate data into Lean-checked algebraic
proofs.  Third, it demonstrates the compiler on seven certificates, including
machine-checked proofs of Mantel's theorem, the Erd\H{o}s pentagon theorem,
and a $C_4$-density bound for triangle-free graphs; outside the certificate
pipeline, the same infrastructure proves Goodman-type inequalities and the
matching lower bounds.  Fourth, it develops the
semantic comparison between built-in-constraint and ensemble approaches to constrained
flag algebras, including the root-plantability criterion and the resulting
generalization beyond hereditary constraints.  A separate, automatically
generated Lean development formalizes this meta-theory on top of the manually
developed flag-algebra library.  The complete Lean formalization, including
the certificate-to-proof compiler and the meta-theory development, is publicly
available at \url{https://github.com/taeyool/lean-flag-algebras-release}.

\paragraph{Organization.}
\Cref{sec:background} recalls the mathematical background of flag algebras.
\Cref{sec:abstract} develops the specification layer and the
semantic treatment of forbidden-graph assumptions.
\Cref{sec:reflection} develops the reflection layer: executable graph
representations and the adequacy theorems that connect them to the
specification.
\Cref{sec:flagmatic} develops the automation layer: SDP-certificate
verification, elaboration-time theorem generation, the tactics that assemble a
certificate proof, and the verified results.
\Cref{sec:lowerbounds} explains how the same infrastructure proves lower
bounds and inequalities outside the certificate pipeline.
\Cref{sec:obstacles} describes the main formalization obstacles and the
bijective proof pattern used for density equalities.
\Cref{sec:metatheory} summarizes our mathematical meta-theory of constrained
semantics and its separate autoformalization, and \Cref{sec:unsure} records the
remaining design questions.
\Cref{sec:related,sec:conclusion} give related work and conclusions.
...
combinatorics is assumed.  
For an integer $n$, we write $[n]$ for the set of all positive integers less than or equal to $n$.
All graphs~$G$ are undirected, finite and simple,
...
forbidden graphs.  Throughout the paper, an unqualified \emph{subgraph} is an
ordinary, not necessarily induced, subgraph; when vertex-induced containment
is intended, we say \emph{induced subgraph} explicitly.  If $G$ is a graph and
$S\subseteq V(G)$, the
...
(that is, $\theta_F$ is a graph embedding of $\sigma$ into $M$).
...
The remaining $|V(F)|-k$ vertices are \emph{unlabeled}.
...
\begin{figure}[t]
  ...
  the filled vertex is the labeled one.  The last two three-vertex flags,
  $P_3^\bullet$ and ${P_3'}^\bullet$, have the same underlying graph and differ
  only in which vertex is labeled.}
...
intermediate flag $G'$ of some size $n'$ with $m \leq n' \leq n$ inside $G$ and
...
combinations of $\sigma$-flags.  For a flag $F$, write $\mathbf e_F$ for its
...
whose value at every such limiting object is non-negative, and establishing
...
number to each flag-algebra element, respects addition, multiplication, and the unit, and
never assigns a negative value to a flag basis element.
...
are exactly the limiting objects of convergent sequences of
...
built from \(\mathcal{H}\)-free graphs, and later sections vary $\mathcal{H}$.
...
limiting object.
Whenever $\phi_0(\langle\sigma\rangle_0) > 0$ (i.e., $\sigma$ has positive
density as an unlabeled graph in the limiting object),
...
\begin{proof}[Proof sketch]
  ...
  If $\phi_0(\langle\sigma\rangle_0) = 0$, then the flag type $\sigma$ has zero
  density in the limiting object, and every $\sigma$-flag density also
...
Our first design choice is to keep the Lean definitions as close to the
mathematics of \Cref{sec:background} as Lean allows, rather than shaping them
for evaluation.  Flags remain isomorphism classes of labeled graphs, densities
remain finite sampling probabilities, the flag algebra remains a quotient by
expansion identities, and positive homomorphisms remain semantic
interpretations of algebra elements; the subsections below take these in turn.
Together with the maps between the roles above and the interface theorems
justifying them, these definitions form the \emph{specification layer}.  Many
of them are consequently \lean{noncomputable}; the executable counterparts, and
the adequacy theorems tying them back to the definitions here, are developed
separately in \Cref{sec:reflection}.

A second design choice concerns where forbidden-subgraph assumptions enter the
development.  In \Cref{sec:background} we fixed $\mathcal{H}$ first and
...
graph whose vertices are the elements of the type \lean{V}.  Finiteness is
supplied separately by assumptions such as \lean{Fintype V}.  For example,
...
implicit parameters, which Lean normally infers from context, and
square brackets mark typeclass assumptions.  Thus \lean{[Fintype T]} tells
Lean that the label type
...
\end{lstlisting}
\lean{Quotient} comes with a double-bracket notation for equivalence classes.
In Lean code, the bare notation \(\llbracket x\rrbracket\), with no subscript,
means ``the quotient class of the representative \(x\),'' with the relevant
quotient inferred from the expected type.  Thus, when the expected type is
...
\bigcup_n \mathcal{F}^\sigma_n$, defined as a dependent pair type in Lean; each
element is a pair $\hat F = \langle n, F\rangle$ where $n$ records the size and $F$ is the
$\sigma$-flag of that size.  In Lean the two components are written
$\hat F.1$ and $\hat F.2$.
\begin{lstlisting}
...
make this a function of flags, we lift these two inputs separately.  The host
...
host or any pattern flag is replaced by an isomorphic representative.
\begin{lstlisting}
...
large enough to contain the two input flags after identifying their common
...
type are sampled disjointly in the two flags, while the \(n_0\) labeled vertices
...
\(\phi:\mathcal{A}^\sigma\to\mathbb{R}\) such that
\(\phi([F])\ge 0\) for every finite \(\sigma\)-flag \(F\).  The Lean definition follows this sentence almost
literally:
\begin{lstlisting}
...
\]
In other words, the only extra condition on an algebra homomorphism is
...
factor.  On a single flag $F$ the intended formula is
\[
...
graphs to flags, a linear extension from flags to flag vectors, and a final
...
\(\phi_0(\llbracket f\rrbracket_0)\) from typed evaluations: randomly label the
limiting object represented by \(\phi_0\) with a copy of \(\sigma\), evaluate
\(f\) in the resulting \(\sigma\)-typed positive homomorphism, average these
values, and multiply by the positive normalizing denominator \(\phi_0(\llbracket \mathbf{1}_\sigma\rrbracket_0)\).
...
a mathematical statement this is still a proposition, not a program.  The
...
def candidateMatches {k m n : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k}
  ...
  -- the graph by H.toLabeledSubgraph.coe, which forgets the subgraph structure.
...
to a finite search.  The size branch is not conceptually necessary for the
...
\lean{Sym2LabeledGraph}, Lean enumerates the finite set of type embeddings that
produce an equivalent flag, takes its cardinality, and divides by the total
number \(n!/(n-k)!\) of injections from $[k]$ to $[n]$.
...
def isoEmbeddingCount_sym2LabeledGraph {k : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k} {n : (*@$\mathbb{N}$@*)}
  ...
  -- isoSym2TypeEmbeddingSetWithSameGraph G is a finite set of embeddings
  -- whose elements can be enumerated by an algorithm, so its cardinality
  -- can be computed.
...
taking the cardinality of the explicitly finite set
\lean{isoSym2TypeEmbeddingSetWithSameGraph G}.  The second performs the
rational normalization.  The final \lean{Quotient.lift} applies this same
...
\end{lstlisting}
and \lean{native\_decide} evaluates that finite computation.  The reflected
normalizing factor makes the downward operator executable on concrete
flags.  The reflection layer computes the underlying unlabeled flag and its
normalizing factor; the adequacy theorem then certifies their product as the
abstract downward image.  By linearity, finite linear combinations of flag
basis elements can be handled term by term, so the downward identities needed in later flag-algebra proofs can
be established computationally rather than by separate counting arguments.

\subsection{From Reflected Computations to Reusable Theorems}
...
its specialization to each concrete flag and to each pattern-pair/host
...
generate statements and proofs for the required specialized facts.  The
resulting theorems are both a reusable interface and a cache: each finite
computation is performed when its theorem family is generated, and later
proofs recover the result by rewriting rather than recomputing it.  For the
...
used in the examples of \Cref{sec:reflection-adequacy}.  Generating the typed
...
this family includes the displayed value \(c=1/2\).  These evaluations are
batched to avoid invoking the native compiler separately for every tuple.
Finally, the multiplication
command uses all of those coefficients to prove the product expansion
\([F_1] \cdot [F_2]=\sum_G c_G\,[G]\) for every pattern pair.  Thus elaboration produces the
flags and their corresponding algebra basis elements, their unlabelings and downward coefficients, the density table,
and the multiplication table, together with proofs of every entry.  Variants
for a fixed forbidden graph first discard flags that cannot contribute and then
perform the same computations on the surviving family.
...
individual results into a reusable table of proved facts.  The next section
uses that table to automate complete flag-algebra arguments.
...
one-vertex type $\sigma$.  The two flags obtained by adding one unlabeled vertex are
...
terms in this derivation: the two labeled flags $\fKtwobull$ and
...
squares.  Choose flags $E_1,\ldots,E_r$ of the same type, let
...
\begin{enumerate}
  \item \textbf{Flags and density data.}  The named flags and corresponding
    ...
    single-flag densities, flag-pair densities, and products of typed flags.
...
\lean{Sym2Graph} term and selects one of two generation routes.  Complete-graph
forbids use the \lean{generate\_pruned\_*} commands, while a general graph
forbidden as a subgraph uses the parallel
\lean{generate\_subgraph\_free\_*} commands.  During elaboration, both routes
enumerate the flags that avoid the forbidden graph, with isomorphic flags
appearing only once.  For each enumerated flag, the commands emit a concrete
...
in Parts 3 and 4 of the proof. Concretely, for each emitted typed flag $F$, Part~1 evaluates the downward normalizing
factor~$q_\sigma(F)$ from \Cref{sec:formal-downward}.
For each relevant pair of typed flags $F_1,F_2$ and host flag $G$, it also
...
always represented by a flag of that size.  This splits the certificates into
...
$\mathcal{H}$-free flags, with coefficients given by the single-flag densities
generated in Part~1.
...
\Cref{def:ensemble-semantic-order}, whereas the host flags were enumerated and
filtered using the concrete graph $K$.  The argument \lean{hmem} connects these
two representations: it proves that the flag decoded from $K$ belongs to
\lean{forbiddenFlags H}, the set of flags whose underlying graphs contain $H$
as a subgraph.
Consequently, the $H$-free condition forces the decoded $K$ to have density
zero, which justifies removing from the objective expansion the host flags
...
each size-$\ell_1$ flag into flags of size $\ell_2$ and apply the chain rule of
...
\]
A size-$\ell_1$ flag $G'$ is therefore stored as the tagged pair
$\langle\ell_1,G'\rangle$, while a size-$\ell_2$ flag $G$ is stored as
...
tagged flags index basis vectors in the same vector space, and both product
...
between two linear combinations whose basis flags carry different size tags.
In the Lean proof, \lean{flagExpansion} constructs the size-$\ell_2$ linear
combination for each tagged size-$\ell_1$ flag, and the zero-space relation
...
type \lean{Flag $\sigma$ $V_i$}, so the type of the returned flag varies with
...
position $0$ now contains a flag on $U_2$ and position $1$ a flag on $U_1$.
...
expressions to the same term.  Because the type of the whole flag tuple depends
...
representatives of its input flags, and to show that
...
intermediate flag.  On paper, these procedures are readily seen as two
...
homomorphisms satisfying that condition, chooses an occurrence of the flag
type at random in the limiting object represented by each homomorphism, and
requires the desired inequality to hold with probability one over that choice.
...
graphs lie outside \(\mathcal K\), span an ideal,\footnote{An ideal \(I\) in a
commutative \(\mathbb{R}\)-algebra \(A\) is a linear subspace, and hence is closed
under addition and real scalar multiplication, that is also closed under
multiplication by arbitrary elements of \(A\): if \(x\in I\) and \(a\in A\),
then \(ax\in I\).  This property makes the quotient \(A/I\) an algebra in which
every element of \(I\) is identified with zero.} and the usual constrained
...
remaining flag to its counterpart in the constrained algebra.
...
flag $F$ appearing in the product of a flag $F'$ outside \(\mathcal K\) with another
flag $F''$ contains the first flag $F'$ as an induced subflag.  It therefore also lies
outside \(\mathcal K\), since otherwise heredity would put that induced subflag $F'$
back in \(\mathcal K\).  Hence multiplying a flag outside \(\mathcal K\) by any
other flag produces a linear combination of flags outside \(\mathcal K\), so
their span is precisely the ideal killed by \(q_\sigma\).  Once this
presentation is established, a homomorphism vanishing on every flag outside~\(\mathcal K\) vanishes on the whole ideal and therefore factors through
...
the countable collection of flags outside \(\mathcal K\) shows that a positive
...
records a flag-density profile relative to a specified embedding
...
\(\widehat\theta:\sigma\to H\) whose flag-density profiles approximate the
...
each such flag \((G,\theta)\), the proof repeatedly applies the preceding
...
finite flag is sampled from the blow-up, its sampled vertices lie in distinct
...
of the replacement graphs \(J_v\) does not affect the sampled flag.
...
Because \(X_\sigma\) has the topology of pointwise convergence on flags,
...
estimate preserves the required flag densities.
...
flag-density profiles approximating the profile of a given sufficiently large
...
density of a particular labeled flag takes one fixed value with probability
one, while the constrained quotient contains a positive homomorphism at which
that flag has a different density.  Finally, the forthcoming work will extend
...
and specialize the adequacy theorems to produce reusable density and
