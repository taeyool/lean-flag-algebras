...
The two examples in the preceding subsection show the basic unit of work.  To
prove the concrete density
...
What repeats in a concrete development is their specialization to every flag,
pair of pattern flags, and host flag that the later algebraic proof will use.
...
automatically generate the statements and proofs of the required specialized
facts.  This both makes those facts available as
ordinary reusable theorems and avoids repeating their finite computations at
every later use: the computation is performed while the theorem family is
generated, after which later proofs reuse the checked result by rewriting.  In
this sense, the generated theorems form a certified cache of the reflected
computations.  For the sizes and flag type used in the two examples above, the
relevant commands have the following form:
\begin{lstlisting}
...
The reader need not remember these numerical parameters, but their convention
explains how the commands fit together.  The argument of
...
The important point is the output of each group, rather than the particular
indices.  The first four commands create the named two- and three-vertex flags
...
generates a theorem \(\lean{flagDensity}_2\,F_1\,F_2\,G=c\) for every pair of
two-vertex patterns \((F_1,F_2)\) and every three-vertex host \(G\); this family includes the
displayed value \(c=1/2\).  These evaluations are batched to avoid invoking the
native compiler separately for every tuple.  Finally, the multiplication
...
Thus the pipeline of this section is: executable concrete representations,
uniform adequacy theorems, and elaboration-time generation of a reusable,
checked table.  The next section uses this table to automate recurring
flag-algebra proofs.
