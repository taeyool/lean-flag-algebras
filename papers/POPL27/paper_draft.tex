...
\lean{LeanFlagAlgebras/Flagmatic} directory within the formalization repository
\begin{lstlisting}[language={}]
python flagmatic_to_lean.py gen-skeleton \
  Certificates/mantel_cert.json Mantel.lean \
...
command
\begin{lstlisting}[language={}]
python flagmatic_to_lean.py inspect Certificates/mantel_cert.json
\end{lstlisting}
...
A wrong reconstruction or factorization therefore makes the generated proof
fail.
...
The first step starts with applying this
theorem with 
the reflexive inequality $[P]\leq_H[P]$, 
and repeatedly applying the theorem 
to the result once per each SDP
block.  After all blocks have been added, its conclusion is precisely
$[P]\leq_H[P]+\sum_tQ_t$.
...
Expanding each quadratic form produces downward images of products of basis algebra
...
$c\cdot\mathbf 1-r\leq_Hc\cdot\mathbf 1$.

\paragraph{The translator.}
The four parts above describe the Lean output.  The external script that
produces this output is not verified; instead, every mathematical claim it
emits must be accepted by Lean.  A certificate names its flags as short strings
...
table to keep in sync.  With the indices in hand, it emits the four parts.  Most
of the output comes from templates keyed to the sizes; the substantial
translation step reconstructs and factors the certificate matrices, producing
the witness consumed by Part~2's kernel check.  The generator also chooses the
branch, the per-block parameters, and the order of the additions.  Why errors in
any of these choices cannot silently establish a false theorem is the subject
of \Cref{sec:flagmatic-trust}.
...
theorem mantel_flagAlgebra                               -- Part 4: main theorem
  ...
  ...                                                     -- rewrite the unit expansion
  rw [forbidLEWith_rw_left_add_right mantel_expand_under_forbid]
  ...
  apply forbidLEWith_of_le; flag_nonneg                  -- close by non-negativity
\end{lstlisting}
Here \lean{K3} is the concrete \lean{Sym2Graph} consumed by the generators,
whereas the theorem is stated using the specification-level graph
\lean{completeGraph (Fin 3)}.  The final membership argument in the generation
and expansion calls supplies the bridge required by the ordinary
forbidden-graph framework.  A Branch~A certificate drops the Part~3 lemma;
extra SDP blocks add one matrix and one
\lean{forbidLEWith\_add\_QuadraticForm} call each.

\subsection{Trust Model}
\label{sec:flagmatic-trust}

The translator computes a great deal, from flag indices to matrix
factorizations, but neither it nor the SDP solver must be trusted: their output
is accepted only through proof obligations generated in Lean.  Those
obligations are closed by \lean{decide +kernel}, by
\lean{native\_decide}, or by tactics that construct proof terms checked by the
kernel.  A mistake in the certificate or translator therefore causes a failed
check rather than silently establishing a false theorem.  The important
trust distinction lies between the two evaluators used for those checks.
...
The tactics are themselves unverified metaprograms, and the same discipline
applies to them.  They traverse elaborated expressions, recover flag constants
through the naming convention, and decompose the normalized forms of sums,
scalar multiples, products, and downward operators.  This machinery is among
the least portable parts of the development because it depends on the shapes
produced by Lean's elaborator.  Nevertheless, a tactic that mishandles such a
shape can only fail or construct a term that the kernel rejects; it cannot make
an invalid proof term valid.

\subsection{Evaluation}
\label{sec:results}\label{sec:flagmatic-scenarios}

To evaluate the compiler, we ran
...
and four forbidden graphs ($K_3$, $K_4$, $K_5$, and the cycle $C_5$).  Every
generated file compiles with no \lean{sorry}
(Table~\ref{tab:flagmatic-scenarios}), and no certificate coefficients had to
be copied into Lean by hand.  Three are classical extremal results, whose upper
bounds we describe below; the remaining cases appear in the table.
...
This is the smallest and most transparent case.  Its single block is small
enough to write the certificate by hand.  As explained in
\Cref{sec:flagmatic-background}, the PSD block is the explicit square
$\llbracket
  \tfrac12(\fKtwobull-\fbarKtwobull)^2
\rrbracket_\sigma$; together with the non-negative
remainder $\tfrac13\fbarPthree$, it proves
$K_2 \leq \tfrac12\cdot\mathbf{1}$ without an external solver.  It therefore
serves as the simplest validation of the compiler.

\paragraph{A $C_4$-density bound.}
This certificate is a sum of two PSD forms, one per size-$2$ type
($4\times4$ and $3\times3$ over $\mathbb{Q}$), again with no externally
generated SDP.  It proves a
$\tfrac38\cdot\mathbf{1}$ bound on $C_4$ density, and is the first example to
require the compiler to combine multiple independently certified PSD blocks into
a single proof.

\paragraph{The Erd\H{o}s pentagon theorem.}
This is the most demanding of the three, with PSD blocks of sizes $8$, $6$,
and $5$ that the tactics must add, expand, and normalize in a single pass.  It proves
...
Together with the remaining scenarios in
Table~\ref{tab:flagmatic-scenarios}, these examples exercise every structural
variation of the compiler described in
\Cref{sec:flagmatic-pipeline}, from a single hand-checkable certificate to
multi-block SDP certificates requiring substantial symbolic normalization.

\paragraph{A non-complete forbid.}
The \lean{C5turan} certificate forbids a non-complete graph: it bounds edge
density at $\tfrac12$ in graphs with no $C_5$ subgraph.  For a complete
forbidden graph, forbidding an induced copy
and forbidding a subgraph coincide, and the generators take a faster induced
path.  The $C_5$ case is the first evaluated certificate in which the two
notions differ, so it instead uses the subgraph-forbidding route.  The four-part
skeleton is otherwise unchanged; the compiler does not assume that the
forbidden graph is complete.
...
not proved that it normalizes every form a future certificate might produce.
This is a limitation of automation rather than soundness: if normalization
fails, the generated proof remains incomplete.  We list the issue alongside
the open questions of Section~\ref{sec:unsure} rather than presenting the
automation as complete.
...
skeleton, the translator reconstructs it from the certificate alone, and Lean
checks the result under the trust model stated above.
...
\[
  ...
  \frac{24}{625},
\]
combined with the convergence theorem
\lean{tendsto\_generalizedTuranDensity}.  Mathematically, the lower bound uses
...
\paragraph{A $K_4$-free $P_4$ bound.}
The theorem \lean{K4\_free\_P4\_density\_upper\_bound} provides a further test
of the automation layer:
\begin{lstlisting}
theorem K4_free_P4_density_upper_bound
    : inducedForbidLE K4.toFinFlag P4_density ((32 / 9 : R) (*@$\bullet$@*) 1) := ...
\end{lstlisting}
Its proof adds three downward-projected squares and then uses the same product
reduction, unit expansion, sum normalization, and basis non-negativity tactics
as the preceding case studies.  This example is stated using the induced-forbid
relation and shows that the automation is not tied to triangle-free problems.


\section{Engineering Obstacles}
\label{sec:obstacles}

The specification layer of \Cref{sec:abstract} exposed four recurring
engineering obstacles.  Quotient eliminators carry compatibility proofs;
size-changing combinatorial operations produce dependent-type transports;
function extensionality appears when pointwise information must be turned into
a function-level equality; and \lean{Fintype} instances synthesized through
different elaboration paths can be extensionally equal without being
definitionally equal.  We report these as obstacles rather than design
principles and describe how the implementation confines each one.  The point
is not the particular workarounds; it is that an
...
After describing the four obstacles, we close with a recurring proof pattern
that emerged from the last one: proving density equalities through explicit
bijections between finite sample spaces.
...
\subsection{Bijective Proofs for Density Equalities}
...
compilation preserves the specification's meaning.  In this development, when
