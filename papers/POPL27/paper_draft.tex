...
\Cref{sec:flagmatic-background}.  The $C_4$-density certificate combines blocks
of sizes $4$ and $3$, while the Erd\H{o}s pentagon certificate combines blocks
of sizes $8$, $6$, and $5$.  The $K_5$- and $C_5$-free edge-density cases each
use four blocks of size $8$.  Together, these examples exercise both objective
...
This evaluation establishes coverage of the seven cases above, not
...
forbidden graph, and one of the currently supported finite block sizes.
...
normalization shape not handled by the custom tactics.  Each situation is
visible as the absence of a complete Lean proof, not as an unchecked theorem.

\medskip
For the seven evaluated certificates, the translator therefore carries all
coefficients and structural choices into the fixed proof skeleton without
manual editing.  Lean checks the resulting theorem under the trust model above;
the separate question of whether the generated file faithfully transcribes
the source certificate remains at the translator boundary.
