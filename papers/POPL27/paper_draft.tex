...
\end{lstlisting}
The notation \lean{$\to_a$[$\mathbb{R}$]} denotes Lean's type of
\(\mathbb{R}\)-algebra homomorphisms: functions preserving addition,
multiplication, scalar multiplication by real numbers, and the unit.  Thus
\lean{Hom}~\(\sigma\) abbreviates the type of algebra homomorphisms from
\lean{FlagAlgebra}~\(\sigma\) to \(\mathbb{R}\).  The second definition then uses
Lean's subtype notation
\(\{\ \phi : T\ //\ P\ \phi\ \}\), meaning elements \(\phi\) of \(T\) together
with a proof that they satisfy \(P\).  Here \(P\) is the positivity condition:
for every finite \(\sigma\)-flag \(F\), \lean{basisVector F} is the formal basis
vector representing \(F\), \(\llbracket\cdot\rrbracket\) is the quotient map
from \lean{FlagVector}~\(\sigma\) to \lean{FlagAlgebra}~\(\sigma\), and the
inequality says precisely that
\[
  \phi\bigl(\llbracket\lean{basisVector F}\rrbracket\bigr)\ge 0 .
\]
