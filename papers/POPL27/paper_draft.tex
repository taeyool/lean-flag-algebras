...
graphs avoiding a fixed forbidden graph, prove that the density of a target
graph $P$ is at most some constant $c$.  A \emph{flag-algebra certificate} is a
...
$[P] \leq_{\mathcal H} c\cdot\mathbf 1$ works by decomposing the gap
$c\cdot\mathbf 1-[P]$ into expressions that evaluate to non-negative real
numbers under every positive homomorphism satisfying the $\mathcal H$-free
condition.  A \emph{certificate} is finite algebraic data specifying such a
decomposition.  Flagmatic~\cite{vaughan2013flagmatic} searches for this data
using an SDP solver and outputs it as a certificate.  Our compiler consumes
that certificate as untrusted input: it translates the proposed decomposition
into Lean and generates a proof that each of its terms is non-negative and
that their sum is the claimed gap.

Mantel's theorem gives a small example of the whole idea.  Fix the one-vertex
type $\sigma$.  The two flags obtained by adding one unlabeled vertex are
...
Combining the two expansions now gives the desired decomposition:
\[
  \tfrac12\cdot\mathbf 1-K_2
  = \tfrac12\cdot\fbarKthree+\tfrac16\cdot\fbarPthree-\tfrac16\cdot\fPthree
  = \bigl\llbracket
      \tfrac12((\fKtwobull)-(\fbarKtwobull))^2
    \bigr\rrbracket_\sigma
    + \tfrac13\cdot\fbarPthree.
\]
The downward square is non-negative by
Theorem~\ref{thm:downward-nonneg}, and the basis element $\fbarPthree$ is
non-negative because graph densities are non-negative.  Hence the right-hand side is non-negative, proving
$K_2\leq_{K_3}\tfrac12\cdot\mathbf 1$.

The essential certificate data for this example are precisely the data on the
right-hand side of the displayed decomposition: the two rooted flags
$\fKtwobull$ and $\fbarKtwobull$, the halved squared
difference between them, and the non-negative remainder $\tfrac13\cdot\fbarPthree$.  We
chose this small certificate by hand; in larger examples, Flagmatic's SDP
search supplies the analogous flags, PSD matrices, and remainder coefficients.
...
In general, the resulting certificate specifies an identity, modulo the
$\mathcal H$-free condition, of the form
\[
  c\cdot\mathbf{1} - [P]
  \;=\;
  ...
  \;+\; r,
\]
where every $M_t$ is PSD and $r$ has non-negative coefficients.  It records the
chosen flags, the rational matrices $M_t$, and the target-density data needed
to state this identity.  Our compiler does not trust the certificate's derived
density or multiplication tables: it recomputes them in Lean, proves the
matrices are PSD, and checks the identity.  The rest of this section explains the
four recurring parts of that verification and how the compiler generates them.
...
\begin{enumerate}
  \item \textbf{Flags and density data.}  The named flags and flag-algebra
    basis elements of the sizes the proof needs (only those that are free of
    the chosen forbidden graph), together with the proved pair-density and
    typed-multiplication identities used for rewriting.
  \item \textbf{Certificate matrices and basis-element vectors.}  For each
    subflag type $\sigma_t$, a rational matrix $M_t$ paired with a list of
    $\sigma_t$-flags $E_{t,i}$; their basis elements
    $e_{t,i}:=[E_{t,i}]$ form the vector $v_t$ and the quadratic form
    $\langle v_t, M_t\, v_t\rangle$.
  \item \textbf{Lifting the objective (when needed).}  If the flag $P$
    representing the objective is smaller than the host size, one auxiliary lemma first lifts $[P]$ to host size,
  ...
  \item \textbf{The main theorem.}  The bound $[P] \leq_{\mathcal{H}}
    ...
    reducing every downward-averaged product, expanding the unit at host size,
...
For example, when run from the \lean{LeanFlagAlgebras/Flagmatic} directory, the
command
\begin{lstlisting}[language={}]
...
\lean{generate\_subgraph\_free\_*} commands.  Both routes enumerate the
surviving concrete flags during elaboration.  For every retained isomorphism
class, the commands emit a concrete quotient flag---a
\lean{Sym2EmptyTypedFlag} at the empty type or a \lean{Sym2Flag} at a nonempty
type---and its decoded specification-level \lean{Flag} and
\lean{FlagAlgebra} basis element.

The finite downward coefficients and pair densities are evaluated on the
concrete flags.  The adequacy theorems from \Cref{sec:reflection} turn those
evaluations into proved statements about the decoded flags.  The multiplication
generator then derives the required product expansions from the general
specification-level multiplication theorem, the proved pair-density identities,
and the completeness theorem for the generated flag set.  Thus Part~1 leaves
the rest of the proof with specification-level basis elements and rewrite
theorems, although reflection performed the finite coefficient computations
behind them.  These commands compute the enumerations and density data during
elaboration; they do not load precomputed graph, flag, density, or
...
matrices $Q'_t$ and $R_t$, from which the
translator constructs $M_t=R_tQ'_tR_t^\top$.  It then computes an exact rational
factorization $M_t=L_tD_tL_t^\top$ and emits $M_t,L_t$, and the diagonal of
$D_t$.  Lean proves positive semidefiniteness from these emitted witnesses: the
\lean{psd\_real\_ldlt} tactic checks diagonal non-negativity by
\lean{fin\_cases} and \lean{norm\_num}, checks the rational matrix identity with
\lean{decide +kernel}, and applies the proved lemma that transports the result
to $\mathbb{R}$, the scalar field of the flag algebra.  No eigenvalue or
floating-point calculation enters this check.  A wrong reconstruction or
factorization therefore makes the generated proof fail.
...
two branches.  When the objective is smaller (\emph{Branch~B}: Mantel, $K_4$- and
$K_5$-Tur\'an), a single \lean{flag\_expand\_hfree N K hmem} rewrites it over
the $N$-vertex $\mathcal{H}$-free flags, dropping the forbidden host terms;
here \lean{hmem} proves that the concrete forbidden flag represents a member
of the ordinary forbidden-graph condition.  When it is already host-size
(\emph{Branch~A}: the $C_4$-density bound,
Erd\H{o}s pentagon) this step is empty.  The non-complete subgraph-forbid route
uses the corresponding \lean{flag\_expand\_hfree\_subgraph} tactic.

\paragraph{The main theorem (Part~4).}
Part~4 assembles the outputs of the preceding parts.  The standard upper-bound
move is to add one or more PSD quadratic forms to the target basis element and
then check that the augmented algebra element is
$\leq_{\mathcal{H}} c\cdot\mathbf{1}$.  Each addition is performed by
\lean{forbidLEWith\_add\_QuadraticForm}:
\begin{lstlisting}
theorem forbidLEWith_add_QuadraticForm
    (M : Matrix (Fin n) (Fin n) R) (hM : M.PosSemidef)
    (v : FlagAlgebraVec sigma n)
    : forbidLEWith C f g ->
      forbidLEWith C f (g + (*@$\llbracket$@*)flagQuadraticForm M v(*@$\rrbracket_0$@*)) := ...
\end{lstlisting}
iterated once per type to build a transitive chain of additions.  After the
quadratic forms are expanded, the downward-averaged products are discharged by
\lean{reduce\_downward\_flagmul}.  This tactic recognizes each
summand $\llbracket c\,(f\cdot g)\rrbracket$, where $f=[F]$ and $g=[G]$ are
basis elements, and rewrites it with the generated product identity
\lean{flagMul\_<F>\_<G>}.  The unit is then expanded to host size by
\lean{expand\_one\_hfree\_at N K} (rewriting $\mathbf{1}$ as the sum of
$N$-vertex \(\mathcal{H}\)-free flags), or by
\lean{expand\_one\_hfree\_at\_subgraph N K} on the general subgraph-forbid
route.  The resulting algebraic expression is normalized
by \lean{flagsum\_ac\_sort\_rhs\_pipeline}, an index-guided sort of the
reordered sum.
...
\lean{add\_nonneg}, and discharges each basis-element leaf by
