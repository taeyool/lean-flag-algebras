...
described on a concrete labeled graph must be shown to be independent of
the chosen representative before it descends to flags.  A flag's size is
part of its Lean type, so statements comparing flags of different sizes,
such as the expansion identity, are not even well typed as written.  A
theorem about tuples of flags, once proved in general, must still be
instantiated, and the instantiation is itself work: two terms for the same
reordered tuple can inhabit types that Lean does not identify.  Counting a
finite set requires an enumeration of it, and Lean's typeclass search can
synthesize different enumerations for the same set at different
occurrences, so a single choice must be recorded and reused.  None of these
issues changes the underlying mathematics, but each recurs often enough to
become a substantial proof-engineering cost.

These obligations have related but not identical causes.  Representative
independence is an ordinary quotient obligation.  The size and tuple issues
share a root in definitional equality: Lean identifies types only when
their definitions reduce to a common expression, so mathematically equal
objects may still require explicit transport.  Ambiguity between
enumerations, by contrast, is specific to Lean's elaboration and typeclass
mechanisms.  We therefore report the concrete obstacles without claiming
that they all arise from a single foundational feature.

A final recurring challenge is to prove equality between densities whose
numerators count differently presented finite sets of configurations.  Our
most reliable method is to name the two sets, construct an explicit
bijection between them, and deduce equality of their cardinalities; the
bijection carries the mathematical content of the proof.  The final
subsection describes this pattern after the representation and enumeration
...
have count_eq : S0.toFinset.card = S1.toFinset.card := by
  ...
  exact card_eq
\end{lstlisting}
This pattern appears throughout the development.  One example is
\lean{flagDensity\allowbreak\_permute} from
\Cref{sec:reordered-flag-tuples}.  There, unfolding the two densities reduces the theorem to an equality of
counts: the tuples of
pairwise-disjoint induced copies realizing the entries of \lean{Fl} in a
host must be as numerous as those realizing the entries of
\lean{Fl.permute $\pi$}.  These two sets are the \lean{S0} and \lean{S1} of
the listing, the mathematical step is the reindexing bijection
\lean{h\_iso\_S0\_S1} between them, and \lean{Fintype.card\_congr} converts
it into the cardinality equality \lean{card\_eq}.  The \lean{Fintype}
bookkeeping enters through the counts themselves.  In the goal they have
the form \lean{S0.toFinset.card}, and constructing a \lean{toFinset}
depends on a \lean{Fintype} instance, as does each side of
\lean{Fintype.card\_congr}.  Lean fills these hidden arguments
automatically, and that is exactly what could not be relied on: the
instance synthesized at one occurrence need not be the instance synthesized
at another, and \lean{card\_eq} then fails to rewrite the counts in the
goal.  Binding \lean{hS0} and \lean{hS1} the moment the two sets are named
is what makes the proof go through; every later \lean{toFinset} and the
equality produced by \lean{Fintype.card\_congr} mention exactly these
instances, and the rewrite through \lean{Set.toFinset\_card}
(\lean{count\_eq}) closes the theorem.  The
...
specification layer, especially for establishing counting identities, from
subflag densities to the coefficients of the downward operator.  We did not set out to make bijective arguments the default: we
...
In \lean{flagDensity\allowbreak\_permute}, the bijection is between the
tuples of pairwise-disjoint induced copies realizing a flag tuple and the
tuples realizing its reordering.  For
\lean{flagDensity\allowbreak\_insert\allowbreak\_empty}, which states that
appending the empty flag to a tuple does not change its density in a host,
the two sets are the realizations of the two tuples, matched by inserting
or deleting the unique extra component.  The adequacy proof for \lean{Sym2InducedLabeledSubgraph} matches
the induced subgraphs of a host in the concrete representation used for
computation with those in the abstract \lean{LabeledSubgraph}
representation used in the specification.  Although these statements serve different purposes, each
proof identifies the two finite sets being counted and gives an explicit
correspondence between their elements.

Carrying out this pattern in Lean uses the instance discipline of
\Cref{sec:fintype}, whose listing is exactly the skeleton of such a proof.
The instance bookkeeping adds no mathematical obligation: the bijection is
already the mathematical proof.
...
rules, permutation invariance, empty-flag insertion, the downward-operator
counts, and adequacy, and
