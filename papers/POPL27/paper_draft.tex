...
\]
Indeed, applying the downward operator to a \(\sigma\)-flag outside
\(\mathcal K\) produces a non-negative multiple of its underlying graph, which
also lies outside \(\mathcal K\).  Every \(\phi_0\in Q_0\) evaluates that graph
to zero.  Therefore, the expected value of the flag under
\(\Ext_\sigma(\phi_0)\) is zero.  Since every
positive homomorphism evaluates the flag non-negatively, its value is zero
with probability one under this measure.  Applying this argument to the
countable collection of flags outside \(\mathcal K\) shows that a positive
