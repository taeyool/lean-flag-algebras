...
\(m\) is the position of the type's graph in the enumeration of the
\(k\)-vertex graphs, and \(i\) is the position of the flag itself in the
enumeration of the \(n\)-vertex flags of that type.  Both enumerations are in
a fixed canonical order: graphs are listed by edge count, with ties broken
lexicographically by the smallest edge list over all vertex relabelings, and
the flags over a graph are listed by the placement of their labels.  The
reflection-layer constants \lean{Sym2Flag\_n\_k\_m\_i} follow the same
scheme, and the empty type is written \(k=m=0\).  Thus
...
We automate this repetition, the specialization of the uniform adequacy
theorems to concrete flags and pattern-pair/host combinations, using Lean's
metaprogramming facilities.  More
...
The numerical arguments are the \(n\), \(k\), \(m\) of the naming scheme.
\lean{generate\_empty\_typed\_flags n} generates the empty-typed flags on
\(n\) vertices, and \lean{generate\_flags n k m} generates the \(n\)-vertex
flags of the type selected by \(k\) and \(m\); here \(k=1,m=0\) is the unique
type on one labeled vertex.  The density and multiplication commands take a
pattern size \(p\) and a host size \(h\), followed by the same \(k,m\)
describing the type, and the suffix \lean{no\_forbid} says that no
...
\begin{table}
  ...
  \begin{tabular}{@{}lp{0.34\textwidth}@{}}
    ...
    \lean{generate\_empty\_typed\_flags n} &
      the empty-typed flags on \(n\) vertices, with completeness \\
    \lean{generate\_flags n k m} &
      the \(n\)-vertex flags of type \((k,m)\), with unlabelings, downward
      coefficients, and completeness \\
...
certificate compiler of \Cref{sec:flagmatic} uses this table when the flag
whose density is being bounded is smaller than the certificate's host size.

\subsubsection{Optimizations}
\label{sec:reflection-generation-optimizations}

The pipeline just described is organized so that elaboration remains
tractable as the flag sizes grow.  The main optimizations are the following.

\paragraph{Size-inductive enumeration.}
...
typed flags on \(n\) vertices are obtained by placing the type's labels on
...
\paragraph{Deduplication by cheap invariants.}
...
\paragraph{Completeness by proof rather than search.}
Each generation run must prove its completeness lemma, and deciding that
statement directly would mean enumerating the full quotient type and
comparing it against the named flags, which is exactly the brute force the
inductive enumeration avoids.  Instead, the statement that the inductive
enumeration misses no isomorphism class is proved once and for all, by
induction on the size, as a theorem about the enumeration itself; the
evaluation performed at generation time then only checks that the emitted
constants coincide with the enumeration's output list.

\paragraph{Pruning under a forbidden graph.}
...
\paragraph{Batched certification.}
...
collect them here.  Both lower bounds follow the pattern described after
\Cref{lem:semantic-bound}: an explicit family of triangle-free graphs (the
balanced complete bipartite graphs for Mantel's theorem, the balanced
blow-ups of the pentagon for the Erd\H{o}s pentagon theorem) attains the
target density at infinitely many orders, and the convergence of the
normalized extremal numbers carries these finite bounds to the Tur\'an
density.

\paragraph{Mantel's theorem.}
\Cref{ex:mantel} states $\tdensity{K_2}{K_3}=\tfrac12$.  The upper bound is
the square certificate worked through in \Cref{sec:flagmatic-background}.
For the lower bound, the balanced complete bipartite graph
$K_{\lfloor n/2\rfloor,\lceil n/2\rceil}$ is triangle-free, since among any
three of its vertices some two lie on the same side and are non-adjacent, and
its $\lfloor n^2/4\rfloor$ edges make up at least half of the $\binom{n}{2}$
vertex pairs.  The Lean development records the certificate bound, the
construction, and the resulting Tur\'an density:
\begin{lstlisting}
theorem Mantel_theorem : K2 (*@$\leq$@*) (1 / 2 : (*@$\mathbb{R}$@*)) (*@$\leansmul$@*) 1 + K3 := ...
...
\lean{Mantel\_theorem} restates the certificate derivation of
\Cref{sec:flagmatic-background} in the ambient algebra of
\Cref{sec:forbidden}, where no triangle-free condition is available: the
triangle terms that the constrained calculation drops are instead collected
in the extra summand \lean{K3} on the right, and evaluating at a positive
homomorphism $\phi$ with $\phi([K_3])=0$ recovers the conditional bound
$\phi([K_2])\leq\tfrac12$.  \lean{extremal\_density\_K3\_ge} formalizes the
construction.  In its statement, \lean{extremalNumber} is Mathlib's classical
extremal number, the maximum number of edges of a $K_3$-free graph on $n$
vertices; it agrees with $\mathrm{ex}(n,\,K_2;\,K_3)$ because the induced
copies of $K_2$ in a graph are exactly its edges.  For even orders $n=2k$,
the witness is \lean{completeBipartiteGraph (Fin k) (Fin k)}: the formal
proof derives its triangle-freeness from two-colorability and compares its
$k^2$ edges with $\tfrac12\binom{2k}{2}$; odd orders then follow because the
normalized extremal numbers are non-increasing.  Finally,
\lean{Turan\_density\_K3} combines the two bounds, mirroring the compactness
proof of \Cref{lem:semantic-bound}: if the normalized extremal numbers, which
are at least $\tfrac12$ by the construction, exceeded $\tfrac12+\varepsilon$
at infinitely many orders, then extremal triangle-free graphs of those orders
would converge, by \Cref{thm:convergent-hom}, to a positive homomorphism
$\phi$ with $\phi([K_3])=0$ and $\phi([K_2])\geq\tfrac12+\varepsilon$,
contradicting \lean{Mantel\_theorem}.  The normalized extremal numbers
therefore converge to $\tfrac12$, and Mathlib's \lean{tendsto\_turanDensity}
identifies their limit with \lean{turanDensity}.  Since
\lean{extremalNumber} and \lean{turanDensity} are Mathlib's own notions, the
final statement composes directly with the rest of the library.
...
converge to $\tdensity{C_5}{K_3}$, it suffices to exhibit, at infinitely many
orders, one triangle-free graph whose normalized count of induced pentagons
is at least $\tfrac{24}{625}$.  The
witness is the \emph{$n$-fold blow-up} of $C_5$: replace each vertex of the
pentagon by a class of $n$ vertices, and join two vertices of distinct
classes exactly when the underlying pentagon vertices are adjacent; vertices
...
contains many pentagons: picking one vertex from each of the five classes
yields five vertices that inherit exactly the adjacencies of $C_5$, so each
of the $n^5$ choice functions induces a copy of $C_5$, and distinct choices
give distinct vertex sets.  These copies are induced, as the density
...
chord of the cycle would create a triangle.)  The blow-up is therefore a
triangle-free graph on $5n$ vertices with at least $n^5$ induced pentagons,
...
\[
  ...
  \frac{24}{625}.
\]
The bound thus holds along the subsequence of orders $5n$, and taking the
...
\end{lstlisting}
\lean{blowUp G n} is the blow-up of an arbitrary base graph $G$: its vertex
type is the product $V\times\Fin{n}$, so the class of a base vertex $v$ is
the fiber $\{v\}\times\Fin{n}$, and its \lean{Adj} field compares first
coordinates through $G$.  Both clauses of the informal description follow:
...
\lean{blowUp\_K3\_free} is the triangle-freeness property.  Its proof
...
triangle-freeness is a finite check, shows that the blow-up of the pentagon
is triangle-free.
\lean{subgraphCount\_blowUp\_C5\_ge} is the counting property; here
...
injection from \lean{(Fin 5 $\to$ Fin n)} into the pentagon subgraphs of the
blow-up: a choice function $g$ is sent to the subgraph induced on
$\{(i,g(i)) \mid i\in\Fin{5}\}$, and distinct choice functions give distinct
subgraphs.
\lean{generalizedExtremalNumber\_K3\_C5\_ge} combines the previous two
results into the finite bound $\mathrm{ex}(5n,\,C_5;\,K_3)\geq n^5$; the Lean
function \lean{generalizedExtremalNumber} is the extremal number of
\Cref{sec:intro}, the maximum of \lean{subgraphCount C5} over the $K_3$-free
graphs on \lean{Fin (5 * n)}.  One representation detail intervenes: this
maximum ranges over graphs on \lean{Fin (5 * n)}, whereas \lean{blowUp C5 n}
lives on a product type, so a \lean{Fin}-indexed variant \lean{blowUp\_fin},
proved isomorphic to \lean{blowUp}, supplies the witness (an instance of the
representation issues of \Cref{sec:obstacles}).  The remaining steps, the
displayed normalization and the passage to the limit along the subsequence of
orders $5n$, produce \lean{ErdosPentagon\_Turan\_lowerBound}.

\paragraph{Additional Goodman-style consequences.}
The formalization also includes two further flag-algebra inequalities, both
due to Goodman~\cite{goodman1959} and both proved with the same basic
infrastructure.
\lean{Goodman\_\allowbreak bound\_\allowbreak on\_\allowbreak triangle\_\allowbreak density}
proves the algebraic lower bound
\[
...
of $K_2$ with a Cauchy--Schwarz inequality for the downward operator, applied
to the edge flag labeled at one endpoint.
\lean{Goodman\_theorem\_on\_Ramsey\_multiplicity} proves
\[
  \overline{K_3} + K_3 \;\geq\; \tfrac14\cdot\mathbf{1},
\]
where $\overline{K_3}$ is the empty graph on three vertices, written
\lean{O3} in the development: asymptotically, at least a quarter of all
...
theorem.  These statements illustrate that the formalized algebraic layer
supports inequalities among flag-algebra expressions beyond Tur\'an problems
that maximize the induced density of one graph subject to forbidding another
as a subgraph.
