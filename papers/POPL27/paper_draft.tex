...
measure \(P\) on \lean{PositiveHomSpace}~\(\sigma\).  This \(P\) is the formal
...
\begin{lstlisting}
theorem exists_probMeasure_extend_emptyType_positiveHom {n(*@$_0$@*) : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : FlagType (Fin n(*@$_0$@*))}
    {(*@$\phi_0$@*) : PositiveHom (*@$\emptyt$@*)} (h(*@$\sigma$@*) : (*@$\phi_0$@*) ((*@$\langle\sigma\rangle_0$@*)) > 0)
    : (*@$\exists$@*) P : ProbabilityMeasure (PositiveHomSpace (*@$\sigma$@*)), (*@$\forall$@*) f : FlagAlgebra (*@$\sigma$@*),
...
\lean{PositiveHomSpace.toPosHom}~\(\phi\)~\(f\), and the measure is \(P\).
Since \(P\) is a probability measure, this integral is simply expectation.
Thus the line
\[
  \int \bigl(\phi \mapsto \phi(f)\bigr)\, dP
\]
should be read as
\[
  \mathbb{E}_{\phi\sim P}[\phi(f)].
\]
With this reading, the theorem says exactly that \(P\) satisfies
