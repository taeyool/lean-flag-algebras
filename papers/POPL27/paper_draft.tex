...
and $C_5$-free edge-density certificates, kernel reduction of the density
calculations was too slow to be practical, so those two files prove them by
...
these 2,832 identities in 15 batched \lean{decide +kernel} evaluations and then uses the
...
chosen for principled reasons, most prominently the residual use of
\lean{native\_decide} in the two largest certificates, we have said so.
...
$C_5$-free edge-density proofs, kernel evaluation of the batched density
computations is not practical at the current scale, so those files prove them
by \lean{native\_decide}; their SDP matrix equalities are small enough for
