...
To supply these choices, the reflection layer introduces a family of more rigid
edge-set representations: vertices are fixed finite types, edges are explicit
finite data, and isomorphism is reduced to finite search over permutations
compatible with the labels.  The empty-type core is \lean{Sym2Graph n}; typed
flags use the same representation style through \lean{Sym2FlagType} and
...
structure Sym2LabeledGraph {k : Nat} (sigma : Sym2FlagType k) (n : Nat) where
  ...
  edges_valid : forall e in edges, not e.IsDiag
  type_embed  :
    fromEdgeSet (SetLike.coe sigma.edges) (*@$\hookrightarrow_g$@*)
      fromEdgeSet (SetLike.coe edges)
\end{lstlisting}
...
interpreting adjacency as membership in \lean{edges}. \gw{I'm here.}
