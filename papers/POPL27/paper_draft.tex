...
flags to arbitrary \lean{FlagVector}~\(\sigma\) terms.  Second,
\lean{downwardFlagVectorQuot} sends the resulting empty-type flag vector into
\lean{FlagAlgebra}~\(\emptyt\).  Third, \lean{downward} descends
the input through the quotient defining \lean{FlagAlgebra}~\(\sigma\).
...
that the downward image is unchanged by exactly those zero-space relations.
Thus the formalized operator is not merely a linear map on representatives: it
is a well-defined map on the quotient algebra.
\begin{lstlisting}
...
empty-typed algebra.  The notation is compact, but the implementation
contains the full descent from concrete labeled graphs to flags, from flags to
flag vectors, and finally from flag vectors to the quotient algebra.

It remains to connect this algebraic map to the semantics of positive
homomorphisms.
%The relevant construction lives in \lean{FlagAlgebra/RandomHom.lean}.
Given an empty-type positive homomorphism
\(\phi_0\) in which the type \(\sigma\) has positive density, Lean constructs a
probability measure on \lean{PositiveHomSpace}~\(\sigma\).  This measure is the
formal version of the random extension \(\Ext_\sigma(\phi_0)\): it represents
the experiment of choosing a random occurrence of the type \(\sigma\) in the
limiting object represented by \(\phi_0\).  Its defining property is the formal
version of Equation~\eqref{eq:ensemble}:
\begin{lstlisting}
theorem (*@\texttt{exists\_probMeasure\_extend\_emptyType\_positiveHom}@*)
    ...
    (*@$\exists$@*) (P : ProbabilityMeasure (PositiveHomSpace (*@$\sigma$@*))),
      (*@$\forall$@*) (f : FlagAlgebra (*@$\sigma$@*)),
        (*@$\int$@*) ((*@$\phi$@*) : PositiveHomSpace (*@$\sigma$@*)), PositiveHomSpace.toPosHom (*@$\phi$@*) f (*@$\partial$@*)P
        = (*@$\phi_0$@*) (*@$\llbracket$@*)f(*@$\rrbracket_0$@*) / (*@$\phi_0$@*) (*@$\llbracket$@*)(1 : FlagAlgebra (*@$\sigma$@*))(*@$\rrbracket_0$@*)
\end{lstlisting}
The statement says that averaging the typed evaluation \(\phi(f)\) over this
random extension recovers the empty-type evaluation of the downward image,
divided by the normalizing value of the typed unit.  Equivalently,
\(\phi_0(\llbracket f\rrbracket_0)\) is that typed average multiplied by
\(\phi_0(\llbracket \mathbf{1}_\sigma\rrbracket_0)\).  This is exactly the bridge needed for
semantic soundness: a typed inequality can be tested by first randomly rooting
the unlabeled limiting object, evaluating in the typed algebra, and then
averaging back down.

The payoff is the formal semantic transfer theorem corresponding to
\Cref{thm:downward-nonneg}: if \(f\) is semantically non-negative in the
\(\sigma\)-typed algebra, then its downward image is semantically non-negative
in the empty-typed algebra.
\begin{lstlisting}
...
\end{lstlisting}
To prove the theorem, Lean tests the claimed empty-type non-negativity against
an arbitrary positive homomorphism \(\phi_0\).  If
\(\phi_0(\langle\sigma\rangle_0)=0\), then every downward value has evaluation
zero at \(\phi_0\).  If \(\phi_0(\langle\sigma\rangle_0)>0\), the
random-extension theorem rewrites \(\phi_0(\llbracket f\rrbracket_0)\) as a
positive normalizing factor times the average of \(\phi(f)\) over typed
positive homomorphisms.  Since \(f\in\mathcal{C}^\sigma\), every integrand is
non-negative, and so the average is non-negative.  \gw{This proof sketch may be unnecessary.}
This is the formal bridge
from typed inequalities to the unlabeled semantic order used by certificate
proofs.
