...
\begin{abstract}
Razvorov's flag algebras are a powerful method for proving asymptotic inequalities in
...
takes a limit; we call this the \emph{Turán density} of $P$ with respect to $\mathcal{H}$:
\[
  \tdensity{P}{\mathcal{H}}
  :=
  \lim_{n\to\infty}
...
bound must certify that the proportion of induced copies of $P$ never exceeds
...
representations to the quotient definitions by adequacy theorems, verifies
...
with graph limits and then imposes the constraint by testing inequalities on
random labeled views of those constrained limits.  These two viewpoints agree
...
to an extremal-density or equality slice.
...
forbidden induced subgraphs; a graph is \emph{$\mathcal{H}$-free} if it
contains no induced subgraph isomorphic to any member of $\mathcal{H}$.
All types, flags, and flag algebras below are defined relative to this
fixed $\mathcal{H}$.
...
% asymptotic meaning.
% A small pattern flag $A$ can be counted directly in a large host flag $G$.
% Equally, one can first sample a larger intermediate flag $B$ inside $G$, then
% count $A$ inside $B$, and finally average over the choice of $B$.
% These two sampling procedures give the same limiting density; the flag algebra
% encodes this as a definitional equality, so a proof can replace a pattern flag
% by any sufficiently large expansion of it---which is exactly the bookkeeping
% needed by flag algebra certificates.
% The flag algebra builds this sampling-averaging identity into the syntax by
% quotienting it out.  As a result, a proof can replace a pattern flag by any
% sufficiently large expansion of it, which is exactly the bookkeeping needed
% by flag algebra certificates.
% This is the same kind of move one makes when quotienting
% programs by an equational theory: the quotient removes representation choices
% that should not affect observable behavior.
Fix a flag type $\sigma$ of size $k$.
The flag algebra $\mathcal{A}^\sigma$ is the algebraic structure in which
Lemma~\ref{lem:chain-rule} holds by definition.
\gw{A better phrasing for this sentence would be welcome.}
Let
...
% The generator $A - \sum_{B \in \mathcal{F}^\sigma_n} \den{A}{B} \cdot B$ lies
% in $\mathcal{Z}^\sigma$ because \Cref{lem:chain-rule} says that $A$ and
% $\sum_{B \in \mathcal{F}^\sigma_n} \den{A}{B} \cdot B$ are the same pattern density-wise, agreeing on every
% host flag. \gw{A better phrasing for this sentence would be welcome.}  Quotienting by $\mathcal{Z}^\sigma$ makes these two definitionally equal
% in $\mathcal{A}^\sigma$, so we may replace $A$ by
% $\sum_{B \in \mathcal{F}^\sigma_n} \den{A}{B} \cdot B$ at any size $n \geq m$.
...
Quotienting by $\mathcal{Z}^\sigma$ makes any two pattern expressions that
have the same density in every host flag definitionally equal in
$\mathcal{A}^\sigma$.  We may therefore
freely replace $A$ by its expansion at any size $n \geq m$.
...
In $\mathcal{A}^\sigma$ the following identity holds for every $\ell \geq |\sigma|$:
\begin{equation}
...
For a $\sigma$-flag $A=(G,\theta_A)$ of size $m$ (flag type of size $k$), let
\[
  q_\sigma(A)
  \;:=\;
  \frac{
...
A flag algebra proof of $P \leq c\cdot\mathbf{1}$ typically follows a fixed pattern:
(1) exhibit $f \in \mathcal{A}^\sigma$ with $0 \leq_\sigma f$ for some non-empty flag type $\sigma$
(e.g., a square $f = g^2$),
(2) apply \Cref{thm:downward-nonneg} to obtain $0 \leq \llbracket f \rrbracket_\sigma$, and
(3) add $\llbracket f \rrbracket_\sigma$ to the zero-space expansion of $P$
and use non-negativity of flags and \eqref{eq:sum-to-one} to conclude $P \leq c\cdot\mathbf{1}$.
...
the constructor that produces the basis element for each flag:
\begin{lstlisting}
...
def basisVector (F : FinFlag (*@$\sigma$@*)) : FlagVector (*@$\sigma$@*) :=
  Finsupp.single F 1
\end{lstlisting}
Here $\to_0$ denotes the type of finitely supported functions, and \lean{Finsupp.single F 1} defines the basis function for $F \in \mathcal{F}^\sigma$, taking value $1$ at $F$ and $0$ elsewhere.
...
\end{lstlisting}
In the resulting type \lean{FlagAlgebra}, a flag and its density expansion at any larger size
...
a commutative associative $\mathbb{R}$-algebra, completing the formalization of the algebraic structure of $\mathcal{A}^\sigma$. \gw{I'm here.}
...
$\mathbb{N}$, and \lean{ConvergesTo} encodes convergence.
\begin{lstlisting}
...
$\den{F}{s(n)} \to a(F)$ as $n \to \infty$ for every $\sigma$-flag $F$.
...
We formalize Theorem~\ref{thm:convergent-hom} as two separate theorems:
\begin{lstlisting}
...
\end{lstlisting}
Here \lean{FlagDensitySpace} $\sigma$ is the compact subspace of
$[0,1]^{\mathcal{F}^\sigma}$ in which density profiles land, and
\lean{$\phi$.coe} is the underlying function $\mathcal{F}^\sigma \to \mathbb{R}$
of a positive homomorphism $\phi$.  The first theorem formalizes part (a) of
Theorem~\ref{thm:convergent-hom}: any limit of a convergent flag sequence is
the evaluation function of some positive homomorphism.  The second formalizes
part (b), proved via a probabilistic argument.

With positive homomorphisms characterized, we use them to define the semantic
non-negativity cone $\mathcal{C}^\sigma$ and the order it induces on
\lean{FlagAlgebra} $\sigma$.
\lean{semanticCone} $\sigma$ is the set of elements that every positive
homomorphism evaluates non-negatively, and we define $f \leq g$ to mean
$g - f \in \mathcal{C}^\sigma$ via a Lean \lean{LE} instance:
\begin{lstlisting}
...
\begin{lstlisting}
def downward : FlagAlgebra (*@$\sigma$@*) (*@$\to$@*) FlagAlgebra emptyType := ...
...
def flagQuadraticForm (M : Matrix (Fin n) (Fin n) R)
    (v : Fin n (*@$\to$@*) FlagAlgebra (*@$\sigma$@*)) : FlagAlgebra (*@$\sigma$@*) :=
  (*@$\sum$@*) i, (*@$\sum$@*) j, M i j * (v i * v j)

theorem flagQuadraticForm_nonneg
    (M : Matrix (Fin n) (Fin n) R) (hM : M.PosSemidef)
    (v : Fin n (*@$\to$@*) FlagAlgebra (*@$\sigma$@*)) :
    flagQuadraticForm M v (*@$\geq$@*) 0

theorem flagQuadraticForm_downward_nonneg
    (hM : M.PosSemidef) (v : Fin n (*@$\to$@*) FlagAlgebra (*@$\sigma$@*)) :
    [[flagQuadraticForm M v]]0 (*@$\geq$@*) 0
\end{lstlisting}
