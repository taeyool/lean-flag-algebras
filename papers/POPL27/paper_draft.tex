...
(the \emph{pattern flag}) and a $\sigma$-flag $G = (M, \theta_G)$ of size $n \geq m$
(the \emph{host flag}), the \emph{density of $A$ in $G$}, written $\den{A}{G}$, is
...
``algebra'' is used in the standard algebraic sense: a vector space equipped
with a bilinear multiplication and a distinguished multiplicative unit,
satisfying the usual associativity and unit laws.  The flag algebras in this
paper are commutative $\mathbb{R}$-algebras.} of density expressions generated
...
its expansion at any size $n \geq m$ have the same density in every host flag.
...
consider sequences $\{G_n\}_{n\in\mathbb{N}}$ of host flags with $|V(G_n)|\to\infty$ and ask
...
$|V(G_1)| < |V(G_2)| < \cdots$.  Each $G$ defines a point
...
$\phi(f)\leq\phi(g)$ for every $\phi \in \poshom{\sigma}$.  When $\sigma=\emptyset$, we write $f \leq g$ in place of $f \leq_\emptyset g$.
...
\begin{proof}
  ...
  subsequence; by \Cref{thm:convergent-hom}(a) its limit is some
  $\phi \in \poshom{\emptyset}$.  Since $\phi$ is the pointwise limit of
  the densities $\den{P}{G_k}$, we have
  $\phi(P) = \lim_k \den{P}{G_k} \geq c + \varepsilon > c$.  But
...
Our first design choice is to build this typed mathematical interface
separately from the proof-by-computation machinery used later for certificates.
Here computation means evaluation inside Lean as part of proof checking, not a
numerical experiment outside the prover.  For example, Lean may evaluate a
finite flag-density table or normalize an algebraic expression, and the
resulting equality can be used as a proof step only because it is connected to
the mathematical definitions by previously proved theorems.  We call the
interface supporting these proof-producing computations the
\emph{specification layer}: it states the mathematical objects and proves their
basic interface theorems.  It provides the formal counterpart of the informal
interface in
...
expansion identities, and positive homomorphisms as the semantic interpretation
of algebra elements.  Once this interface has been established, later proofs
checking particular flag-algebra certificates can work with algebraic
expressions rather than repeatedly returning to the underlying graph
representatives and finite sampling definitions.

A second design choice concerns where forbidden-subgraph assumptions enter
the development.  In the background section we fixed
$\mathcal{H}$ first and defined all flags and algebras relative to the
$\mathcal{H}$-free world.  In Lean we instead build the ambient flag algebra
without fixing $\mathcal{H}$, and impose $\mathcal{H}$-freeness later as a
predicate on the semantic order.  The practical reason for this choice is that
it keeps the basic flag-algebra formalization as close as possible to the
unconstrained theory.  If every definition carried an arbitrary forbidden
family $\mathcal{H}$ from the start, then even the elementary constructions of
flags, densities, expansion identities, and multiplication would acquire an
extra parameter and extra side conditions.  We expected this to make the first
formalization of the core theory harder.  By postponing $\mathcal{H}$-freeness,
we can develop the ambient algebra once and introduce the constraint only at
the point where it is needed for extremal applications.  This also keeps the
algebraic infrastructure reusable across different problems; the price is that
we must prove that the deferred predicate has the same meaning as the usual
constrained flag-algebra semantics when it is used to derive Tur\'an-density
bounds.
...
Throughout this section, the following Lean context is in scope:
\begin{lstlisting}
variable {T : Type} [Fintype T] {(*@$\sigma$@*) : FlagType T}
         {U : Type}  [Fintype U]  [DecidableEq U]
         {U(*@$_1$@*) : Type} [Fintype U(*@$_1$@*)] [DecidableEq U(*@$_1$@*)]
         {U(*@$_2$@*) : Type} [Fintype U(*@$_2$@*)] [DecidableEq U(*@$_2$@*)]
         {W : Type} [Fintype W] [DecidableEq W]
         {t : (*@$\mathbb{N}$@*)} {Vl : Fin t (*@$\to$@*) Type} [FintypeList Vl] [DecidableEqList Vl]
\end{lstlisting}
For readers not familiar with Lean syntax, this block is a list of standing
assumptions rather than a mathematical statement.  The symbols \lean{T},
\lean{U}, \lean{U$_1$}, \lean{U$_2$}, and \lean{W} are finite vertex types;
\lean{[Fintype U]} records that Lean may enumerate the elements of \lean{U},
and \lean{[DecidableEq U]} records that equality on \lean{U} can be decided by
computation.  Curly braces mark arguments that Lean usually infers from
context, while square brackets mark typeclass assumptions that Lean searches
for automatically.  The variable $\sigma$ is a flag type on the label type
\lean{T}; \lean{FlagType} is introduced in the next subsection.  Finally,
\lean{Fin t} is the finite type with elements $0,1,\ldots,t-1$, so
\lean{Vl : Fin t -> Type} is a finite list of vertex types.  The assumptions
\lean{FintypeList Vl} and \lean{DecidableEqList Vl} are custom typeclass
instances asserting \lean{Fintype (Vl i)} and \lean{DecidableEq (Vl i)} for
each index \lean{i : Fin t}.

\subsection{Flag Types and Flags}
\label{sec:flagtypes}

Informally, the word \emph{flag} refers both to a concrete pair $(M,\theta_A)$
and to its isomorphism class.  In Lean, however, the two must be distinct types.  We define \lean{LabeledGraph}
for the concrete pair $(M,\theta_A)$ and \lean{Flag} for its quotient by flag
isomorphism, corresponding to an element of $\mathcal{F}^\sigma_n$ where $n = |V(M)|$.
...
\end{lstlisting}
The structure \lean{LabeledGraph} takes the flag type $\sigma$ and a vertex type \lean{V} as parameters, and encodes a concrete $\sigma$-flag $(M,\theta_A)$. \lean{FlagType T} is the type of $\sigma$, i.e., simple graphs with vertex type \lean{T}. The field \lean{graph} stores the underlying graph $M$, and \lean{type\_embed} records the graph embedding $\theta_A$.
...
\begin{lstlisting}
structure LabeledGraphIso (G : LabeledGraph (*@$\sigma$@*) V) (G' : LabeledGraph (*@$\sigma$@*) W) where
...
\end{lstlisting}
% The field \lean{graph\_iso} is a graph isomorphism between the underlying graphs,
% and \lean{type\_preserve} encodes the commutativity condition $\alpha \circ
% \theta_A = \theta_{A'}$ from Section~\ref{sec:background}.
Lean's quotient type requires a \emph{setoid}, which is a type explicitly
equipped with an equivalence relation.  \lean{labeledGraphSetoid} serves this role, equipping
\lean{LabeledGraph} with the equivalence relation induced by 
\lean{LabeledGraphIso}.

We can now define $\mathcal{F}^\sigma_n$ in Lean as \lean{Flag}~$\sigma$~\lean{(Fin}~$n$\lean{)},
abbreviated \lean{FlagWithSize}~$\sigma$~$n$.  \lean{FinFlag}~$\sigma$ is the all-size union $\mathcal{F}^\sigma =
...
\begin{lstlisting}
abbrev FlagWithSize {n(*@$_0$@*) : (*@$\mathbb{N}$@*)} ((*@$\sigma$@*) : FlagType (Fin n(*@$_0$@*))) (n : (*@$\mathbb{N}$@*)) :=
  Flag (*@$\sigma$@*) (Fin n)

def FinFlag {n(*@$_0$@*) : (*@$\mathbb{N}$@*)} ((*@$\sigma$@*) : FlagType (Fin n(*@$_0$@*))) : Type :=
  (*@$\Sigma$@*) (n : (*@$\mathbb{N}$@*)), FlagWithSize (*@$\sigma$@*) n
\end{lstlisting}

Unlike Section~\ref{sec:background}, none of the definitions above carries the
$\mathcal{H}$-free condition: the underlying graphs are arbitrary.  The
$\mathcal{H}$-free assumption is deferred to the semantic predicate on the
final unlabeled inequality, as described in the section introduction.
\gw{This sentence needs revision.}

\subsection{Subflag Densities}
\label{sec:formal-subflag-densities}

Formalizing density function is not straightforward.  The density
...
\lean{LabeledGraph} inputs, then prove that replacing any input by an
isomorphic \lean{LabeledGraph} gives the same result.

We first introduce \lean{LabeledGraphList} as an abbreviation for a
list of $t$ \lean{LabeledGraph}s, one per index in \lean{Fin t}.
Given such a list of pattern \lean{LabeledGraph}s and a host \lean{LabeledGraph},
...
\begin{lstlisting}
abbrev LabeledGraphList ((*@$\sigma$@*) : FlagType T) (t : (*@$\mathbb{N}$@*)) (Vl : Fin t (*@$\to$@*) Type)
  := (*@$\forall$@*) (i : Fin t), LabeledGraph (*@$\sigma$@*) (Vl i)

noncomputable def labeledGraphListDensity
    (Hl : LabeledGraphList (*@$\sigma$@*) t Vl) (G : LabeledGraph (*@$\sigma$@*) W) : (*@$\mathbb{Q}$@*) :=
  let r_list (i : Fin t) := (Hl i).size - (*@$\sigma$@*).size
  labeledGraphListCount Hl G / multinomialCoefficient r_list (G.size - (*@$\sigma$@*).size)
\end{lstlisting}
The \lean{noncomputable} keyword signals that Lean cannot synthesize an algorithm
to evaluate the function.  This poses no obstacle to formalizing flag algebra
itself, but becomes relevant when applying the algebra to concrete graphs, a
point we return to in Section~\ref{sec:reflection}.

Lifting \lean{labeledGraphListDensity} to the quotient type \lean{Flag} is
slightly layered.  We first lift the host argument \lean{G}, then lift the list of
pattern flags \lean{Hl}, and finally package the result as a function on \lean{FlagList}.
The congruence proofs, omitted below, show that the density is unchanged when
the host or any pattern flag is replaced by an isomorphic representative.
\begin{lstlisting}
abbrev FlagList ((*@$\sigma$@*) : FlagType T) (t : (*@$\mathbb{N}$@*)) (Vl : Fin t (*@$\to$@*) Type)
  := (*@$\forall$@*) (i : Fin t), Flag (*@$\sigma$@*) (Vl i)

noncomputable def labeledGraphListDensityLifted
    (Hl : LabeledGraphList (*@$\sigma$@*) t Vl) : Flag (*@$\sigma$@*) W (*@$\to$@*) (*@$\mathbb{Q}$@*) :=
  Quotient.lift
...
noncomputable def flagListDensity : FlagList (*@$\sigma$@*) t Vl (*@$\to$@*) Flag (*@$\sigma$@*) W (*@$\to$@*) (*@$\mathbb{Q}$@*) :=
  fun Fl => quotLabeledGraphListDensity Fl.coe
\end{lstlisting}
\lean{flagDensity}$_1$ and \lean{flagDensity}$_2$ are specializations of \lean{flagListDensity}
...
\end{lstlisting}
Here \lean{[}$\cdot$\lean{]}$^{\mathtt{f}}$ denotes the notation for constructing a \lean{FlagList}.

Building on the definitions above, we prove the following chain rule (Lemma~\ref{lem:chain-rule}) and several of its variants.
\gw{The proof of the chain rule was also very difficult, but I'm not sure whether it would be worth discussing that here.}
\begin{lstlisting}
theorem flagDensity_eq_sum_density_prods
    {(*@$\ell_0$@*) : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : FlagType (Fin (*@$\ell_0$@*))} {(*@$\ell_1$@*) (*@$\ell$@*) : (*@$\mathbb{N}$@*)}
    ((*@$\ell'$@*) : (*@$\mathbb{N}$@*)) (F(*@$_1$@*) : Flag (*@$\sigma$@*) (Fin (*@$\ell_1$@*))) (G : Flag (*@$\sigma$@*) (Fin (*@$\ell$@*)))
...
For the remainder of Section~\ref{sec:abstract}, $\sigma$ denotes a \lean{FlagType (Fin $n_0$)} for a fixed $n_0 \in \mathbb{N}$:
\begin{lstlisting}
variable {(*@$n_0$@*) : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : FlagType (Fin (*@$n_0$@*))}
\end{lstlisting}
...
\begin{lstlisting}
abbrev FlagVector ((*@$\sigma$@*) : FlagType (Fin (*@$n_0$@*))) : Type :=
...
Next, we define the zero space $\mathcal{Z}^\sigma$.  We encode the generating element of
Equation~\eqref{eq:zero-element} as \lean{zeroElement}, \lean{zeroSet} $\sigma$
collects all elements of this form, and \lean{ZeroSpace} $\sigma$ is the submodule
they span:
\begin{lstlisting}
def zeroElement (F : FinFlag (*@$\sigma$@*)) ((*@$\ell$@*) : (*@$\mathbb{N}$@*)) : FlagVector (*@$\sigma$@*) :=
  basisVector F - flagExpansion F (*@$\ell$@*)

noncomputable def zeroSet ((*@$\sigma$@*) : FlagType (Fin (*@$n_0$@*))) : Set (FlagVector (*@$\sigma$@*)) :=
  {k | (*@$\exists$@*) (F : FinFlag (*@$\sigma$@*)) ((*@$\ell$@*) : (*@$\mathbb{N}$@*)), F.1 (*@$\leq$@*) (*@$\ell$@*) (*@$\wedge$@*) k = zeroElement F (*@$\ell$@*)}

def ZeroSpace ((*@$\sigma$@*) : FlagType (Fin (*@$n_0$@*))) : Submodule (*@$\mathbb{R}$@*) (FlagVector (*@$\sigma$@*)) :=
  Submodule.span (*@$\mathbb{R}$@*) (zeroSet (*@$\sigma$@*))
\end{lstlisting}
Here \lean{flagExpansion F} $\ell$ is defined as
$\sum_{G \in \mathcal{F}^\sigma_\ell} \den{F}{G} \cdot G$,
the expansion of $F$ at size $\ell$.
...
abbrev FlagAlgebra ((*@$\sigma$@*) : FlagType (Fin (*@$n_0$@*))) : Type :=
  Quotient (flagVectorSetoid (*@$\sigma$@*))
\end{lstlisting}
In the resulting type \lean{FlagAlgebra} $\sigma$, a flag and its density expansion at any larger size
...
\lean{flagMul} linearly in each argument to give multiplication on
\lean{FlagVector} $\sigma$, and \lean{Quotient.map$_2$} lifts this to
\lean{FlagAlgebra} $\sigma$.
\begin{lstlisting}
def flagMulWithSize (F F' : FinFlag (*@$\sigma$@*)) ((*@$\ell$@*) : (*@$\mathbb{N}$@*)) : FlagVector (*@$\sigma$@*) :=
  (*@$\sum$@*) G : FlagWithSize (*@$\sigma$@*) (*@$\ell$@*), flagDensity(*@$_2$@*) F.2 F'.2 G * basisVector (*@$\langle$@*)(*@$\ell$@*), G(*@$\rangle$@*)

noncomputable def flagMul (F F' : FinFlag (*@$\sigma$@*)) : FlagVector (*@$\sigma$@*) :=
  flagMulWithSize F F' (F.1 + F'.1 - (*@$n_0$@*))

noncomputable instance : Mul (FlagVector (*@$\sigma$@*)) where
  mul := bilinearExtension flagMul

noncomputable instance : Mul (FlagAlgebra (*@$\sigma$@*)) where
  mul := by
...
comment above.  We also prove \lean{flagMulWithSize\_indep\_on\_size}: for any two choices of $\ell$, the results of \lean{flagMulWithSize F F'} $\ell$ are
equivalent under \lean{flagVectorEqv}, justifying the choice of minimal size
in \lean{flagMul}.
...
exactly the limiting objects of convergent flag sequences.  \lean{FlagSeq} $\sigma$ is the type of sequences of $\sigma$-flags indexed by
$\mathbb{N}$, and \lean{ConvergesTo} encodes its convergence.
\begin{lstlisting}
abbrev FlagSeq ((*@$\sigma$@*) : FlagType (Fin (*@$n_0$@*))) := (*@$\mathbb{N}$@*) (*@$\to$@*) FinFlag (*@$\sigma$@*)
...
\lean{PositiveHom} $\sigma$ is the subtype of $\mathbb{R}$-algebra homomorphisms from
\lean{FlagAlgebra} $\sigma$ to $\mathbb{R}$ that are non-negative on every $\sigma$-flag:
\begin{lstlisting}
abbrev Hom ((*@$\sigma$@*) : FlagType (Fin (*@$n_0$@*))) :=
  FlagAlgebra (*@$\sigma$@*) (*@$\to_a$@*)[(*@$\mathbb{R}$@*)] (*@$\mathbb{R}$@*)

def PositiveHom ((*@$\sigma$@*) : FlagType (Fin (*@$n_0$@*))) : Type :=
...
map \lean{FlagVector} $\sigma \to$ \lean{FlagAlgebra} $\sigma$.
...
\lean{semanticCone} $\sigma$ is the set of elements in \lean{FlagAlgebra} $\sigma$
that every positive homomorphism evaluates non-negatively.  The induced order
is installed as a Lean \lean{LE} instance, with $f \leq g$ defined to mean
$g - f \in \mathcal{C}^\sigma$.
\begin{lstlisting}
def semanticCone ((*@$\sigma$@*) : FlagType (Fin (*@$n_0$@*))) : Set (FlagAlgebra (*@$\sigma$@*)) :=
  { f | (*@$\forall$@*) (*@$\phi$@*) : PositiveHom (*@$\sigma$@*), (*@$\phi$@*) f (*@$\geq$@*) 0 }

instance : LE (FlagAlgebra (*@$\sigma$@*)) where
...
noncomputable def downwardNormalizingFactor_labeledGraph
  ...
  isomor(*@\texttt{ph}@*)ismCount G / num_of_all_injections
...
def unlabeledGraph (G : LabeledGraph (*@$\sigma$@*) V) : LabeledGraph (*@$\emptyt$@*) V := ...

noncomputable def unlabel : Flag (*@$\sigma$@*) V (*@$\to$@*) Flag (*@$\emptyt$@*) V :=
  Quotient.lift
    (fun G : LabeledGraph (*@$\sigma$@*) V => unlabeledGraphQuot G)
    (fun _ _ h => unlabeledGraphQuot_respect_eqv h)

noncomputable def downwardFlag (F : Flag (*@$\sigma$@*) (Fin n)) : FlagVector (*@$\emptyt$@*) :=
  downwardNormalizingFactor F (*@$\leansmul$@*) basisVector ((*@$n$@*), unlabel F)
\end{lstlisting}
...
final quotient lift: \gw{I'm here.}
\begin{lstlisting}
...
\begin{lstlisting}
def forbidLE (H : FinFlag (*@$\emptyt$@*)) (f g : FlagAlgebra sigma) : Prop :=
  forall (phi0 : PositiveHom (*@$\emptyt$@*)), phi0 <sigma>0 > 0 ->
    phi0 (*@$\llbracket$@*)basisVector H(*@$\rrbracket_0$@*) = 0 ->
  ...
  exists P : ProbabilityMeasure (PositiveHomSpace sigma),
    forall f : FlagAlgebra sigma,
      integral (fun phi => phi f) P =
        phi0 (*@$\llbracket$@*)f(*@$\rrbracket_0$@*) /
          phi0 (*@$\llbracket$@*)(1 : FlagAlgebra sigma)(*@$\rrbracket_0$@*)
\end{lstlisting}
...
theorem generalizedTuranDensity_le_of_forbidLE
    (hc : 0 <= c) (h : F.toFlagAlgebra <=[H.toFinFlag] c (*@$\leansmul$@*) 1) :
