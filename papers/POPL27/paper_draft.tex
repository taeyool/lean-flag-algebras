...
We first identify this common proof structure, then show how each step becomes a
reusable, kernel-checked component in the certificate-to-Lean compiler.
...
Across these proofs the same four parts recur; only the sizes and the numbers
inside each part change.  That invariance is what the next subsection exploits:
it discharges each part with a single reusable, kernel-checked component fed the
per-proof data, so a new theorem is new data, not a new proof.
...
Part 2 discharges the positive-semidefiniteness the method depends on.  A
flag-algebra bound $P \leq_{\mathcal{H}} c\cdot\mathbf{1}$ is proved not by
manipulating $P$ but by adding to it a provably non-negative quantity, the
flag-algebra analogue of a sum of squares: a quadratic form
$\langle v_t, M_t\,v_t\rangle$ whose matrix $M_t$ is positive semidefinite.  The
solver supplies each $M_t$, and re-establishing that each is PSD is exactly
Lean's task.  Testing that spectrally, by asking every eigenvalue to be
...
\lean{gen-skeleton} on six certificates chosen to span the cases it must handle:
both branches, host sizes $N\in\{3,4,5\}$, block counts $1$ through $4$, and
three forbids ($K_3$, $K_4$, $K_5$).  Every one compiles to a file the kernel
accepts with no \lean{sorry} (Table~\ref{tab:flagmatic-scenarios}), and none
required hand transcription of the certificate into Lean.  Three are classical
extremal results, whose upper bounds we describe below; the rest appear only in
the table.
...
\paragraph{What is not automated.}
...
not proved that no certificate produces a form it fails to normalize.  Second,
the identifier and forbid machinery is written to be generic in the forbidden
graph: nothing in the proof body or the generators assumes a complete graph.
But only $K_3$, $K_4$, $K_5$ have been exercised end to end, so a non-$K_n$
forbid such as $C_4$ or $P_4$ would be the first empirical test of the
generality we claim.
We list these alongside the open questions of \Cref{sec:unsure}
