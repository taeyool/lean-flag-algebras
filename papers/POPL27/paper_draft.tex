...
for each obligation in turning a certificate into a verified proof.  In the
order in which a finished proof presents them, the parts are:

\begin{enumerate}
  \item \textbf{Flags and density data.}  The flags of the sizes the proof needs
    (only those that are free of the chosen forbidden graph), together with
    the pair-density and typed-multiplication identities used for rewriting.
...
The compiler assigns each part to a small set of reusable proof-producing
...
\end{lstlisting}
The diagnostic subcommand \lean{inspect} checks that the certificate can be
parsed and that its flags can be matched to Lean's enumeration before
compilation.  The generated file fills the four parts of
\Cref{sec:flagmatic-shape}; we take them in turn and then show the assembled
output.

\paragraph{Flags and density data (Part~1).}
Part 1 defines the certificate's forbidden graph as a \lean{Sym2Graph} term and
selects one of two generation routes.  Complete-graph forbids use the
\lean{generate\_pruned\_*} commands, while a general graph forbidden as a
subgraph uses the parallel \lean{generate\_subgraph\_free\_*} commands.  Both
routes enumerate the surviving flags, prove their pair-density identities, and
then prove their product expansions, supplying the subflag-to-host bookkeeping
used by the main proof.  All of this runs inside Lean by the machinery of
...
task.  Rather than reason about numerically approximated eigenvalues, we certify
PSD from a rational factorization $M_t = L D L^\top$ with $D$ diagonal and
non-negative.  Lean checks the diagonal entries by exact rational arithmetic
and the matrix identity with \lean{decide +kernel}, without eigenvalues or
floating-point arithmetic, and then lifts the result from $\mathbb{Q}$ to
$\mathbb{R}$, the scalar field of the flag algebra.  A wrong factorization makes one of these
checks fail; this is the one place where we deliberately pay for in-kernel
evaluation.

\paragraph{Lifting the objective (Part~3).}
The densities live at the host size $N$, but the objective basis element is not
always represented by a flag of that size, which splits the certificates into two branches.  When it is
...
target basis element and then check that the augmented algebra element is
...
route.  The downward-averaged products
are discharged by \lean{reduce\_downward\_flagmul}.  This tactic recognizes each
...
\lean{flagMul\_<F>\_<G>}.  The resulting equality is normalized
...
The script translates certificate data but does not establish its correctness;
that remains Lean's task.  A certificate names its flags as short strings
