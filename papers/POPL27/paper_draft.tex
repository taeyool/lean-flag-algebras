...
Checking even this certificate requires expanding a product, computing a
downward average, expanding the objective and the unit to a common host size,
...
\(\phi_0(\langle\sigma\rangle_0)>0\).  For every such \(\phi_0\), Razborov's
extension theorem gives a probability measure
\(\Ext_\sigma(\phi_0)\) on \(X_\sigma\), obtained by choosing a uniformly
random occurrence of \(\sigma\) in finite graphs approaching \(\phi_0\) and
passing to the limit.  For a probability measure \(\mu\) on \(X_\sigma\), its
support \(\operatorname{supp}(\mu)\) consists of those points \(\chi\in X_\sigma\) for
which every open neighborhood of \(\chi\) has positive \(\mu\)-measure.
Define
\begin{equation}\label{eq:S-sigma}
  ...
  \subseteq X_\sigma,
\end{equation}
where the closure is taken in \(X_\sigma\).  Thus \(S_\sigma\) is exactly the
closed set generated by labeled views that remain visible with positive
probability under random rooting from constrained empty-type positive
homomorphisms.

Random rooting preserves the constraint:
\[
  \operatorname{supp}\!\bigl(\Ext_\sigma(\phi_0)\bigr)
  \subseteq Q_\sigma
  \qquad
...
\]
Indeed, the downward average of a \(\sigma\)-flag outside \(\mathcal K\) is a
non-negative multiple of its underlying graph, which also lies outside
\(\mathcal K\).  Every \(\phi_0\in Q_0\) evaluates that graph to zero, so the
expected density of the flag under the random extension is zero.  Its density
is pointwise non-negative and therefore zero almost surely.  Applying this
argument to the countable collection of flags outside \(\mathcal K\) shows that
the random extension lies in \(Q_\sigma\) almost surely.  Since \(Q_\sigma\) is
closed, \Cref{eq:S-sigma} yields the fundamental
inclusion
\begin{equation}\label{eq:S-subset-Q}
  S_\sigma\subseteq Q_\sigma.
\end{equation}
We say that \((\mathcal G,\mathcal K,\sigma)\) is
\emph{root-plantable} when \(S_\sigma=Q_\sigma\).
...
\(\psi(q_\sigma f)\leq\psi(q_\sigma g)\).  The second asks for the inequality
to hold almost surely after randomly labeling the limiting object represented
by a constrained empty-type positive homomorphism.
...
represented by finite rooted graphs in \(\mathcal K\), and enlarges the fibers
containing the roots until random rooting hits them with probability bounded
away from zero.
...
Their adjacencies are then determined entirely by the original rooted graph;
...
Every large \(C_4\)-free graph has \(o(n^2)\) edges, so a uniformly random root
has normalized degree tending to zero.  If \(d\) denotes the one-root flag
density that the root is adjacent to one additional sampled vertex, then
\(-d\) is ensemble positive.  On the other hand, arbitrarily large stars are
\(C_4\)-free, and selecting the center as the root gives \(d=1\).  This
...
gap between the two typed orders: quotient semantics may select an exceptional
root, whereas ensemble semantics sees only roots visible to random sampling.
