...
identity matters, because our development works in the ambient algebra.  In
...
whose flags are \(H\)-free there.  In the ambient algebra the flags containing
...
Part~1, however, selected the $H$-free flags by a concrete
...
The script then reads the target and bound, the forbidden graph, the
host size, and the SDP blocks; reconstructs the rational matrices; computes the
$LDL^\top$ witnesses; and selects the target-lifting branch and the form in
...
proof, and one addition of a non-negative quadratic form.  When the target is
smaller than the host size, the file additionally contains the Part~3
expansion.  A
...
certificate flag and converts the target, bound, forbidden graph, and matrix
...
We evaluated the compiler on seven certificates spanning both target-size
...
all with~$8\times8$ matrices.  Together, these examples exercise both target-size
...
supported simple-graph certificate format, a parseable target and singleton
