...
\(\mathtt{flagDensity}_2\) are the specification-level densities.  Their reflected
counterparts run the finite search from \Cref{sec:reflection-concrete} over concrete induced
subflags, count the isomorphic placements, and then lift the result through the
...
\end{lstlisting}
After rewriting a density goal with one of these theorems, the right-hand side
is a concrete finite computation over \lean{Finset}s and \lean{Bool} tests, so
\lean{native\_decide} can evaluate it directly.  For example, one generated
pair-density computation proves the first \(1/2\)-density from
\Cref{ex:typed-edge-nonedge-product}:
\begin{lstlisting}
-- Sym2Flag_2_1_0_0 is (*@$\overline{K_2}^{\bullet}$@*).
-- Sym2Flag_2_1_0_1 is (*@$K_2^{\bullet}$@*).
-- Sym2Flag_3_1_0_1 is (*@$\overline{P_3}^{\bullet}$@*).
example
    : flagDensity(*@$_2$@*)
  ...
  native_decide
\end{lstlisting}
The statement already presents the abstract flags as the decoding of concrete
\lean{Sym2Flag}s.  The proof rewrites the abstract density with the adequacy
theorem and then lets \lean{native\_decide}
evaluate the reflected density computation.

The downward operator from \Cref{sec:formal-downward} has the same shape.  The abstract
definition uses the normalizing factor \(q_\sigma(A)\).  In the reflection
layer, we compute this factor by counting concrete type-preserving embeddings
of the type into a \lean{Sym2LabeledGraph} and dividing by the number of all
injections; then we lift the result to \lean{Sym2Flag}s:
\begin{lstlisting}
...
\end{lstlisting}
Thus the same reflection pattern covers both density and downward
computations: define the executable version on the concrete graph structure,
prove that decoding it gives the abstract operation, and use that theorem as a
rewrite rule before evaluation.  For the downward normalizing factor, the proof
has the same schematic shape:
\begin{lstlisting}
example : downwardNormalizingFactor F.toFlag = q := by
  rw [downwardNormalizingFactor_eq]
  native_decide
\end{lstlisting}
The external tables determine which statements Lean should try to prove and
which rational number \(q\) appears in the statement.  They are not trusted as
proof certificates: after the adequacy rewrite, Lean's kernel checks that the
reflected evaluator really computes the stated value.
