...
This obstacle is the most pervasive in the counting layer.  Sets defined
...
\end{lstlisting}
This pattern appears repeatedly in the list-density proofs: for the
quotient-respect lemma, for permutation of sampled flags, and for insertion of
an empty flag.  Without the explicit \lean{let} bindings,
...
as referring to the same enumeration terms.  The flag-operator proofs use a
similar explicit \lean{Fintype.ofFinite} construction for the isomorphism set
with a fixed underlying graph.
...
multiple ways depending on which prior instances are in scope.  When defining
the executable \lean{Fintype} instances for these types, we had to give the
elaborator explicit guidance via \lean{Finset.univ} with a type ascription:
\begin{lstlisting}
