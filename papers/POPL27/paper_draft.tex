...
\lean{FlagSeq}~$\sigma$ is the type of sequences of
...
def ConvergesTo {n(*@$_0$@*) : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : FlagType (Fin n(*@$_0$@*))}
    (s : FlagSeq (*@$\sigma$@*)) (a : FinFlag (*@$\sigma$@*) (*@$\to$@*) (*@$\mathbb{R}$@*)) : Prop :=
...
\lean{PositiveHom}~$\sigma$ is the subtype of
$\mathbb{R}$-algebra homomorphisms from
\lean{FlagAlgebra}~$\sigma$ to $\mathbb{R}$ that are
...
theorem flagSeq_limit_mem_positiveHom
    ...
    (s : FlagSeq (*@$\sigma$@*)) {a : FlagDensitySpace (*@$\sigma$@*)}
    (hs_conv : ConvergesTo s a)
...
\end{lstlisting}
Here \lean{FlagDensitySpace}~$\sigma$ denotes the function space $[0,1]^{\mathcal{F}^\sigma}$, and
...
def semanticCone {n(*@$_0$@*) : (*@$\mathbb{N}$@*)}
    ((*@$\sigma$@*) : FlagType (Fin n(*@$_0$@*))) : Set (FlagAlgebra (*@$\sigma$@*)) :=
  { f | (*@$\forall$@*) (*@$\phi$@*) : PositiveHom (*@$\sigma$@*), (*@$\phi$@*) f (*@$\geq$@*) 0 }

instance {n(*@$_0$@*) : (*@$\mathbb{N}$@*)}
    ((*@$\sigma$@*) : FlagType (Fin n(*@$_0$@*))) : LE (FlagAlgebra (*@$\sigma$@*)) where
...
\lean{unlabel} is its lifted version on \lean{Flag}s.  Each lift requires a proof that
...
noncomputable def downwardNormalizingFactor
  ...
  Quotient.lift
    (fun G : LabeledGraph (*@$\sigma$@*) (Fin n) => downwardNormalizingFactor_labeledGraph G)
...
noncomputable def unlabel {n(*@$_0$@*) : (*@$\mathbb{N}$@*)} {V : Type}
  ...
  Quotient.lift
    (fun G : LabeledGraph (*@$\sigma$@*) V => (*@$\llbracket$@*)unlabeledGraph G(*@$\rrbracket$@*))
    (fun _ _ h => unlabeledGraphQuot_respect_eqv h)

noncomputable def downwardFlag
    {n(*@$_0$@*) n : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : FlagType (Fin n(*@$_0$@*))}
    (F : Flag (*@$\sigma$@*) (Fin n)) : FlagVector (*@$\emptyt$@*) :=
...
Lean first extends \lean{downwardFlag} linearly to
\lean{downwardFlagVector}, a map from \lean{FlagVector} $\sigma$ to
\lean{FlagVector} $\emptyt$.  The difficulty is the next step, descending this
linear map through the quotient defining \lean{FlagAlgebra} $\sigma$.
At the single-flag level, the construction only has to respect isomorphism of
representatives; at the algebra level, it must also respect the expansion
equations generating the zero space $\mathcal{Z}^\sigma$.

The code below shows this second lift.  The intermediate
\lean{downwardFlagVectorQuot} applies \lean{downwardFlagVector} and then
quotients the resulting \lean{FlagVector} $\emptyt$, so its output lies in
\lean{FlagAlgebra} $\emptyt$.  The second lift then quotients the input
parameter of \lean{downwardFlagVectorQuot}.  To do so, Lean needs the proof
\lean{downwardFlagVectorQuot\_respect\_eqv}: two \lean{FlagVector} $\sigma$
inputs that differ by a zero-space element produce the same element of
\lean{FlagAlgebra} $\emptyt$.
\begin{lstlisting}
noncomputable def downwardFlagVector
    {n(*@$_0$@*) : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : FlagType (Fin n(*@$_0$@*))}
    : FlagVector (*@$\sigma$@*) (*@$\to$@*) FlagVector (*@$\emptyt$@*) :=
  linearExtension (fun F : FinFlag (*@$\sigma$@*) => downwardFlag F.2)

noncomputable def downwardFlagVectorQuot 
    {n(*@$_0$@*) : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : FlagType (Fin n(*@$_0$@*))}
    (f : FlagVector (*@$\sigma$@*)) : FlagAlgebra (*@$\emptyt$@*) :=
  (*@$\llbracket$@*)downwardFlagVector f(*@$\rrbracket$@*)
...
noncomputable def downward
  ...
  Quotient.lift
    (fun f : FlagVector (*@$\sigma$@*) => downwardFlagVectorQuot f)
    (fun _ _ h => downwardFlagVectorQuot_respect_eqv h)
\end{lstlisting}
The last definition is the quotient-level downward operator.  In mathematical
prose we write it as $\llbracket-\rrbracket_\sigma$, following
Section~\ref{sec:background-downward}; in Lean snippets below, the same map is
rendered as $\llbracket-\rrbracket_0$, emphasizing that its codomain is the
$\emptyt$-typed algebra.  In either notation, the concise brackets hide two
successive quotient lifts and a substantial compatibility proof with the
zero-space presentation of the algebra. \gw{I'm here.}
...
probability measure on \lean{PositiveHomSpace}~$\sigma$ corresponding to
...
def forbidLE {n(*@$_0$@*) : (*@$\mathbb{N}$@*)} ((*@$\sigma$@*) : FlagType (Fin n(*@$_0$@*)))
    (H : FinFlag (*@$\emptyt$@*)) (f g : FlagAlgebra (*@$\sigma$@*)) : Prop :=
  forall ((*@$\phi_0$@*) : PositiveHom (*@$\emptyt$@*)), (*@$\phi_0$@*) (*@$\langle\sigma\rangle_0$@*) > 0 ->
    (*@$\phi_0$@*) (*@$\llbracket$@*)basisVector H(*@$\rrbracket_0$@*) = 0 ->
    P[(*@$\phi_0$@*)] {(*@$\phi$@*) : PositiveHomSpace (*@$\sigma$@*) | (*@$\phi$@*) f <= (*@$\phi$@*) g} = 1
\end{lstlisting}
...
and enumerates permutations only of the unlabeled vertices outside the type,
reducing the pentagon proof's search from $5! = 120$ to $(5-3)! = 2$ candidate
maps per check.  This constant-factor improvement is multiplied across
thousands of generated density lemmas, making the difference between a viable
evaluator and a stuck proof search.
...
\sh{I'm here.}
The mathematical specification of Section~\ref{sec:abstract} and the
reflection layer of Section~\ref{sec:reflection} are enough, in principle,
to write a flag-algebra proof in Lean by hand.  When we did so for the first
case studies, a different observation emerged: the proofs were almost
identical.  Mantel's theorem, the $C_4$-density bound, and the
Erd\H{o}s pentagon theorem differ in their flags, their rational
coefficients, and their positive semidefinite matrices, but their
\emph{proof scripts} have the same shape---the same sequence of tactic
calls, in the same order, parametrized only by sizes and data.

This regularity is itself a result.  If the only thing that changes between
two flag-algebra proofs is finite data, then the human-written part is
mechanizable: a script can read that data from an external certificate and
emit the proof.  This section describes the pattern
(Section~\ref{sec:flagmatic-shape}), the verified Lean building blocks that
each part of it rests on (Section~\ref{sec:flagmatic-blocks}), and the
translator that fills the pattern from a Flagmatic certificate alone
(Section~\ref{sec:flagmatic-script}).  The translator is the second
contribution promised in Section~\ref{sec:intro}: a certificate-to-proof
pipeline whose output the Lean kernel checks, and whose own computation is
never trusted (Section~\ref{sec:flagmatic-trust}).

\subsection{The recurring four-part shape}
\label{sec:flagmatic-shape}

Every API-style proof in the development is a populated instance of the same
four-part skeleton.

\begin{enumerate}
  \item \textbf{Per-block PSD certificates.}  For each subflag type $\sigma_t$
    the certificate carries a rational positive semidefinite matrix $M_t$.
    Part~1 declares each $M_t$ together with a proof that it is PSD over
    $\mathbb{R}$.
  \item \textbf{Flag, density, and product data.}  Part~2 brings into scope,
    for the relevant sizes, the $\mathcal{H}$-free flags, their completeness
    lemmas, and the pair-density and typed-multiplication identities that the
    proof body rewrites with.
  \item \textbf{Subflag vectors.}  Part~3 names the type $\sigma_t$ and the
    vector $v_t$ of $\sigma_t$-flags that the quadratic form
    $\langle v_t, M_t\, v_t\rangle$ is built from.
  \item \textbf{The theorem and its proof body.}  Part~4 states the bound
    $P \leq_{\mathcal{H}} c\cdot\mathbf{1}$ and discharges it by adding each
    block's quadratic form, expanding the unit at host size, reducing every
    downward-averaged product, normalizing, and closing by non-negativity.
\end{enumerate}

The shape has exactly two structural variants, selected by comparing the
objective's vertex count $n_{\mathit{obj}}$ to the host size $N$ at which the
certificate's admissible graphs live.  In \emph{Branch~A}
($n_{\mathit{obj}}=N$) the objective is already a host-size flag, so the
proof works directly with it ($C_4$-Tur\'an, Erd\H{o}s pentagon).  In
\emph{Branch~B} ($n_{\mathit{obj}}<N$) the objective must first be expanded
up to host size, dropping the $\mathcal{H}$-containing contributions on the
way; this costs one extra auxiliary lemma (Mantel, $K_4$- and $K_5$-Tur\'an).
Everything else is shared.

\subsection{The building blocks}
\label{sec:flagmatic-blocks}

Each part of the shape is discharged by a verified component.  These
components are reusable on their own---one can invoke them by hand---but in
this section their role is to be the targets that the translator emits into.
We describe them in turn, deferring to the reflection layer
(Section~\ref{sec:reflection}) for the density computation they ultimately
rest on.

\paragraph{Names as machine-readable metadata.}
A design decision underlies every tactic below: each flag and flag-algebra
constant is named \lean{Flag\_n\_k\_m\_i} / \lean{FlagAlgebra\_n\_k\_m\_i},
where $n$ is the vertex count, $k$ and $m$ identify the type graph, and $i$ is
the canonical index within the isomorphism class.  The convention is not
merely organizational: it is a machine-readable encoding of mathematical
structure.  By parsing the trailing integers from a constant's \lean{Name}---a
syntactic object Lean~4's elaboration monad inspects without triggering
type-checking or attribute resolution---a tactic recovers the canonical order
of a flag term in a linear combination, the name of its precomputed density or
multiplication lemma, and the type parameters needed to instantiate the
adequacy theorem of Section~\ref{sec:reflection}.  Every tactic below runs its
name parsing in the fast elaboration path; this is what lets it scale to
pentagon-sized expressions.

\paragraph{PSD via rational \texorpdfstring{LDL$^\top$}{LDL\^T} (Part~1).}
A real symmetric matrix is positive semidefinite iff it factors as
$M = L\,\mathrm{diag}(d)\,L^\top$ with $d\geq 0$.  We verify the factorization
over $\mathbb{Q}$ and lift it to $\mathbb{R}$.  Each block is emitted as a
cluster of rational definitions closed by a one-line proof:
\begin{lstlisting}
...
\end{lstlisting}
The tactic \lean{psd\_real\_ldlt M LM dM} (in \lean{API/Matrix/PosSemiDef.lean})
refines the goal through the generic lemma \lean{posSemidef\_real\_of\_LDLt} and
discharges its two side conditions: the diagonal non-negativity $d_M \geq 0$ by
\lean{fin\_cases}/\lean{norm\_num}, and the factorization equality
$M = L_M\,\mathrm{diag}(d_M)\,L_M^\top$ by \lean{decide +kernel}---a decidable
rational matrix equality the kernel evaluates entry by entry, using no compiled
native code.  The lemma bundles the $\mathbb{Q}\!\to\!\mathbb{R}$ plumbing, so
the emitter need only supply the rational triple $(M, L_M, d_M)$.  Because the
equality is kernel-checked, a wrong $L_M$ or $d_M$ surfaces as a compile error
...
\paragraph{Forbid-conditioned flag and density generation (Part~2).}
...
read from precomputed tables.  The commands emit exactly the constants the
proof needs:
\begin{lstlisting}
...
\end{lstlisting}
These commands enumerate only the $\mathcal{H}$-free flags: they filter the
small underlying-graph enumeration by an isomorphism-invariant forbid-freeness
predicate \emph{before} building the typed flags, so the
$\mathcal{H}$-containing flags are pruned before they are ever
materialized---this is what keeps the typed enumeration tractable at host sizes
where the full typed \lean{native\_decide} would otherwise wall.  Completeness
is a lemma of the form \lean{flagSetHfree\_N\_k\_m\_<K>}~$=$~\lean{univ.filter p},
proved by a single \lean{native\_decide} that references only the surviving
constants; the pair
densities and product expansions are each proved by the adequacy rewrite of
Section~\ref{sec:reflection} followed by an evaluator call.  All numerical
content is therefore recomputed and re-checked in Lean every build; the
certificate never asserts a density value the kernel does not reproduce.
Crucially, the generators take the forbidden graph as a \emph{term argument}
(an \lean{ident} naming a \lean{Sym2Graph}) and build the forbid-freeness
predicate from it, so a new forbidden graph is a new \lean{Sym2Graph}
definition rather than a new per-forbid branch in the meta-commands.

\paragraph{Flag expansion and multiplication.}
Two structural identities recur in every proof.  The \emph{expansion} of a flag
$F\in\Flag{\sigma}_m$ at a larger size $n$ writes it as a weighted sum of all
$n$-vertex flags; the \emph{product} of two typed flags is a linear combination
at the product size.  Under a forbid both are produced in forbid-free form.
The tactic \lean{flag\_expand\_hfree N K} (in \lean{API/FlagExpand.lean}) closes
a goal $F =_{K} \sum_j c_j\,F_j$ by rewriting with
\lean{basisVector\_quot\_forbidEq\_sum} directly onto the generated
\lean{flagSetHfree\_N\_k\_m\_<K>} set---never materializing the full
\lean{flagSet}, and dropping the forbidden host terms automatically (no manual
\lean{basisVector\_forbidEq\_zero} step).  Its closing arithmetic needs the
relevant \lean{flagDensity\textsubscript{1}} values present as \lean{@[simp]}
lemmas, which the translator emits (each closed by \lean{native\_decide} after
an adequacy rewrite).  Products are symmetric:
\lean{generate\_pruned\_forbid\_free\_mul\_theorems} registers a
\lean{flagMul\_<A>\_<B>} expansion for each pattern pair, and the proof body
consumes them with \lean{reduce\_downward\_flagmul}.

\paragraph{Linear normalization.}
After expansion, the obligation is an equality between two linear combinations
of flag terms that are equal but syntactically reordered.  Generic tactics
(\lean{ring}, \lean{ac\_rfl}) handle this in principle but search over all
permutations of summands and become prohibitively slow past ${\sim}20$ terms.
The \lean{flagsum\_ac\_sort\_rhs\_pipeline} tactic (in \lean{API/FlagSumSort.lean})
instead flattens the expression into a list of (base, coefficient) pairs,
assigns each base the sort key read from the trailing integer of its name,
stable-sorts, merges like coefficients, and proves the original-equals-sorted
equality up to add-AC.  The recorded counterexample is in
\lean{ErdosPentagon.Lemmas}: a commented call reads
...
expressions of ${\sim}25$ flag terms; the index-guided sort completes the same
step in under a second.  It is a correctness-enabling optimization, not a
convenience wrapper.

\paragraph{Adding quadratic forms and closing (Part~4).}
Two API lemmas frame the body.  The standard upper-bound move---add one or more
PSD quadratic forms to the target, then check that the result lies in the
forbid-cone---is encapsulated by
\begin{lstlisting}
...
\end{lstlisting}
iterated once per type to build a transitive chain of additions.  The unit is
expanded to host size by \lean{expand\_one\_hfree\_at N K} (rewriting
$\mathbf{1}$ as the sum of $N$-vertex $\mathcal{H}$-free flags); the
...
identity, moving each rewritten term across the inequality; and the final
non-negativity goal is closed by \lean{flag\_nonneg}, which distributes an
arbitrary positive homomorphism over the sum, splits with \lean{add\_nonneg},
and discharges each leaf by \lean{positiveHom\_basisVector\_ge\_zero}.
...
The translator \lean{flagmatic\_to\_lean.py} turns a Flagmatic certificate
into a Lean module populating Parts~1--4.  Its main entry point is
\begin{lstlisting}[language={}]
python flagmatic_to_lean.py gen-skeleton <cert>.json <target>.lean
\end{lstlisting}
and the output is a self-contained file that \lean{lake build} accepts with
no \lean{sorry}.  Two diagnostic subcommands, \lean{inspect} (resolve every
certificate string to its Lean identifier) and \lean{check-deps} (report the
generators and imports the file will use), let one catch mismatches before
any Lean compilation.

\paragraph{Resolving certificate strings to indices.}
A Flagmatic certificate names graphs by compact strings: \lean{"3:1213"} is
the $3$-vertex path, and \lean{"3:12(2)"} is a $3$-vertex flag on a size-$2$
type.  The Lean side names them by canonical index, \lean{Flag\_n\_k\_m\_i}.
The script parses each string into $(n, E, \mathit{labels})$ and
isomorphism-matches it against the canonical enumeration, recovering the
index $i$ by brute-forcing the $n!$ vertex permutations---ample, since
$n\leq 5$ here.  The same enumeration that Lean uses to define the flags is
reused by the script, so the indices agree by construction rather than by a
separately maintained table.

\paragraph{Assembling Part~1.}
The certificate stores each block as a pair $(Q_t', R_t)$, not as the
assembled matrix.  The script reconstructs $M_t = R_t\,Q_t'\,R_t^\top$ in
exact rationals, then factors it as $L_t\,\mathrm{diag}(d_t)\,L_t^\top$ by a
textbook elimination loop that handles the rank deficiency the assembled
matrix typically has.  It emits $M_t$, $L_t$, $d_t$, and the one-line
\lean{psd\_real\_ldlt} proof of Section~\ref{sec:flagmatic-blocks}.  None of
the script's matrix arithmetic is trusted; the kernel re-checks the
factorization.

\paragraph{Emitting the body: Branch~A vs.\ Branch~B.}
For Branch~A the body is a fixed sequence parametrized only by $N$, the block
count $T$, and the per-block dimensions.  For the $C_4$-Tur\'an certificate
($K_3$-free, $N=4$, $T=2$) the script emits:
\begin{lstlisting}
theorem K3forbidC4_flagAlgebra
    : FlagAlgebra_4_0_0_8
      (*@$\leq$@*)[((*@$\langle$@*)_, Sym2EmptyTypedFlag.toFlag (*@$\llbracket$@*)K3(*@$\rrbracket$@*)(*@$\rangle$@*) : FinFlag (*@$\emptyt$@*))] (3 / 8 : (*@$\mathbb{R}$@*)) (*@$\bullet$@*) (1 : FlagAlgebra (*@$\emptyt$@*)) := by
  have quadraticForm_trans : ... := by
    apply forbidLE_add_QuadraticForm M(*@$_2$@*)_real M(*@$_2$@*)_real_posSemidef v(*@$_2$@*)
    apply forbidLE_add_QuadraticForm M(*@$_1$@*)_real M(*@$_1$@*)_real_posSemidef v(*@$_1$@*)
    exact forbidLE_refl _ FlagAlgebra_4_0_0_8
  apply forbidLE_trans quadraticForm_trans
  apply forbidLE_trans_forbidEq_right ?_
        (forbidEq_smul (forbidEq_symm (one_forbidEq_forbidExpand_one _ 4)))
  simp [flagQuadraticForm, v(*@$_1$@*), M(*@$_1$@*)_real, ratMatrixToReal, M(*@$_1$@*), Fin.sum_univ_four, add_assoc]
  simp [v(*@$_2$@*), M(*@$_2$@*)_real, ratMatrixToReal, M(*@$_2$@*), Fin.sum_univ_three, add_assoc]
  ...
  flag_nonneg
\end{lstlisting}
The \lean{forbidLE\_add\_QuadraticForm} calls are emitted in reverse of the
addition order so the outermost summand peels off first; the per-block
\lean{Fin.sum\_univ\_<n>} and the argument to \lean{expand\_one\_hfree\_at}
are chosen from the block dimensions and $N$.  For Branch~B the script
additionally emits the host-size expansion lemma (a single
\lean{flag\_expand\_hfree N K} body plus the \lean{native\_decide}
coefficient table) and a \lean{rw} that splices it in after the
unit-expansion step.  Selecting the branch, choosing the per-block
parameters, and ordering the additions is the entire ``intelligence'' of the
generator; everything downstream is verified by Lean.
...
$L\,\mathrm{diag}(d)\,L^\top$, enumerates induced subgraphs to fill the
...
kernel), or a verified tactic of Section~\ref{sec:flagmatic-blocks}.  A
...
transfer theorem.  The forbid-conditioned generators (Section~\ref{sec:flagmatic-blocks})
...
Section~\ref{sec:flagmatic-blocks}: $LDL^\top$ decomposition over $\Q$, with the matrix
