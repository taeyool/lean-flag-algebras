...
command \lean{generate\_empty\_typed\_flags n} generates the empty-typed flags
...
written in this language into proofs that Lean can check.  We evaluate the
compiler on seven upper-bound certificates, including those needed for
Mantel's theorem and the Erd\H{o}s pentagon theorem
($\tdensity{C_5}{K_3} = 24/625$), an inequality bounding the $C_4$-density of
triangle-free graphs, and a $K_4$-free $P_4$ density bound.  The direct
developments show that the formalization is useful beyond compilation: they
give a separate proof of Mantel's upper bound, formalize the constructions
that complete the Mantel and Erd\H{o}s pentagon results, and prove two
inequalities of Goodman.  We also summarized a separately developed
