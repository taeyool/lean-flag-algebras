...
Here the target is $F=K_2$, and we abbreviate its basis element $[K_2]$ by
$K_2$.
...
non-negativity explicit: the right-hand side is a positive multiple of a square
of a real number.  Thus $f$ is non-negative in $\mathcal A^\sigma$, and
...
of $K_2$ from \eqref{eq:k2-expansion} is
\[
  K_2 = \tfrac13\cdot\fbarPthree + \tfrac23\cdot\fPthree.
\]
Applying \eqref{eq:sum-to-one} to the triangle-free empty-type flags of size
three gives
\[
...
Combining these identities now gives a forward derivation of the desired bound:
\begin{align*}
  K_2
  &\leq_{K_3} K_2+Q \\
...
its own flag vector $v_t$ and matrix $M_t$; the downward quadratic form it
produces is
\[
  Q_t := \bigl\llbracket
