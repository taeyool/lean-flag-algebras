...
layer described in Section~\ref{sec:abstract}.  For each finite operation that would otherwise require a hand proof,
...
Section~\ref{sec:abstract} gave the specification layer: definitions close to
...
The specification layer of Section~\ref{sec:abstract}, however, is not designed
...
isomorphism-checking algorithm when filtering the \lean{Fintype} enumeration. \gw{I'm here.}

\subsection{Correctness as Adequacy}

For density computations, we define computable density functions such as
\lean{sym2EmptyTypeFlagDensity1} entirely over \lean{Finset} operations.  The
decoding map from the concrete representation to the abstract graph type makes
abstract definitions \emph{applicable} to \lean{Sym2Graph} inputs, but
applicability is not computability:
\lean{flagDensity} composed with this decoding still unfolds to non-computable
...
Section~\ref{sec:flagmatic-shape}, each discharged by a reusable, kernel-checked
...
Section~\ref{sec:reflection}, so nothing is read from a precomputed table.
...
none of it has to be trusted is the subject of Section~\ref{sec:flagmatic-trust}.
...
tactic of Section~\ref{sec:flagmatic-pipeline}.  A
...
certificate-to-Lean pipeline introduced in Section~\ref{sec:flagmatic-pipeline},
...
The certificates of Section~\ref{sec:results} establish the flag-algebra upper
...
\end{lstlisting}
is the conjunction of the upper bound of Section~\ref{sec:results} and a lower
bound
\begin{lstlisting}
...
Section~\ref{sec:bridge}'s reflection, tactic, and Automation layers are
