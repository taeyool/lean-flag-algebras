...
\lean{FlagAlgebra}~\(\emptyt\).  Third, \lean{downward} turns this
representative-level construction into a function whose input is an element of
the quotient algebra \lean{FlagAlgebra}~\(\sigma\), rather than a chosen
...
appearing in the statement.  The type \lean{PositiveHomSpace}~\(\sigma\) is the
...
\(\phi : \lean{PositiveHomSpace}~\sigma\) can therefore be converted back to
the corresponding positive homomorphism by \lean{PositiveHomSpace.toPosHom}.
At this point \(\phi\) is just a point of this measurable space, not yet a
random sample.  The expression
\lean{PositiveHomSpace.toPosHom}~\(\phi\)~\(f\) is simply the value of the
positive homomorphism represented by \(\phi\) on \(f\).

The hypothesis
\(\phi_0(\langle\sigma\rangle_0)>0\) says that the type \(\sigma\), viewed as
an empty-type flag-algebra element, has positive density under the base
homomorphism \(\phi_0\).  Under this hypothesis Lean constructs a probability
measure \(\mathbb{P}\) on \lean{PositiveHomSpace}~\(\sigma\).  This
...
\(\mathbb{P}\) is present, a point
\(\phi : \lean{PositiveHomSpace}~\sigma\) may be regarded as a random positive
homomorphism drawn from this distribution.  The displayed theorem states the
defining identity of that distribution:
\begin{lstlisting}
...
\[
  \int x, g(x)\,\partial\mu
\]
for the integral of the function \(x\mapsto g(x)\) with respect to the measure
\(\mu\).  The comma separates the bound variable from the integrand, and
\(\partial\mu\) supplies the measure; the symbol \(\partial\) is only notation
for ``with respect to'', not a derivative.  In the theorem above, the bound
variable is
\(\phi : \lean{PositiveHomSpace}~\sigma\), the integrand is
\lean{PositiveHomSpace.toPosHom}~\(\phi\)~\(f\), and the measure is
\(\mathbb{P}\).  Since \(\mathbb{P}\) is a probability measure, this integral
is simply expectation.
Thus the line
\[
  \int \bigl(\phi \mapsto \phi(f)\bigr)\, d\mathbb{P}
\]
should be read as
\[
...
\(\llbracket f\rrbracket_0\) lies in \lean{semanticCone}~\(\emptyt\), Lean
...
In Section~\ref{sec:background}, following Razborov's usual presentation, the
forbidden graphs are fixed before the flag algebra is built.  For example, in
an \(H\)-free problem, the \(\sigma\)-typed algebra is already the algebra of
\(H\)-free \(\sigma\)-flags, and the semantic order of
Section~\ref{sec:background-semantic-order} is defined inside that constrained
world.  Thus, for \(H\)-free algebra elements \(f\) and \(g\), the statement
\(f\leq_\sigma g\) means
\[
  g-f\in\mathcal{C}^\sigma,
\]
where \(\mathcal{C}^\sigma\) is the semantic non-negativity cone of the
\(H\)-free \(\sigma\)-typed flag algebra.  We call this the
\emph{quotient-semantics} view: the forbidden configurations have already
been quotiented away before the order is tested.

Our formalization takes the opposite route.  The core development constructs
the ambient flag algebra \(\mathcal{A}^\sigma\) without choosing any forbidden
graph.  This keeps the definitions of flags, densities, products, the downward
operator, and positive homomorphisms independent of the extremal problem.  It
also creates an obligation: once a forbidden graph \(H\) is supplied later, we
must still be able to express the constrained semantic order between two
ambient algebra elements \(f,g\in\mathcal{A}^\sigma\).

The order we use for this purpose is the \emph{ensemble semantic order}.  For
readability we state it for a single forbidden graph \(H\), which is the case
implemented by the Lean predicate below; the later semantic comparison applies
to arbitrary forbidden families. \gw{The current framework only supports
forbidding a single graph. I plan to extend it to support forbidding multiple
graphs.}
...
homomorphism \(\phi_0\) such that
\[
  \phi_0(\llbracket H\rrbracket_0)=0
  \qquad\text{and}\qquad
...
The first condition says that the base limit is \(H\)-free; the second says
that the type \(\sigma\) can actually be sampled from that limit.  The last
line then compares \(f\) and \(g\) not against all positive homomorphisms on a
separate quotient algebra, but against almost every typed random extension of
every \(H\)-free base limit.  Section~? proves that, for arbitrary
forbidden graph sets, this ensemble order agrees with the quotient semantic
order above.  The Lean predicate \lean{forbidLE} is the formal definition of
...
notation f "(*@$\leq$@*)[" F_forbid "]" g => forbidLE F_forbid f g
\end{lstlisting}
In the displayed Lean notation,
\(\mathbb{P}[\phi_0]\) denotes the random extension probability measure
\(\Ext_\sigma(\phi_0)\).
The implementation also introduces the Lean notation
\(\lean{f}\mathrel{\leq[\lean{H}]}\lean{g}\) for
\lean{forbidLE H f g}.

What remains is to turn this constrained semantic inequality into the
Tur\'an-density upper bound described in
\Cref{sec:background-semantic-order}.  Lean formulates the target as
\lean{generalizedTuranDensity H F}; here ``generalized'' only indicates that
the target \(F\) is an arbitrary graph, rather than just an edge.  For each
\(n\), it first maximizes the number of induced copies of \(F\) over all \(H\)-free
graphs on \(n\) vertices, and then takes the limiting normalized density. The
relevant definitions are:
\begin{lstlisting}
noncomputable def generalizedExtremalNumber
    {U W : Type}
    (n : (*@$\mathbb{N}$@*)) (H : SimpleGraph U) (F : SimpleGraph W) : (*@$\mathbb{N}$@*) :=
  sup { G : SimpleGraph (Fin n) | H.Free G }
    (fun (G : SimpleGraph (Fin n)) => GraphAlgebras.subgraphCount F G)

noncomputable def generalizedTuranDensity
    {U W : Type} [Fintype W]
    (H : SimpleGraph U) (F : SimpleGraph W) : (*@$\mathbb{R}$@*) :=
  limUnder atTop fun n =>
    (generalizedExtremalNumber n H F / n.choose (Fintype.card W) : (*@$\mathbb{R}$@*))
\end{lstlisting}
Here \lean{H.Free G} says that the host graph \(G\) is \(H\)-free, and
\lean{subgraphCount F G} counts induced copies of the target graph \(F\) in \(G\).
After normalization by \(\binom{n}{|V(F)|}\), this is exactly the
Tur\'an density $\pi(F;H)$.

The following bridge theorem is the point at which the
ensemble semantic order becomes an extremal bound.  It is the
ensemble-semantic version of \Cref{lem:semantic-bound}: the semantic inequality
is stated with \lean{forbidLE} rather than with the quotient semantic order:
\begin{lstlisting}
theorem generalizedTuranDensity_le_of_forbidLE
    {n m : (*@$\mathbb{N}$@*)} {H : SimpleGraph (Fin n)} {F : SimpleGraph (Fin m)}
    {c : (*@$\mathbb{R}$@*)} (hc : 0 (*@$\leq$@*) c)
...
graph \(H\).  The theorem concludes exactly the combinatorial statement
\(\lean{generalizedTuranDensity H F}\leq \lean{c}\).  This is the main payoff of the
specification layer: a concrete certificate proof only has to establish one
semantic inequality in the ambient algebra, and the passage from that
inequality to a Tur\'an-density bound is handled once and reused.
...
to reduce them as executable programs.  Quotients, \lean{Classical.choice},
\lean{Prop}-valued adjacency relations, and abstract finite enumerations are
all appropriate for stating the theory of flag algebras, but they are a poor
interface for repeatedly evaluating small graph computations.

Certificate checking needs exactly those finite computations.  It must decide
whether two concrete flags represent the same isomorphism class, compute
subflag densities, and reduce products among named flags.  These tasks are
finite, but if we ask the evaluator to unfold the abstract definitions directly,
it ends up reducing quotient eliminators, proof-valued fields, and generic
finite-search instances rather than running a simple graph algorithm.

Isomorphism testing is the smallest example of the gap.  Abstractly, two
flags are equal when there exists a type-preserving graph isomorphism between
representatives.  In the computable layer, a graph is instead stored as a
finite set of unordered edges, so the same question becomes a Boolean search
over vertex permutations together with edge-membership checks.  Correctness
theorems for both Boolean outcomes connect this executable test back to the
quotient relation.  In this sense the computation \emph{reflects} the
mathematical proposition: Lean runs the Boolean program on concrete data, then
uses a theorem saying that the answer has the intended abstract meaning.

The files in \lean{FlagAlgebra/Compute/} provide this executable mirror of the
specification layer.  They introduce concrete graph and flag representations,
...
\begin{enumerate}
  \item \textbf{Flags and density data.}  The flags of the sizes the proof needs
    (only those that avoid the forbidden graph), together with the pair-density
  ...
  \item \textbf{Lifting the objective (when needed).}  If the objective is
    ...
    dropping the contributions that contain the forbidden graph.
...
cluster of rational definitions closed by a one-line proof, and is paired with
the vector of $\sigma$-flags it will be contracted against:
\begin{lstlisting}
...
The vector $v_t$ alongside it merely lists the $\sigma_t$-flags by name;
together $(M_t, v_t)$ form the quadratic form
$\llbracket\langle v_t, M_t\,v_t\rangle\rrbracket$ that the main theorem adds to
the target.

\subsection{Lifting the objective to host size}
\label{sec:flagmatic-expand}

Whether this step is present is the proof's one structural choice, fixed by
comparing the objective's vertex count $n_{\mathit{obj}}$ to the host size $N$
fixed by the certificate.  In \emph{Branch~A} ($n_{\mathit{obj}}=N$), the
objective is already a host-size flag and this subsection is skipped
($C_4$-Tur\'an, Erd\H{o}s pentagon).  In \emph{Branch~B} ($n_{\mathit{obj}}<N$;
Mantel, $K_4$- and $K_5$-Tur\'an), the objective must first be lifted, by
rewriting it as its forbid-free expansion at size $N$.  Concretely,
\lean{flag\_expand\_hfree N K} closes a goal $P =_{K} \sum_j c_j\,F_j$ by
rewriting with \lean{basisVector\_quot\_forbidEq\_sum} directly onto the
generated \lean{flagSetHfree\_N\_0\_0\_<K>} set---never materializing the full
\lean{flagSet}, and dropping the $\mathcal{H}$-containing host terms
automatically (no manual \lean{basisVector\_forbidEq\_zero} step).  Its closing
arithmetic needs the relevant \lean{flagDensity\textsubscript{1}} values present
as \lean{@[simp]} lemmas, which the translator emits (each closed by
\lean{native\_decide} after an adequacy rewrite).

\subsection{The main theorem}
\label{sec:flagmatic-close}

The body assembles the pieces in a fixed sequence.  First, each block's
quadratic form is added to the target by \lean{forbidLE\_add\_QuadraticForm}:
\begin{lstlisting}
...
\end{lstlisting}
iterated once per type to build a transitive chain of additions.  The unit on
the right is then expanded to host size by \lean{expand\_one\_hfree\_at N K},
which rewrites $\mathbf{1}$ as the sum of $N$-vertex $\mathcal{H}$-free flags by
the same \lean{basisVector\_quot\_forbidEq\_sum} machinery as the objective lift
of Section~\ref{sec:flagmatic-expand}.

The downward-averaged products are discharged by
\lean{reduce\_downward\_flagmul}.  The generation step
...
single call dispatches all of a block's products.

What remains is an equality between two linear combinations of flag terms that
are equal but syntactically reordered.  Generic tactics (\lean{ring},
\lean{ac\_rfl}) search over all permutations of summands and become
prohibitively slow past ${\sim}20$ terms, so we normalize with
\lean{flagsum\_ac\_sort\_rhs\_pipeline} (in \lean{API/FlagSumSort.lean}): it
flattens the expression into a list of (base, coefficient) pairs, assigns each
base the sort key read from the trailing integer of its name, stable-sorts,
merges like coefficients, and proves the original-equals-sorted equality up to
add-AC.  The recorded counterexample is in \lean{ErdosPentagon.Lemmas}: a
commented call reads ``\texttt{-- sort\_at -- takes more than 10 minutes}''; the
generic \lean{sort\_at}, which uses \lean{simp} without index guidance, times out
on ${\sim}25$-term expressions, while the index-guided sort completes the same
step in under a second.

The final non-negativity goal is closed by \lean{flag\_nonneg}: it distributes
an arbitrary positive homomorphism over the sum, splits with \lean{add\_nonneg},
