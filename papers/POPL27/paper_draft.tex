...
The definition delegates to the condition-generic \lean{forbidLEWith},
instantiated at the forbidden condition \lean{forbiddenCondition H}.
...
hypothesis \lean{C $\phi_0$} is the forbidden-graph assumption.  Here
\lean{forbiddenCondition H} unfolds to
\lean{familyForbiddenCondition (forbiddenFlags H)}: it requires
\lean{$\phi_0$ $\llbracket$basisVector F$\rrbracket_0$ = 0} for every empty-type
flag \lean{F} in \lean{forbiddenFlags H}, the family of flags whose underlying
graph contains \(H\) as a (not necessarily induced) subgraph.
...
\lean{SimpleGraph}, and the resulting condition \lean{forbiddenCondition H} is
the semantic form of being \(H\)-free.
...
theorem generalizedTuranDensity_le_of_forbidLE
    ...
    (h : F.toFlagAlgebra <=[H] c (*@$\leansmul$@*) (1 : FlagAlgebra (*@$\emptyt$@*)))
    : generalizedTuranDensity H F <= c := ...
\end{lstlisting}
The hypothesis is an empty-typed \lean{forbidLE} inequality: the target
...
therefore provides an enumeration of all induced subflags of \(G\).  Concretely,
the instance contains an \lean{elems} field listing all elements of this type,
...
the pattern \(F\) when computing \(\den{F}{G}\). \gw{I'm here.}
