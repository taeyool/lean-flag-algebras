...
distinguished labeled induced subgraph.  By treating the densities of such partially
...
strong inequalities can be obtained from positive semidefinite matrices: a
...
\begin{equation}
  ...
  \mathbf e_F - \sum_{H \in \mathcal{F}^\sigma_n} \den{F}{H} \cdot \mathbf e_H,
...
Throughout the paper, capital letters $F,G,H$ denote finite flags, while
...
We call $\sum_{H \in \mathcal{F}^\sigma_n} \den{F}{H} \cdot \mathbf e_H$ the
...
element $[P]$ of a target graph $P$ satisfies
$[P]\leq_{\mathcal H}c\cdot\mathbf 1$.  A \emph{flag-algebra certificate} is a
...
$[P] \leq_{\mathcal H} c\cdot\mathbf 1$ supplies downward quadratic forms $Q_t$
and a remainder $r$ that, under the $\mathcal H$-free condition, justify the
forward chain
\[
  [P]
  \leq_{\mathcal H} [P]+\sum_t Q_t
  = c\cdot\mathbf 1-r
...
$c\cdot\mathbf 1-[P]=\sum_t Q_t+r$.  Flagmatic~\cite{vaughan2013flagmatic}
...
\begin{enumerate}
  ...
  \item \textbf{Certificate matrices and basis-element vectors.}  For each SDP
    ...
    $\langle v_t, M_t\, v_t\rangle$.
  \item \textbf{Lifting the objective (when needed).}  If the target graph $P$
    has fewer than $N$ vertices, one auxiliary lemma first lifts $[P]$ to the
  ...
  \item \textbf{The main theorem.}  With
    ...
    the bound $[P] \leq_{\mathcal{H}} c\cdot\mathbf{1}$ through the same chain as
...
$[P]+\sum_tQ_t=c\cdot\mathbf 1-r$ supplies its middle equality, while the
...
$[P]\leq_H[P]+\sum_tQ_t$.  It uses the following general lemma:
\begin{lstlisting}
...
The proof starts from the reflexive inequality $[P]\leq_H[P]$ and applies this
lemma once for each SDP block.  After all blocks have been added, its conclusion is precisely
$[P]\leq_H[P]+\sum_tQ_t$.

It remains to prove $[P]+\sum_tQ_t\leq_H c\cdot\mathbf 1$.  The compiler checks
this comparison by rewriting both sides in the basis of $N$-vertex $H$-free
flags.  When necessary, the Part~3 lemma first expands $[P]$ to host size.
...
conceptual equality $[P]+\sum_tQ_t=c\cdot\mathbf 1-r$ and the last inequality
...
\]
A size-$\ell_1$ flag $H$ is therefore stored as the tagged pair
$\langle\ell_1,H\rangle$, while a size-$\ell_2$ flag $G$ is stored as
...
For every
$H\in\mathcal{F}_{\ell_1}^{\sigma}$, the expansion identity gives
\[
  [H]
  =
...
\[
\begin{aligned}
  \sum_{H\in\mathcal{F}_{\ell_1}^{\sigma}}
    \den{F_1,F_2}{H}\,[H]
  &=
  \sum_{G\in\mathcal{F}_{\ell_2}^{\sigma}}
    \left(
      \sum_{H\in\mathcal{F}_{\ell_1}^{\sigma}}
        \den{F_1,F_2}{H}\,\den{H}{G}
    \right)[G] \\
  &=
  \sum_{G\in\mathcal{F}_{\ell_2}^{\sigma}}
    \den{F_1,F_2}{G}\,[G].
...
that a positive homomorphism \(\chi\sim\Ext_\sigma(\phi_0)\) satisfy
\(\chi(f)\leq\chi(g)\) with probability one.
For the \(H\)-free class---graphs containing no subgraph
isomorphic to \(H\)---
\(\lean{forbidLE H f g}\) implements this ensemble order.  The corresponding
notions of positivity are \(0\leq^{\mathrm{quot}}_{\mathcal K,\sigma}f\) and
\(0\leq^{\mathrm{ens}}_{\mathcal K,\sigma}f\); an arbitrary comparison of
\(f\) and \(g\) is the positivity comparison for \(g-f\).
