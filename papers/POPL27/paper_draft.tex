...
the pattern \(F\) when computing \(\den{F}{G}\). \gw{The current density-counting
path appears to enumerate all induced subflags and filter them using generic
isomorphism checker rather than the fast same-size checker below; revisit this
after the planned density-count refactor.}

The core reflection definitions already give a generic decidability instance for
the relation \(G \sim_{\mathsf{sf}} G'\) by asking Lean to enumerate graph
isomorphisms.  For the evaluator, the fast isomorphism module installs a
high-priority replacement with the same specification but a more direct
algorithm.  It first rejects typed labeled graphs with different edge counts,
then maps the labeled vertices according to the two embeddings and searches
only over permutations of the remaining vertices:
\begin{lstlisting}
def isIsoFast_bool
...
theorem isIsoFast_bool_true_correct
    {k n : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k} {G1 G2 : Sym2LabeledGraph (*@$\sigma$@*) n}
...
\end{lstlisting}
The Boolean program is not trusted on its own: the \lean{true\_correct} and
\lean{false\_correct} theorems turn its two possible outcomes into proofs about
the quotient relation.  Thus equality still means equality up to flag
isomorphism; only the procedure used to decide it has changed.
