...
The specification layer of Section~\ref{sec:abstract} deliberately starts from
Mathlib's graph type \lean{SimpleGraph V}.  Here \lean{V} is the
type whose elements are the vertices of the graph, and the adjacency relation
\lean{Adj} takes two vertices and returns a proposition:
\begin{lstlisting}
...
\end{lstlisting}
This is exactly the right language for the mathematics, but it is too general to
be the data structure of the evaluator.  The type \lean{V} need not come with a
computable enumeration of its vertices, and the relation \lean{Adj} need not
come with a Boolean decision procedure.  Since \lean{LabeledGraph}s and
