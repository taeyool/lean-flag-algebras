...
\lean{Fintype}-instance ambiguity, together with the bijective proof pattern
that emerged for density equalities.
...
chosen representatives.  An operation that changes the number of vertices
moves between differently indexed types, so the corresponding data must be
transported explicitly.  Pointwise agreement between two embeddings must
sometimes be reassembled into equality of the functions or structures
themselves.  Finally, when a density is defined by counting a finite set, Lean
must be made to use a consistent finite enumeration.  None of these issues
changes the underlying mathematics, but each recurs often enough to become a
substantial proof-engineering cost.

These obligations have related but not identical causes.  Lean automatically
identifies expressions only when they reduce to the same term; other
mathematically evident identifications require explicit proofs.  This accounts
for many of the size-indexing and function-equality obligations.  By contrast,
well-definedness on quotient classes is an ordinary quotient obligation, while
ambiguity between finite enumerations is more specific to Lean's elaboration
and typeclass mechanisms.  We therefore report the concrete obstacles without
claiming that they all arise from a single foundational feature.
...
Both \lean{Flag} and \lean{FlagAlgebra} are quotients.  Defining a function on
representatives therefore requires \lean{Quotient.lift} and a proof that the
function respects the equivalence relation; proving two quotient classes equal
from representative-level data typically requires \lean{Quotient.sound} and a
witness of that relation.
...
\subsection{From Pointwise Agreement to Equality of Structures}

In Lean~4's intensional type theory, function extensionality
($(\forall x,\, f\,x = g\,x) \Rightarrow f = g$) is not part of definitional
equality; it is available through \lean{funext}.  This
...
\(\neg\,\lean{e.IsDiag}\).  When two \lean{Sym2Graph} values are constructed
through different proof paths, their \lean{edges\_valid} fields still appear
as congruence goals even after the edge data has been identified.  The
adequacy proofs discharge those goals with \lean{proof\_irrel\_heq}:
\begin{lstlisting}
...
The mitigations of \Cref{sec:obstacles}---explicit \lean{Fintype} bindings,
systematic use of \lean{HEq} and \lean{cast} for size changes, and
proof-irrelevance bridges---are responses to
...
propositionally equal, and a definitional equality that does not include
