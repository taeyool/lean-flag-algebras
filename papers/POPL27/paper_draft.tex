...
abbrev Sym2FlagType (k : Nat) := Sym2Graph k
...
structure Sym2InducedLabeledSubgraph
    {k : Nat} {sigma : Sym2FlagType k} {n : Nat}
...
def isIsoFast_bool
  ...
  if G1.edges.card != G2.edges.card then false
    ...
    perms.any fun p =>
      let fullMap :=
        buildFullMap n k G1.type_embed G2.type_embed nonType1 p
      edges.all fun e =>
...
instance (priority := high) fastDecidableSym2LabeledGraphEqv
  ...
  if h : isIsoFast_bool G1 G2 = true then
    ...
    isFalse (isIsoFast_bool_false_correct ...)

instance (priority := high) fastFintypeSym2Flag
    {k : Nat} {sigma : Sym2FlagType k} {n : Nat} :
    Fintype (Sym2Flag sigma n) := by
  refine @Quotient.fintype _ _ (sym2LabeledGraphSetoid sigma n) ?_
  intro G G'
