...
abstract downward image.  By linearity, finite flag expressions can be handled
term by term, so the downward identities needed in later flag-algebra proofs can
...
basis flags, their unlabelings and downward coefficients, the density table,
...
the flags and rational matrices needed to rewrite the difference between the
...
squares.  For flags
$e_1,\ldots,e_r$ of the same type, collect them into a vector
$v=(e_1,\ldots,e_r)$.  A symmetric matrix $M$ is \emph{positive semidefinite}
...
positive homomorphism: evaluating the flags turns it into the ordinary real
...
We organize the recurring structure into a four-part skeleton, with one part
...
The next subsection exploits this structure by assigning each part to a small
set of reusable proof-producing components.  Consequently, a new theorem
primarily requires new certificate data rather than a new proof architecture.


\subsection{Compiling Certificates into Lean Proofs}
...
$\mathbb{R}$, where the flags live.  A wrong factorization makes one of these
...
summand $\llbracket c\,(A\cdot B)\rrbracket$ and rewrites it with the generated
product identity \lean{flagMul\_<A>\_<B>}.  The resulting equality is normalized
...
separating from the engineering choices and the certificate-to-Lean
construction.  We do not claim that it generalizes beyond formalizations of
extremal combinatorics; we report it because the same pattern arose repeatedly.
