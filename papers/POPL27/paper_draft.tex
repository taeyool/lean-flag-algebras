...
transcribing its numerical data into the proof assistant, reproducing the
bookkeeping that links subflags to host flags, and checking
positive-semidefiniteness, densities, and product expansions within Lean's
proof pipeline.
...
semidefinite matrices certify extremal bounds.  We then identify the common
proof structure and show how the certificate-to-Lean compiler realizes each
step with reusable proof-producing components.
...
variation of the certificate-to-Lean pipeline introduced in
...
skeleton is otherwise unchanged; the pipeline does not assume that the
forbidden graph is complete.

The pipeline itself proves only upper bounds.  For Mantel and the pentagon,
...
do not know which part of the pipeline---flag enumeration, density-theorem
...
end-to-end pipeline from such certificates to combinatorial theorems.
