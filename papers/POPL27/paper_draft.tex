...
the reflection generators for the entry isomorphic to it.  The index of the
matching entry determines the corresponding \lean{FlagAlgebra\_n\_k\_m\_i}
constant, and hence the entries of each flag vector.
...
$LDL^\top$ witnesses; and decides whether the target must be lifted.
...
expansion.  Thus, the script varies the
data, but the argument assembled in Part~4 has the same
structure in every generated file.

The script itself determines which \lean{FlagAlgebra\_n\_k\_m\_i} constant
corresponds to each
...
final bound, but it does not compare the file with the source certificate.  This
