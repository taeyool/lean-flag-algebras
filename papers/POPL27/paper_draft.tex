...
A \emph{flag-algebra certificate} is a finite witness for a proposed density
inequality $P \leq_{\mathcal H} c\cdot\mathbf{1}$.  It specifies concrete flags,
rational coefficients, and positive semidefinite matrices arranged so that,
after all terms are expanded at a common host size, the difference
$c\cdot\mathbf{1}-P$ is a sum of downward-averaged positive semidefinite
quadratic forms and a remainder with non-negative coefficients.  Checking this
decomposition proves the inequality.  Flagmatic~\cite{vaughan2013flagmatic}
uses semidefinite programming to search for such finite witnesses.
...
We first explain why positive semidefinite matrices provide the non-negative
terms in a certificate.  We then identify the four obligations common to our
...
used by the certificate, represents these coefficients as the entries of a
...
exact rational matrices.  The resulting \emph{certificate} records the
admissible graphs and flags, the target-density data, and the rational PSD
matrices $M_t$ paired with vectors $v_t$ of $\sigma_t$-flags.  The density and
multiplication tables needed to check the certificate are then recomputed in
Lean rather than trusted as part of the input.
...
enough to write the certificate by hand as an explicit square,
...
the simplest validation of the compiler.
