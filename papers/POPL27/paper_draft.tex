...
such as testing flag isomorphism, computing a density, or expanding a product,
...
checking them already involves many finite computations: flag isomorphism,
subflag densities, multiplication tables, and rational arithmetic.  A formal
proof should not trust an external table, but it also should not replay every
table entry as a long hand proof.  Reflection gives the middle path: verify the
evaluator once, then use the verified evaluator repeatedly.  Each time Lean runs
the evaluator, the correctness theorem explains why the computed answer has the
claimed mathematical meaning.

\Cref{sec:abstract} gave the specification layer: definitions close to
the mathematics, where flags are identified up to isomorphism and quotient
classes hide arbitrary choices of representatives.  Those definitions are the
right ones for stating theorems, but many are marked \lean{noncomputable}
because they are specifications rather than efficient programs.  This section
develops the executable mirror of that specification layer: concrete graph and
flag representations, optimized procedures for isomorphism and density
computations, and adequacy theorems proving that those procedures agree with the
quotient-based definitions.  The same pattern is used throughout the rest of the
development: a concrete representation, an evaluator, and a theorem connecting
the evaluator to the abstract specification.
...
structure Sym2InducedLabeledSubgraph
    {k : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k} {n : (*@$\mathbb{N}$@*)}
...
def Sym2InducedLabeledSubgraph.edges
    ...
    Finset (Sym2 (Fin n)) :=
  G.edges.filter (fun e => forall v in e, v in H.verts)

instance
    {k : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k} {n : (*@$\mathbb{N}$@*)}
    (G : Sym2LabeledGraph (*@$\sigma$@*) n) :
...
of \(G\): it stores the chosen vertex set together with a proof that the
labeled vertices are included.  Its edge set is then computed by filtering the
...
with the proof of containment.  With this instance available, the evaluator can
iterate through all induced subflags of \(G\) and count the ones isomorphic to
the pattern \(F\) when computing \(\den{F}{G}\).

The remaining task in this count is the second basic ingredient: for each
subflag, the evaluator must decide whether it is isomorphic to the pattern
\(F\).  \gw{The current
density computing framework does not appear to use the fast isomorphism-checking
instance below.  This likely needs to be revised later.}
The following \lean{Decidable} instance provides this concrete
isomorphism test for two \lean{Sym2LabeledGraph}s \(G_1\) and \(G_2\) with the
same vertex type \lean{Fin n}:
\begin{lstlisting}
def isIsoFast_bool
    {k n : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k}
    (G1 G2 : Sym2LabeledGraph (*@$\sigma$@*) n) : Bool :=
  if G1.edges.card != G2.edges.card then false
    ...
    let perms := nonType2.permutations
    let edges := allEdges n
    perms.any fun p =>
      let fullMap := buildFullMap n k G1.type_embed G2.type_embed nonType1 p
      edges.all fun e =>
        decide (e (*@$\in$@*) G1.edges) ==
        decide (applyPermEdge fullMap e (*@$\in$@*) G2.edges)

theorem isIsoFast_bool_true_correct
    {k n : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k}
    {G1 G2 : Sym2LabeledGraph (*@$\sigma$@*) n}
    (h : isIsoFast_bool G1 G2 = true) :
    G1 (*@$\sim_{\mathsf{sf}}$@*) G2 := ...

theorem isIsoFast_bool_false_correct
    {k n : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k}
    {G1 G2 : Sym2LabeledGraph (*@$\sigma$@*) n}
    (h : isIsoFast_bool G1 G2 = false) :
    (*@$\neg$@*) (G1 (*@$\sim_{\mathsf{sf}}$@*) G2) := ...

instance (priority := high) fastDecidableSym2LabeledGraphEqv
    {k n : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k}
    (G1 G2 : Sym2LabeledGraph (*@$\sigma$@*) n) :
    Decidable (G1 (*@$\sim_{\mathsf{sf}}$@*) G2) :=
...
\end{lstlisting}
\lean{isIsoFast\_bool} is the executable Boolean procedure for testing
isomorphism.  It first rejects graphs with different edge counts.  If the edge counts agree, the type embeddings
determine where the labeled vertices must go, so the algorithm only searches
over permutations of the remaining vertices and checks edge membership in the
two explicit edge sets.

The \lean{Decidable} instance packages this Boolean algorithm as a
proof-producing decision procedure for the predicate
\(G_1 \sim_{\mathsf{sf}} G_2\), meaning that the two \lean{Sym2LabeledGraph}s are
isomorphic.  In Lean, a \lean{Decidable P} value is either \lean{isTrue h},
carrying a proof \lean{h : P}, or \lean{isFalse h}, carrying a proof of
\(\neg P\).  The two correctness theorems above turn the Boolean outcomes of
\lean{isIsoFast\_bool} into exactly these proofs.  With this instance available,
the evaluator can use \lean{isIsoFast\_bool} as a trusted
isomorphism-checking algorithm when filtering the \lean{Fintype} enumeration.

\subsection{Adequacy Theorems for Reflection}
\label{sec:reflection-adequacy}

\Cref{sec:flagtypes} defined flags by quotienting labeled graphs by flag isomorphism.
The reflection layer repeats the same construction for the concrete graph
representation from \Cref{sec:reflection-concrete}.  A \lean{Sym2Flag} is the quotient of
\lean{Sym2LabeledGraph}s by the isomorphism relation
\(\sim_{\mathsf{sf}}\), and \lean{Sym2Flag.toFlag} decodes the resulting
concrete flag back to the abstract \lean{Flag} used in the specification layer:
\begin{lstlisting}
def Sym2Flag {k : (*@$\mathbb{N}$@*)}
    ((*@$\sigma$@*) : Sym2FlagType k) (n : (*@$\mathbb{N}$@*)) : Type :=
  Quotient (sym2LabeledGraphSetoid (*@$\sigma$@*) n)

def Sym2Flag.toFlag
    {k : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k}
    {n : (*@$\mathbb{N}$@*)} (G : Sym2Flag (*@$\sigma$@*) n) :
...
The density functions follow the same pattern for the constructions of
\Cref{sec:formal-subflag-densities}.  The functions \(\mathtt{flagDensity}_1\) and
\(\mathtt{flagDensity}_2\) are the specification-level densities.  Their
reflected counterparts, \(\mathtt{sym2FlagDensity}_1\) and
\(\mathtt{sym2FlagDensity}_2\), are built by running the finite search from
\Cref{sec:reflection-concrete} over concrete induced subflags, counting the
isomorphic placements, and then lifting the result through the
\lean{Sym2Flag} quotients:
\begin{lstlisting}
def sym2FlagDensity(*@$_1$@*)
    {k : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k} {m n : (*@$\mathbb{N}$@*)}
    (F : Sym2Flag (*@$\sigma$@*) m) (G : Sym2Flag (*@$\sigma$@*) n) : (*@$\mathbb{Q}$@*) := ...

def sym2FlagDensity(*@$_2$@*)
    {k : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k} {m(*@$_0$@*) m(*@$_1$@*) n : (*@$\mathbb{N}$@*)}
    (F(*@$_0$@*) : Sym2Flag (*@$\sigma$@*) m(*@$_0$@*)) (F(*@$_1$@*) : Sym2Flag (*@$\sigma$@*) m(*@$_1$@*)) (G : Sym2Flag (*@$\sigma$@*) n) : (*@$\mathbb{Q}$@*) := ...
\end{lstlisting}
Unlike the specification-level density functions, these definitions do not
...
density functions from the specification layer.  We therefore prove
\emph{adequacy theorems}: theorems stating that the reflected computation agrees
with the abstract definition after decoding \lean{Sym2Flag}s back to ordinary
\lean{Flag}s. 
\begin{lstlisting}
theorem flagDensity(*@$_1$@*)_eq_sym2FlagDensity(*@$_1$@*)
    {k : (*@$\mathbb{N}$@*)} {(*@$\sigma$@*) : Sym2FlagType k} {m n : (*@$\mathbb{N}$@*)}
