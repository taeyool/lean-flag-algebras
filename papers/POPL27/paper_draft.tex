...
to obtain a function on \lean{FlagAlgebra} $\sigma$. \gw{I'm here.}
\begin{lstlisting}
...
noncomputable def unlabel : Flag (*@$\sigma$@*) V (*@$\to$@*) Flag emptyType V :=
  Quotient.lift
    (fun G : LabeledGraph (*@$\sigma$@*) V => unlabeledGraphQuot G)
    (fun _ _ h => unlabeledGraphQuot_respect_eqv h)
\end{lstlisting}
These lifts let us define the downward image of a single flag:
\begin{lstlisting}
...
The main new obligation appears when this single-flag operation is lifted to
\lean{FlagAlgebra}.  Since \lean{FlagAlgebra} $\sigma$ is a quotient of
\lean{FlagVector} $\sigma$ by the zero-space relations, the linear extension of
\lean{downwardFlag} must be shown to send every typed zero-space relation to an
empty-typed zero-space relation.
