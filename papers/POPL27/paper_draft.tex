...
well-definedness on quotient classes, transports between size-indexed
representations, and \lean{Fintype}-instance ambiguity, together with the
...
Lean packages this step as \lean{Quotient.lift}.  In the opposite direction,
to prove that two representatives determine the same flag, one supplies their
isomorphism to \lean{Quotient.sound}.  These two Lean operations are short;
the real work is the accompanying proof that the representative-level
construction is invariant under isomorphism.

This pattern occurs when lifting densities and graph operators to flags and
again at the reflection boundary.  For example, decoding an executable
edge-set representation must yield the same abstract flag for every
isomorphic concrete encoding.  Rather than expose the two representations in
...
\subsection{Relating Size-Indexed Representations}

Flag algebra operations change the vertex size used to represent their output:
the product of basis elements represented by a size-$m_1$ and a size-$m_2$
flag is expanded at a size $\ell \geq m_1 + m_2 - k$.  In the formalization,
this product is computed as a sum over
