...
Finally, we give a clean counterexample showing that heredity alone is not enough:
the hereditary class of $C_4$-free graphs is not root-plantable.  For the one-vertex
type, the element $-e$, where $e$ is the rooted edge flag, is non-negative under
every ensemble extension yet negative on the centre-rooted star homomorphism in the
quotient $C_4$-free theory.
\end{abstract}
