...
\[
  \qmap_\sigma:\A^\sigma[\T_0]\twoheadrightarrow\A^\sigma[\T_1]
\]
pulls every $\psi\in\Homp(\A^\sigma[\T_1],\R)$ back to a positive homomorphism on
$\A^\sigma[\T_0]$: as a surjection of algebras $\qmap_\sigma$ sends each $\T_0$-flag to a
$\T_1$-flag or to $0$, so $\psi\circ\qmap_\sigma$ is an algebra homomorphism that is
non-negative on every flag, hence a point of $X_\sigma$.  We write
\[
  Q_\sigma
  :=
  ...
  \subseteq X_\sigma.
\]
Thus $Q_\sigma$ is the constrained, or $\T_1$-supported, part of the ambient
homomorphism space.  It is closed in $X_\sigma$: it is cut out by the conditions
that all forbidden finite $\sigma$-flags have value zero.

Equivalently, $\chi\in X_\sigma$ lies in $Q_\sigma$ iff it vanishes on every
$\sigma$-flag that belongs to $\T_0$ but not to $\T_1$.  This uses that $\T_1$ is
hereditary: a $\sigma$-flag is forbidden in $\A^\sigma[\T_1]$ exactly when its underlying
