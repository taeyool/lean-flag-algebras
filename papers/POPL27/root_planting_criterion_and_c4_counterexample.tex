...
decisive, and the genuine obstruction is that the rooted edge density is pinned to a
boundary value across all constrained limits.
\end{abstract}
...
\begin{enumerate}[label=\textup{(\arabic*)}, leftmargin=2.8em]
  ...
  \emph{dense} co-degenerate classes, so density is not the dividing line.  The
  governing invariant is boundary pinning of the rooted edge density.  The remaining
  problem is to characterise root-plantability exactly: is the absence of boundary
  pinning sufficient?  The dense, non-clone-closed, non-pinned class of $C_5$-free
  graphs is the natural first test.
\end{enumerate}
