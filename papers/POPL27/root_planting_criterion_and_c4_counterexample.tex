...
independent blow-ups (which holds for the clique-free theories, including triangle-free) implies root-plantability and ensures that these two notions of non-negativity coincide. But heredity alone does not lead to root-plantability.  Every class so
...
limits while the quotient theory admits a labelled limit where it takes another.  We conjecture
...
our sufficient condition and our obstruction, and since they exhibit no pinning
obstruction from the edge or triangle the conjecture predicts them
root-plantable---which we are not yet able to prove.
...
correspondence for the clique-free theories, including triangle-free.  Heredity alone, however,
...
sufficient condition and our obstruction, and carrying no pinning obstruction from the
edge or triangle they are predicted root-plantable, though we cannot yet prove it.  This incompleteness, moreover, never
...
structure---for graphs, a choice of $k$ distinguished \emph{labelled} vertices (its
\emph{roots}) and the
adjacencies among them---and a \emph{$\sigma$-flag} is a finite structure carrying a
marked copy of $\sigma$, that is, a structure labelled by $\sigma$; the \emph{empty type}
...
of $M$.  Write $\one_\sigma\in\A^\sigma[\T]$ for the flag consisting of the type alone
...
\]
pulls every $\psi\in\Homp(\A^\tau[\T_1],\R)$ back to a positive homomorphism on
$\A^\tau[\T_0]$: as a surjection of algebras $\qmap_\tau$ sends each $\T_0$-flag to a
$\T_1$-flag or to $0$, so $\psi\circ\qmap_\tau$ is an algebra homomorphism that is
non-negative on every flag, hence a point of $X_\tau$.  We write
\[
  Q_\tau
  :=
  \{\psi\circ\qmap_\tau:
...
labelled limit in the quotient of positive labelled edge density; it may be replaced by the existence
of any sequence of labelled graphs in $\K$ whose labelled edge density stays bounded
away from $0$.
\end{remark}
...
quotient theory still contains a labelled limit attaining the opposite boundary.  The
...
$\Ext_\sigma(\phi_0)$.  If the quotient theory contains a labelled limit attaining a different
...
\begin{proof}
The empty type admits a unique embedding into any structure, so the random labelling carries
...
whose constrained labelled limits are dense and locally bipartite rather than collapsed to a
...
\cref{thm:clone-root-plantable} every $C_5$-free labelled limit realised by
triangle-free graphs lies in $S_1$; and by \cref{lem:c5-nbhd} the
neighbourhood of a positive-degree vertex is asymptotically independent (its induced
edges have vanishing density), so dense $C_5$-free labelled limits are locally bipartite.
