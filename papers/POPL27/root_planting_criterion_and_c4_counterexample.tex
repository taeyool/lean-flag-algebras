...
stars---for example $C_4$-free, $K_{s,t}$-free, even-cycle-free, and planar
...
$\sigma=1$, witnessed by $f:=e-1$.  In particular the complements of the classes in
...
$-e$ for $\overline\K$ pulls back to $-(1-e)=e-1$ for $\K$.  Directly:
$\avg{e}{1}=\rho$ gives $\E[\phi^1(e)]=\phi_0(\rho)=1$, and $\phi^1(e)\le1$ forces
$\phi^1(e)=1$ almost surely, so $e-1$ satisfies ensemble semantics; while the
co-star rooted at its isolated vertex gives a $\psi\in Q_1$ with $\psi(e)=0$, whence
$\psi(e-1)=-1<0$ and quotient semantics fails.
\end{proof}
...
Both obstructions are instances of one mechanism: a rooted density pinned to a
boundary of its range.

\begin{theorem}[Pinning obstruction]\label{thm:pinning}
Let $\sigma$ be a non-degenerate type and let $g$ be a $\sigma$-flag, so that
$0\le\chi(g)\le1$ for every $\chi\in X_\sigma$.  Suppose the typical density of $g$
is pinned to an endpoint of $[0,1]$ across all constrained unlabelled limits:
\begin{itemize}[leftmargin=2.8em]
  \item[\textup{(0)}] $\phi_0(\avg{g}{\sigma})=0$ for every $\phi_0\in Q_0$ with
        $\phi_0(\brak\sigma)>0$; or
  \item[\textup{(1)}] $\phi_0(\avg{g}{\sigma})=\phi_0(\brak\sigma)$ for every such
        $\phi_0$.
\end{itemize}
If moreover the quotient theory contains a root attaining a different value---some
$\psi\in Q_\sigma$ with $\psi(g)>0$ in case \textup{(0)}, or $\psi(g)<1$ in case
\textup{(1)}---then $(\T_0,\T_1,\sigma)$ is not root-plantable, witnessed by $-g$,
respectively $g-1_\sigma$.
\end{theorem}

\begin{proof}
In case \textup{(0)}, \eqref{eq:extension-expectation} gives
$\E_{\phi^\sigma\sim\Ext_\sigma(\phi_0)}[\phi^\sigma(g)]
   =\phi_0(\avg{g}{\sigma})/\phi_0(\brak\sigma)=0$ for every admissible $\phi_0$;
as $\phi^\sigma(g)\ge0$ pointwise, $\phi^\sigma(g)=0$ almost surely, so $-g$ is
ensemble-nonnegative and, by Lemma~\ref{lem:support-as}, vanishes on $S_\sigma$.
The root $\psi$ gives $\psi(-g)<0$, so quotient semantics fails.  Case \textup{(1)}
is identical using $\phi^\sigma(g)\le1$ and the witness $g-1_\sigma$; equivalently it
is case \textup{(0)} for $\overline\K$ via Lemma~\ref{lem:complementation}.
\end{proof}

\begin{remark}
The degeneracy obstruction (Theorem~\ref{thm:degenerate-obstruction}) and its dense
counterpart (Corollary~\ref{cor:codegenerate}) are the instances $\sigma=1$, $g=e$,
pinned at $0$ and at $1$.  Larger types and higher flags can in principle pin other
rooted densities, giving further obstructions.
\end{remark}

\begin{conjecture}[Characterisation]\label{conj:characterisation}
$(\T_{\mathrm{Graph}},\T_\K,\sigma)$ is root-plantable if and only if it admits no
pinning obstruction: for every $\sigma$-flag $g$ whose typical density is pinned to
an endpoint of $[0,1]$ across all constrained unlabelled limits, the quotient theory
attains only that endpoint.
\end{conjecture}

The forward direction is Theorem~\ref{thm:pinning}; the content of the conjecture is
that the absence of pinning is sufficient.
...
triangle-free graphs is root-plantable; and by Lemma~\ref{lem:c5-nbhd} the
