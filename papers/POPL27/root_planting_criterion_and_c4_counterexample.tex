...
\]
that identify each flag with the average of its one-vertex extensions; these relations are
what make ``density'' a well-defined linear functional on the whole space.  The space
...
\paragraph{Positive homomorphisms and limit objects.}
...
$\Homp(\A^\sigma[\T],\R)$.  By Razborov's theory these are exactly the \emph{limit
objects}: a sequence of finite $\sigma$-flags of growing size \emph{converges} when every
density $p(F,\cdot)$ converges, its limit is the homomorphism $\phi$ with
$\phi(F)=\lim p(F,\cdot)$, and conversely every positive homomorphism arises in this way
\cite[Theorem 3.3]{Razborov2007}.  We may therefore picture a positive homomorphism
$\phi$ as a limit object and $\phi(F)$ as the density of the flag $F$ in it.  Since each
density lies in $[0,1]$, evaluation on all flags embeds $\Homp(\A^\sigma[\T],\R)$ as a
closed subspace of the compact metrizable cube $[0,1]^{\F^\sigma[\T]}$; in particular it
is compact.
...
Let $\T_1$ be a hereditary subclass of $\T_0$.  For a non-degenerate type $\tau$ of
$\T_1$, define
\[
...
$\tau$-flag that belongs to $\T_0$ but not to $\T_1$.  This uses that $\T_1$ is
hereditary: a $\tau$-flag is forbidden in $\A^\tau[\T_1]$ exactly when its underlying
graph is one of the forbidden graphs that define the constrained class.
...
\begin{proof}
Let $F$ be a $\sigma$-flag of $\T_0$ whose underlying unlabelled graph $M$ does not
belong to $\T_1$; these are exactly the forbidden $\sigma$-flags.  The downward average
$\avg{F}{\sigma}$ is a
...
neighbourhood; yet the constrained class may still retain an \emph{exceptional}
...
The $C_5$-free class is root-plantable at every non-degenerate type; in particular
