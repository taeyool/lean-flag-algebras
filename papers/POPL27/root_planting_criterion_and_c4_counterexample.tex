...
$\A^\sigma[\T_0]$.  Indeed, as a surjection of algebras $\qmap_\sigma$ sends each
$\T_0$-flag to a $\T_1$-flag or to $0$, so $\psi\circ\qmap_\sigma$ is an algebra
homomorphism, non-negative on every flag, and hence a point of $X_\sigma$.  We write
\[
  Q_\sigma
  :=
...
homomorphism space.  Equivalently, $\chi\in X_\sigma$ lies in $Q_\sigma$ if and only if it vanishes
on every $\sigma$-flag that belongs to $\T_0$ but not to $\T_1$.  This uses that $\T_1$ is
hereditary: whether a $\sigma$-flag is forbidden in $\A^\sigma[\T_1]$ is determined by its
underlying unlabelled graph alone---the flag is forbidden exactly when that graph fails to
lie in $\T_1$.  Since each evaluation $\chi\mapsto\chi(F)$ is continuous, $Q_\sigma$ is an
intersection of closed sets---one for each forbidden flag---and hence closed in $X_\sigma$.
