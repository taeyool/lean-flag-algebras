...
supporting local structure.  Finally we show that this incompleteness is confined to
labelled types and never reaches a density bound: at the empty type the two semantics
coincide, and for the degenerate and co-degenerate classes the constrained roots
collapse to a single point, so the ensemble relaxation certifies exactly the same
density bounds as the quotient.  We further prove that pinning is to a boundary value at
the one-vertex and edge types, so every obstruction exhibited here is boundary; a
practically relevant gap would require an interior or non-pinning obstruction, of which
none is known.
\end{abstract}
...
below exhibit its \emph{incompleteness} for certain classes.  That incompleteness is,
however, confined to labelled types.  At the empty type---where every density bound an
application outputs actually lives---the two semantics provably coincide
(\cref{prop:empty-type}), and the witnesses that expose the gap unlabel to zero
(\cref{prop:ideal-zero,prop:single-point}); so the phenomenon concerns intermediate
rooted certificates, not the final density bounds one wants to prove
(\cref{sec:empty-type}).
...
\subsection{Pinning is to a boundary value at the smallest types}\label{subsec:boundary}
...
constancy as a genuine hypothesis.  At the smallest types we can show more: interior
pinning does not occur, so every pinning obstruction there is a boundary one.  This
matters because, as the next section shows, boundary obstructions are inert for density
bounds.

\begin{proposition}[No interior pinning at edgeless types]\label{prop:no-interior-edgeless}
Let $\K$ forbid a family of graphs as (not necessarily induced) subgraphs, and let
$\sigma$ be an edgeless type.  If a $\sigma$-flag $g$ is pinned to $c$ on $S_\sigma$, then
$c\in\{0,1\}$.
\end{proposition}

\begin{proof}
Every empty graph is $F$-free (each forbidden $F$ has an edge), so the zero graphon
$o$---their limit---lies in $Q_0$, and the edgeless type $\sigma$ embeds in it with
density $1$.  For any flag $h$ containing an edge,
$\E_{\Ext_\sigma(o)}[\phi^\sigma(h)]=\phi_o(\avg{h}{\sigma})/\phi_o(\brak\sigma)=0$, since
$o$ assigns density $0$ to every structure with an edge; as $\phi^\sigma(h)\ge0$ this
forces $\phi^\sigma(h)=0$ almost surely.  Hence $\Ext_\sigma(o)$ is the point mass at the
unique homomorphism $o^\sigma$ that vanishes on all edge-containing flags, and
$o^\sigma\in S_\sigma$.  Pinning then gives $c=\phi_{o^\sigma}(g)$, which is $1$ if $g$ is
the all-edgeless flag and $0$ otherwise.
\end{proof}

In particular this holds at the one-vertex type $\sigma=1$, where all the obstructions of
this paper live.

This generalises to every complete-multipartite type below the chromatic threshold of
$\K$.  The key is to root $\sigma$ in the \emph{minimal} multipartite family that
contains it, where each further vertex has its colour forced and the rooted density
becomes a single monomial in the part sizes.

\begin{theorem}[No interior pinning at complete-multipartite types]\label{thm:no-interior-cm}
Let $\sigma$ be a complete-multipartite type with $s:=\chi(\sigma)\ge2$, and suppose the
complete $s$-partite graphs lie in $\K$ (equivalently, no forbidden subgraph is
$s$-colourable; this holds whenever $s$ is below the least chromatic number of a forbidden
subgraph, e.g.\ $s=2$ exactly when $\K$ is non-degenerate).  Then no $\sigma$-flag is
pinned to an interior value: a pinned flag is pinned to $0$ or $1$.
\end{theorem}

\begin{proof}
For $\alpha=(\alpha_1,\dots,\alpha_s)$ in the open simplex, let $\phi_0^\alpha\in Q_0$ be
the limit of complete $s$-partite graphs with parts of relative sizes $\alpha_j$.  As
$\sigma$ is complete $s$-partite, it embeds in $\phi_0^\alpha$ with its $s$ parts filling
the $s$ colours; fixing the embedding that sends the $i$-th part of $\sigma$ to colour
$i$ gives a rooted limit $\psi_\alpha\in S_\sigma$.  In $\psi_\alpha$ a further
independent vertex takes colour $c$ with probability $\alpha_c$ and is adjacent to every
part of $\sigma$ except the one of colour $c$, while two further vertices are adjacent iff
their colours differ.

Let $g$ be a $\sigma$-flag with further vertices $v_1,\dots,v_m$.  Because $\sigma$
already occupies all $s$ colours, a further vertex adjacent to all of $\sigma$ cannot be
placed, and one non-adjacent to two parts cannot either; so each $v_t$ is non-adjacent to
exactly one part of $\sigma$, which forces its colour (otherwise $\psi_\alpha(g)=0$).
When these forced colours are consistent with the adjacencies among the $v_t$,
\[
  \psi_\alpha(g)=\prod_{j=1}^{s}\alpha_j^{\ell_j},
  \qquad
  \ell_j=\#\{t:v_t\text{ has colour }j\}.
\]
A flag pinned across $S_\sigma$ is constant on $\{\psi_\alpha\}$ over the open simplex.  A
monomial $\prod_j\alpha_j^{\ell_j}$ is constant there only when every $\ell_j=0$, i.e.\
$g=\sigma$ (value $1$); any flag with a further vertex gives a non-constant monomial and
so is not pinned, while a non-realizable flag is pinned to $0$.  No interior value occurs.
\end{proof}

The edge type is the case $s=2$: here $\K$ contains all complete bipartite graphs
(equivalently, by Erd\H{o}s--Stone--Simonovits, no forbidden subgraph is bipartite), and
$\psi_\alpha(g)=\alpha^{a}(1-\alpha)^{b}$.  Together with
\cref{prop:no-interior-edgeless} (the case $s=1$), this covers the one-vertex type, all
non-edges and edges, triangles and larger cliques, and every complete-multipartite type
below the chromatic threshold.

\begin{remark}[The remaining types]
\Cref{thm:no-interior-cm} roots $\sigma$ in the complete $\chi(\sigma)$-partite family, so
it reaches exactly the complete-multipartite types below the chromatic threshold.  Two
kinds of type remain.  Complete-multipartite types with $\chi(\sigma)$ at or above that
threshold---such as the triangle type for a theory of chromatic threshold three---cannot
be rooted in any multipartite family inside $\K$.  More fundamentally, a graph embeds as
an \emph{induced} subgraph of a complete multipartite graph iff it is itself complete
multipartite (induced non-adjacency must be transitive), so types that are not complete
multipartite---the induced $P_4$ type, for instance---are not rooted by this family at
all, and a different boundary-forcing family in $S_\sigma$ would be needed.  We
nonetheless conjecture that interior pinning is impossible at every type, so that every
pinning obstruction is to a boundary value.  Under \cref{conj:characterisation} this would
make every obstruction boundary pinning, and hence---by the inertness of boundary
obstructions established next (\cref{sec:empty-type})---confine any practically relevant
gap to a genuinely non-pinning obstruction, of which none is known.
\end{remark}
...
\begin{proof}
The empty type admits a unique embedding into any structure, so the random root carries
...
\]
for the empty-type contributions level $\sigma$ can make to the two methods, so that
...
$\avg{a}{\sigma}$ is a combination of unlabelled structures each containing a copy of
...
edge.  A random $k$-subset of an $n$-vertex graph spans, in expectation, $\binom k2$
times the edge density many edges, so the induced density obeys
...
homomorphism $o\in X_1$ does so (the rooted empty-graph limit, the others being
...
for the exhibited classes: a degenerate root carries no information, so its only
...
\cref{prop:no-interior-edgeless,thm:no-interior-cm} the pinning at the one-vertex and
edge types---hence at every obstruction exhibited here---is boundary.  \Cref{conj:characterisation}
predicts that the only obstructions are pinning; if moreover pinning is always boundary
(as \cref{subsec:boundary} conjectures) and the boundary support sets remain
...
whose constrained roots are dense and locally bipartite rather than collapsed to a
point, is the natural place where a richer $S_\sigma$, and with it a first practically
relevant gap, could appear.
\end{remark}
...
\begin{enumerate}[label=\textup{(\arabic*)}, leftmargin=2.8em]
  ...
  roots.  This question also carries the practical stakes of the whole theory: by
  \cref{sec:empty-type} the pinning obstructions are inert for density bounds, so only
  a non-pinning obstruction---a support set rich enough to carry a non-negative element
  that is no restricted sum of squares, for which the dense $C_5$-free roots are the
  natural candidate---could let the quotient/ensemble gap affect a bound one actually
  wants to prove.
\end{enumerate}
