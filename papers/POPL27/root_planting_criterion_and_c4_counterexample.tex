...
\begin{remark}[Degeneracy is essential, and marks the frontier]
Degeneracy is the crux, not heredity, and not the absence of clone-closure.  Odd
girth constraints are \emph{not} covered: for $k\ge2$ the class of
$C_{2k+1}$-subgraph-free graphs is dense---it contains all bipartite graphs, of edge
density up to $1/2$---so the argument above does not apply.  The $C_5$-free graphs
are a natural test case: the class is dense yet not clone-closed (the blow-up of a
triangle into the octahedron $K_{2,2,2}$ contains a $C_5$), and whether it is
root-plantable is open.  This is precisely the gap between
Theorem~\ref{thm:clone-root-plantable} (clone-closed $\Rightarrow$ root-plantable)
and Theorem~\ref{thm:degenerate-obstruction} (degenerate $\Rightarrow$ not
root-plantable).
\end{remark}
...
\begin{enumerate}[label=\textup{(\arabic*)}, leftmargin=2.8em]
  ...
  $K_{s,t}$-free, even-cycle-free, planar, and bounded-degeneracy classes.  These
  examples are strong because the classes are already hereditary and universal;
  non-hereditary constraints are not needed to witness failure.  The complementary
  question is to chart the frontier between degeneracy (failure) and clone-closure
  (success); the dense, non-clone-closed class of $C_5$-free graphs is the natural
  first test.
\end{enumerate}
