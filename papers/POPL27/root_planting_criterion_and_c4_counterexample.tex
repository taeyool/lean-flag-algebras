...
ways: in the \emph{quotient} flag algebra of the constrained theory, the algebra one
actually computes in, or by \emph{ensemble} semantics in the ambient theory, which
asks that an inequality hold almost surely along the random labelled extensions of
constrained limits and is what a constrained density bound asserts.  Quotient
...
condition holds that we call \emph{root-plantability}: every constrained labelled limit
must be approximable by rooted views of constrained unlabelled limits.  Closure under
independent blow-ups implies root-plantability and recovers the correspondence for
clique-free and triangle-free theories; but heredity alone does not.  Every class so
sparse that its $n$-vertex graphs have only $o(n^2)$ edges, yet that still contains
arbitrarily large stars---the $C_4$-free, $K_{s,t}$-free, even-cycle-free, and planar
graphs among them---fails at the one-vertex type; by complementation so do their dense
complements, so neither sparsity nor density is the dividing line.  The uniform cause is
\emph{pinning}: a rooted quantity that takes one fixed value across all constrained
limits while the quotient theory admits a root where it takes another.  We conjecture
that the absence of pinning is exactly what makes a class root-plantable.  The $C_5$-free
graphs are the critical test case: dense yet not closed under blow-ups, they escape both
our sufficient condition and our obstruction, and since they exhibit no pinning the
conjecture predicts them root-plantable---which we are not yet able to prove.

This incompleteness, however, never affects a density bound of the kind a flag-algebra
computation is run to produce.  Such a bound is an inequality about whole graphs, whereas
the gap between the two semantics lives entirely in the auxiliary inequalities---about
graphs rooted at chosen vertices---that a proof passes through on the way to it.  We show
that on whole-graph statements the two semantics always coincide, that the rooted
examples witnessing the gap vanish once averaged back into whole-graph statements, and
...
the constrained class.  This paper determines exactly when they agree.
...
approximable by positive-probability rooted views of constrained unlabelled limits.  We
call this condition \emph{root-plantability} and prove that it is exactly equivalent to
the quotient/ensemble correspondence for every flag-algebra element
(\cref{thm:support-criterion}).

\paragraph{Results.}
Root-plantability is implied by closure under independent blow-ups, which recovers the
correspondence for clique-free and triangle-free theories.  Heredity alone, however,
...
$K_{s,t}$-free, even-cycle-free, and planar graphs among them) fails at the one-vertex
type, witnessed by the rooted edge flag, and by complementation so do their dense
complements; neither sparsity nor density is the dividing line.  The uniform cause is
\emph{pinning}: a rooted quantity that takes one fixed value across all constrained
limits while the quotient theory admits a root where it takes another.  We conjecture
that the absence of pinning characterises root-plantability.  The $C_5$-free graphs are
the natural test case: dense yet not closed under blow-ups, they fall between our
sufficient condition and our obstruction, and being free of pinning they are predicted
root-plantable, though we cannot yet prove it.  This incompleteness, moreover, never
reaches a density bound.  A density bound is a statement about whole graphs---the
\emph{empty type}---where the two semantics provably coincide (\cref{prop:empty-type});
the rooted examples that witness the gap vanish when averaged back to whole graphs
(\cref{prop:ideal-zero}); and the gap can open only at a degenerate extreme, since
pinning is always to a boundary value (\cref{thm:no-interior}).  The incompleteness is
thus confined to the intermediate, rooted certificates a proof passes through; for the
obstructions we exhibit it never affects a density bound (\cref{sec:empty-type}), and
whether some constrained class with a sufficiently rich rooted support makes it do so is
left open.

\section{Flag-algebra background}

We use Razborov's notation and terminology \cite{Razborov2007}, recalled here in
graph-theoretic terms.  A \emph{type} $\sigma$ is a fixed finite labelled
structure---for graphs, a choice of $k$ distinguished \emph{root} vertices and the
adjacencies among them---and a \emph{$\sigma$-flag} is a finite structure carrying a
marked copy of $\sigma$, that is, a structure rooted at $\sigma$; the \emph{empty type}
($k=0$) marks nothing.  For a universal theory $\T$ and a type $\sigma$, $\A^\sigma[\T]$
denotes the flag algebra of $\sigma$-flags.  The compact space of positive homomorphisms
is
\[
  \Homp(\A^\sigma[\T],\R).
\]
...
homomorphism arises in this way \cite[Theorem 3.3]{Razborov2007}.  We may therefore
picture a positive homomorphism $\phi$ as a \emph{limit object} and $\phi(F)$ as the
density of the flag $F$ in it.  We write
...
\end{equation}
Intuitively, $\Ext_\sigma(\phi_0)$ is the law of the rooted limit seen from a uniformly
random copy of $\sigma$ inside $\phi_0$; \eqref{eq:extension-expectation} records that
the expected rooted density of $g$ is the unlabelled density of $g$, normalised by that
of $\sigma$.  Moreover, if a finite sequence converges to $\phi_0$, then the
corresponding finite random-root distributions converge weakly to $\Ext_\sigma(\phi_0)$
...
Informally, the next lemma says that randomly rooting a constrained unlabelled limit
...
positive density, so a configuration the quotient exhibits only at an exceptional root
can instead be realised at a random---hence typical---root, which is just what
root-plantability requires.  Let
...
sufficient, the frontier left open between \cref{thm:clone-root-plantable}
...
genuine hypothesis---but in fact never arises, as \cref{subsec:boundary} shows all
pinning is to a boundary value.
\end{remark}
...
constancy as a genuine hypothesis.  We now show that for a subgraph-forbidden theory no
...
realises $\sigma$; so $\phi_0^{\lambda}$ roots $\sigma$.
...
the root, or that two further vertices are adjacent, is at most $\lambda$.  Hence as
$\lambda\to0$ the rooted density of a $\sigma$-flag $g$ tends to $1$ when $g$ extends
...
concentrates at the rooted limit $o_\sigma$---the type $\sigma$ together with an isolated
...
\cref{conj:characterisation} predicts that the $C_5$-free graphs---dense,
...
\cref{thm:clone-root-plantable} every $C_5$-free rooted limit realised by
triangle-free graphs lies in $S_1$; and by \cref{lem:c5-nbhd} the
...
\cref{conj:characterisation} holds for it.
\end{conjecture}

\section{Strengthening by a further theory \texorpdfstring{$\T'$}{T'}}

The framework is not tied to a single forbidden-subgraph constraint.  It applies
unchanged when the constraint is a conjunction $\T\wedge\T'$ of two theories, provided
the conjunction is again universal.

\subsection{When \texorpdfstring{$\T'$}{T'} is universal}

If $\T'$ is universal in the same relational language as $\T$, then $\T_1:=\T\wedge\T'$
is universal, the quotient flag algebra $\A^\sigma[\T_1]$ is well-defined, and the
support-closure criterion (\cref{thm:support-criterion}) applies verbatim.  The
equivalence of the two semantics is thus governed by root-plantability and by nothing
weaker: it does \emph{not} hold for all universal strengthenings, as the degeneracy
...
substitutions, true- or false-twin duplications, and model-theoretic amalgamation
properties.
...
algebra} than as quotient semantics of a new universal theory, and lies outside the
...
\begin{enumerate}[label=\textup{(\arabic*)}, leftmargin=2.8em]
  ...
  or local replacements, as long as the labelled finite model can be amplified into
  ...
  already sufficient for root-plantability.  The $C_5$-free graphs (\cref{conj:c5}) are
  the natural first test: they carry no pinning obstruction from the edge or triangle
  (\cref{cor:c5-no-pin}), and by \cref{lem:c5-nbhd,lem:c5-blowup} the question reduces to
  planting dense, locally bipartite roots.
  ...
  rooted support is rich enough to carry a non-negative element that is no restricted sum
  of squares.  None is known; the dense $C_5$-free roots, which do not collapse to a
