...
clique-free and triangle-free theories, but heredity alone does not.  Every
edge-degenerate class with arbitrarily large stars---$C_4$-free, $K_{s,t}$-free,
even-cycle-free, planar---fails at the one-vertex type, and by complementation so do
their dense complements, so neither sparsity nor density is the dividing line.  The
uniform cause is \emph{pinning}: a rooted density forced to a constant value across all
constrained limits while the quotient still attains a different value.  We conjecture
that the absence of pinning characterises root-plantability, the $C_5$-free graphs being
the critical open case.

This incompleteness is nevertheless invisible to the density bounds that a flag-algebra
computation actually outputs.  Such bounds are statements of the empty type, where the
two semantics provably coincide; the pinning witnesses unlabel to the zero element; and
pinning, we prove, is always to a boundary value.  The gap is therefore confined to the
intermediate labelled certificates: for the obstructions we exhibit it never reaches a
density bound, and whether any constrained class makes it do so is left open.
\end{abstract}
...
does not imply it: every edge-degenerate class containing arbitrarily large
stars---$C_4$-free, $K_{s,t}$-free, even-cycle-free, planar---fails at the one-vertex
...
\emph{pinning}: a rooted density forced to a constant value across all constrained
limits while the quotient theory still attains a different one.  We conjecture that the
absence of pinning characterises root-plantability, with the $C_5$-free graphs as the
critical open case.  Yet this incompleteness never reaches a density bound.  Density
bounds are statements of the empty type, where the two semantics provably coincide
(\cref{prop:empty-type}); the pinning witnesses unlabel to the zero element
(\cref{prop:ideal-zero}); and pinning is always to a boundary value
(\cref{thm:no-interior}).  The gap is therefore confined to the intermediate, labelled
certificates; for the obstructions we exhibit it never affects a density bound
(\cref{sec:empty-type}), and whether some constrained class with a sufficiently rich
rooted support makes it do so is left open.
