...
scaffolding script that turns it into a Lean source file, are both
unverified.  Neither is trusted for the logical validity of a theorem
accepted by Lean.  The certificate supplies flags, matrices,
factorizations, and a target inequality; Lean recomputes and checks the
resulting PSD obligations, density and multiplication identities, algebraic
calculation, and final statement.  Candidate data that do not support that
statement cause proof construction to fail.
...
quadratic form.  The residual audit obligation is anchored at the statement
rather than at the certificate: Lean proves exactly the theorem written in
the source file, so mistranslated certificate data can make a proof fail
but cannot change what was proved (\Cref{sec:flagmatic-trust}).  The lesson
is to separate these questions: external tools can propose witnesses and
proof structure without becoming authorities for logical validity, provided
the stated goal remains a small, human-readable object of audit.
