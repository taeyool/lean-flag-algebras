...
Both orders ask that the same continuous function
\(\chi\mapsto\chi(g-f)\) be non-negative, and differ only in where it is
tested: almost-sure non-negativity under a measure is non-negativity on its
support, so the ensemble order tests \(S_\sigma\), while the built-in order
tests \(Q_\sigma\) by definition.  The implication is then exactly the
inclusion \(S_\sigma\subseteq Q_\sigma\) of
Equation~\eqref{eq:S-subset-Q}, and when \(S_\sigma=Q_\sigma\) the two tests
coincide, so the orders agree on every pair.  Conversely, if
\(S_\sigma\subsetneq Q_\sigma\), a point of \(Q_\sigma\setminus S_\sigma\)
can be separated from \(S_\sigma\) by a continuous function, and a
flag-algebra element approximating it is ensemble-non-negative but not
built-in non-negative, so some pair disagrees.
Thus, \Cref{thm:support-closure-summary} is a completeness theorem:
...
\lean{S$\sigma$\_subset\_Q$\sigma$}, formalizes the strict inclusion
\(S_{\sigma_1}\subsetneq Q_{\sigma_1}\) for the \(C_4\)-free class.
