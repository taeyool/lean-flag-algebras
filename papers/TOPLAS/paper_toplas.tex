...
Some of the best-known results of modern combinatorics were established by
proofs that lean on large external computations.  The four-color theorem was
settled by extensive machine case analysis and later replayed inside a proof
assistant~\cite{gonthier2008fourcolour}; the Kepler conjecture was resolved
by optimization-heavy computation whose formal verification took a decade of
further work~\cite{hales2017kepler}; the empty-hexagon number was determined
by a SAT solver and verified against the solver's clausal
proof~\cite{subercaseaux2024hexagon}.  These results share a shape: a search
tool produces a witness, such as a case analysis, an optimization bound, or
an unsatisfiability proof, and the theorem is only as trustworthy as
whatever checks that witness.
...
solvers emit certificates that are validated by formally verified
checkers~\cite{lammich2020unsat,tan2023cakelpr}; SMT proof witnesses are
reconstructed as proof terms inside a proof
assistant~\cite{armand2011modular}; numerical sums-of-squares certificates
for polynomial inequalities are rounded to rational form and reverified
exactly~\cite{harrison2007verifying,besson2006fast,morrison2026sos}; and
when a tool is too complex to verify, each of its runs can be validated
after the fact, as in translation
validation~\cite{pnueli1998translation}.  This article applies that playbook
to a proof method of extremal graph theory whose computer-generated
certificates have so far lived entirely outside proof assistants:
Razborov's flag algebra method~\cite{razborov2007flag}.
...
triangle-free graph, at most half of all vertex pairs are edges.  Such
statements are hard to prove because they quantify over \emph{all}
sufficiently large $\mathcal{H}$-free graphs: no finite collection of
...
a bound to checking finitely many algebraic conditions.  A
...
reconstructed as proof terms inside Rocq (previously called
