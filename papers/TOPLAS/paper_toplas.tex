...
density bound for large graphs into the task of finding matrices and numerical
weights that satisfy a finite system of algebraic identities and positivity
...
recorded in a certificate.  Independently checking the certificate against this
system establishes the original bound.  This approach is especially valuable
for flag algebras, which have been central to proofs of major results in
extremal graph theory, including the Erd\H{o}s pentagon theorem and the
...
Many central questions of extremal graph theory concern finite simple
graphs.\footnote{By a finite simple graph we mean an undirected graph
with a finite vertex set, no self-loops, and at most one edge between any two
distinct vertices.  Formally, it is a pair $G=(V,E)$, where $V$ is finite and
$E\subseteq\binom{V}{2}$; here $\binom{V}{2}$ denotes the set of two-element
subsets of $V$.  We write $V(G)$ and $E(G)$ for the vertex set $V$ and the edge
set $E$, respectively.}  Such questions ask how often one pattern can occur when
another is forbidden.  A basic example is Mantel's theorem~\cite{mantel}, which
...
\[
  ...
  = \frac{1}{2}.
\]
The limit on the left is the Tur\'an density of edges in triangle-free graphs.
The equality combines two bounds of different character.  The lower bound
comes from a concrete family known as the balanced complete bipartite graphs.
For present purposes, only the existence of this family matters, not its
construction.  Proving the matching upper bound is more difficult because it
must control all triangle-free graphs as their size grows, rather than exhibit
a single family.

Flag algebras address this difficulty through the finite algebraic reduction
outlined above.  They use small, partially labeled graphs, called
\emph{flags}, to encode the local configurations that enter the calculation.
A \emph{candidate certificate} selects finitely many flags, supplies matrices
whose entries are rational numbers, and gives rational-number coefficients that
weight the corresponding algebraic terms.  The certificate claims that these
data satisfy specified algebraic identities and that the matrices are positive
semidefinite.  The flag-algebra theory guarantees that checking these finite
claims establishes the desired asymptotic upper bound over the full class of
graphs.

The raw certificates, however, cannot themselves serve as trusted proof
artifacts.  A nontrivial certificate combines many flags, so validating it
requires large tables of exact counting results for finite graphs.  Each table
entry is a rational number recording the fraction of suitable vertex selections
in one small, partially labeled graph that induce another specified pattern.
The certificate behind the Erd\H{o}s pentagon theorem depends on thousands of
such entries, together with matrices that must be proved positive semidefinite
using exact rational arithmetic.  These obligations are far beyond what a human
referee can audit, and the candidate data come from a large, unverified numerical
toolchain.  A formal development should therefore check these obligations
automatically, without trusting the raw data or requiring thousands of
individually written proofs.
