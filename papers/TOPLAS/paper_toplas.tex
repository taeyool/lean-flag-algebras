...
for a certificate that witnesses them.  Certificates behind notable
theorems contain thousands of exact rational values and are produced by
large, numerical, unverified toolchains, so they can be neither audited by
hand nor sensibly trusted.  This article presents a certificate-to-proof
compiler for the method: a Lean~4 development that formalizes flag
algebras for finite simple graphs and compiles such untrusted certificates
into complete, machine-checked proofs.

The development has three layers: a specification layer that formalizes
the mathematics of the method, a reflection layer that makes the
specification computable and generates the thousands of verified
identities a certificate requires, and an automation layer that drives the
compilation.  Compilation happens at elaboration time, inside Lean: the
theorem statement is fixed in the source file, the certificate enters only
as untrusted hint data, and Lean recomputes every claimed fact against the
mathematical definitions.  We evaluate the compiler on seven Tur\'an-type
upper bounds and report a trust profile that states, for each class of
proof obligation, the mechanism by which it is discharged.  Combined with matching extremal constructions
proved directly in the formalization, the compiled bounds give, to our
knowledge, the first machine-checked proof of the Erd\H{o}s pentagon
theorem, whose known proofs all use flag algebras.

Our formalization also departs from Razborov's development in where the
forbidden-subgraph constraint enters: rather than building each
constraint into the algebra, it formalizes the algebra of unconstrained
graphs once and imposes the constraint only where an inequality is
interpreted.  This ensemble semantics keeps the constraint out of the
algebra's definitions and lemmas, so one formalization serves every
forbidden family.  We prove the departure sound, characterize exactly
when the two semantics agree, and autoformalize the resulting
meta-theory in a separate Lean library extending the main development.
\end{abstract}
...
This article presents a certificate-to-proof compiler for the flag
algebra method: a Lean~4~\cite{moura2021lean} development that
formalizes the method for finite simple graphs and compiles candidate
certificates into complete, machine-checked proofs, structured so that
certificate data never needs to be trusted.  The development has three
layers.
...
semantics through positive homomorphisms representing graph limits.  It
...
Razborov's development builds the constraint in from the start,
constructing the flag algebra from $H$-free graphs only; we call this
the \emph{built-in} (quotient) semantics.  Our implementation instead
retains the ambient algebra of all finite graphs and imposes
$H$-freeness only where an inequality is interpreted; we call this the
\emph{ensemble} semantics.  This keeps the constraint out of the
algebra's definitions and lemmas, so one formalization serves every
forbidden family.  The departure raises a soundness question: do the two
semantics validate the same inequalities?  \Cref{sec:metatheory} answers
it, comparing the two designs the way one compares two semantics of a
language.  Every inequality valid in the built-in semantics is valid in
the ensemble semantics; the converse holds at the empty flag type, can
fail at a nonempty flag type, and holds at a fixed flag type exactly
when the constrained class is \emph{root-plantable} there.  We prove
that blow-up closure guarantees root-plantability and give a $C_4$-free
counterexample showing that heredity alone does not.

\paragraph{Contributions.}
\begin{itemize}
\item \emph{A Lean~4 formalization of the flag algebra method} for
  finite simple graphs, and \emph{a certificate-to-proof compiler} built
  on it and invoked as an elaboration-time tactic, turning externally
  generated, untrusted certificates into complete, machine-checked
  proofs of Tur\'an-type bounds.
\item \emph{Machine-checked theorems}: seven compiled Tur\'an-type upper
  bounds, including those of Mantel's theorem and the Erd\H{o}s pentagon
  theorem, combined with matching lower-bound constructions proved
  directly in the formalization.  To our knowledge, this gives the first
  machine-checked proof of the Erd\H{o}s pentagon theorem, whose known
  proofs all use flag algebras.
\item \emph{A meta-theory of ensemble semantics}, autoformalized in a
  separate Lean library that extends the development above.
\end{itemize}
...
reflection layer: the executable graph representations, their adequacy
theorems, and the elaboration-time generation commands.
\Cref{sec:flagmatic} describes the automation layer and the compilation
it drives: the exact checks of positive semidefiniteness, the proof
automation, and the evaluation on seven certificates.
...
meta-theory of ensemble semantics together with its autoformalization.
...
those shared by a group of definitions into a \lean{variable} block, leaving
...
probabilistic sampling formulation of \Cref{sec:background-densities}, this direct ratio is easier to work with in Lean.
\begin{lstlisting}
...
information for every type \lean{Vl i}.  Lean normally supplies all of these
automatically through its typeclass inference mechanism.
...
\end{lstlisting}
Here \lean{labeledGraphListEqv} says that two concrete tuples are equivalent
exactly when their entries are isomorphic as labeled graphs, and
\lean{QuotLabeledGraphList} is the resulting quotient, formed from that
relation by the same setoid step as in \Cref{sec:flagtypes}.  The abbreviation
\lean{FlagList} is the more convenient tuple of already-quotiented
$\sigma$-flags.
...
\lean{labeledGraphListDensityLifted Hl} quotients the host argument, giving a
function \lean{Flag}~$\sigma$~\lean{W}~\(\to \mathbb{Q}\), and
\lean{quotLabeledGraphListDensity} lifts the pattern tuple as well.  In the
final definition, the input \lean{Fl} has type
\lean{FlagList}~$\sigma$~\lean{t Vl}, so \lean{Fl i} is already a
$\sigma$-flag for each \lean{i : Fin t}.  The term \lean{FlagList.coe Fl}
chooses a representative \lean{(Fl i).out} for each entry and forms the
quotient class of the concrete tuple, converting \lean{Fl} into the
quotient-of-tuples input that \lean{quotLabeledGraphListDensity} expects.
...
\Cref{sec:background-densities}, its formal proof must account for the quotient
representation:
...
instance flagVectorSetoid ((*@$\sigma$@*) : FlagType (Fin k)) : Setoid (FlagVector (*@$\sigma$@*)) :=
  ... -- flagVectorEqv, with the proof that it is an equivalence relation
...
\lean{bilinearExtension flagMul}.  The proof obligation is that this
multiplication respects \lean{flagVectorEqv}, and its load-bearing lemma is
that \lean{ZeroSpace} $\sigma$ is closed under multiplication by arbitrary
flag vectors:
\begin{lstlisting}
theorem flagVector_mul_zeroSpace (f : FlagVector (*@$\sigma$@*)) {z : FlagVector (*@$\sigma$@*)}
    (hz_zero : z (*@$\in$@*) ZeroSpace (*@$\sigma$@*)) : f * z (*@$\in$@*) ZeroSpace (*@$\sigma$@*) := ...
\end{lstlisting}
Together with commutativity, this says that \lean{ZeroSpace} $\sigma$ is an
ideal in the algebraic sense.  Hence, if two representatives differ by a
zero-space element, so do their products with any fixed vector: the product
does not depend on the chosen representatives, which is the compatibility
condition \lean{Quotient.map$_2$} requires.  A separate theorem handles the
choice of output size:
\begin{lstlisting}
...
assumptions on the two output sizes, where \(F_j.1\) is the total number of
vertices of \(F_j\) and \(k\) the number of labeled vertices in \(\sigma\):
each \(\ell_j\) must be at least \(F_1.1 + F_2.1 - k\), since the unlabeled
vertices outside the flag type are sampled disjointly in the two
$\sigma$-flags while the \(k\) labeled vertices are shared.  Any two admissible
sizes therefore give equivalent flag vectors, justifying the choice of the
minimal size in \lean{flagMul}.
...
large size: a \(\sigma\)-flag \(G\) is sampled with weight \(\phi([G])\).  These
weights are indeed probabilities, since \(\phi\) is non-negative on flag basis
elements and Equation~\eqref{eq:sum-to-one} gives
\(\sum_{G\in\mathcal{F}^{\sigma}_{\ell}} \phi([G])=\phi(\mathbf{1}_\sigma)=1\).
One then samples an entire sequence \(X=(X_0,X_1,\ldots)\), where \(X_n\) is
drawn from the distribution on \(\sigma\)-flags of size \(n^2+k\).  The key
estimate says that, for each fixed test \(\sigma\)-flag \(F\), the density
\(\den{F}{X_n}\) is very likely to be close to \(\phi([F])\) once \(n\) is
large, so almost every sampled sequence has the limiting density profile
prescribed by \(\phi\).  Selecting one such realization gives the deterministic
\(\sigma\)-flag sequence required by Theorem~\ref{thm:convergent-hom}(b).  The
Lean proof constructs the product measure on flag sequences, proves that the
required coordinatewise convergence event has probability one, and selects a
...
The other ingredient, $F|_\emptyset$, is computed by
\lean{unlabel\_labeledGraph}, which forgets the type embedding while retaining
the underlying graph.  Both ingredients are defined on concrete
$\sigma$-typed labeled graphs and then lifted to flags, by the same
\lean{Quotient.lift} pattern used for densities in
\Cref{sec:formal-subflag-densities}.
\begin{lstlisting}
...
\end{lstlisting}
Thus \lean{downwardFlag} forms the empty-type basis vector indexed by the pair
...
and, for each such \(\phi_0\) for which \(\sigma\) has positive density, we
require \(\phi(f)\leq\phi(g)\) with probability one when \(\phi\) is sampled
from the random-extension measure \(\Ext_\sigma(\phi_0)\) of
\Cref{sec:background-downward}.

The forbidden-graph condition therefore does not define a separate
flag-algebra universe in the implementation; it restricts the empty-type
positive homomorphisms from which those random extensions are formed.  The
elements \(f\) and \(g\), as well as the definitions of flags, densities,
products, the downward operator, and positive homomorphisms, remain those of
the ambient algebra and can therefore be reused across extremal problems.
...
\Cref{sec:reflection-adequacy} turn all these reflection-layer evaluations into
...
required a formalization of the method itself.  We first describe the
...
(\Cref{sec:reflection-generation}).
...
(\Cref{sec:flagmatic-trust}) contribute datapoints from computational mathematics, where the
