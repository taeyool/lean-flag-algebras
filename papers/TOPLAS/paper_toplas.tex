...
\begin{abstract}
Razborov's flag algebra method reduces asymptotic questions in extremal graph
theory, including upper bounds on the density of a fixed pattern in large
graphs avoiding specified patterns, to finite algebraic verification.  In
practice, semidefinite-programming solvers produce certificates containing
rational matrices and coefficients whose identities and positivity conditions
imply the desired bound.  The method has played a central role in extremal graph
theory, but its certificates may contain thousands of exact rational values and
arise from toolchains involving unverified numerical computation, making them
impractical to audit by hand and unsuitable as trusted proof artifacts without
independent checking.

We present a Lean~4 formalization of flag algebras for finite simple graphs and
a certificate-to-proof compiler built on it.  During Lean elaboration, the
compiler treats certificates as untrusted input, independently checks their
algebraic identities and positivity conditions against the formal definitions,
and constructs complete, machine-checked Lean proofs.  The development has
three layers: a specification layer formalizing the mathematics, a reflection
layer providing verified computation and generating thousands of verified
identities required by a certificate, and an automation layer driving
compilation.

We evaluate the compiler on seven certificates for Tur\'an-type upper bounds
and describe how Lean checks each proof obligation.  For Mantel's theorem and
the Erd\H{o}s pentagon theorem, we also prove matching lower-bound constructions
directly in Lean, establishing the exact values.  In particular, this yields,
to our knowledge, the first proof-assistant formalization of the asymptotic
Erd\H{o}s pentagon theorem, all known proofs of which use flag algebras.
\end{abstract}
...
\item \emph{Machine-checked extremal bounds}: seven compiled Tur\'an-type upper
  ...
  asymptotic Erd\H{o}s pentagon theorem; all known proofs use flag algebras.
...
first proof-assistant formalization of the Tur\'an-density identity
$\tdensity{C_5}{K_3}=\tfrac{24}{625}$.
...
\lean{ErdosPentagon\_Turan} is the first proof-assistant formalization of the
Erd\H{o}s pentagon Tur\'an-density identity
$\tdensity{C_5}{K_3}=\tfrac{24}{625}$; all known proofs of this asymptotic
theorem use flag algebras~\cite{grzesik2012,hatami2012}, so a machine-checked
proof required a formalization of the method itself.
...
machine-checks the new bounds of both papers, while recording the
SDP-certificate-bridge and cited-literature steps required by some bounds as
named assumptions documented on a result-by-result basis.  It also
independently arrives at the reflection pattern of \Cref{sec:reflection}: a
...
aimed at carrying out flag-algebra inequality proofs automatically inside Lean.
...
peak resident set, taken as the maximum over the five runs.  Wall-clock times
are highly repeatable (standard deviation below $1\%$ of the mean in every
configuration except the $P_3$- and $C_4$-density cases under
\lean{native\_decide}, at $2.3\%$ and $1.8\%$).
