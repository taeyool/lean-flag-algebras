...
occurrences.  Only the first is forced by the representation itself and would
arise in any proof assistant that models flags as isomorphism classes; the
others are shaped by Lean's treatment of dependent equality and instance
synthesis, so a port to Rocq, Agda, or Isabelle/HOL\@ would likely meet them
in a different form.  The following subsections treat these obstacles
separately.
...
type.  Thus, the identity is not merely hard to prove; it is ill-typed as
written.
...
whose relaxations go back to Parrilo and
Sturmfels~\cite{parrilo2003minimizing}.  Validating each run of a complex
...
themselves been verified in a proof assistant~\cite{alkassar2014framework};
...
are correspondingly mixed: thousands of density identities decided by kernel
evaluation, normalization in a quotient algebra, and forbidden-subgraph
reasoning.
...
gives an extended treatment~\cite{chlipala2013cpdt}.  Cohen et
...
which our \lean{Sym2Graph}/\lean{SimpleGraph} pair and its adequacy theorems
...
through a bytecode machine whose correctness is part of the
...
\lean{native\_decide} occupy the same two positions, and our certificates sit
at both; we report which obligation rests on which mechanism
(\Cref{sec:flagmatic-trust}) rather than quote a single trust level.
...
computational mathematics, where the dominant costs are computing with
quotients, comparing objects up to isomorphism, and controlling finite
enumerations.
...
polynomial Freiman--Ruzsa conjecture in characteristic two.  Our development
is aimed at the method rather than at a single theorem: it formalizes the flag
algebra, its semantics, and the finite machinery its certificates need.
...
representation with adequacy lemmas.  Our development formalizes the classical
theory instead: the quotient algebra, positive homomorphisms, and the ensemble
semantics.  It is organized as a reusable library with a certificate-to-proof
compiler on top, rather than around individual bounds, so that certificates
produced outside Lean become Lean proofs.
...
\boldparagraph{Graph limits and Tur\'an densities.}
