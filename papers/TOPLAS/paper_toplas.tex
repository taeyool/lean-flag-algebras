...
\title[A Certificate-to-Proof Compiler for Flag-Algebra Bounds]{A Certificate-to-Proof Compiler for Flag-Algebra Bounds in Lean~4}
...
\begin{abstract}
Razborov's flag algebra method proves asymptotic inequalities of extremal
graph theory: statements about all sufficiently large graphs that avoid
given patterns.  The method reduces such an inequality to finitely many
algebraic conditions, and a semidefinite-programming (SDP) solver searches
for a certificate that witnesses them: lists of flags, exact rational
density facts, and positive-semidefinite matrices.  Certificates behind
notable theorems contain thousands of exact values and are produced by
large, numerical, unverified toolchains, so they can be neither audited by
hand nor sensibly trusted.  This article presents a Lean~4 formalization of
the method for finite simple graphs, together with a certificate-to-proof
compiler that turns such untrusted certificate data into complete,
machine-checked proofs.

The development has three layers.  A specification layer formalizes the
mathematics of the method: partially labeled graphs, their densities in
large graphs, the quotient algebra of density expressions, and its
graph-limit semantics through positive homomorphisms.  A reflection layer
supplies executable graph representations, adequacy theorems relating their
computations to the specification, and elaboration-time commands that
generate the thousands of verified density and multiplication identities a
certificate requires.  An automation layer contains the certificate-to-proof
compiler itself.  The compiler treats the SDP output as candidate data only:
Lean recomputes every claimed fact against the mathematical definitions,
verifies positive semidefiniteness exactly over the rationals by $LDL^\top$
factorization, and assembles the target bound.  We evaluate the compiler on
seven Tur\'an-type upper bounds, including those of Mantel's theorem and the
Erd\H{o}s pentagon theorem, and report a trust profile that states, for each
class of proof obligation, the mechanism by which it is discharged.

The compiled bounds all concern graphs that avoid a forbidden subgraph,
such as a triangle, so the formalization must choose where this constraint
enters the design.  Razborov's original development builds the constraint
into the algebra from the start; we call this the built-in semantics.  Our
implementation instead formalizes the algebra of unconstrained graphs once
and imposes each constraint only where an inequality is interpreted; we
call this the ensemble semantics.  This keeps the constraint out of the
algebra's definitions and lemmas, so one formalization serves every
forbidden family.  The choice raises a soundness question: do the two
semantics validate the same inequalities?
We prove that every inequality valid in the built-in semantics remains
valid in the ensemble semantics, characterize the flag types at which the
two agree by a criterion we call root-plantability, and give a $C_4$-free
counterexample where they differ.  A
forthcoming companion paper develops the complete mathematical and formal
account of this meta-theory.
\end{abstract}
...
\keywords{flag algebras, Lean 4, proof by reflection, semidefinite
  programming, Tur\'an density, interactive theorem proving, tactic
  metaprogramming, extremal combinatorics}
...
Some of the best-known results of modern discrete mathematics were
established by proofs that lean on large external computations.  The
four-color theorem was settled by extensive machine case analysis, later
re-implemented and checked inside a proof
assistant~\cite{gonthier2008fourcolour}; the Kepler conjecture was resolved
by optimization-heavy computation, and a complete formal proof required
more than a decade of further work~\cite{hales2017kepler}; the
empty-hexagon number was determined with a SAT solver, and the result was
later verified by formalizing the geometric reduction in Lean and checking
the solver's clausal proof with a formally verified
checker~\cite{heule2024hexagon,subercaseaux2024hexagon}.  These results
share a shape: a search tool produces candidate evidence, such as a case
analysis, an optimization certificate, or an unsatisfiability proof, and
the theorem is only as trustworthy as the checks applied to that evidence
and the reduction that connects it to the mathematical statement.

For consuming such untrusted computation, the programming-languages and
verification communities have an established playbook.  Untrusted SAT
solvers emit unsatisfiability certificates that are validated by formally
verified checkers~\cite{lammich2020unsat,tan2023cakelpr}; SMT proof
witnesses are reconstructed inside a proof assistant, step by step or
through a verified checker~\cite{bohme2010z3,armand2011modular}; numerical
sums-of-squares certificates for polynomial inequalities are rounded to
rational form and reverified
exactly~\cite{harrison2007verifying,besson2006fast,morrison2026sos}; and
when verifying the tool itself is impractical, each of its runs can be
validated after the fact, as in translation
validation~\cite{pnueli1998translation}.  This article applies that
playbook to a proof method of extremal graph theory whose
computer-generated certificates are, in established practice, produced and
checked entirely outside proof assistants: Razborov's flag algebra
method~\cite{razborov2007flag}.

\paragraph{Flag algebras and their certificates.}
Many central questions of extremal combinatorics ask how often one finite
pattern can occur in graphs that avoid another.  For a finite simple graph
$F$ and a finite collection $\mathcal{H}$ of finite simple graphs, write
$\mathrm{ex}(n,\,F;\,\mathcal{H})$ for the maximum number of induced copies
of $F$ in an $n$-vertex graph that contains no member of $\mathcal{H}$ as a
(not necessarily induced) subgraph.  The associated \emph{Tur\'an density}
\[
  \tdensity{F}{\mathcal{H}}
  :=
  \lim_{n\to\infty}
...
\]
records the asymptotic proportion of $|V(F)|$-vertex subsets that induce a
copy of $F$.  (Whenever $\mathcal{H}=\{H\}$ is a singleton, we write $H$ in
place of $\mathcal{H}$.)  Mantel's theorem~\cite{mantel}, in this language,
states that $\tdensity{K_2}{K_3}=1/2$: in a sufficiently large
triangle-free graph, at most half of all vertex pairs are edges.  Such an
equality combines two bounds of very different character.  The lower bound
is witnessed by explicit constructions; for Mantel's theorem, the balanced
complete bipartite graphs achieve edge density $1/2$ without containing a
triangle.  The upper bound is the hard half: it quantifies over \emph{all}
sufficiently large $\mathcal{H}$-free graphs, so no finite collection of
examples suffices, and the extremal graphs may change unpredictably as $n$
grows.

Razborov's flag algebra method~\cite{razborov2007flag} reduces proving such
an upper bound to checking finitely many algebraic conditions.  A
semidefinite-programming (SDP) solver searches for a finite package of data
intended to satisfy these conditions: lists of flags, exact rational
density facts, and positive-semidefinite matrices.  We call this package a
\emph{candidate certificate}; the flag-algebra framework explains why a
candidate certificate that passes the checks establishes the bound for the
whole infinite class of graphs.  This combination of algebraic abstraction
and computer search has been unusually effective.  It proved the Erd\H{o}s
pentagon theorem~\cite{hatami2012,grzesik2012}, determined the minimum
triangle density for a given edge density~\cite{razborov2008triangles}, the
maximum induced $C_5$ density~\cite{balogh2016induced5}, and the
rainbow-triangle density~\cite{balogh2017rainbow}, and produced the best
known upper bound on Tur\'an's tetrahedron
problem~\cite{razborov2010tetrahedron} as well as exact Tur\'an densities
for 3-uniform hypergraphs~\cite{baber2012turan}.

The certificates themselves, however, are nothing one would want to trust.
The certificate behind the Erd\H{o}s pentagon theorem relies on thousands of
exact density values and on rational matrices whose positive
semidefiniteness must be established exactly, far beyond what a human
referee can audit; and the solvers that produce such data sit at the end of
a large, numerical, unverified toolchain.  A formal proof should neither
trust this data nor require thousands of individually written proofs.

\paragraph{A certificate-to-proof compiler.}
This article presents a Lean~4~\cite{moura2021lean} formalization of the
flag algebra method for finite simple graphs, structured so that certificate
data never needs to be trusted.  The development has three layers.

The \emph{specification layer} (\Cref{sec:abstract}) formalizes the
mathematics of the method: partially labeled graphs and their densities in
large graphs, an algebra of density expressions modulo the identities that
relate different graph sizes, positive homomorphisms representing graph
limits, and the downward operator that averages labels away.  Its
definitions follow the standard mathematics closely: flags are isomorphism
classes, and densities are sampling probabilities.  Flag-algebra arguments
can therefore be stated in Lean much as they appear on paper.

The \emph{reflection layer} (\Cref{sec:reflection}) makes the specification
computable.  It supplies executable graph representations together with
adequacy theorems proving that the computations they support agree with the
specification, and elaboration-time generation commands that specialize
these theorems into the verified density, downward, and multiplication
identities that a given certificate requires; a single case study
routinely needs thousands of such lemmas.

The \emph{automation layer} (\Cref{sec:flagmatic}) is the
certificate-to-proof compiler.  It treats the output of the external SDP
search as candidate data only: Lean recomputes the required density and
multiplication facts and checks them against the mathematical definitions,
verifies positive semidefiniteness exactly over $\mathbb{Q}$ through
$LDL^\top$ factorizations, expands the target when necessary, and uses the
reflected identities together with custom tactics to normalize the
certificate calculation under the forbidden-graph assumption and assemble
the final bound.  The compiler is packaged as a tactic: the theorem statement
is fixed in the source file, and \lean{flag\_certificate} reads the
certificate file during elaboration and synthesizes a proof of exactly that
statement, so the certificate functions as untrusted hint data.  A companion
\lean{flag\_certificate?} materializes the synthesized proof script in the
source file with one editor click, for auditing or archiving.

\paragraph{Trust.}
The proofs produced by the compiler are ordinary Lean theorems, so in the
best case the trusted base is Lean's kernel.  Five of our seven case studies
reach that best case: every generated proof obligation is discharged by
kernel evaluation, through \lean{decide +kernel}.  For the two largest
certificates, kernel evaluation of the bulk density computations is not yet
practical, and the generated files discharge those obligations with
\lean{native\_decide}, which additionally trusts Lean's compiler; their
positive-semidefiniteness checks remain kernel-evaluated even there.
\Cref{sec:flagmatic} states this per-obligation trust profile explicitly.
We regard the residual \lean{native\_decide} use as a feasibility constraint
of the current encoding rather than a limitation of the method.

\paragraph{Two semantics for forbidden subgraphs.}
A formalization of the constrained method must also decide where the
forbidden-graph condition enters.  The usual mathematical development builds
the constraint in from the start, constructing the flag algebra from
$H$-free graphs only.  Our implementation instead retains the ambient
algebra of all finite graphs and imposes $H$-freeness when an inequality is
interpreted: the order \lean{forbidLE} considers $H$-free empty-type
positive homomorphisms, chooses an occurrence of the flag type uniformly at
random, uses its vertices as labels, and requires the inequality to hold
with probability one.  \Cref{sec:metatheory} compares the two designs the
way one compares two semantics of a language.  Every inequality valid in the
built-in (quotient) semantics is valid in this \emph{ensemble} semantics;
the converse holds at the empty flag type, can fail at a nonempty flag type,
and holds at a fixed flag type exactly when the constrained class is
\emph{root-plantable} there.  We prove that blow-up closure guarantees
root-plantability and give a $C_4$-free counterexample showing that
heredity alone does not.

\paragraph{Contributions.}
\begin{itemize}
\item \emph{A certificate-to-proof compiler}, packaged as an elaboration-time
  tactic, that translates externally generated flag-algebra certificates into
  machine-checked Lean proofs of Tur\'an-type bounds: the statement is fixed
  in the source file and the certificate is untrusted hint data, exact
  rational $LDL^\top$ factorizations are checked for positive
  semidefiniteness, and the final bound is assembled with custom tactics.
  The compiler comes with an explicit per-obligation trust model and a
  one-click materialization of the synthesized proof script
  (\Cref{sec:flagmatic}).
\item \emph{An executable reflection layer} whose computations are connected
  to the specification by adequacy theorems, and whose elaboration-time
  commands generate the flags and the density, downward, and multiplication
  theorems that a certificate needs, under generated names that carry
  metadata for later automation (\Cref{sec:reflection}).
\item \emph{A specification-layer library} formalizing the core theory of
  flag algebras for finite simple graphs: flags and their densities, the
  quotient algebra, positive homomorphisms and the semantic order, and the
  downward operator with its random-extension machinery
  (\Cref{sec:abstract}).
\item \emph{A meta-theory of constrained semantics} relating the ensemble
  and quotient orders: the root-plantability criterion, a sufficient
  condition based on blow-up closure, and a $C_4$-free counterexample.  The
  meta-theory is formalized in a separate, automatically generated Lean
  development that extends the manually written library
  (\Cref{sec:metatheory}).
\item \emph{An evaluation and experience report}: seven compiled
  upper-bound certificates, including the bounds of Mantel's theorem and the
  Erd\H{o}s pentagon theorem and a $C_4$-density bound for triangle-free
  graphs; direct uses of the library without the compiler, namely a separate
  proof of Mantel's bound, the matching lower-bound constructions completing
  both exact Tur\'an-density results, and two inequalities of Goodman
  (\Cref{sec:lowerbounds}); compilation-time and memory measurements
  (\Cref{sec:compile-times}); and an analysis of the engineering obstacles
  the development had to overcome (\Cref{sec:obstacles}).
\end{itemize}
The complete Lean development, including the certificate-to-proof compiler
and the autoformalized meta-theory, is publicly available at
\url{https://github.com/taeyool/lean-flag-algebras-release}.

\paragraph{Beyond flag algebras.}
Although the subject matter of this article is extremal graph theory, the
design questions it answers are the general ones of certificate-consuming
verification.  The compiler instantiates a \emph{candidate-generator
pattern}: an external tool proposes data, and the proof assistant
independently re-derives every fact the proof needs, so that the tool sits
entirely outside the trusted base.  The specification/reflection split with
machine-checked adequacy theorems, the elaboration-time generation of
theorem families whose names carry metadata for later automation, and
per-obligation trust reporting are, we believe, directly reusable in
formalizations built around other certificate-producing methods, from
sums-of-squares and linear-programming bounds to spectral arguments.

\paragraph{Organization.}
\Cref{sec:background} reviews the mathematical background of flag algebras
and fixes the notation used throughout the article.  \Cref{sec:abstract}
presents the specification layer, including its semantic treatment of
forbidden-graph assumptions.  \Cref{sec:reflection} introduces the
executable graph representations, their adequacy theorems, and the
elaboration-time generation commands.  \Cref{sec:flagmatic} describes the
...
certificate compilation.  \Cref{sec:obstacles} analyzes the main engineering
obstacles, and \Cref{sec:metatheory} presents the core results of the
meta-theory of constrained semantics together with its autoformalization.
\Cref{sec:unsure} records the remaining design questions.  Finally,
\Cref{sec:related} discusses related work, \Cref{sec:conclusion} presents
our conclusions, and \Cref{sec:compile-times} reports compilation times for
the seven compiler examples.
...
input, matches its flags, matrices, and target data against Lean's own
enumerations, regenerates the required flags, recomputes their density and
multiplication identities, verifies the rational matrix factorizations, and
assembles the final inequality.  The compiler runs inside Lean's elaborator:
the desired theorem is stated in the source file, and the
\lean{flag\_certificate} tactic reads the certificate file at elaboration
time and synthesizes a proof of exactly that statement.  Automating this
translation is important because nontrivial certificates contain extensive
finite bookkeeping that would be tedious and error-prone to reproduce
manually.  The resulting theorem is accepted only after Lean has checked
every mathematical obligation.
...
four-part process is exposed as a tactic.  A source file states the desired
bound and proves it by naming the certificate:
...
\end{lstlisting}
During elaboration, \lean{flag\_certificate} parses the certificate file,
resolves each of its compact graph strings to the isomorphic entry of Lean's
canonical enumeration, reconstructs the rational block matrices together with
exact $LDL^\top$ witnesses, and synthesizes the proof described below, with
the certificate data inlined as literals.  Before doing any of this, it
checks that the theorem families of Part~1 are present in the current
namespace; if some are missing, it fails with the exact block of generation
commands to add.  The variant \lean{flag\_certificate?} additionally offers
...
The scaffolding surrounding the statement, namely the forbidden-graph
definition, the \lean{set\_option} block, and the Part~1 generation commands,
is written once per certificate, by hand or by the \lean{gen-skeleton}
subcommand of the \lean{flagmatic\_to\_lean.py} script:
...
generated Lean source file.  The output file contains the flag and density
generation commands of Part~1, the theorem statement read off the
certificate with its one-line \lean{flag\_certificate} proof, and a closing
restatement of the bound as a generalized Tur\'an density
(\Cref{sec:results}).  Passing \lean{--materialize} instead emits the fully
expanded legacy form, in which the matrices of Part~2, the target-lifting
lemma of Part~3, and the complete tactic proof of Part~4 appear in the
source file itself and the certificate is not read at build time.  A
separate diagnostic command
\begin{lstlisting}[language={}]
python flagmatic_to_lean.py inspect <mantel-cert>.json
\end{lstlisting}
checks that the certificate can be parsed and that its flags can be matched
to Lean's enumeration, printing the resulting identifier mapping and the
Part~1 generation commands.  We now explain the synthesized proof one part
at a time.
...
file, and the prerequisite the \lean{flag\_certificate} tactic checks for: it
...
\Cref{sec:reflection-generation} on it.  The commands choose
...
lemma.  To use it, the tactic reconstructs $M_t$ from $Q'_t$ and $R_t$ in
...
enters this check.  Lean does not separately compare $M_t$ with the original
$Q'_t$ and $R_t$ fields of the certificate; that reconstruction is performed
by the tactic's unverified elaboration-time code.  An incorrect $LDL^\top$
witness is therefore rejected, while fidelity to the source certificate is
discussed in \Cref{sec:flagmatic-trust}.  In either case, a reconstructed
matrix can contribute to the final theorem only if both its PSD proof and the
...
and $r=\tfrac13\cdot\fbarPthree$.  The synthesized proof does not
...
\paragraph{The checked artifact.}
The certificate-specific assembly just described is performed by the tactic's
elaboration-time code: it parses the target, bound, forbidden graph, host
size, and SDP blocks; finds, for each compact graph string, the isomorphic
entry of Lean's canonical enumeration, whose index selects the corresponding
\lean{FlagAlgebra\_n\_k\_m\_i} constant; reconstructs the matrices and their
$LDL^\top$ witnesses; and decides whether the target requires lifting.  Because the matching runs against the enumeration
Lean itself computes, no agreement between an external enumeration order and
Lean's is assumed.  Certificates change the assembled data and the number of
SDP blocks, but not the four-part proof structure.  In both frontends, what
Lean accepts is a proof of the stated theorem, checked from scratch: every
intermediate proposition and the final bound are carried by proof terms, and
the certificate can influence the outcome only by making the stated theorem
provable.  The next subsection makes the resulting trust boundary precise.
...
For the first question, neither the SDP solver nor the compiler frontends
...
example files used here introduce neither axiom declarations nor
...
source file.  The tactic derives every ingredient of the proof (the flag
indices, the matrices, the expansion coefficients) from the certificate,
and it can only close the goal that the statement poses.  A disagreement
between the two, say a mistranscribed target or forbidden graph, or a stated
bound below what the certificate supports, therefore surfaces as an
elaboration failure; no choice of certificate data can make an unsupported
statement elaborate.  What remains to audit is the statement itself: that
$[F]\leq_{\mathcal H}c\cdot\mathbf 1$, as written, expresses the intended
extremal claim.  When the statement is written by hand, it is the author's
declaration of intent; in files emitted by \lean{gen-skeleton}, the script
writes the statement, so reading it remains the auditability boundary.  In
the \lean{--materialize} form, the script is additionally trusted to
transcribe the certificate data into the file, as the certificate is not
...
\lean{gen-skeleton} produces a complete Lean source file whose main theorem
is proved by a single \lean{flag\_certificate} call.  All seven files compile
...
mentions no generated constants and can be audited against the
specification-layer definitions alone.  The certificate argument establishes only this
...
\Cref{sec:flagmatic-trust} but does not eliminate it: the theorem statement
must still be read to confirm that it expresses the intended extremal claim,
and in generated files that statement is written by the unverified
scaffolding script.  Two smaller costs of the elaboration-time design are
...
trigger rechecking of the file that reads it; and the certificate data is no
longer visible in the source file, so recovering it takes either the
\lean{--materialize} form or one \lean{flag\_certificate?} click.
...
\paragraph{Certificate checking and proof reconstruction.}
Consuming the output of untrusted solvers without trusting them is a
well-developed theme in verification.  For SAT, solvers emit clausal proofs
that are validated by formally verified
checkers~\cite{lammich2020unsat,tan2023cakelpr}; this is how the
empty-hexagon number, another computer-generated result in combinatorics,
was verified~\cite{subercaseaux2024hexagon}.  For SMT, proof witnesses are
replayed step by step in Isabelle/HOL and HOL4~\cite{bohme2010z3} or
validated by a checker verified inside Rocq (previously called
Coq)~\cite{armand2011modular}.  For
polynomial inequalities, numerical solvers find sums-of-squares
certificates, whose underlying relaxations go back to Parrilo and
Sturmfels~\cite{parrilo2003minimizing}; the certificates are rounded to
rational form and verified exactly: by Harrison in HOL Light~\cite{harrison2007verifying}, by
reflexive tactics in Rocq~\cite{besson2006fast}, and in Lean~4 by Morrison's
\lean{sos} tactic~\cite{morrison2026sos}, which finds a certificate with an
external semidefinite solver, rounds it, and verifies it through an
$LDL^\top$ factorization; Roux et al.\ study the validation of numerical
SDP-solver output in the context of program
invariants~\cite{roux2018validating}.  Validating each run of a complex
untrusted tool, rather than the tool itself, is the translation-validation
strategy~\cite{pnueli1998translation}.  Our compiler uses the same
discovery-versus-verification separation and, like the \lean{sos} tactic, is
invoked as a tactic that treats its certificate input as hint data for a
stated goal, verified through an exact rational $LDL^\top$ check.  What distinguishes the flag-algebra setting is, first, the
heterogeneity and scale of the obligations: a single certificate mixes
thousands of graph-counting identities, algebraic normalization in a
quotient algebra, and forbidden-subgraph reasoning.  Second, we reconstruct
proofs inside the object logic rather than compile a standalone verified
checker.

\paragraph{Proof by reflection in proof assistants.}
Proof by reflection is a classical technique in the Rocq community; Chlipala
...
\paragraph{Metaprogramming and elaboration-time generation.}
Our generation commands and custom tactics are built on Lean~4's
metaprogramming framework~\cite{moura2021lean}, which exposes the elaborator
to user code in the tradition of the Lean~3 framework of Ebner et
al.~\cite{ebner2017structured}; MetaCoq plays a similar role in the Rocq
ecosystem~\cite{sozeau2020metacoq}.  Where reflection-based developments
typically reify one goal at a time, our commands run at elaboration time to
emit whole families of definitions and theorems ahead of any particular
proof, and they encode enough metadata in the generated names that later
tactics can retrieve exactly the lemmas a certificate step needs
(\Cref{sec:reflection}).

\paragraph{Proof engineering at scale.}
Sustaining large mechanized developments raises engineering questions that
are increasingly documented in their own right, from the survey of Ringer et
al.~\cite{ringer2019qed} to the retrospective on Project
Everest~\cite{ahman2026everest}.  Our analysis of engineering obstacles
(\Cref{sec:obstacles}) and our per-obligation trust reporting
(\Cref{sec:flagmatic}) contribute datapoints from computational mathematics, where the
dominant costs (computing with quotients, comparing objects up to
isomorphism, and controlling finite enumerations) differ from those of
...
\paragraph{Concurrent formalizations of flag algebras.}
...
\paragraph{Graph limits and Tur\'an densities.}
...
expansion, and the main theorem.  The measurements were taken on the fully
materialized form of the files (\lean{--materialize}); the
\lean{flag\_certificate} form performs the same proof obligations, and in our
spot checks the two differed by less than run-to-run variance.  The generated
