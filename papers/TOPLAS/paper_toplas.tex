...
\end{lstlisting}
The tactic \lean{decide +kernel} closes this decidable goal by evaluating the
reflection-layer density computation inside the kernel.
...
non-negativity of $Q$ and $r$ proves the bound.
...
\begin{enumerate}
  ...
  \item \textbf{Lifting the target (when needed).}  If the target graph $F$
    ...
    search in \Cref{sec:flagmatic-background}, the proof must lift $[F]$ to the
    host-size basis, dropping contributions that contain the forbidden graph.
...
\boldparagraph{Generating the flags and density data (Part~1).}
...
\boldparagraph{Lifting the target (Part~3).}
...
the \lean{flagsum\_ac\_sort\_rhs\_pipeline} tactic collects like basis terms.
...
\boldparagraph{The checked artifact.}
...
exercise targets at and below the host size and increasingly large multi-block
normalizations.
...
obligations that ordinary combinatorial notation hides.  They do not change
the underlying mathematics, but they recur often enough to impose a
substantial proof-engineering cost, and their causes differ.  Invariance
under isomorphism is the well-definedness condition required to define an
operation on a quotient.  Comparing flags of different sizes, and comparing a
tuple with its reordering, both come from carrying data in types, which puts
related objects beyond the reach of ordinary equality.  The enumeration issue,
by contrast,
arises from elaboration and instance synthesis, which may produce
different enumerations at different occurrences.  Only the first is forced by
the representation itself and would arise in any proof assistant that models
flags as isomorphism classes.  The others are artifacts of a dependently
typed setting with typeclass-driven elaboration: a port to Rocq (formerly
Coq) or Agda would meet close analogues, whereas Isabelle/HOL's simple type
theory cannot index flags by size at all, so a port there would trade these
obstacles for a different set of obligations.  A final
subsection collects the proof method these obligations kept demanding:
identifying two finite sets by an explicit bijection.

\subsection{Computing with Graphs up to Isomorphism}

As described in \Cref{sec:flagtypes}, a flag is an isomorphism class of
concrete labeled graphs, so that later statements do not depend on arbitrary
vertex names.  Every construction first described on a representative
therefore owes a well-definedness proof: a rule $G\mapsto A(G)$ descends to
flags only once
\[
  G \simeq_f G' \quad\Longrightarrow\quad A(G)=A(G')
\]
is established.  \lean{Quotient.lift} takes the rule and that proof and returns the
function on flags, so the mechanism costs a line; the proof is what costs.

The obligation recurs for every density and every graph operator, and again
at the reflection boundary, where defining the decoding
\lean{Sym2Flag.toFlag} of \Cref{sec:reflection-adequacy} requires showing
that equivalent concrete representations decode to the same abstract flag.  Rather than exposing both
representations in every later theorem, we discharge the obligation once and
let subsequent proofs work with flags.
...
theorem flagListDensity_HEq_eq
    {T W : Type} {(*@$\sigma$@*) : FlagType T} {t : (*@$\mathbb{N}$@*)} {Vl(*@$_0$@*) Vl(*@$_1$@*) : Fin t (*@$\to$@*) Type}
...
heterogeneous equality can be used as an ordinary one, and the lemma
...
former; an instance of \lean{Fintype T} provides the latter:
\begin{lstlisting}
class Fintype (T : Type*) where
  elems : Finset T
  complete : (*@$\forall$@*) x : T, x (*@$\in$@*) elems
\end{lstlisting}
The field \lean{elems} is a concrete finite set of elements of \lean{T} and
\lean{complete} proves that every element belongs to it, so \lean{Fintype} is
the interface counting needs.  The sets this development counts are described
by conditions, not by explicit enumerations: the tuples of induced copies
realizing a given flag list, or the type embeddings producing a given flag.
Lean can usually
infer that such a set is \lean{Finite}, but \lean{Fintype.card} and the lemmas
about it require a \lean{Fintype}, and \lean{Fintype.ofFinite} supplies one by
invoking \lean{Classical.choice}.  That instance is noncomputable, which costs
nothing here, since these proofs relate cardinalities by lemmas rather than
evaluating them.

The elaborator synthesizes such instances automatically, and the difficulty is
not obtaining one but obtaining the same one every time.  In a development
with multiple overlapping constructions (quotient types, embedded subgraphs,
spaces of label placements and constrained embeddings), the same type can
receive different \lean{Fintype} instances from different elaboration paths.  Their \lean{elems} fields cannot
disagree as sets, since \lean{complete} forces each to contain every element;
what differs is the term denoting that set, and when the two terms do not
reduce to a common one definitionally, a rewrite or a direct application
fails, even though the instances are provably equal.
...
-- and h_iso_S0_S1 a bijection between them.
...
have count_eq : S0.toFinset.card = S1.toFinset.card := by
  ...
  exact card_eq
\end{lstlisting}
It names the instance for each set, derives the equality of cardinalities from
the bijection, and rewrites the \lean{toFinset} counts into that form.  The
\lean{@} notation writes out the instance arguments that Lean would otherwise
infer for itself, so one can see that \lean{card\_eq} and the two
\lean{toFinset} terms use the same ones; without the bindings, each
occurrence would receive whatever instance its own elaboration path supplies,
and the rewrite fails whenever that term differs from the one inside the
count being rewritten.

This pattern appears in the proof of \lean{flagDensity\allowbreak\_permute},
the invariance theorem of \Cref{sec:reordered-flag-tuples}: there \lean{S0}
and \lean{S1} are the sets of tuples of pairwise-disjoint induced copies that
realize \lean{Fl} and \lean{Fl.permute $\pi$}, the mathematical step is the
reindexing bijection, and the named instances ensure that the resulting
cardinality equality rewrites the counts occurring in the density definition.
The downward-operator proofs of \Cref{sec:formal-downward} use the same
pattern for the set of type embeddings that produce a given flag.
...
\lean{flagListDensity\allowbreak\_HEq\allowbreak\_eq}, the other theorem of
\Cref{sec:reordered-flag-tuples}, is closed exactly this way: once its
families and tuples have been identified, the two sides differ only in the
\lean{Fintype} instances that its \lean{FintypeList} hypothesis supplies for
the component types, and \lean{congr!} resolves those.
...
specification layer.  We expected many of these equalities to follow from
existing counting lemmas or symbolic simplification, but unfolding the
definitions repeatedly exposed the same structure: the two sides are
cardinalities of finite sets of configurations presented in different ways.
Our most reliable method is to identify the two sets and construct an explicit
...
\begin{equation}\label{eq:Q-sigma-intrinsic}
  ...
  \right\}.
\end{equation}
One inclusion is immediate from \(q_\sigma([F])=0\); the other holds because
the flags outside \(\mathcal K\) span exactly the ideal that \(q_\sigma\)
kills, so a homomorphism vanishing on them factors through \(q_\sigma\).
...
The equality \(S_\sigma=Q_\sigma\) is the right condition because each order is
non-negativity of the continuous function \(\chi\mapsto\chi(g-f)\) on one of
the two spaces: almost-sure non-negativity under a measure is non-negativity
on its support, so the ensemble order tests \(S_\sigma\), while the built-in
order tests \(Q_\sigma\) by definition.  Equation~\eqref{eq:S-subset-Q} then
gives the forward implication.  For the converse, a point of
\(Q_\sigma\setminus S_\sigma\) can be separated from \(S_\sigma\) by a
continuous function and the separating function approximated by a
flag-algebra element, which is then ensemble-non-negative but not built-in
non-negative.  Thus, \Cref{thm:support-closure-summary} is a completeness theorem:
built-in proofs are always sound for ensemble semantics, and they capture all
...
The proof takes a target \(\chi\in Q_\sigma\), represented by a sequence of
finite \(\sigma\)-flags in \(\mathcal K\), and blows up each of them one vertex
at a time, closure supplying an interior at every step that keeps the graph in
\(\mathcal K\).  Making the fibers over the labeled vertices large enough
forces a uniformly chosen embedding of \(\sigma\) to land on them with
probability bounded away from zero, while a fixed sample lands in distinct
fibers with probability tending to one, so the interiors do not affect the
densities it sees.  A positive fraction of the embeddings therefore realizes
the profile of \(\chi\) to any prescribed accuracy, which places \(\chi\) in
the support of \(\Ext_\sigma(\phi_0)\) for a limit \(\phi_0\) of the unlabeled
blow-ups, hence in \(S_\sigma\).  With
Equation~\eqref{eq:S-subset-Q} this gives \(S_\sigma=Q_\sigma\).
...
\(-d\) is non-negative in the ensemble semantics.  Arbitrarily large stars are
\(C_4\)-free, however, since they contain no cycle at all.  Label the center
of \(K_{1,t}\) to obtain a \(\sigma_1\)-flag, and let
\(\chi_*\in Q_{\sigma_1}\) be the limit of that sequence.  Every leaf is
adjacent to the center, so \(\chi_*(d)=1\) and
\(\chi_*\notin S_{\sigma_1}\), whence
...
present implementation cannot settle on its own.  For each, we distinguish what
the development demonstrates from what would require alternative
implementations or larger computations to determine.

\boldparagraph{When should a forbidden-graph constraint enter the formalization?}
We deferred it: the ensemble semantics (\Cref{sec:forbidden}) imposes the
constraint only at the point of comparison, and the seven certificates of
\Cref{sec:results}, which forbid four different graphs, share one ambient
algebra and one body of algebraic lemmas.  The choice costs nothing in
soundness: the ensemble semantics yields the asymptotic bounds on its own, and
the comparison with Razborov's built-in semantics in \Cref{sec:metatheory} is
mathematics rather than a proof obligation.

This does not settle which design is cheaper overall.  Incorporating the
constraint from the outset may streamline arguments specific to a fixed
extremal problem, and we have not formalized comparable certificate proofs
under that design.  Even when root-plantability makes the two semantics
validate the same inequalities, the implementations need not cost the same.

\boldparagraph{Should specification and computation use separate representations?}
Maintaining the two representations of \Cref{sec:abstract,sec:reflection} kept
the abstract development close to the mathematics, and every density and
multiplication fact used by the seven case studies was obtained by executable
evaluation and carried across by an adequacy theorem.  The bridge itself is
substantial.  One alternative is to state the theory directly on a concrete,
canonical representation: this would reduce the bridge, but representation and
canonicalization choices would become visible in the statements and proofs of
the abstract theory.  Deciding between the two on cost would take a second
development of comparable scope.  The separation is at least a well-trodden
one: it is the established methodology of Cohen et
al.~\cite{cohen2013refinements}, and the concurrent formalization of local
flag algebras~\cite{davey2026local}, a calculus that normalizes by maximum
degree and takes no quotient at all, independently makes the same split
between a noncomputable specification and a computable representation.
...
calculations over graphs on three, four, or five vertices.  At this scale, the
optimized enumeration of \Cref{sec:reflection-generation} keeps the required
families of finite identities manageable: the Erd\H{o}s pentagon certificate
needs 2,832 pair-density theorems across its three flag types, all checked by
\lean{decide +kernel}.  Certificates at the five-vertex scale therefore do go
through the current reflection and automation layers, though only just, since
the Erd\H{o}s pentagon is the largest carried entirely on the kernel-checked
path.
...
kernel-checked evaluation grows with them: the two certificates that fall back
to \lean{native\_decide} (\Cref{sec:results}) need 10,332 and 8,172
pair-density theorems.  Scaling further will therefore likely require more
efficient executable graph representations and counting algorithms in the
reflection layer, as well as improvements to theorem generation and algebraic
normalization.  This
...
well-developed theme in verification: clausal proofs for
SAT~\cite{lammich2020unsat,tan2023cakelpr}, proof reconstruction and verified
checkers for SMT~\cite{bohme2010z3,armand2011modular}, and rounded
sums-of-squares certificates for polynomial
inequalities~\cite{harrison2007verifying,besson2006fast,morrison2026sos},
whose underlying semidefinite relaxation goes back to Shor and
Parrilo~\cite{parrilo2003minimizing}.  Validating each run of a complex
untrusted tool, rather than the tool itself, is the translation-validation
strategy~\cite{pnueli1998translation}, and the general discipline of having a
computation emit a witness that a cheap checker can verify is that of
certifying algorithms~\cite{mcconnell2011certifying}, whose checkers have
themselves been formally verified~\cite{alkassar2014framework};
Roux et al.\ study the discipline for numerical SDP-solver output in the
context of program invariants~\cite{roux2018validating}.  Our compiler is closest to Morrison's
...
$LDL^\top$ factorization: we too are invoked on a stated goal with the
certificate as hint data, and check positive semidefiniteness by exact
rational $LDL^\top$.  What distinguishes the flag-algebra setting is that the
certificate refers to objects the checker must first build: a \lean{sos} or
clausal certificate is checked against objects the goal already names, whereas
here Lean regenerates the flags up to isomorphism and proves the enumeration
exhaustive before any coefficient can be checked.  The obligations that follow
are correspondingly mixed: thousands of density identities decided by
trusted evaluation, normalization in a quotient algebra, and
forbidden-subgraph reasoning.

\boldparagraph{Proof by reflection and trusted evaluation.}
Proof by reflection is a classical technique in the Rocq community, going
back to Boutin~\cite{boutin1997reflection}; Chlipala gives an extended
treatment~\cite{chlipala2013cpdt}.  Cohen et
al.~\cite{cohen2013refinements} develop a systematic methodology for building
efficiently computable representations alongside their abstract counterparts,
which our \lean{Sym2Graph}-based reflection layer and its adequacy theorems
follow.  Reflection then raises the question of what performs the evaluation.
Rocq has developed this design space explicitly: \lean{vm\_compute} evaluates
through a bytecode machine implemented inside the
kernel~\cite{gregoire2002compiled}, whereas \lean{native\_compute} compiles to
native code and adds the OCaml compiler and runtime to the trusted
base~\cite{boespflug2011full}.  Lean's \lean{decide +kernel} and
\lean{native\_decide} occupy the same two trust positions (though the Lean
kernel reduces terms directly rather than through a bytecode machine), and
our certificates sit at both; we report which obligation rests on which
mechanism (\Cref{sec:flagmatic-trust}) rather than quote a single trust
level.
...
Everest~\cite{ahman2026everest}.  That literature draws mostly on program
verification; the obstacles we report (\Cref{sec:obstacles}) come from
computational mathematics: in our development the dominant costs were
computing with quotients of graphs up to isomorphism, casting between
mathematically equal but not definitionally equal types, and controlling
finite enumerations.
...
The theory of graph limits, to which flag algebras are closely related
(convergent graph sequences, homomorphism densities, the graphon limit), is
developed in depth by Lov\'asz and Szegedy and by
Lov\'asz~\cite{lovasz2006limits,lovasz2012large}.
Freer has independently formalized a substantial graphon theory in Lean~4,
covering cut distance, weak regularity, counting lemmas, and
compactness~\cite{freer2026graphon}.  Our development works with
...
certificate-to-proof compiler that consumes them without trusting them:
invoked as an elaboration-time tactic against a theorem stated in the source
file, it recomputes every fact a certificate claims against formal definitions
of the flag algebra and assembles a complete proof.  Underneath it are a
...
One design choice shaped the development throughout.  Rather than build a
separate algebra for each forbidden family, we kept one ambient algebra and
imposed the forbidden-subgraph constraint only when its elements are compared.
This ensemble semantics keeps the core definitions independent of the extremal
problem being solved, and it yields the intended asymptotic bounds on its own
terms.  Asking how it relates to the built-in semantics turned into
mathematics of its own: built-in validity always implies ensemble validity, and
the converse holds for all expressions exactly when the class is
root-plantable.

We do not claim that this is the right way to formalize flag algebras.  Our aim
...
the questions left open.  Some choices reflect what is currently feasible
rather than a principled preference; most visibly, the two largest certificates
still use \lean{native\_decide}, a trust tradeoff we report per obligation
...
Section~4.3 of his original paper~\cite{razborov2007flag}, which would exercise
the quotient-level semantics well beyond the certificate arguments considered
here.  We have not tested whether it extends to richer combinatorial structures
such as hypergraphs and directed graphs, which change the objects the whole
development is built on.  A further direction is a formal bridge between our
positive-homomorphism semantics and graphon theory, connecting the development
to the Lov\'asz theory~\cite{lovasz2012large} and to the existing Lean graphon
formalization~\cite{freer2026graphon}.  Most immediately, closing the
\lean{native\_decide} gap, by extending the kernel-checked path or by
delegating to a verified external checker, would remove the last dependency on
Lean's native compiler: a feasibility constraint of the current encoding, not a
