...
reduces its certificate inequalities over partially labeled graphs to this
unlabeled case, so equivalence with the standard semantics is not required for
the soundness of the compiled bounds.

The alternative semantics nevertheless exposed a natural mathematical
question: how does it relate to Razborov's built-in (quotient) semantics when
one or more vertices are labeled?  This comparison is separate from the
preceding justification, but it led to new mathematics.
...
\item \emph{A formalization-driven semantics for forbidden subgraphs.}  The
  ...
  family, and we prove directly that its unlabeled inequalities yield the
  intended asymptotic bounds.  We also present selected results from the
...
vertices.  The downward normalizing factor \(q_\sigma(F)\) is a similar finite
count, but of label placements rather than subflags.
...
\lean{Decidable} instance gives a procedure for deciding a proposition.  With
these instances, a definition that says ``count all objects satisfying this
predicate'' can be evaluated by iterating over the finite collection and
filtering with the decision procedure.
...
unordered pairs of vertices.  This representation is packaged as
\lean{Sym2Graph} and reused to define the concrete flag type
...
always implies non-negativity in the ensemble semantics, while the converse at
a given flag type holds for all expressions exactly when the class is
root-plantable there.
