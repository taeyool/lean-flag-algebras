...
\begin{itemize}
\item \emph{A certificate-to-proof compiler} that translates externally
  generated flag-algebra certificates into machine-checked Lean proofs of
  Tur\'an-type bounds, checking exact rational $LDL^\top$ factorizations for
  positive semidefiniteness and assembling the final bound with custom
  tactics, together with an explicit per-obligation trust model
  (\Cref{sec:flagmatic}).
...
discovery-versus-verification separation and, like the \lean{sos} tactic, an
exact rational $LDL^\top$ check.  What distinguishes the flag-algebra setting is, first, the
