...
upper bounds, including that of Mantel's theorem, and report a trust
profile that states, for each class of proof obligation, the mechanism by
which it is discharged.  Combined with matching extremal constructions
...
The compiled bounds all concern graphs that avoid a forbidden subgraph, so
the formalization must decide where this constraint enters.  Razborov's
development builds the constraint into the algebra from the start; we call
this the built-in semantics.  Ours instead formalizes the algebra of
unconstrained graphs once and imposes each constraint only where an
inequality is interpreted; we call this the ensemble semantics.  This
keeps the constraint out of the algebra's definitions and lemmas, so one
formalization serves every forbidden family.  The choice raises a
soundness question: do the two semantics validate the same inequalities?
