...
independent checking.  This paper presents a Lean~4 formalization of flag
...
Large external computations have played a central role in landmark results of
discrete mathematics.  The four-color theorem was settled by extensive machine
case analysis and later reimplemented and checked inside a proof
...
search tool produces candidate evidence, such as a case analysis, an
optimization certificate, or an unsatisfiability proof, and trust in the final
theorem rests on both the validation of that evidence and the correctness of
the reduction connecting it to the mathematical statement.

The programming languages and formal verification communities have an
established approach to such untrusted computation: treat the search tool as
untrusted and validate the evidence it produces.  SAT solvers emit
unsatisfiability certificates that are validated by formally verified
checkers~\cite{lammich2020unsat,tan2023cakelpr}; SMT proof witnesses are
reconstructed inside a proof assistant, step by step or through a verified
checker~\cite{bohme2010z3,armand2011modular}; and numerical sums-of-squares
certificates for polynomial inequalities are rounded to rational form and
reverified exactly~\cite{harrison2007verifying,besson2006fast,morrison2026sos}.
More generally, when verifying the tool itself is impractical, each run can be
validated after the fact, as in translation
validation~\cite{pnueli1998translation}.  This paper applies the same approach
to Razborov's flag algebra method, a proof technique in extremal graph theory
whose computer-generated certificates are currently produced and checked
entirely outside proof assistants~\cite{razborov2007flag}.
...
This paper presents a certificate-to-proof compiler for the flag
...
and fixes the notation used throughout the paper.  \Cref{sec:abstract}
