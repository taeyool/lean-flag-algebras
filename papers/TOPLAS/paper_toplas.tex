...
theory: we prove that an unlabeled inequality in the ensemble semantics yields
the corresponding asymptotic bound in extremal graph theory.  The compiler
reduces its typed certificate inequalities to this unlabeled case, so
equivalence with the standard semantics is not required for the soundness of
the compiled bounds.

The alternative semantics nevertheless exposed a natural mathematical
question: how does it relate, at nonempty flag types, to Razborov's built-in
(quotient) semantics?  This comparison is separate from the preceding
justification, but it led to new mathematics.  \Cref{sec:metatheory} proves that
built-in validity always implies ensemble validity and characterizes when the
converse holds through a new structural condition, \emph{root-plantability}.
This result illustrates how formalization can do more than check existing
arguments: it can also reveal new mathematical structures and questions.
...
\item \emph{Machine-checked theorems}: seven compiled Tur\'an-type upper
  bounds.  Two of them, for Mantel's theorem and the Erd\H{o}s pentagon
  theorem, are paired with matching lower-bound constructions proved directly
  in the formalization.  To our knowledge, this gives the first machine-checked
  proof of the Erd\H{o}s pentagon theorem, whose known proofs all use flag
  algebras.
\item \emph{A meta-theory of ensemble semantics} relating our treatment of
  forbidden subgraphs to Razborov's built-in semantics, together with an
  autoformalization in a separate Lean library that extends the development
  above.
\end{itemize}
