...
Since every $\phi \in \poshom{\sigma}$ is linear, $\mathcal{C}^\sigma$ is
moreover closed under addition and under multiplication by non-negative reals.
...
\begin{proof}
  ...
  $k$.  The points $p^{G_k}$ lie in $[0,1]^{\mathcal{F}^\emptyset}$, which is
  compact and metrizable because $\mathcal{F}^\emptyset$ is countable, so some
  subsequence $\{p^{G_{k_j}}\}$ converges; by \Cref{thm:convergent-hom}(a) its
  limit is some $\phi \in \poshom{\emptyset}$.  As a pointwise limit, $\phi$
  satisfies $\phi([F]) = \lim_j \den{F}{G_{k_j}} \geq c + \varepsilon > c$,
  contradicting $[F] \leq_{\mathcal{H}} c \cdot \mathbf{1}$, which forces
  $\phi([F]) \leq c$ for every~$\phi \in \poshom{\emptyset}$.
\end{proof}
...
\Cref{sec:flagmatic-background} carries this pattern out in full for Mantel's
theorem, taking $\sigma$ to be the one-vertex flag type and $f$ a multiple of
$((\fKtwobull) - (\fbarKtwobull))^2$.  There the resulting downward image and
the non-negative remainder left over at the end are exactly the data of the
smallest certificate our compiler consumes, so the same derivation doubles as
the introduction to certificates.
