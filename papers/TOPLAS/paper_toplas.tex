...
\begin{itemize}
\item \emph{A certificate-to-proof compiler} for Tur\'an-type bounds,
  built as the three layers described above and invoked as an
  elaboration-time tactic; it comes with an explicit per-obligation
  trust model, a one-click materialization of the synthesized proof
  script, and generated theorem families whose names carry metadata for
  later automation (\Cref{sec:abstract,sec:reflection,sec:flagmatic}).
\item \emph{A meta-theory of ensemble semantics}, formalized in a
  separate, automatically generated Lean development that extends the
  manually written library (\Cref{sec:metatheory}).
