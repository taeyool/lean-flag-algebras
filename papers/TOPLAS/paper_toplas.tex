...
on both the validation of that evidence and the correctness of the reduction
...
numerical sums-of-squares certificates for polynomial inequalities are rounded
to rational form and reverified
exactly~\cite{harrison2007verifying,besson2006fast,morrison2026sos}.
Across these examples, the search procedure remains unverified; assurance comes
from independently validating the evidence produced by each run, much as
translation validation checks individual compilation
results~\cite{pnueli1998translation}.

This paper applies the same validation principle to Razborov's flag algebra
method~\cite{razborov2007flag}.  At a high level, the method turns an asymptotic
density bound for large graphs into the task of finding matrices and
coefficients that satisfy a finite system of algebraic identities and positivity
constraints.  A semidefinite-programming (SDP) solver searches numerically for
such data, which are then converted to exact rational form and recorded in a
certificate.  Independently checking the certificate against this system
establishes the original bound.  This approach is especially valuable for flag
algebras, which have been central to proofs of major results in extremal graph
theory, including the Erd\H{o}s pentagon theorem and the
determination of the maximum induced $C_5$
density~\cite{hatami2012,grzesik2012,balogh2016induced5}.  Yet existing
flag-algebra toolchains produce and check computer-generated certificates
entirely outside proof assistants.  To close this gap, we build a reusable
certificate-to-proof compiler that accepts externally generated certificates
as untrusted input and constructs complete Lean proofs.

\paragraph{Flag algebras and their certificates.}
Many central questions of extremal graph theory ask how often one finite
...
states that $\tdensity{K_2}{K_3}=1/2$: the maximum edge density of
triangle-free graphs tends to $1/2$ as the number of vertices grows.  Such an
equality combines two bounds of very different character.  The lower bound is
witnessed by explicit constructions; for Mantel's theorem, balanced complete
bipartite graphs are triangle-free and have edge density tending to $1/2$.
The upper bound is the hard half: it quantifies over \emph{all}
...
For the difficult upper-bound half, flag algebras provide the finite reduction
outlined above.  Concretely, a \emph{candidate certificate} identifies the
relevant flags and supplies rational matrices and coefficients that purport to
satisfy the required identities and positive-semidefinite constraints.  The
flag-algebra framework proves that, once these finite claims are checked, the
desired bound holds for the entire infinite class of graphs.  The method's
applications extend well beyond the examples already mentioned: they include
the determination of the rainbow-triangle density~\cite{balogh2017rainbow},
the best known upper bound on Tur\'an's tetrahedron
problem~\cite{razborov2010tetrahedron}, and exact Tur\'an densities for
3-uniform hypergraphs~\cite{baber2012turan}.

The raw certificates, however, are not suitable as trusted proof artifacts.
The certificate behind the Erd\H{o}s pentagon theorem relies on thousands of
exact density values and on rational matrices whose positive semidefiniteness
must be established exactly, far beyond what a human referee can audit.
Moreover, the solvers that produce such data sit at the end of a large,
numerical, unverified toolchain.  A formal proof should neither trust this data
nor require thousands of individually written proofs.
