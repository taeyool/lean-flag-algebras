...
downward image, expanding the target and the unit to a common host size, and
collecting coefficients: exactly what our compiler automates.
...
checked after all terms have been expanded in the common host-size
basis.  The certificate records the chosen
...
\begin{enumerate}
  ...
  \item \textbf{Assembling the main theorem.}  With
    ...
    the chain~\eqref{eq:cert-chain}, whose middle equality is checked using the
    identities from Part~1 and, when needed, the target-lifting identity from
    Part~3.
\end{enumerate}
...
\end{lstlisting}
The output file contains the flag and density
...
discussed in \Cref{sec:flagmatic-trust}.
...
multiplication and downward identities generated in Part~1,
...
elaboration-time code, which resolves each compact graph string to the entry
of Lean's canonical enumeration whose index selects the corresponding
\lean{FlagAlgebra\_n\_k\_m\_i} constant.  Because the matching runs against the
enumeration Lean itself computes, no agreement between an external enumeration
order and Lean's is assumed.  Certificates change the assembled data and the number of
...
file introduces an axiom declaration or a \lean{sorry}
(\Cref{sec:results}), so every claim is carried by a proof term Lean checks: the
...
matrices.  These certificates are Flagmatic's output as
