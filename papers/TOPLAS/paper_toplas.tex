...
tuple with its reordering, reflect a limitation of Lean's underlying type
theory: mathematically equal types need not be definitionally equal, and
therefore may require explicit casts.  The enumeration issue, by contrast,
...
A flag's size is part of its Lean type, so the expansion
identity~\eqref{eq:expansion-identity}, which equates a size-$m$ flag with a
combination of size-$n$ flags, is ill-typed as written.  The dependent sum of
\Cref{sec:formal-subflag-densities} absorbs that index: flags of every size
inhabit \lean{FinFlag}~$\sigma$ and index basis vectors of one vector space,
and identifying the two sides is then the quotient's job rather than a
separate argument, since their difference is a generator of the zero space.
Reordering a tuple is not absorbed this way, because the vertex-type family
is an argument of the tuple's type.
...
def FlagList.permute {T : Type} {(*@$\sigma$@*) : FlagType T} {t : (*@$\mathbb{N}$@*)} {Vl : Fin t (*@$\to$@*) Type}
  ...
  fun i => Fl ((*@$\pi$@*) i)
\end{lstlisting}
The permuted tuple \lean{Fl.permute $\pi$} places \lean{Fl ($\pi$ i)} at
position \lean{i}, so the entry types are reordered along with the entries:
the vertex-type family of the result is \lean{listTypePermute Vl $\pi$}, and
it appears in the type of the result.  The invariance is proved once, for
...
\lean{Vl' 0 = U}$_1$ and \lean{Vl' 1 = U}$_0$.  The two families are
pointwise equal, since both send $0$ to $U_1$ and $1$ to $U_0$, but Lean need
not reduce them to the same term.  Because the family is an argument of the
tuple's type, the permuted tuple and the directly constructed pair inhabit
types that are not definitionally the same, and so cannot yet be compared by
ordinary equality.  The formal proof must first establish equality of the
vertex-type families and then use it to compare the tuples, a step with no
counterpart on paper.

The bridging lemma \lean{flagListDensity\_HEq\_eq} packages these requirements:
if the vertex-type families are equal and the tuples agree after alignment,
then their densities coincide.
\begin{lstlisting}
...
\end{lstlisting}
Here \lean{h\_Vl\_eq} states \lean{listTypePermute Vl $\pi$ = Vl'}, proved by
checking the two indices of \lean{Fin 2}, and it aligns the two tuple types.
Once the families are identified, \lean{h\_HEq}, which expresses that both
tuples place $F_1$ at position $0$ and $F_0$ at position $1$, can be used as
an ordinary equality, and the lemma concludes that the densities agree.  The
...
the interface counting algorithms need.  \lean{Fintype.ofFinite} produces an
instance from a proof of finiteness by invoking Lean's choice axiom
\lean{Classical.choice}; the result is noncomputable, so every lemma about
\lean{Fintype} applies to it but Lean cannot evaluate its \lean{elems} to
list the elements of $T$.  That costs nothing when a proof relates
cardinalities to one another by lemmas, which is how the counting below
proceeds.  Proving a concrete numeric value of \lean{Fintype.card T} by
computation instead requires a computable instance, which is the form the
reflection layer (\Cref{sec:reflection}) builds.
...
have count_eq : S0.toFinset.card = S1.toFinset.card := by
  ...
  exact card_eq
\end{lstlisting}
The \lean{@} notation makes explicit where the two named instances enter:
they are the enumerations used by the two \lean{Fintype.card} terms and the
arguments of \lean{Fintype.card\_congr}, and the \lean{toFinset} expressions
use them implicitly, so \lean{Set.toFinset\_card} rewrites the concrete
counts to exactly the terms appearing in \lean{card\_eq}.  Without the local
bindings, Lean could synthesize those hidden arguments independently at the
different occurrences, producing terms that are propositionally equal but do
not match for rewriting.
...
Our most reliable method is to identify the two sets, construct an explicit
bijection, and use it to prove equality of their cardinalities; the bijection
...
\(S_\sigma\).  Non-negativity in the built-in semantics is, by definition,
...
non-negative in the ensemble semantics but not in the built-in semantics,
so the two orders
...
fibers with probability tending to one.  On this event, the corresponding
vertices of \(G\) determine their adjacencies, so the internal structure of the
replacement graphs \(J_v\) does not affect the sample.
...
could therefore build the new constrained layer---the forbidden ideal and
constrained quotient---on this foundation.
