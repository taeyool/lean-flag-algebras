...
by contrast,
arises from elaboration and typeclass inference, which may synthesize
...
flags as isomorphism classes; the others are shaped by Lean's treatment of
dependent equality and instance synthesis, so a port to Rocq (formerly Coq),
Agda, or Isabelle/HOL\@ would likely meet them in a different form.  A final
...
\]
is established.  \lean{Quotient.lift} takes that proof and returns the
...
at the reflection boundary, where the decoding \lean{Sym2Flag.toFlag} of
\Cref{sec:reflection-adequacy} must be shown to send equivalent concrete
representations to the same abstract flag.  Rather than exposing both
...
\Cref{sec:formal-subflag-densities} suffices, since it puts every size in
...
\end{lstlisting}
Its hypothesis \lean{h\_Vl\_eq} states \lean{listTypePermute Vl $\pi$ = Vl'},
...
instances from different elaboration paths.  Their \lean{elems} fields cannot
...
\lean{let}, so that every later occurrence elaborates against the same binding
rather than a fresh search.  The fragment below proves that two finite sets
...
\lean{toFinset} terms use the same ones; without the bindings, instance search
would run afresh at each occurrence and could return different terms, so the
rewrite would fail.
