...
Razborov's development builds this constraint into the algebra from the
start; we call this the built-in semantics.  Ours instead formalizes the
algebra of unconstrained graphs once and imposes each constraint only
where an inequality is interpreted; we call this the ensemble semantics.
This keeps the constraint out of the algebra's definitions and lemmas, so
one formalization serves every forbidden family.  The departure raises a
...
counterexample where they differ.  The meta-theory is formalized in a
separate, automatically generated Lean library that extends the main
development.
\end{abstract}
...
The compiled bounds all concern graphs that avoid a forbidden subgraph.
Razborov's development builds this constraint into the algebra from the
start; we call this the \emph{built-in} (quotient) semantics.  Our
implementation instead formalizes the algebra of unconstrained graphs
once and imposes each constraint only where an inequality is interpreted;
we call this the \emph{ensemble} semantics.  This keeps the constraint
out of the algebra's definitions and lemmas, so one formalization serves
every forbidden family.  The departure raises a soundness question: do
the two semantics validate the same inequalities?  \Cref{sec:metatheory}
answers it, comparing the two designs the way one compares two semantics
of a language.  Every inequality valid in the built-in semantics is valid
in the ensemble semantics; the converse holds at the empty flag type, can
fail at a nonempty flag type, and holds at a fixed flag type exactly when
the constrained class is \emph{root-plantable} there.  We prove that
blow-up closure guarantees root-plantability and give a $C_4$-free
