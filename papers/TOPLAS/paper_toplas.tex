...
resulting sums-of-squares decomposition through rational $LDL^\top$.  The final
tactic in our pipeline plays a similar role: it treats the certificate as
untrusted hint data and, for each sums-of-squares block, constructs an exact
...
floating-point arithmetic.  The main difference lies in the verified context
available to the checker.  For \lean{sos}, the stated goal already supplies the
polynomial inequality; after reifying the goal, the tactic can check the
rational sums-of-squares identity directly.  A flag-algebra certificate, by
contrast, names types, flags, and host graphs considered up to isomorphism,
while the theorem statement neither enumerates these objects nor supplies the
density and multiplication identities needed to check the certificate.
Earlier elaboration commands therefore generate canonical representatives,
prove the enumerations exhaustive, and establish the required identities before
the final tactic matches the certificate flags, expands the quadratic forms,
and checks the resulting coefficients.  Verification also requires
