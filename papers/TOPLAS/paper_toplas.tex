...
graph.  The tactic matches each of the certificate's compact graph strings
against this enumeration, selecting the corresponding
\lean{FlagAlgebra\_n\_k\_m\_i} constant, so no agreement between Flagmatic's
ordering and Lean's is assumed.
...
expanded in Part~3, the generated file similarly contains proved evaluations of
...
chain~\eqref{eq:cert-chain}.  In the
Mantel example there is one $Q_t$, the $Q$ of
\Cref{sec:flagmatic-background}, and $r=\tfrac13\cdot\fbarPthree$.  The synthesized proof does not
...
this section.  Thus,
\lean{forbidLEWith C f g} is the implementation-level form of $f\leq_H g$, and
the hypothesis \lean{hM} is the positive-semidefiniteness proof generated in
Part~2.
