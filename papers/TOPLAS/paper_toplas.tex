...
\item \emph{A Lean~4 formalization of the flag algebra method} for
  ...
  (\Cref{sec:abstract,sec:reflection,sec:flagmatic}).
\item \emph{A meta-theory of ensemble semantics}, formalized in a
  separate, automatically generated Lean library that extends the
  development above (\Cref{sec:metatheory}).
\item \emph{Machine-checked theorems}: seven compiled Tur\'an-type upper
  ...
  directly in the library (\Cref{sec:results,sec:lowerbounds}).  To our
  knowledge, this gives the first machine-checked proof of the
  Erd\H{o}s pentagon theorem, whose known proofs all use flag algebras.
\end{itemize}
