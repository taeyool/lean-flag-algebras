...
lemma predIsoLabeledHl_related_indep
    ...
    {Hl₀ : Fin t → LabeledSubgraph σ G₀} {Hl₁ : Fin t → LabeledSubgraph σ G₁} (h_rel : relOfLabeledSubgraphList φ Hl₀ Hl₁)
    : ∀ (i j : Fin t),
...
lemma predIsoLabeledHl_related
  ...
  · intro ⟨h_ind₀, ⟨h_1₀, h_2₀⟩⟩
    ...
    have h_2₁ : ∀ (i j : Fin t), i ≠ j → ((Hl₁ i).subgraph.verts \ G₁.type_verts) ∩ ((Hl₁ j).subgraph.verts \ G₁.type_verts) = ∅ := by
      ...
      exact (predIsoLabeledHl_related_indep h_rel) i j h_empty_i
    exact ⟨h_ind₁, ⟨h_1₁, h_2₁⟩⟩
  · intro ⟨h_ind₁, ⟨h_1₁, h_2₁⟩⟩
    ...
    have e_rels : ∀ (i : Fin t), ∀ (u v : W), (Hl₀ i).subgraph.Adj (φ.symm.graph_iso u) (φ.symm.graph_iso v) ↔ (Hl₁ i).subgraph.Adj u v := by
      intro i u v
      ...
      have := (h_rel i).2 (φ.symm.graph_iso u) (φ.symm.graph_iso v)
      simp_all only [Equiv.toFun_as_coe, RelIso.coe_fn_toEquiv]
    have h_ind₀ : ∀ (i : Fin t), (Hl₀ i).IsInduced := by
      intro i
      have v_rel := v_rels i
      have e_rel := e_rels i
    ...
    have h_2₀ : ∀ (i j : Fin t), i ≠ j → ((Hl₀ i).subgraph.verts \ G₀.type_verts) ∩ ((Hl₀ j).subgraph.verts \ G₀.type_verts) = ∅ := by
      intro i j h_ij
      exact (predIsoLabeledHl_related_indep (relOfLabeledSubgraphList_symm h_rel)) i j (h_2₁ i j h_ij)
    exact ⟨h_ind₀, ⟨h_1₀, h_2₀⟩⟩
