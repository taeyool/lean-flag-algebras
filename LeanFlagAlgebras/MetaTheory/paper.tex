...
$i$, and every resulting ordered choice is one of the planted induced embeddings
counted above.  Hence the number of planted ordered embeddings is at least
\[
...
the base flag $(G,\theta)$.  Since the clone classes are distinct, these choices
are independent: for label $i$ there are $|C_{\theta(i)}|$ possible images, and
the product counts the ordered choices of all $k$ labelled images.  The
...
lower bounds gives the same lower bound on the number of planted ordered
embeddings,
\[
  \prod_{i=1}^k |C_{\theta(i)}|
  \ge
...
The proofs of \cref{thm:clone-root-plantable,thm:true-clone-root-plantable} are, in all
but one detail, the same proof.  Inspecting them shows that the planted estimate and
the planted-mass bound never use the \emph{internal} structure of a blown-up vertex:
the good event keeps the sampled vertices in distinct clone classes, where every
observed adjacency is a between-class adjacency, governed by the base graph $G$ alone
(\cref{lem:planted-estimate,lem:true-planted-estimate}).  Independent blow-ups (clone
classes empty) and complete blow-ups (clone classes complete) differ only in the
unobserved interior, so the same argument applies to \emph{any} way of replacing each
vertex by a graph.  This section isolates the exact closure hypothesis the argument
needs, of which clone-closure, true-clone-closure, and the substitution-closure below
are all special cases.

\begin{definition}[Vertex blow-up]\label{def:vertex-blowup}
Let $G$ be a finite graph, $v\in V(G)$, and $H$ a finite graph disjoint from $G$.  The
...
$v\in V(G)$, and every $N\in\mathbb N$ there is a graph $H$ with $|H|\ge N$ such that
$G[v\to H]\in\K$.
\end{definition}
...
this is the only difference from substitution-closure, which would demand that
\emph{every} in-class interior be allowed (see \cref{rem:strictness} below).
...
\begin{proof}
Process the vertices $v_1,\dots,v_n$ of $G$ one at a time.  Put $G_0=G$.  Given
$G_{i-1}\in\K$, apply \cref{def:blow-up-closed} to $G_{i-1}$, its vertex $v_i$, and
$N=m$: there is $H'_i$ with $|H'_i|\ge m$ and $G_{i-1}[v_i\to H'_i]\in\K$.  By heredity,
restrict $H'_i$ to an induced subgraph $H_i$ on exactly $m$ vertices; then
$G_{i-1}[v_i\to H_i]$ is an induced subgraph of $G_{i-1}[v_i\to H'_i]$, so it lies in
$\K$.  Set $G_i=G_{i-1}[v_i\to H_i]\in\K$.  Blowing up $v_i$ does not disturb the clone
classes already created at $v_1,\dots,v_{i-1}$, and all between-class adjacencies remain
those of $G$ (each vertex of $H_i$ inherits the adjacencies of $v_i$).  Hence
$G_n=G[H_v:v\in V(G)]$ with $|H_v|=m$ for all $v$, and $G_n\in\K$.
\end{proof}
...
Let $\K$ be hereditary and let $F=(G,\theta)$ be a $\sigma$-flag with $|\sigma|=k$ and
$|G|=n>k$.  Let $B=G[H_v:v\in V(G)]$ be any simultaneous blow-up of $G$, with the clone
class $H_v$ of size $s_v\ge1$ at each $v$.  Then the estimates of
\cref{lem:planted-estimate,lem:planted-mass} hold for $B$ in place of the independent
blow-up, with the same constants and the same hypotheses on the clone sizes $s_v$; the
interiors $H_v$ play no role.
\end{lemma}

\begin{proof}
This is the proof of \cref{lem:true-planted-estimate} with ``complete'' replaced by
``arbitrary''.  For the approximation estimate, sample the $q=|F_0|-k$ extra vertices and
condition on the good event that no sampled vertex lies in a labelled clone class and that
no two sampled vertices lie in the same clone class.  On this event every sampled vertex
lies in a distinct clone class, so every adjacency involving a sampled vertex is an
adjacency between two \emph{distinct} clone classes, and therefore equals the corresponding
base adjacency in $G$---whatever the interiors $H_v$ are.  Hence the induced $\sigma$-flag
seen in $B$ is exactly the one seen in $(G,\theta)$ after projecting to $G$, and an interior
edge is never observed.  The probability of the bad event and the total-variation
comparison with a uniform sample depend only on the sizes $s_v$ and on sampling without
replacement, exactly as in \cref{lem:planted-estimate}.  For the planted-mass bound, every
ordered choice of one vertex from each labelled clone class $H_{\theta(i)}$ is an induced
copy of $\sigma$: the labelled clone classes are distinct and their mutual adjacencies are
inherited from $(G,\theta)$.  So the planted count $\prod_i s_{\theta(i)}$ and its lower
bound are unchanged.  By the same token an induced copy of $\sigma$ already arises from any
transversal of $k$ labelled clone classes spanning an induced $\sigma$ in $G$, so the
positive lower bound on the $\sigma$-type density of $B$ also carries over.
\end{proof}
...
\cref{lem:support-passes-general}.  For the reverse inclusion we repeat the proof of
\cref{thm:clone-root-plantable} verbatim, with one substitution.  As there, fix
$\psi\in Q_\sigma$, represent it by in-class flags $(G_t,\theta_t)$ with $|G_t|\to\infty$,
and fix one such base flag $(G_t,\theta_t)$.  For each $j$, \cref{lem:blowup-iterate}
supplies a uniform blow-up $B_j=G_t[H_v^{(j)}:v\in V(G_t)]$ with $|H_v^{(j)}|=j$ for all
$v$, and $B_j\in\K$.  By \cref{lem:general-planting-estimate} the planted estimate, the
planted-mass bound, and the positive lower bound on the $\sigma$-type density all hold for
$B_j$ with uniform clone sizes $j$, exactly as for the independent blow-ups of
\cref{thm:clone-root-plantable}.  The closed-cylinder and Portmanteau argument of
\cref{thm:clone-root-plantable} then puts positive mass arbitrarily close to $\psi$, so
$\psi\in S_\sigma$.
\end{proof}

\Cref{thm:blowup-root-plantable} subsumes the two previous root-plantability theorems and
extends them, because each of the three closure hypotheses already considered implies
blow-up-closure.
...
true-clone-closure; for (3), $G[v\to H]=G[H_w:w\in V(G)]$ with the in-class fibre $H$ at
$v$ and single vertices elsewhere, in $\K$ by substitution-closure.  Apply
...
whereas root planting only needs \emph{one} large blow-up of each base graph to stay in
...
and global substitution-closure (\cref{thm:substitution-root-plantable}) are three such
