...
-- `generate_empty_typed_flags n`: the JSON-free counterpart of
-- `load_empty_typed_flags`. Instead of reading `graphs_n.json`, it evaluates the
-- self-contained Lean enumeration `genSym2Graphs n` (one canonical representative
-- per isomorphism class) at elaboration time and synthesizes the *same* named
-- constants (`Sym2Graph_n_0_0_i`, `Sym2Flag_n_0_0_i`, `Flag_n_0_0_i`,
-- `FlagAlgebra_n_0_0_i`, the finset defs and the `… = Finset.univ` lemmas). The
-- crucial difference: `Sym2FlagSet_n_0_0_eq_univ` is discharged by the
...
-- flag enumeration) rather than the prohibitive `native_decide` that enumerated
-- the entire quotient `Fintype` via `Finset.univ`.
...
-- `generate_flags k m n`: the JSON-free counterpart of `load_flags`. Instead of
-- reading `flags_n_k_m.json`, it evaluates the self-contained Lean enumeration
-- `genFlagData k m n` at elaboration time (one orbit representative per flag, in
-- the canonical JSON order) and synthesizes the *same* named constants
-- (`Sym2FlagType_k_m`, `FlagType_k_m`, per flag `Sym2LabeledGraph_n_k_m_i`,
-- `Sym2Flag_n_k_m_i`, `Flag_n_k_m_i`, `FlagAlgebra_n_k_m_i`, the `simp` lemmas
-- `unlabel_n_k_m_i` / `downward_n_k_m_i`, and the finset/`= univ` lemmas). The
-- type's edges and each flag's underlying edges are the canonical edge lists
-- `canonicalEdgeList (genSym2Graphs ·)`, so the generated `Sym2LabeledGraph`'s
-- edge Finset matches `Sym2Graph_n_0_0_j`'s exactly (preserving `unlabel`/
-- `downward` defeq); the downward coefficient is the reduced orbit ratio computed
-- by `genFlagData` and independently re-checked by the per-flag `native_decide`.
elab "generate_flags" kStx:num mStx:num nStx:num : command => do
  ...
  -- Crucial difference from `load_flags`: `sym2FlagSet_{n}_{k}_{m}_eq_univ` is
  -- discharged by the mathematically-proved completeness theorem
  -- `genFlagSet_eq_univ` (bridged to the named flag list by one cheap
  -- `native_decide` over the tight `genFlagSet` enumeration) rather than the
  -- prohibitive `native_decide` that materialised `Finset.univ : Finset (Sym2Flag …)`
  -- via the full `Fintype (Sym2LabeledGraph σ n)` enumeration over all
  -- `2 ^ (C(n,2)+n)` edge subsets × embeddings.
  let env ← getEnv
  if ¬ env.contains setEqUnivName.getId then
    elabCommand (← `(
      theorem $setEqUnivName : $setName = Finset.univ := by
