...
Restates `Mantel_flagAlgebra` through the spec-level bridge
`generalizedTuranDensity_le_of_forbidLE`: the asymptotic density of induced
copies of `TargetGraph` among K3-free graphs is at most '1/2'.
...
theorem Mantel_turanDensity
    : generalizedTuranDensity (completeGraph (Fin 3)) TargetGraph.toLabeledGraph.graph ≤ (1 / 2 : ℝ)
  := by
