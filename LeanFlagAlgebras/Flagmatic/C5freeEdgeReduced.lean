...
--     LeanFlagAlgebras/Flagmatic/C5freeEdgeReduced.lean --namespace C5freeEdgeReduced --mask-density --mask-flagsets --force
...
theorem C5freeEdge_reduced_flagAlgebra
  ...
  := by
  flag_certificate "LeanFlagAlgebras/Flagmatic/Certificates/C5freeEdge_reduced_cert.json" ForbidGraph
...
Restates `C5freeEdge_reduced_flagAlgebra` through the spec-level bridge
...
theorem C5freeEdge_reduced_turanDensity
  ...
  have hobj : TargetGraph.toLabeledGraph.graph.toFlagAlgebra
    ...
    rw [htarget]; rfl
  rw [hobj]
  exact C5freeEdge_reduced_flagAlgebra
